	...
	struct guard_genericAI: public GuardAI {
		...
		void EnterCombat(Unit* who) {
			if (me->GetEntry() == NPC_CENARION_HOLD_INFANTRY)
				DoScriptText(
						RAND(SAY_GUARD_SIL_AGGRO1, SAY_GUARD_SIL_AGGRO2,
								SAY_GUARD_SIL_AGGRO3), me, who);
		...
		void UpdateAI(const uint32 diff) {
			...
			if (me->isAlive() && !me->isInCombat()) {
				if (buffTimer <= diff) {
					...
					SpellEntry const *info = SelectSpell(me, 0, 0,
							SELECT_TARGET_ANY_FRIEND, 0, 0, 0, 0,
							SELECT_EFFECT_AURA);
			...
			if (me->isAttackReady() && !me->IsNonMeleeSpellCasted(false)) {
				...
				if (me->IsWithinMeleeRange(me->getVictim())) {
					...
					if (me->HealthBelowPct(30))
						info = SelectSpell(me, 0, 0, SELECT_TARGET_ANY_FRIEND,
					...
					if (info)
						...
						info = SelectSpell(me->getVictim(), 0, 0,
								SELECT_TARGET_ANY_ENEMY, 0, 0, 0, 0,
				...
				if (!me->IsNonMeleeSpellCasted(false)) {
					...
					if (info)
						...
						info = SelectSpell(me->getVictim(), 0, 0,
								SELECT_TARGET_ANY_ENEMY, 0, 0,
								NOMINAL_MELEE_RANGE, 0, SELECT_EFFECT_DONTCARE);
					...
					else if (me->GetMotionMaster()->GetCurrentMovementGeneratorType()
