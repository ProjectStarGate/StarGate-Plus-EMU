...
public:
	...
	void OnChat(Player* player, uint32 type, uint32 lang, std::string msg) {
		...
		case CHAT_MSG_ADDON:
			if (sWorld->getBoolConfig(CONFIG_CHATLOG_ADDON))
				sLog->outChat("[ADDON] Player %s sends: %s", player->GetName(),
						msg.c_str());
			break;
	...
	void OnChat(Player *player, uint32 /*type*/, uint32 /*lang*/,
		...
		if (sWorld->getBoolConfig(CONFIG_CHATLOG_WHISPER))
			sLog->outChat("[WHISPER] Player %s tells %s: %s", player->GetName(),
					receiver ? receiver->GetName() : "<unknown>", msg.c_str());
	}
	...
	void OnChat(Player *player, uint32 type, uint32 lang, std::string msg,
		...
		case CHAT_MSG_GUILD:
			if (lang != LANG_ADDON
					&& sWorld->getBoolConfig(CONFIG_CHATLOG_GUILD))
				sLog->outChat("[GUILD] Player %s tells guild %s: %s",
						player->GetName(),
						guild ? guild->GetName().c_str() : "<unknown>",
						msg.c_str());
			else if (lang == LANG_ADDON
					&& sWorld->getBoolConfig(CONFIG_CHATLOG_ADDON))
	...
	void OnChat(Player *player, uint32 /*type*/, uint32 /*lang*/,
		...
		if (sWorld->getBoolConfig(CONFIG_CHATLOG_SYSCHAN) && isSystem)
			...
			sLog->outChat("[CHANNEL] Player %s tells channel %s: %s",
					player->GetName(),
					channel ? channel->GetName().c_str() : "<unknown>",
					msg.c_str());
	}
};
