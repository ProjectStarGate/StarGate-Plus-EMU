...
public:
	go_northern_crystal_pylon() :
			GameObjectScript("go_northern_crystal_pylon") {
	}
...
public:
	go_eastern_crystal_pylon() :
			GameObjectScript("go_eastern_crystal_pylon") {
	}
...
public:
	go_western_crystal_pylon() :
			GameObjectScript("go_western_crystal_pylon") {
	}
...
public:
	go_field_repair_bot_74A() :
			GameObjectScript("go_field_repair_bot_74A") {
	}
...
public:
	go_orb_of_command() :
			GameObjectScript("go_orb_of_command") {
	}
...
public:
	...
	bool OnGossipHello(Player *pPlayer, GameObject * /*pGO*/) {
		if (pPlayer->HasSkill(SKILL_ALCHEMY)
				&& pPlayer->GetSkillValue(SKILL_ALCHEMY) >= 300
				&& !pPlayer->HasSpell(24266)) {
...
const uint32 NpcPrisonEntry[] = { 22810, 22811, 22812, 22813, 22814, 22815, //good guys
			...
			if (!pCreature->IsHostileTo(pPlayer)) {
				...
				if (FactionTemplateEntry const* pFaction = pCreature->getFactionTemplateEntry()) {
					...
					if (Spell)
						...
						sLog->outError(
								"TSCR: go_ethereum_prison summoned Creature (entry %u) but faction (%u) are not expected by script.",
...
public:
	...
	bool OnGossipHello(Player *pPlayer, GameObject *pGO) {
		...
		pPlayer->SummonCreature(NpcStasisEntry[Random], pGO->GetPositionX(),
				pGO->GetPositionY(), pGO->GetPositionZ(),
				pGO->GetAngle(pPlayer), TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
				30000);
...
public:
	...
	bool OnGossipHello(Player *pPlayer, GameObject *pGO) {
		if (pGO->GetGoType() == GAMEOBJECT_TYPE_GOOBER)
			pPlayer->SummonCreature(NPC_ARIKARA, -5008.338f, -2118.894f,
					83.657f, 0.874f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
					30000);
...
public:
	...
	bool OnGossipHello(Player *pPlayer, GameObject *pGO) {
		...
		float fX, fY, fZ;
		pGO->GetClosePoint(fX, fY, fZ, pGO->GetObjectSize(),
		...
		if (BirdEntry)
			pPlayer->SummonCreature(BirdEntry, fX, fY, fZ,
					pGO->GetOrientation(),
...
enum eDalaranCrystal {
	QUEST_LEARN_LEAVE_RETURN = 12790, QUEST_TELE_CRYSTAL_FLAG = 12845
};
...
enum eFelCrystalforge {
	SPELL_CREATE_1_FLASK_OF_BEAST = 40964, SPELL_CREATE_5_FLASK_OF_BEAST = 40965,
};
...
public:
	...
	bool OnGossipSelect(Player *pPlayer, GameObject *pGO, uint32 /*uiSender*/,
			uint32 uiAction) {
...
public:
	...
	bool OnGossipSelect(Player *pPlayer, GameObject *pGO, uint32 /*uiSender*/,
		...
		switch (uiAction) {
		case GOSSIP_ACTION_INFO_DEF:
			pPlayer->CastSpell(pPlayer, SPELL_CREATE_1_FLASK_OF_SORCERER,
					false);
			...
			break;
		case GOSSIP_ACTION_INFO_DEF + 1:
			pPlayer->CastSpell(pPlayer, SPELL_CREATE_5_FLASK_OF_SORCERER,
					false);
			pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_BASHIR_CRYSTALFORGE_ITEM_RETURN, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
			pPlayer->SEND_GOSSIP_MENU(GOSSIP_BASHIR_CRYSTALFORGE_ITEM_TEXT_RETURN, pGO->GetGUID());
			break;
...
public:
	go_matrix_punchograph() :
			GameObjectScript("go_matrix_punchograph") {
	}
...
public:
	...
	bool OnGossipHello(Player *pPlayer, GameObject *pGO) {
		if (Creature *pNearestPrisoner = pGO->FindNearestCreature(NPC_SCOURGE_PRISONER, 5.0f, true)) {
			pGO->SetGoState(GO_STATE_ACTIVE);
			pPlayer->KilledMonsterCredit(NPC_SCOURGE_PRISONER,
...
enum eArcanePrison {
	QUEST_PRISON_BREAK = 11587, SPELL_ARCANE_PRISONER_KILL_CREDIT = 45456
};
...
public:
	...
	bool OnGossipHello(Player *pPlayer, GameObject *pGO) {
		if (pPlayer->GetQuestStatus(QUEST_PRISON_BREAK)
				== QUEST_STATUS_INCOMPLETE) {
			pGO->SummonCreature(25318, 3485.089844f, 6115.7422188f, 70.966812f,
					0, TEMPSUMMON_TIMED_DESPAWN, 60000);
			pPlayer->CastSpell(pPlayer, SPELL_ARCANE_PRISONER_KILL_CREDIT,
					true);
			return true;
		} else
			return false;
...
public:
	go_blood_filled_orb() :
			GameObjectScript("go_blood_filled_orb") {
	}

	bool OnGossipHello(Player *pPlayer, GameObject *pGO) {
		if (pGO->GetGoType() == GAMEOBJECT_TYPE_GOOBER)
			pPlayer->SummonCreature(NPC_ZELEMAR, -369.746f, 166.759f, -21.50f,
					5.235f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000);
...
public:
	...
	bool OnGossipHello(Player* pPlayer, GameObject* pGO) {
		Creature* pPrisoner = pGO->FindNearestCreature(
				NPC_EBON_BLADE_PRISONER_HUMAN, 5.0f, true);
		if (!pPrisoner) {
			pPrisoner = pGO->FindNearestCreature(NPC_EBON_BLADE_PRISONER_TROLL,
					5.0f, true);
			if (!pPrisoner) {
				pPrisoner = pGO->FindNearestCreature(
...
public:
	...
	bool OnGossipHello(Player* pPlayer, GameObject* pGO) {
		if (pPlayer->GetQuestStatus(QUEST_SPIDER_GOLD)
				== QUEST_STATUS_INCOMPLETE)
...
public:
	...
	bool OnGossipHello(Player *pPlayer, GameObject* /*pGO*/) {
		...
		pPlayer->CastSpell(pPlayer,
				SPELL_SUMMON_PIRATES_TREASURE_AND_TRIGGER_MOB, true);
...
public:
	...
	bool OnGossipHello(Player *pPlayer, GameObject *pGO) {
		if (pPlayer->GetQuestStatus(QUEST_PRISONERS_OF_WYRMSKULL)
				!= QUEST_STATUS_INCOMPLETE)
			return true;

		Creature* pPrisoner = pGO->FindNearestCreature(NPC_PRISONER_PRIEST,
				2.0f);
		if (!pPrisoner) {
			pPrisoner = pGO->FindNearestCreature(NPC_PRISONER_MAGE, 2.0f);
			if (!pPrisoner) {
				pPrisoner = pGO->FindNearestCreature(NPC_PRISONER_WARRIOR,
						2.0f);
				if (!pPrisoner)
					pPrisoner = pGO->FindNearestCreature(NPC_PRISONER_PALADIN,
							2.0f);
			}
		}
...
enum eTadpoles {
	QUEST_OH_NOES_THE_TADPOLES = 11560, NPC_WINTERFIN_TADPOLE = 25201
};
...
public:
	...
	bool OnGossipSelect(Player *pPlayer, GameObject *pGO, uint32 /*uiSender*/,
		...
		if (uiAction == GOSSIP_ACTION_INFO_DEF + 1) {
			...
			Creature* pTarget = GetClosestCreatureWithEntry(pPlayer,
					NPC_OUTHOUSE_BUNNY, 3.0f);
			if (pTarget) {
...
enum eHives {
	QUEST_HIVE_IN_THE_TOWER = 9544, NPC_HIVE_AMBUSHER = 13301
};
...
public:
	...
	bool OnGossipHello(Player *pPlayer, GameObject *pGO) {
		...
		pGO->SummonCreature(NPC_HIVE_AMBUSHER, pGO->GetPositionX() + 1,
				pGO->GetPositionY(), pGO->GetPositionZ(),
				pGO->GetAngle(pPlayer), TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,
				60000);
		pGO->SummonCreature(NPC_HIVE_AMBUSHER, pGO->GetPositionX(),
				pGO->GetPositionY() + 1, pGO->GetPositionZ(),
				pGO->GetAngle(pPlayer), TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,
				60000);
		return true;
	}
