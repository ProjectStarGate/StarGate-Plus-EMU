...
bool OutdoorPvPSI::HandleAreaTrigger(Player *plr, uint32 trigger) {
	...
	case SI_AREATRIGGER_A:
		if (plr->GetTeam() == ALLIANCE && plr->HasAura(SI_SILITHYST_FLAG)) {
			...
			plr->GetReputationMgr().ModifyReputation(
					sFactionStore.LookupEntry(609), 20);
			// complete quest
	...
	if (spellId == SI_SILITHYST_FLAG) {
		...
		case ALLIANCE: {
			...
			if (atEntry) {
				...
				if (plr->GetDistance(atEntry->x, atEntry->y, atEntry->z)
						> 5.0f + atEntry->radius) {
					...
					if (!go->Create(
							sObjectMgr->GenerateLowGuid(HIGHGUID_GAMEOBJECT), SI_SILITHYST_MOUND, map, plr->GetPhaseMask(), plr->GetPositionX(), plr->GetPositionY(), plr->GetPositionZ(), plr->GetOrientation(), 0, 0, 0, 0, 100, GO_STATE_READY))
							{
								delete go;
							}
							else
...
bool OutdoorPvPSI::HandleCustomSpell(Player *plr, uint32 spellId,
		GameObject *go) {
