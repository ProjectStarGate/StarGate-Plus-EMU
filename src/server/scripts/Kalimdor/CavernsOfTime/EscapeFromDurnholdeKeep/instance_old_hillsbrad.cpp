...
public:
	instance_old_hillsbrad() :
			InstanceMapScript("instance_old_hillsbrad", 560) {
	}
	...
	struct instance_old_hillsbrad_InstanceMapScript: public InstanceScript {
		instance_old_hillsbrad_InstanceMapScript(Map* pMap) :
				InstanceScript(pMap) {
			Initialize();
		}
		;
		...
		Player* GetPlayerInMap() {
			...
			sLog->outDebug(
					LOG_FILTER_TSCR,
					"TSCR: Instance Old Hillsbrad: GetPlayerInMap, but PlayerList is empty!");
			return NULL;
		}
		...
		void SetData(uint32 type, uint32 data) {
			...
			if (!pPlayer) {
				sLog->outDebug(
						LOG_FILTER_TSCR,
						"TSCR: Instance Old Hillsbrad: SetData (Type: %u Data %u) cannot find any player.",
						type, data);
				return;
			}
			...
			case TYPE_BARREL_DIVERSION: {
				if (data == IN_PROGRESS) {
					...
					sLog->outDebug(
							LOG_FILTER_TSCR,
							"TSCR: Instance Old Hillsbrad: go_barrel_old_hillsbrad count %u",
							mBarrelCount);
					...
					if (mBarrelCount == 5) {
						UpdateQuestCredit();
						pPlayer->SummonCreature(DRAKE_ENTRY, 2128.43f, 71.01f,
								64.42f, 1.74f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,
								1800000);
			...
			case TYPE_THRALL_EVENT: {
				if (data == FAIL) {
					if (mThrallEventCount <= 20) {
						...
						sLog->outDebug(
								LOG_FILTER_TSCR,
								"TSCR: Instance Old Hillsbrad: Thrall event failed %u times. Resetting all sub-events.",
								mThrallEventCount);
				...
				sLog->outDebug(
						LOG_FILTER_TSCR,
						"TSCR: Instance Old Hillsbrad: Thrall escort event adjusted to data %u.",
						data);
				break;
			}
			case TYPE_THRALL_PART1:
				...
				sLog->outDebug(
						LOG_FILTER_TSCR,
						"TSCR: Instance Old Hillsbrad: Thrall event part I adjusted to data %u.",
						data);
				break;
			case TYPE_THRALL_PART2:
				...
				sLog->outDebug(
						LOG_FILTER_TSCR,
						"TSCR: Instance Old Hillsbrad: Thrall event part II adjusted to data %u.",
						data);
				break;
			case TYPE_THRALL_PART3:
				...
				sLog->outDebug(
						LOG_FILTER_TSCR,
						"TSCR: Instance Old Hillsbrad: Thrall event part III adjusted to data %u.",
						data);
				break;
			case TYPE_THRALL_PART4:
				...
				sLog->outDebug(
						LOG_FILTER_TSCR,
						"TSCR: Instance Old Hillsbrad: Thrall event part IV adjusted to data %u.",
						data);
				break;
			}
		}
