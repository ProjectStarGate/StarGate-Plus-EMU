...
public:
	go_barrel_old_hillsbrad() :
			GameObjectScript("go_barrel_old_hillsbrad") {
	}
...
static Location DrakeWP[] = { { 0, 2125.84f, 88.2535f, 54.8830f }, { 1,
		2111.01f, 93.8022f, 52.6356f }, { 2, 2106.70f, 114.753f, 53.1965f }, {
		3, 2107.76f, 138.746f, 52.5109f }, { 4, 2114.83f, 160.142f, 52.4738f },
...
public:
	boss_lieutenant_drake() :
			CreatureScript("boss_lieutenant_drake") {
	}
	...
	struct boss_lieutenant_drakeAI: public ScriptedAI {
		boss_lieutenant_drakeAI(Creature *c) :
				ScriptedAI(c) {
		}
		...
		void UpdateAI(const uint32 diff) {
			//TODO: make this work
			if (CanPatrol && wpId == 0) {
				me->GetMotionMaster()->MovePoint(DrakeWP[0].wpId, DrakeWP[0].x,
						DrakeWP[0].y, DrakeWP[0].z);
				++wpId;
			}
