...
public:
	mob_ancient_wisp() :
			CreatureScript("mob_ancient_wisp") {
	}
...
public:
	mob_doomfire_targetting() :
			CreatureScript("mob_doomfire_targetting") {
	}
	...
	struct mob_doomfire_targettingAI: public ScriptedAI {
		mob_doomfire_targettingAI(Creature* c) :
				ScriptedAI(c) {
		}
		...
		void UpdateAI(const uint32 diff) {
			if (ChangeTargetTimer <= diff) {
				if (Unit *temp = Unit::GetUnit(*me, TargetGUID)) {
					...
					me->GetMotionMaster()->MovePoint(0, pos.m_positionX,
							pos.m_positionY, pos.m_positionZ);
				}
...
public:
	...
	struct boss_archimondeAI: public hyjal_trashAI {
		...
		bool CanUseFingerOfDeath() {
			...
			if (victim
					&& me->IsWithinDistInMap(victim,
							me->GetAttackDistance(victim)))
				return false;
			...
			if (pTarget) {
				if (!me->IsWithinDistInMap(pTarget,
						me->GetAttackDistance(pTarget)))
					return true; // Cast Finger of Death
				else
					// This target is closest, he is our new tank
					me->AddThreat(pTarget,
							me->getThreatManager().getThreat(me->getVictim()));
			}
		...
		void JustSummoned(Creature *summoned) {
			...
			if (summoned->GetEntry() == CREATURE_DOOMFIRE) {
				...
				summoned->CastSpell(summoned, SPELL_DOOMFIRE, true, 0, 0,
						me->GetGUID());

				if (Unit *DoomfireSpirit = Unit::GetUnit(*me, DoomfireSpiritGUID)) {
					summoned->GetMotionMaster()->MoveFollow(DoomfireSpirit,
							0.0f, 0.0f);
					DoomfireSpiritGUID = 0;
				}
			}
		...
		void SummonDoomfire(Unit *pTarget) {
			me->SummonCreature(CREATURE_DOOMFIRE_SPIRIT,
					pTarget->GetPositionX() + 15.0f,
					pTarget->GetPositionY() + 15.0f, pTarget->GetPositionZ(), 0,
		...
		void UpdateAI(const uint32 diff) {
			if (!me->isInCombat()) {
				if (pInstance) {
					...
					if ((pInstance->GetData(DATA_AZGALOREVENT) < DONE)
							&& ((!me->IsVisible()) || (me->getFaction() != 35))) {
						me->SetVisible(false);
						me->setFaction(35);
					} else if ((pInstance->GetData(DATA_AZGALOREVENT) >= DONE)
							&& ((!me->IsVisible()) || (me->getFaction() == 35))) {
						me->setFaction(1720);
						me->SetVisible(true);
					}
				...
				if (DrainNordrassilTimer <= diff) {
					if (!IsChanneling) {
						Creature *temp = me->SummonCreature(
								CREATURE_CHANNEL_TARGET, NORDRASSIL_X,
								NORDRASSIL_Y, NORDRASSIL_Z, 0,
						...
						if (Unit *Nordrassil = Unit::GetUnit(*me, WorldTreeGUID)) {
							Nordrassil->SetFlag(UNIT_FIELD_FLAGS,
					...
					if (Unit *Nordrassil = Unit::GetUnit(*me, WorldTreeGUID)) {
						Nordrassil->CastSpell(me, SPELL_DRAIN_WORLD_TREE_2,
								true);
			...
			if (!Enraged) {
				...
				if (CheckDistanceTimer <= diff) {
					...
					Creature* Check = me->SummonCreature(
							CREATURE_CHANNEL_TARGET, NORDRASSIL_X, NORDRASSIL_Y,
							NORDRASSIL_Z, 0, TEMPSUMMON_TIMED_DESPAWN, 2000);
					if (Check) {
			...
			if (BelowTenPercent) {
				...
				if (SummonWispTimer <= diff) {
					DoSpawnCreature(CREATURE_ANCIENT_WISP, float(rand() % 40),
							float(rand() % 40), 0, 0,
