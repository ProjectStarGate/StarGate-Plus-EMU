...
public:
	...
	struct instance_mount_hyjal_InstanceMapScript: public InstanceScript {
		instance_mount_hyjal_InstanceMapScript(Map* pMap) :
				InstanceScript(pMap) {
			Initialize();
		}
		;
...
#define YELL_EFFORTS_NAME   "Archimonde"
		...
		void SetData(uint32 type, uint32 data) {
			...
			case DATA_AZGALOREVENT: {
				...
				if (data == DONE) {
					...
					if (pCreature) {
						Creature* pUnit = pCreature->SummonCreature(21987,
								pCreature->GetPositionX(),
								pCreature->GetPositionY(),
								pCreature->GetPositionZ(), 0,
						...
						if (pMap->IsDungeon() && pUnit) {
							...
							for (Map::PlayerList::const_iterator i =
								...
								if (i->getSource()) {
									...
									pUnit->BuildMonsterChat(&data,
											CHAT_MSG_MONSTER_YELL, YELL_EFFORTS,
											0, YELL_EFFORTS_NAME,
											i->getSource()->GetGUID());
									i->getSource()->GetSession()->SendPacket(
											&data);
				...
				if (data == SPECIAL) {
					if (!m_uiAncientGemGUID.empty()) {
						for (std::list<uint64>::const_iterator itr =
								m_uiAncientGemGUID.begin();
								itr != m_uiAncientGemGUID.end(); ++itr) {
			...
			sLog->outDebug(
					LOG_FILTER_TSCR,
					"TSCR: Instance Hyjal: Instance data updated for event %u (Data=%u)",
					type, data);
			...
			if (data == DONE) {
				...
				saveStream << m_auiEncounter[0] << " " << m_auiEncounter[1]
						...
						<< " " << m_auiEncounter[4] << " " << allianceRetreat
						<< " " << hordeRetreat << " " << RaidDamage;
		...
		void Load(const char* in) {
			...
			loadStream >> m_auiEncounter[0] >> m_auiEncounter[1]
					>> m_auiEncounter[2] >> m_auiEncounter[3]
					>> m_auiEncounter[4] >> allianceRetreat >> hordeRetreat
					>> RaidDamage;
			for (uint8 i = 0; i < MAX_ENCOUNTER; ++i)
