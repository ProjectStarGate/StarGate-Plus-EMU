...
public:
	...
	struct boss_azgalorAI: public hyjal_trashAI {
		...
		void UpdateAI(const uint32 diff) {
			...
			if (RainTimer <= diff) {
				DoCast(SelectTarget(SELECT_TARGET_RANDOM, 0, 30, true),
						SPELL_RAIN_OF_FIRE);
			...
			if (DoomTimer <= diff) {
				DoCast(SelectTarget(SELECT_TARGET_RANDOM, 1, 100, true),
						SPELL_DOOM); //never on tank
...
public:
	mob_lesser_doomguard() :
			CreatureScript("mob_lesser_doomguard") {
	}
	...
	struct mob_lesser_doomguardAI: public hyjal_trashAI {
		mob_lesser_doomguardAI(Creature *c) :
				hyjal_trashAI(c) {
			pInstance = c->GetInstanceScript();
			if (pInstance)
