	...
	bool OnGossipSelect(Player* pPlayer, Creature* pCreature,
		...
		case GOSSIP_ACTION_INFO_DEF:
			...
			sLog->outDebug(LOG_FILTER_TSCR,
					"TSCR: HyjalAI - Debug mode has been toggled");
			break;
		}
		return true;
	}
...
public:
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature) {
		...
		if (AnetheronEvent == DONE
				&& ai->GetInstanceData(DATA_ALLIANCE_RETREAT)) {
...
public:
	...
	bool OnGossipSelect(Player* pPlayer, Creature* pCreature,
		...
		if (uiAction == GOSSIP_ACTION_INFO_DEF) {
			...
			uint8 msg = pPlayer->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest,
					ITEM_TEAR_OF_GODDESS, 1);
			if (msg == EQUIP_ERR_OK) {
				Item* item = pPlayer->StoreNewItem(dest, ITEM_TEAR_OF_GODDESS,
						true);
				if (item && pPlayer)
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature) {
		...
		if (AzgalorEvent == DONE
				&& !pPlayer->HasItemCount(ITEM_TEAR_OF_GODDESS, 1))
