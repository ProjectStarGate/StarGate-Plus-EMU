...
float PortalLocation[4][4] = { { -2041.06f, 7042.08f, 29.99f, 1.30f }, {
		-1968.18f, 7042.11f, 21.93f, 2.12f }, { -1885.82f, 7107.36f, 22.32f,
		3.07f }, { -1928.11f, 7175.95f, 22.11f, 3.44f } };
...
static Wave RiftWaves[] = { { RIFT_BOSS, 0 }, { C_DEJA, 0 },
		{ RIFT_BOSS, 120000 }, { C_TEMPO, 140000 }, { RIFT_BOSS, 120000 }, {
				C_AEONUS, 0 } };

class instance_dark_portal: public InstanceMapScript {
public:
	instance_dark_portal() :
			InstanceMapScript("instance_dark_portal", 269) {
	}
	...
	struct instance_dark_portal_InstanceMapScript: public InstanceScript {
		instance_dark_portal_InstanceMapScript(Map* pMap) :
				InstanceScript(pMap) {
			Initialize();
		}
		;
				...
				if (data == SPECIAL && m_auiEncounter[0] == IN_PROGRESS) {
					...
					if (!mShieldPercent) {
						if (Creature* pMedivh = instance->GetCreature(MedivhGUID)) {
							if (pMedivh->isAlive()) {
								pMedivh->DealDamage(pMedivh,
										pMedivh->GetHealth(), NULL,
					...
					if (data == IN_PROGRESS) {
						sLog->outDebug(LOG_FILTER_TSCR,
								"TSCR: Instance Dark Portal: Starting event.");
						InitWorldState();
					...
					if (data == DONE) {
						...
						sLog->outDebug(LOG_FILTER_TSCR,
								"TSCR: Instance Dark Portal: Event completed.");
		...
		void DoSpawnPortal() {
			if (Creature* pMedivh = instance->GetCreature(MedivhGUID)) {
				...
				sLog->outDebug(
						LOG_FILTER_TSCR,
						"TSCR: Instance Dark Portal: Creating Time Rift at locationId %i (old locationId was %u).",
						tmp, CurrentRiftId);

				CurrentRiftId = tmp;

				Creature *pTemp = pMedivh->SummonCreature(C_TIME_RIFT,
						PortalLocation[tmp][0], PortalLocation[tmp][1],
						PortalLocation[tmp][2], PortalLocation[tmp][3],
		...
		void Update(uint32 diff) {
			...
			if (NextPortal_Timer) {
				if (NextPortal_Timer <= diff) {
					...
					DoSpawnPortal();
					NextPortal_Timer =
							RiftWaves[GetRiftWaveId()].NextPortalTime;
				} else
