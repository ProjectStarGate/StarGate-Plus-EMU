/*
 * Copyright (C) 2009-2011 StarGate
...
public:
	npc_highlord_demitrian() :
			CreatureScript("npc_highlord_demitrian") {
	}
	...
	bool OnGossipHello(Player* player, Creature* creature) {
		...
		if (player->GetQuestStatus(7785) == QUEST_STATUS_NONE
				&& (player->HasItemCount(18563, 1, false)
						|| player->HasItemCount(18564, 1, false)))
...
static QuestCinematic EventAnim[] = { { ANACHRONOS_SAY_1, 0, 2000 }, {
		FANDRAL_SAY_1, 1, 4000 }, { MERITHRA_EMOTE_1, 3, 500 }, {
		MERITHRA_SAY_1, 3, 500 }, { ARYGOS_EMOTE_1, 2, 2000 }, {
		CAELESTRASZ_SAY_1, 4, 8000 }, { MERITHRA_SAY_2, 3, 6000 },
		{ 0, 3, 2000 }, { MERITHRA_YELL_1, 3, 2500 }, { 0, 3, 3000 }, //Morph
		{ 0, 3, 4000 }, //EmoteLiftoff
		{ 0, 3, 4000 }, // spell
		{ 0, 3, 1250 }, //fly
		{ 0, 3, 250 }, //remove flags
		{ ARYGOS_SAY_1, 2, 3000 }, { 0, 3, 2000 }, { ARYGOS_YELL_1, 2, 3000 }, {
				0, 3, 3000 }, //Morph
		{ 0, 3, 4000 }, //EmoteLiftoff
		{ 0, 3, 4000 }, // spell
		{ 0, 3, 1000 }, //fly
		{ 0, 3, 1000 }, //remove flags
		{ CAELESTRASZ_SAY_2, 4, 5000 }, { 0, 3, 3000 }, { CAELESTRASZ_YELL_1, 4,
				3000 }, { 0, 3, 3000 }, //Morph
		{ 0, 3, 4000 }, //EmoteLiftoff
		{ 0, 3, 2500 }, // spell
		{ ANACHRONOS_SAY_2, 0, 2000 }, { 0, 3, 250 }, //fly
		{ 0, 3, 25 }, //remove flags
		{ FANDRAL_SAY_2, 1, 3000 }, { ANACHRONOS_SAY_3, 0, 10000 }, //Both run through the armies
		{ 0, 3, 2000 }, // Sands will stop
		{ 0, 3, 8000 }, // Summon Gate
		{ ANACHRONOS_SAY_4, 0, 4000 }, { 0, 0, 2000 }, //spell 1-> Arcane cosmetic (Mobs freeze)
		{ 0, 0, 5000 }, //Spell 2-> Arcane long cosmetic (barrier appears) (Barrier -> Glyphs)
		{ 0, 0, 7000 }, //BarrieR
		{ 0, 0, 4000 }, //Glyphs
		{ ANACHRONOS_SAY_5, 0, 2000 }, { 0, 0, 4000 }, // Roots
		{ FANDRAL_SAY_3, 1, 3000 }, //Root Text
		{ FANDRAL_EMOTE_1, 1, 3000 }, //falls knee
		{ ANACHRONOS_SAY_6, 0, 3000 }, { ANACHRONOS_SAY_7, 0, 3000 }, {
				ANACHRONOS_SAY_8, 0, 8000 }, { ANACHRONOS_EMOTE_1, 0, 1000 }, //Give Scepter
		{ FANDRAL_SAY_4, 1, 3000 }, { FANDRAL_SAY_5, 1, 3000 }, //->Equip hammer~Scepter, throw it at door
		{ FANDRAL_EMOTE_2, 1, 3000 }, //Throw hammer at door.
		{ ANACHRONOS_SAY_9, 0, 3000 }, { FANDRAL_SAY_6, 1, 3000 }, //fandral goes away
		{ ANACHRONOS_EMOTE_2, 0, 3000 }, { ANACHRONOS_EMOTE_3, 0, 3000 }, { 0,
				0, 2000 }, { 0, 0, 2000 }, { 0, 0, 4000 }, { ANACHRONOS_SAY_10,
				0, 3000 }, { 0, 0, 2000 }, { 0, 0, 3000 }, { 0, 0, 15000 }, { 0,
				0, 5000 }, { 0, 0, 3500 }, { 0, 0, 5000 }, { 0, 0, 3500 }, { 0,
				0, 5000 }, { 0, 0, 0 } };
...
static WaveData WavesInfo[] = { { 30, 0, 15423, 0, 0, 24000, 0 }, // Kaldorei Soldier
...
public:
	...
	struct npc_anachronos_the_ancientAI: public ScriptedAI {
		npc_anachronos_the_ancientAI(Creature* c) :
				ScriptedAI(c) {
		}
		...
		void HandleAnimation() {
			...
			Unit* Fandral = player->FindNearestCreature(C_FANDRAL_STAGHELM,
					100.0f, me);
			Unit* Arygos = player->FindNearestCreature(C_ARYGOS, 100.0f, me);
			Unit* Caelestrasz = player->FindNearestCreature(C_CAELESTRASZ,
					100.0f, me);
			Unit* Merithra = player->FindNearestCreature(C_MERITHRA, 100.0f,
					me);
			...
			if (eventEnd == false) {
				...
				case 7:
					...
					Merithra->GetMotionMaster()->MoveCharge(-8065, 1530, 2.61f,
							10);
					break;
				case 8:
				...
				case 10:
					...
					Merithra->GetMotionMaster()->MoveCharge(-8065, 1530, 6.61f,
							3);
					break;
				case 11:
					...
					break;
				case 12:
					Merithra->GetMotionMaster()->MoveCharge(-8100, 1530, 50,
							42);
					break;
				case 13:
					break;
				case 14:
					...
					break;
				case 15:
					Arygos->GetMotionMaster()->MoveCharge(-8065, 1530, 2.61f,
							10);
					Merithra->GetMotionMaster()->MoveCharge(-8034.535f,
							1535.14f, 2.61f, 42);
					break;
				case 16:
				...
				case 18:
					...
					Arygos->GetMotionMaster()->MoveCharge(-8065, 1530, 6.61f,
							42);
					break;
				case 19:
				...
				case 22:
					...
					break;
				case 23:
					Caelestrasz->GetMotionMaster()->MoveCharge(-8065, 1530,
							2.61f, 10);
					Arygos->SetVisible(false);
					Arygos->GetMotionMaster()->MoveCharge(-8034.535f, 1535.14f,
							2.61f, 10);
					break;
				case 24:
				...
				case 28:
					...
					break;
				case 29:
					Caelestrasz->GetMotionMaster()->MoveCharge(-8095, 1530, 50,
							42);
				...
				case 32:
					...
					Caelestrasz->GetMotionMaster()->MoveCharge(-8034.535f,
							1535.14f, 2.61f, 42);
					Fandral->GetMotionMaster()->MoveCharge(-8108, 1529, 2.77f,
							8);
					me->GetMotionMaster()->MoveCharge(-8113, 1525, 2.77f, 8);
					break; //both run to the gate
				case 33:
					DoScriptText(ANACHRONOS_SAY_4, me);
					Caelestrasz->GetMotionMaster()->MoveCharge(-8050, 1473, 65,
							15);
				...
				case 36:
					...
					break;
				case 37:
					me->SummonGameObject(GO_GATE_OF_AHN_QIRAJ, -8130, 1525,
							17.5f, 0, 0, 0, 0, 0, 0);
					break;
				case 38:
					DoCast(player, 25166, true);
					me->SummonGameObject(GO_GLYPH_OF_AHN_QIRAJ, -8130, 1525,
							17.5f, 0, 0, 0, 0, 0, 0);
					break;
				case 39:
				...
				case 40:
					...
					break;
				case 41:
					Fandral->SummonGameObject(GO_ROOTS_OF_AHN_QIRAJ, -8130,
							1525, 17.5f, 0, 0, 0, 0, 0, 0);
				...
				case 52:
					Fandral->GetMotionMaster()->MoveCharge(-8028.75f, 1538.795f,
							2.61f, 4);
				...
				case 57:
					...
					break;
				case 58:
					me->GetMotionMaster()->MoveCharge(-8112.67f, 1529.9f, 2.86f,
							4);
					break;
				case 59:
					me->GetMotionMaster()->MoveCharge(-8117.99f, 1532.24f,
							3.94f, 4);
					break;
				case 60:
					if (player)
						DoScriptText(ANACHRONOS_SAY_10, me, player);
					me->GetMotionMaster()->MoveCharge(-8113.46f, 1524.16f,
							2.89f, 4);
					break;
				case 61:
					me->GetMotionMaster()->MoveCharge(-8057.1f, 1470.32f, 2.61f,
							6);
...
public:
	mob_qiraj_war_spawn() :
			CreatureScript("mob_qiraj_war_spawn") {
	}
	...
	struct mob_qiraj_war_spawnAI: public ScriptedAI {
		mob_qiraj_war_spawnAI(Creature* c) :
				ScriptedAI(c) {
		}
...
public:
	...
	struct npc_anachronos_quest_triggerAI: public ScriptedAI {
		npc_anachronos_quest_triggerAI(Creature* c) :
				ScriptedAI(c) {
		}
			...
			for (uint8 i = 0; i < 67; ++i) {
				if (Creature* spawn = me->SummonCreature(WavesInfo[WaveCount].CreatureId, SpawnLocation[locIndex + i], TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, WavesInfo[WaveCount].DespTimer)) {
					...
					if (WaveCount < 5) //1-4 Wave
							{
						mob_qiraj_war_spawn::mob_qiraj_war_spawnAI* spawnAI =
								CAST_AI(mob_qiraj_war_spawn::mob_qiraj_war_spawnAI, spawn->AI());
		...
		void CheckEventFail() {
			...
			if (Group* EventGroup = player->GetGroup()) {
				...
				const Group::MemberSlotList members =
	...
	bool OnQuestAccept(Player* player, GameObject* go, Quest const* quest) {
		if (quest->GetQuestId() == QUEST_A_PAWN_ON_THE_ETERNAL_BOARD) {
			if (Unit* Anachronos_Quest_Trigger = go->FindNearestCreature(15454, 100, player)) {
				Unit* Merithra = Anachronos_Quest_Trigger->SummonCreature(15378,
						-8034.535f, 1535.14f, 2.61f, 0,
						TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 220000);
				Unit* Caelestrasz = Anachronos_Quest_Trigger->SummonCreature(
						15379, -8032.767f, 1533.148f, 2.61f, 1.5f,
						TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 220000);
				Unit* Arygos = Anachronos_Quest_Trigger->SummonCreature(15380,
						-8034.52f, 1537.843f, 2.61f, 5.7f,
						TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 220000);
				/* Unit* Fandral = */
				Anachronos_Quest_Trigger->SummonCreature(15382, -8028.462f,
						1535.843f, 2.61f, 3.141592f,
						TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 220000);
				Creature* Anachronos = Anachronos_Quest_Trigger->SummonCreature(
						15381, -8028.75f, 1538.795f, 2.61f, 4,
						TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 220000);
				...
				if (Anachronos) {
					CAST_AI(npc_anachronos_the_ancient::npc_anachronos_the_ancientAI, Anachronos->AI())->PlayerGUID =
							player->GetGUID();
					CAST_AI(npc_anachronos_quest_trigger::npc_anachronos_quest_triggerAI, CAST_CRE(Anachronos_Quest_Trigger)->AI())->Failed =
							false;
					CAST_AI(npc_anachronos_quest_trigger::npc_anachronos_quest_triggerAI, CAST_CRE(Anachronos_Quest_Trigger)->AI())->PlayerGUID =
							player->GetGUID();
					CAST_AI(npc_anachronos_quest_trigger::npc_anachronos_quest_triggerAI, CAST_CRE(Anachronos_Quest_Trigger)->AI())->EventStarted =
							true;
					CAST_AI(npc_anachronos_quest_trigger::npc_anachronos_quest_triggerAI, CAST_CRE(Anachronos_Quest_Trigger)->AI())->Announced =
							true;
				}
			}
		}
...
enum WSGossip {
	GOSSIPID_LESSER_WS = 6540, GOSSIPID_WS = 6542, GOSSIPID_GREATER_WS = 6543
};
...
enum WS {
	TEMPLAR = 0, DUKE = 1, ROYAL = 2,

	FIRE = 0x1, WATER = 0x2, EARTH = 0x4, AIR = 0x8
};

enum WSTexts {
	SAY_TEMPLAR_AGGRO = 0, SAY_DUKE_AGGRO = 0, YELL_ROYAL_AGGRO = 0
};
...
private:
	...
	void SummonNPC(GameObject* go, Player* player, uint32 npc, uint32 spell) {
		...
		TempSummon* summons = go->SummonCreature(npc, go->GetPositionX(),
				go->GetPositionY(), go->GetPositionZ(),
				player->GetOrientation() - M_PI,
				TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 10 * 60 * 1000);
	...
	bool OnGossipSelect(Player* player, GameObject* go, uint32 /*sender*/,
		...
		case GOSSIP_ACTION_INFO_DEF + 1:
			SummonNPC(
					go,
					player,
					RAND(NPC_TEMPLAR_WATER, NPC_TEMPLAR_FIRE, NPC_TEMPLAR_EARTH,
							NPC_TEMPLAR_AIR), SPELL_TEMPLAR_RANDOM);
			break;
		case GOSSIP_ACTION_INFO_DEF + 2:
		...
		case GOSSIP_ACTION_INFO_DEF + 6:
			SummonNPC(
					go,
					player,
					RAND(NPC_DUKE_FIRE, NPC_DUKE_WATER, NPC_DUKE_EARTH,
							NPC_DUKE_AIR), SPELL_DUKE_RANDOM);
			break;
		...
		case GOSSIP_ACTION_INFO_DEF + 11:
			SummonNPC(
					go,
					player,
					RAND(NPC_ROYAL_FIRE, NPC_ROYAL_AIR, NPC_ROYAL_EARTH,
							NPC_ROYAL_WATER), SPELL_ROYAL_RANDOM);
			break;
		case GOSSIP_ACTION_INFO_DEF + 12:
