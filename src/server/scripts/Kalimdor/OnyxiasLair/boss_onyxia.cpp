...
static sOnyxMove aMoveData[] = { { 0, 1, SPELL_BREATH_WEST_TO_EAST, -33.5561f,
		-182.682f, -56.9457f }, //west
...
//Whelps
		{ -30.127f, -254.463f, -89.440f, 0.0f }, { -30.817f, -177.106f,
				-89.258f, 0.0f },
		//Lair Guard
...
public:
	...
	struct boss_onyxiaAI: public BossAI {
		boss_onyxiaAI(Creature* pCreature) :
				BossAI(pCreature, DATA_ONYXIA) {
		}
			...
			if (type == POINT_MOTION_TYPE) {
				...
				case 8:
					...
					if (m_pPointData) {
						...
						me->GetMotionMaster()->MovePoint(m_pPointData->uiLocId,
								m_pPointData->fX, m_pPointData->fY,
								m_pPointData->fZ);
					}
					break;
				case 9:
				...
				case 10:
					me->SetFlying(true);
					me->GetMotionMaster()->MovePoint(11,
							Phase2Location.GetPositionX(),
							Phase2Location.GetPositionY(),
							Phase2Location.GetPositionZ() + 25);
					me->SetSpeed(MOVE_FLIGHT, 1.0f);
			...
			if (m_uiPhase == PHASE_START || m_uiPhase == PHASE_END) {
				...
				if (m_uiPhase == PHASE_START) {
					...
					if (m_uiBellowingRoarTimer <= uiDiff) {
						...
						Trinity::GameObjectInRangeCheck check(
								me->GetPositionX(), me->GetPositionY(),
								me->GetPositionZ(), 15);
						Trinity::GameObjectLastSearcher<
								Trinity::GameObjectInRangeCheck> searcher(me,
								pFloor, check);
						me->VisitNearbyGridObject(30, searcher);
						if (instance && pFloor)
							instance->SetData64(DATA_FLOOR_ERUPTION_GUID,
									pFloor->GetGUID());
						m_uiBellowingRoarTimer = 30000;
					} else
				...
				if (m_uiLairGuardTimer <= uiDiff) {
					me->SummonCreature(NPC_LAIRGUARD,
							aSpawnLocations[2].GetPositionX(),
							aSpawnLocations[2].GetPositionY(),
							aSpawnLocations[2].GetPositionZ(), 0.0f,
				...
				if (m_uiWhelpTimer <= uiDiff) {
					me->SummonCreature(NPC_WHELP,
							aSpawnLocations[0].GetPositionX(),
							aSpawnLocations[0].GetPositionY(),
							aSpawnLocations[0].GetPositionZ(), 0.0f,
							TEMPSUMMON_CORPSE_DESPAWN);
					me->SummonCreature(NPC_WHELP,
							aSpawnLocations[1].GetPositionX(),
							aSpawnLocations[1].GetPositionY(),
							aSpawnLocations[1].GetPositionZ(), 0.0f,
							TEMPSUMMON_CORPSE_DESPAWN);
		...
		void DespawnCreatures(uint32 entry, float distance) {
			...
			for (std::list<Creature*>::iterator iter = m_pCreatures.begin();
					iter != m_pCreatures.end(); ++iter)
