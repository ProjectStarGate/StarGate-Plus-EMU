...
enum eSpells {
	SPELL_ARCHAEDAS_AWAKEN = 10347, SPELL_AWAKEN_VAULT_WALKER = 10258,
};
...
public:
	...
	struct instance_uldaman_InstanceMapScript: public InstanceScript {
		instance_uldaman_InstanceMapScript(Map* pMap) :
				InstanceScript(pMap) {
			Initialize();
		}
		;
		...
		void ActivateStoneKeepers() {
			for (std::vector<uint64>::const_iterator i = vStoneKeeper.begin();
					i != vStoneKeeper.end(); ++i) {
		...
		uint64 GetData64(uint32 identifier) {
			...
			if (identifier == 1)
				return vVaultWalker[0]; // VaultWalker1
			if (identifier == 2)
				return vVaultWalker[1]; // VaultWalker2
			if (identifier == 3)
				return vVaultWalker[2]; // VaultWalker3
			if (identifier == 4)
				return vVaultWalker[3]; // VaultWalker4
