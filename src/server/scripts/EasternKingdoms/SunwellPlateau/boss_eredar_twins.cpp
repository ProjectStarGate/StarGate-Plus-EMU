...
public:
	...
	struct boss_sacrolashAI: public ScriptedAI {
		...
		void Reset() {
			...
			if (pInstance) {
				Unit* Temp = Unit::GetUnit((*me),
						pInstance->GetData64(DATA_ALYTHESS));
				if (Temp) {
					if (Temp->isDead())
						CAST_CRE(Temp)->Respawn();
					else if (Temp->getVictim())
						me->getThreatManager().addThreat(Temp->getVictim(),
								0.0f);
				}
			}
		...
		void UpdateAI(const uint32 diff) {
			...
			if (ShadowimageTimer <= diff) {
				...
				for (uint8 i = 0; i < 3; ++i) {
					pTarget = SelectUnit(SELECT_TARGET_RANDOM, 0);
					temp = DoSpawnCreature(MOB_SHADOW_IMAGE, 0, 0, 0, 0,
...
public:
	...
	struct boss_alythessAI: public Scripted_NoMovementAI {
		...
		void Reset() {
			...
			if (pInstance) {
				Unit* Temp = Unit::GetUnit((*me),
						pInstance->GetData64(DATA_SACROLASH));
				if (Temp) {
					if (Temp->isDead())
						CAST_CRE(Temp)->Respawn();
					else if (Temp->getVictim())
		...
		uint32 IntroStep(uint32 step) {
			Creature* Sacrolash = Unit::GetCreature(*me,
					pInstance ? pInstance->GetData64(DATA_SACROLASH) : 0);
			switch (step) {
			case 0:
				return 0;
			case 1:
		...
		void UpdateAI(const uint32 diff) {
			...
			if (!me->getVictim()) {
				if (pInstance) {
					Creature* sisiter = Unit::GetCreature((*me),
							pInstance->GetData64(DATA_SACROLASH));
...
public:
	mob_shadow_image() :
			CreatureScript("mob_shadow_image") {
	}
