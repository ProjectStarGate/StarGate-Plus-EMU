...
Position DeceiverLocations[3] = { { 1682.045f, 631.299f, 5.936f, 0.0f }, {
		1684.099f, 618.848f, 0.589f, 0.0f },
		{ 1694.170f, 612.272f, 1.416f, 0.0f }, };

// Locations, where Shield Orbs will spawn
float ShieldOrbLocations[4][2] = { { 1698.900f, 627.870f }, // middle pont of Sunwell
...
//Kil Phase 1 -> Phase 2
		{ SAY_KJ_EMERGE, DATA_KILJAEDEN, 0 }, { SAY_KALECGOS_JOIN,
				DATA_KALECGOS_KJ, 26000 },
		//Kil Phase 2 -> Phase 3
		{ SAY_KALECGOS_AWAKEN, DATA_KALECGOS_KJ, 10000 }, {
				SAY_ANVEENA_IMPRISONED, DATA_ANVEENA, 5000 }, { SAY_KJ_PHASE3,
				DATA_KILJAEDEN, 5000 },
		//Kil Phase 3 -> Phase 4
		{ SAY_KALECGOS_LETGO, DATA_KALECGOS_KJ, 10000 }, { SAY_ANVEENA_LOST,
				DATA_ANVEENA, 8000 }, { SAY_KJ_PHASE4, DATA_KILJAEDEN, 7000 },
		//Kil Phase 4 -> Phase 5
		{ SAY_KALECGOS_FOCUS, DATA_KALECGOS_KJ, 4000 }, { SAY_ANVEENA_KALEC,
				DATA_ANVEENA, 11000 }, { SAY_KALECGOS_FATE, DATA_KALECGOS_KJ,
				2000 }, { SAY_ANVEENA_GOODBYE, DATA_ANVEENA, 6000 }, {
...
public:
	boss_kalecgos_kj() :
			CreatureScript("boss_kalecgos_kj") {
	}
	...
	struct boss_kalecgos_kjAI: public ScriptedAI {
		...
		GameObject* GetOrb(int32 index) {
			...
			switch (index) {
			case 0:
				return pInstance->instance->GetGameObject(
						pInstance->GetData64(
...
public:
	go_orb_of_the_blue_flight() :
			GameObjectScript("go_orb_of_the_blue_flight") {
	}

	bool OnGossipHello(Player* pPlayer, GameObject* pGo) {
		if (pGo->GetUInt32Value(GAMEOBJECT_FACTION) == 35) {
			InstanceScript* pInstance = pGo->GetInstanceScript();
			pPlayer->SummonCreature(CREATURE_POWER_OF_THE_BLUE_DRAGONFLIGHT,
					pPlayer->GetPositionX(), pPlayer->GetPositionY(),
					pPlayer->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_DESPAWN,
					121000);
			pPlayer->CastSpell(pPlayer, SPELL_VENGEANCE_OF_THE_BLUE_FLIGHT,
					false);
...
public:
	mob_kiljaeden_controller() :
			CreatureScript("mob_kiljaeden_controller") {
	}
	...
	struct mob_kiljaeden_controllerAI: public Scripted_NoMovementAI {
		mob_kiljaeden_controllerAI(Creature* c) :
				Scripted_NoMovementAI(c), summons(me) {
			pInstance = c->GetInstanceScript();
		}
		...
		void UpdateAI(const uint32 diff) {
			if (uiRandomSayTimer < diff) {
				if (pInstance && pInstance->GetData(DATA_MURU_EVENT) != DONE
						&& pInstance->GetData(DATA_KILJAEDEN_EVENT)
								== NOT_STARTED)
					DoScriptText(
							RAND(SAY_KJ_OFFCOMBAT1, SAY_KJ_OFFCOMBAT2,
									SAY_KJ_OFFCOMBAT3, SAY_KJ_OFFCOMBAT4,
									SAY_KJ_OFFCOMBAT5), me);
				uiRandomSayTimer = 30000;
			} else
				uiRandomSayTimer -= diff;

			if (!bSummonedDeceivers) {
				for (uint8 i = 0; i < 3; ++i)
					me->SummonCreature(CREATURE_HAND_OF_THE_DECEIVER,
							DeceiverLocations[i], TEMPSUMMON_DEAD_DESPAWN, 0);

				DoSpawnCreature(CREATURE_ANVEENA, 0, 0, 40, 0,
			...
			if (deceiverDeathCount > 2 && phase == PHASE_DECEIVERS) {
				...
				phase = PHASE_NORMAL;
				DoSpawnCreature(CREATURE_KILJAEDEN, 0, 0, 0, 0,
...
public:
	...
	struct boss_kiljaedenAI: public Scripted_NoMovementAI {
		...
		void EnterNextPhase() {
			...
			Timer[TIMER_DARKNESS] =
					(Phase == PHASE_SACRIFICE) ? 15000 : urand(10000, 40000);
			Timer[TIMER_ORBS_EMPOWER] =
					(Phase == PHASE_SACRIFICE) ? 10000 : 5000;
		}
				...
				if (Timer[t] < diff && !TimerIsDeactivated[t]) {
					...
					case TIMER_SPEECH:
						...
						if (Speeches[speechCount].timer < SpeechTimer) {
							...
							if (Creature* pSpeechCreature = Unit::GetCreature(*me, pInstance->GetData64(Speeches[speechCount].pCreature)))
								DoScriptText(Speeches[speechCount].textid,
										pSpeechCreature);
							if (speechCount == 12)
								if (Creature* pAnveena = Unit::GetCreature(*me, pInstance->GetData64(DATA_ANVEENA)))
									pAnveena->CastSpell(me,
											SPELL_SACRIFICE_OF_ANVEENA, false);
					...
					case TIMER_SOUL_FLAY:
						if (!me->IsNonMeleeSpellCasted(false)) {
							DoCast(me->getVictim(), SPELL_SOUL_FLAY_SLOW,
									false);
					...
					case TIMER_LEGION_LIGHTNING:
						if (!me->IsNonMeleeSpellCasted(false)) {
							...
							for (uint8 z = 0; z < 6; ++z) {
								pRandomPlayer = SelectTarget(
										SELECT_TARGET_RANDOM, 0, 100, true);
								if (!pRandomPlayer
										|| !pRandomPlayer->HasAura(
							...
							if (pRandomPlayer)
								DoCast(pRandomPlayer, SPELL_LEGION_LIGHTNING,
										false);
							else
								sLog->outError(
										"try to cast SPELL_LEGION_LIGHTNING on invalid target");

							Timer[TIMER_LEGION_LIGHTNING] =
									(Phase == PHASE_SACRIFICE) ? 18000 : 30000; // 18 seconds in PHASE_SACRIFICE
					...
					case TIMER_FIRE_BLOOM:
						if (!me->IsNonMeleeSpellCasted(false)) {
							...
							Timer[TIMER_FIRE_BLOOM] =
									(Phase == PHASE_SACRIFICE) ? 25000 : 40000; // 25 seconds in PHASE_SACRIFICE
					...
					case TIMER_SUMMON_SHILEDORB:
						for (uint8 i = 1; i < Phase; ++i) {
							float sx, sy;
							sx = ShieldOrbLocations[0][0]
									+ sin(ShieldOrbLocations[i][0]);
							sy = ShieldOrbLocations[0][1]
									+ sin(ShieldOrbLocations[i][1]);
							me->SummonCreature(CREATURE_SHIELD_ORB, sx, sy,
									SHIELD_ORB_Z, 0,
					...
					case TIMER_DARKNESS: //Phase 3
						if (!me->IsNonMeleeSpellCasted(false)) {
							...
							if (!IsInDarkness) {
								...
								if (Phase == PHASE_SACRIFICE)
									TimerIsDeactivated[TIMER_ARMAGEDDON] =
											false;
								IsInDarkness = true;
							} else {
								Timer[TIMER_DARKNESS] =
										(Phase == PHASE_SACRIFICE) ?
												15000 : urand(40000, 70000);
								IsInDarkness = false;
								DoCastAOE(
										SPELL_DARKNESS_OF_A_THOUSAND_SOULS_DAMAGE);
								DoScriptText(
										RAND(SAY_KJ_DARKNESS1, SAY_KJ_DARKNESS2,
												SAY_KJ_DARKNESS3), me);
							}
					...
					case TIMER_ORBS_EMPOWER: //Phase 3
						if (Creature* pKalec = Unit::GetCreature(*me, pInstance->GetData64(DATA_KALECGOS_KJ))) {
							switch (Phase) {
							case PHASE_SACRIFICE:
								CAST_AI(boss_kalecgos_kj::boss_kalecgos_kjAI, pKalec->AI())->EmpowerOrb(
										true);
								break;
							default:
								CAST_AI(boss_kalecgos_kj::boss_kalecgos_kjAI, pKalec->AI())->EmpowerOrb(
					...
					case TIMER_ARMAGEDDON: //Phase 4
						...
						if (pTarget) {
							float x, y, z;
							pTarget->GetPosition(x, y, z);
							me->SummonCreature(CREATURE_ARMAGEDDON_TARGET, x, y,
									z, 0, TEMPSUMMON_TIMED_DESPAWN, 15000);
						}
...
public:
	mob_hand_of_the_deceiver() :
			CreatureScript("mob_hand_of_the_deceiver") {
	}
	...
	struct mob_hand_of_the_deceiverAI: public ScriptedAI {
		mob_hand_of_the_deceiverAI(Creature* c) :
				ScriptedAI(c) {
			pInstance = c->GetInstanceScript();
		}
		...
		void UpdateAI(const uint32 diff) {
			...
			if (FelfirePortalTimer <= diff) {
				if (Creature* pPortal = DoSpawnCreature(CREATURE_FELFIRE_PORTAL, 0, 0, 0, 0, TEMPSUMMON_TIMED_DESPAWN, 20000)) {
					...
					for (itr = me->getThreatManager().getThreatList().begin();
							itr != me->getThreatManager().getThreatList().end();
							++itr) {
...
public:
	mob_felfire_portal() :
			CreatureScript("mob_felfire_portal") {
	}
	...
	struct mob_felfire_portalAI: public Scripted_NoMovementAI {
		mob_felfire_portalAI(Creature* c) :
		...
		void UpdateAI(const uint32 diff) {
			...
			if (uiSpawnFiendTimer <= diff) {
				if (Creature* pFiend = DoSpawnCreature(CREATURE_VOLATILE_FELFIRE_FIEND, 0, 0, 0, 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 20000))
					pFiend->AddThreat(SelectUnit(SELECT_TARGET_RANDOM, 0),
							100000.0f);
				uiSpawnFiendTimer = urand(4000, 8000);
			} else
				uiSpawnFiendTimer -= diff;
		}
	};
};
...
public:
	mob_volatile_felfire_fiend() :
			CreatureScript("mob_volatile_felfire_fiend") {
	}
	...
	struct mob_volatile_felfire_fiendAI: public ScriptedAI {
		mob_volatile_felfire_fiendAI(Creature* c) :
				ScriptedAI(c) {
		}
...
public:
	mob_sinster_reflection() :
			CreatureScript("mob_sinster_reflection") {
	}
	...
	struct mob_sinster_reflectionAI: public ScriptedAI {
		mob_sinster_reflectionAI(Creature* c) :
				ScriptedAI(c) {
		}
		...
		void UpdateAI(const uint32 diff) {
			...
			case CLASS_WARLOCK:
				...
				if (uiTimer[2] <= diff) {
					DoCast(SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true),
							SPELL_SR_CURSE_OF_AGONY, true);
