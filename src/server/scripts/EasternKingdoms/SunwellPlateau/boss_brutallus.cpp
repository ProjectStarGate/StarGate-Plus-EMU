	...
	struct boss_brutallusAI: public ScriptedAI {
		...
		void JustDied(Unit* /*Killer*/) {
			...
			if (pInstance) {
				...
				me->SummonCreature(FELMYST, x, y, z + 30, me->GetOrientation(),
		...
		void StartIntro() {
			...
			Creature *Madrigosa = Unit::GetCreature(*me,
					pInstance ? pInstance->GetData64(DATA_MADRIGOSA) : 0);
			if (Madrigosa) {
				Madrigosa->Respawn();
		...
		void DoIntro() {
			...
			case 4:
				...
				break;
			case 5:
				Madrigosa->CastSpell(me, SPELL_INTRO_ENCAPSULATE_CHANELLING,
						false);
				...
				if (IntroPhase == 3 + 1) {
					if (IntroFrostBoltTimer <= diff) {
						if (Creature *Madrigosa = Unit::GetCreature(*me, pInstance ? pInstance->GetData64(DATA_MADRIGOSA) : 0)) {
							Madrigosa->CastSpell(me, SPELL_INTRO_FROSTBOLT,
									true);
