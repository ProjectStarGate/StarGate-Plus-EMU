...
// after felmyst's death spawned and say this
};
...
enum PhaseFelmyst {
	PHASE_NONE, PHASE_GROUND, PHASE_FLIGHT,
};
	...
	struct boss_felmystAI: public ScriptedAI {
		...
		void SpellHit(Unit *caster, const SpellEntry *spell) {
			...
			if (spell->Id == SPELL_FOG_INFORM) {
				...
				me->DealDamage(caster, caster->GetHealth(), NULL, DIRECT_DAMAGE,
		...
		void HandleFlightSequence() {
			...
			case 0:
				...
				break;
			case 1:
				me->GetMotionMaster()->MovePoint(0, me->GetPositionX() + 1,
						me->GetPositionY(), me->GetPositionZ() + 10);
				break;
			case 2: {
				Unit *pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 150,
						true);
				if (!pTarget)
					pTarget = Unit::GetUnit(
							*me,
							pInstance ?
									pInstance->GetData64(DATA_PLAYER_GUID) : 0);
				...
				Creature* Vapor = me->SummonCreature(MOB_VAPOR,
						pTarget->GetPositionX() - 5 + rand() % 10,
						pTarget->GetPositionY() - 5 + rand() % 10,
						pTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN,
						9000);
				if (Vapor) {
			...
			case 3: {
				...
				Creature* pVapor = me->SummonCreature(MOB_VAPOR,
						pTarget->GetPositionX() - 5 + rand() % 10,
						pTarget->GetPositionY() - 5 + rand() % 10,
						pTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN,
						9000);
		...
		void UpdateAI(const uint32 diff) {
			...
			if (phase == PHASE_GROUND) {
				...
				case EVENT_ENCAPSULATE:
					...
					events.ScheduleEvent(EVENT_ENCAPSULATE,
							urand(25000, 30000));
					break;
				case EVENT_FLIGHT:
		...
		void DespawnSummons(uint32 entry) {
			...
			for (std::list<Creature*>::const_iterator i = templist.begin();
				...
				if (entry == MOB_VAPOR_TRAIL && phase == PHASE_FLIGHT) {
					(*i)->GetPosition(x, y, z);
					me->SummonCreature(MOB_DEAD, x, y, z, 0,
...
public:
	mob_felmyst_vapor() :
			CreatureScript("mob_felmyst_vapor") {
	}
	...
	struct mob_felmyst_vaporAI: public ScriptedAI {
		mob_felmyst_vaporAI(Creature *c) :
				ScriptedAI(c) {
...
public:
	mob_felmyst_trail() :
			CreatureScript("mob_felmyst_trail") {
	}
	...
	struct mob_felmyst_trailAI: public ScriptedAI {
		mob_felmyst_trailAI(Creature *c) :
				ScriptedAI(c) {
			me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
