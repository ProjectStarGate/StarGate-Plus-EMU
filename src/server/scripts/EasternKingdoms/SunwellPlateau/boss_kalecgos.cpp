	...
	struct boss_kalecgosAI: public ScriptedAI {
		...
		void UpdateAI(const uint32 diff) {
			if (TalkTimer) {
				...
				if (CheckTimer <= diff) {
					if (me->GetDistance(CENTER_X, CENTER_Y, DRAGON_REALM_Z)
							>= 75) {
					...
					if (!isBanished && HealthBelowPct(1)) {
						if (Creature* Sath = Unit::GetCreature(*me, SathGUID)) {
							...
							sLog->outError(
									"TSCR: Didn't find Shathrowar. Kalecgos event reseted.");
							EnterEvadeMode();
							return;
						}
					}
				...
				if (SpectralBlastTimer <= diff) {
					...
					for (std::list<HostileReference*>::const_iterator itr =
							m_threatlist.begin(); itr != m_threatlist.end();
							++itr)
						if ((*itr)->getTarget()
								&& (*itr)->getTarget()->GetTypeId()
										== TYPEID_PLAYER
								&& (*itr)->getTarget()->GetGUID()
										!= me->getVictim()->GetGUID()
								&& !(*itr)->getTarget()->HasAura(
										AURA_SPECTRAL_EXHAUSTION)
								&& (*itr)->getTarget()->GetPositionZ()
										> me->GetPositionZ() - 5)
...
public:
	kalecgos_teleporter() :
			GameObjectScript("kalecgos_teleporter") {
	}
	...
	bool OnGossipHello(Player* pPlayer, GameObject* pGo) {
		...
		for (Map::PlayerList::const_iterator i = PlayerList.begin();
				i != PlayerList.end(); ++i) {
			if (i->getSource()
					&& i->getSource()->GetPositionZ() < DEMON_REALM_Z + 5)
				++SpectralPlayers;
		}
	...
	struct boss_sathrovarrAI: public ScriptedAI {
		...
		void KilledUnit(Unit *pTarget) {
			if (pTarget->GetGUID() == KalecGUID) {
				...
				if (Creature *Kalecgos = Unit::GetCreature(*me, KalecgosGUID)) {
					CAST_AI(boss_kalecgos::boss_kalecgosAI, Kalecgos->AI())->TalkTimer =
							1;
					CAST_AI(boss_kalecgos::boss_kalecgosAI, Kalecgos->AI())->isFriendly =
							false;
				}
				EnterEvadeMode();
				return;
		...
		void JustDied(Unit * /*killer*/) {
			...
			me->GetMap()->CreatureRelocation(me, me->GetPositionX(),
					me->GetPositionY(), DRAGON_REALM_Z, me->GetOrientation());
			TeleportAllPlayersBack();
			if (Creature *Kalecgos = Unit::GetCreature(*me, KalecgosGUID)) {
				CAST_AI(boss_kalecgos::boss_kalecgosAI, Kalecgos->AI())->TalkTimer =
						1;
		...
		void TeleportAllPlayersBack() {
			...
			for (Map::PlayerList::const_iterator i = PlayerList.begin();
				...
				if (i->getSource()->GetPositionZ() <= DRAGON_REALM_Z - 5) {
					...
					i->getSource()->TeleportTo(me->GetMap()->GetId(),
							i->getSource()->GetPositionX(),
							i->getSource()->GetPositionY(), DRAGON_REALM_Z + 5,
							i->getSource()->GetOrientation());
				}
			}
		}
				...
				for (std::list<HostileReference*>::const_iterator itr =
					...
					if (Unit* pUnit = Unit::GetUnit(*me, (*itr)->getUnitGuid())) {
						if (pUnit->GetPositionZ() > me->GetPositionZ() + 5) {
							me->getThreatManager().modifyThreatPercent(pUnit,
									-100);
						}
					}
				}
