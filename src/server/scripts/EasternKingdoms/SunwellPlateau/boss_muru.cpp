...
float DarkFiends[8][4] = { { 1819.9f, 609.80f, 69.74f, 1.94f }, { 1829.39f,
		617.89f, 69.73f, 2.61f }, { 1801.98f, 633.62f, 69.74f, 5.71f }, {
...
float Humanoides[6][5] = { { CREATURE_FURY_MAGE, 1780.16f, 666.83f, 71.19f,
		5.21f }, { CREATURE_FURY_MAGE, 1847.93f, 600.30f, 71.30f, 2.57f }, {
	...
	struct boss_entropiusAI: public ScriptedAI {
		...
		void JustSummoned(Creature* summoned) {
			...
			case CREATURE_DARKNESS:
				...
				me->SummonCreature(CREATURE_DARK_FIENDS, x, y, z, o,
						TEMPSUMMON_CORPSE_DESPAWN, 0);
				break;
			}
			summoned->AI()->AttackStart(
					SelectTarget(SELECT_TARGET_RANDOM, 0, 50, true));
				...
				if (Timer[i] <= diff) {
					...
					case TIMER_DARKNESS:
						if (!DarkFiend) {
							...
							DarkFiend = false;
							for (uint8 i = 0; i < 8; ++i)
								me->SummonCreature(CREATURE_DARK_FIENDS,
										DarkFiends[i][0], DarkFiends[i][1],
										DarkFiends[i][2], DarkFiends[i][3],
										TEMPSUMMON_CORPSE_DESPAWN, 0);
					...
					case TIMER_HUMANOIDES:
						for (uint8 i = 0; i < 6; ++i)
							me->SummonCreature(uint32(Humanoides[i][0]),
									Humanoides[i][1], Humanoides[i][2],
									Humanoides[i][3], Humanoides[i][4],
									TEMPSUMMON_CORPSE_DESPAWN, 0);
...
public:
	npc_muru_portal() :
			CreatureScript("npc_muru_portal") {
	}
	...
	struct npc_muru_portalAI: public Scripted_NoMovementAI {
		...
		void UpdateAI(const uint32 diff) {
			if (!SummonSentinel) {
				if (InAction && pInstance
						&& pInstance->GetData(DATA_MURU_EVENT) == NOT_STARTED)
					Reset();
				return;
			}
			if (SummonTimer <= diff) {
		...
		void UpdateAI(const uint32 diff) {
			...
			if (WaitTimer <= diff) {
				if (!InAction) {
					...
					AttackStart(
							SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true));
					InAction = true;
					WaitTimer = 500;
				} else {
...
public:
	npc_void_sentinel() :
			CreatureScript("npc_void_sentinel") {
	}
	...
	struct npc_void_sentinelAI: public ScriptedAI {
		npc_void_sentinelAI(Creature *c) :
				ScriptedAI(c) {
		}
		...
		void JustDied(Unit* /*killer*/) {
			for (uint8 i = 0; i < 8; ++i)
				me->SummonCreature(CREATURE_VOID_SPAWN, me->GetPositionX(),
						me->GetPositionY(), me->GetPositionZ(),
						float(rand() % 6), TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN,
						180000);
		}
...
public:
	...
	struct npc_blackholeAI: public ScriptedAI {
		...
		void UpdateAI(const uint32 diff) {
			if (SpellTimer <= diff) {
				Unit* Victim = Unit::GetUnit(*me,
						pInstance ? pInstance->GetData64(DATA_PLAYER_GUID) : 0);
				switch (NeedForAHack) {
				case 0:
					me->ClearUnitState(UNIT_STAT_STUNNED);
