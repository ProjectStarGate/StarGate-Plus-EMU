	...
	struct boss_sulfuronAI: public ScriptedAI {
		...
		void UpdateAI(const uint32 diff) {
			...
			if (Inspire_Timer <= diff) {
				...
				std::list<Creature*> pList = DoFindFriendlyMissingBuff(45.0f,
						SPELL_INSPIRE);
				if (!pList.empty()) {
			...
			if (Flamespear_Timer <= diff) {
				...
				if (pTarget)
					DoCast(pTarget, SPELL_FLAMESPEAR);
...
public:
	mob_flamewaker_priest() :
			CreatureScript("mob_flamewaker_priest") {
	}
	...
	struct mob_flamewaker_priestAI: public ScriptedAI {
		mob_flamewaker_priestAI(Creature *c) :
				ScriptedAI(c) {
		}
