...
public:
	boss_netherspite() :
			CreatureScript("boss_netherspite") {
	}
	...
	struct boss_netherspiteAI: public ScriptedAI {
		...
		bool IsBetween(WorldObject* u1, WorldObject *pTarget, WorldObject* u2) // the in-line checker
			...
			return (abs((xn - xp) * yh + (yp - yn) * xh - xn * yp + xp * yn)
		...
		void UpdatePortals() // Here we handle the beams' behavior
		{
			for (int j = 0; j < 3; ++j) // j = color
				if (Creature *portal = Unit::GetCreature(*me, PortalGUID[j])) {
					...
					if (Map* map = me->GetMap()) {
						...
						for (Map::PlayerList::const_iterator i =
							...
							if (p && p->isAlive() // alive
									&& (!pTarget
											|| pTarget->GetDistance2d(portal)
													> p->GetDistance2d(portal)) // closer than current best
					...
					if (j == RED_PORTAL && me->getVictim() != pTarget
							&& pTarget->GetTypeId() == TYPEID_PLAYER)
						me->getThreatManager().addThreat(pTarget,
								100000.0f + DoGetThreat(me->getVictim()));
				}
		}
		...
		void UpdateAI(const uint32 diff) {
			...
			if (VoidZoneTimer <= diff) {
				DoCast(SelectTarget(SELECT_TARGET_RANDOM, 1, 45, true),
						SPELL_VOIDZONE, true);
				VoidZoneTimer = 15000;
			} else
				VoidZoneTimer -= diff;
