...
float Locations[4][3] = { { -10991.0f, -1884.33f, 0.614315f }, { -10989.4f,
		-1885.88f, 0.904913f }, { -10978.1f, -1887.07f, 2.035550f }, {
		-10975.9f, -1885.81f, 2.253890f }, };

const uint32 Adds[6] = { 17007, 19872, 19873, 19874, 19875, 19876, };

class boss_moroes: public CreatureScript {
public:
	boss_moroes() :
			CreatureScript("boss_moroes") {
	}
		...
		void SpawnAdds() {
			...
			if (isAddlistEmpty()) {
				...
				while (AddList.size() > 4)
					AddList.erase(
							(AddList.begin()) + (rand() % AddList.size()));
				...
				for (std::vector<uint32>::const_iterator itr = AddList.begin();
					...
					pCreature = me->SummonCreature(entry, Locations[i][0],
							Locations[i][1], POS_Z, Locations[i][2],
				...
				for (uint8 i = 0; i < 4; ++i) {
					Creature *pCreature = me->SummonCreature(AddId[i],
							Locations[i][0], Locations[i][1], POS_Z,
							Locations[i][2], TEMPSUMMON_CORPSE_TIMED_DESPAWN,
							10000);
					if (pCreature) {
						AddGUID[i] = pCreature->GetGUID();
					}
		...
		void UpdateAI(const uint32 diff) {
			...
			if (!Enrage) {
				...
				if (Blind_Timer <= diff) {
					...
					SelectTargetList(pTargets, 5, SELECT_TARGET_RANDOM,
							me->GetMeleeReach() * 5, true);
					for (std::list<Unit*>::const_iterator i = pTargets.begin();
							i != pTargets.end(); ++i)
...
struct boss_moroes_guestAI: public ScriptedAI {
	...
	boss_moroes_guestAI(Creature* c) :
			ScriptedAI(c) {
		for (uint8 i = 0; i < 4; ++i)
			GuestGUID[i] = 0;
	...
	void AcquireGUID() {
		...
		if (Moroes) {
			for (uint8 i = 0; i < 4; ++i) {
				uint64 GUID =
						CAST_AI(boss_moroes::boss_moroesAI, Moroes->AI())->AddGUID[i];
				if (GUID)
					GuestGUID[i] = GUID;
			}
		}
...
public:
	boss_baroness_dorothea_millstipe() :
			CreatureScript("boss_baroness_dorothea_millstipe") {
	}
	...
	struct boss_baroness_dorothea_millstipeAI: public boss_moroes_guestAI {
		//Shadow Priest
		boss_baroness_dorothea_millstipeAI(Creature *c) :
				boss_moroes_guestAI(c) {
		}
...
public:
	boss_baron_rafe_dreuger() :
			CreatureScript("boss_baron_rafe_dreuger") {
	}
	...
	struct boss_baron_rafe_dreugerAI: public boss_moroes_guestAI {
		//Retr Pally
		boss_baron_rafe_dreugerAI(Creature *c) :
				boss_moroes_guestAI(c) {
		}
...
public:
	boss_lady_catriona_von_indi() :
			CreatureScript("boss_lady_catriona_von_indi") {
	}
	...
	struct boss_lady_catriona_von_indiAI: public boss_moroes_guestAI {
		//Holy Priest
		boss_lady_catriona_von_indiAI(Creature *c) :
				boss_moroes_guestAI(c) {
		}
...
public:
	boss_lady_keira_berrybuck() :
			CreatureScript("boss_lady_keira_berrybuck") {
	}
	...
	struct boss_lady_keira_berrybuckAI: public boss_moroes_guestAI {
		//Holy Pally
		boss_lady_keira_berrybuckAI(Creature *c) :
				boss_moroes_guestAI(c) {
		}
...
public:
	boss_lord_robin_daris() :
			CreatureScript("boss_lord_robin_daris") {
	}
	...
	struct boss_lord_robin_darisAI: public boss_moroes_guestAI {
		//Arms Warr
		boss_lord_robin_darisAI(Creature *c) :
				boss_moroes_guestAI(c) {
		}
...
public:
	boss_lord_crispin_ference() :
			CreatureScript("boss_lord_crispin_ference") {
	}
	...
	struct boss_lord_crispin_ferenceAI: public boss_moroes_guestAI {
		//Arms Warr
		boss_lord_crispin_ferenceAI(Creature *c) :
				boss_moroes_guestAI(c) {
		}
