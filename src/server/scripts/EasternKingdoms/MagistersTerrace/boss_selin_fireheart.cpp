...
public:
	...
	struct boss_selin_fireheartAI: public ScriptedAI {
		boss_selin_fireheartAI(Creature* c) :
				ScriptedAI(c) {
			...
			if (pInstance) {
				...
				for (uint8 i = 0; i < size; ++i) {
					...
					sLog->outDebug(
							LOG_FILTER_TSCR,
							"TSCR: Selin: Adding Fel Crystal " UI64FMTD " to list",
							guid);
		...
		void Reset() {
			if (pInstance) {
				...
				for (std::list<uint64>::const_iterator itr = Crystals.begin();
						itr != Crystals.end(); ++itr) {
					...
					if (pUnit) {
						...
						pUnit->SetFlag(UNIT_FIELD_FLAGS,
								UNIT_FLAG_NOT_SELECTABLE);
					}
				}

				pInstance->HandleGameObject(
						pInstance->GetData64(DATA_SELIN_ENCOUNTER_DOOR), true);
		...
		void SelectNearestCrystal() {
			...
			if (CrystalChosen) {
				...
				CrystalChosen->CastSpell(CrystalChosen,
						SPELL_FEL_CRYSTAL_COSMETIC, true);

				float x, y, z; // coords that we move to, close to the crystal.
				CrystalChosen->GetClosePoint(x, y, z, me->GetObjectSize(),
						CONTACT_DISTANCE);
		...
		void EnterCombat(Unit* /*who*/) {
			...
			if (pInstance)
				pInstance->HandleGameObject(
						pInstance->GetData64(DATA_SELIN_ENCOUNTER_DOOR), false);
		...
		void MovementInform(uint32 type, uint32 id) {
			if (type == POINT_MOTION_TYPE && id == 1) {
				...
				if (CrystalChosen && CrystalChosen->isAlive()) {
					...
					sLog->outError(
							"TSCR: Selin Fireheart unable to drain crystal as the crystal is either dead or despawned");
					DrainingCrystal = false;
				}
			}
		}
		...
		void JustDied(Unit* /*killer*/) {
			...
			pInstance->HandleGameObject(
					pInstance->GetData64(DATA_SELIN_ENCOUNTER_DOOR), true); // Open the encounter door
			pInstance->HandleGameObject(pInstance->GetData64(DATA_SELIN_DOOR),
					true); // Open the door leading further in
		...
		void UpdateAI(const uint32 diff) {
			...
			if (!DrainingCrystal) {
				...
				if (maxPowerMana
						&& ((me->GetPower(POWER_MANA) * 100 / maxPowerMana) < 10)) {
					if (DrainLifeTimer <= diff) {
						DoCast(SelectUnit(SELECT_TARGET_RANDOM, 0),
								SPELL_DRAIN_LIFE);
					...
					if (IsHeroic()) {
						if (DrainManaTimer <= diff) {
							DoCast(SelectUnit(SELECT_TARGET_RANDOM, 1),
									SPELL_DRAIN_MANA);
...
public:
	mob_fel_crystal() :
			CreatureScript("mob_fel_crystal") {
	}
	...
	struct mob_fel_crystalAI: public ScriptedAI {
		...
		void JustDied(Unit* /*killer*/) {
			if (InstanceScript* pInstance = me->GetInstanceScript()) {
				Creature* Selin = (Unit::GetCreature(*me,
						pInstance->GetData64(DATA_SELIN)));
				if (Selin && Selin->isAlive()) {
					if (CAST_AI(boss_selin_fireheart::boss_selin_fireheartAI, Selin->AI())->CrystalGUID
							== me->GetGUID()) {
						// Set this to false if we are the Creature that Selin is draining so his AI flows properly
						CAST_AI(boss_selin_fireheart::boss_selin_fireheartAI, Selin->AI())->DrainingCrystal =
								false;
						CAST_AI(boss_selin_fireheart::boss_selin_fireheartAI, Selin->AI())->IsDraining =
								false;
						CAST_AI(boss_selin_fireheart::boss_selin_fireheartAI, Selin->AI())->EmpowerTimer =
								10000;
						if (Selin->getVictim()) {
							Selin->AI()->AttackStart(Selin->getVictim());
							Selin->GetMotionMaster()->MoveChase(
									Selin->getVictim());
						}
					}
				}
