...
static SpiritInfoStruct SpiritInfo[4] = { { 23878, 147.87f, 706.51f, 45.11f,
		3.04f }, { 23880, 88.95f, 705.49f, 45.11f, 6.11f }, { 23877, 137.23f,
		725.98f, 45.11f, 3.71f }, { 23879, 104.29f, 726.43f, 45.11f, 5.43f } };
...
static TransformStruct Transform[4] = { { SOUND_TRANSFORM_TO_BEAR,
		YELL_TRANSFORM_TO_BEAR, SPELL_SHAPE_OF_THE_BEAR, SPELL_WHIRLWIND }, {
		SOUND_TRANSFORM_TO_EAGLE, YELL_TRANSFORM_TO_EAGLE,
		SPELL_SHAPE_OF_THE_EAGLE, SPELL_SHAPE_OF_THE_BEAR }, {
		SOUND_TRANSFORM_TO_LYNX, YELL_TRANSFORM_TO_LYNX,
		SPELL_SHAPE_OF_THE_LYNX, SPELL_SHAPE_OF_THE_EAGLE }, {
		SOUND_TRANSFORM_TO_DRAGONHAWK, YELL_TRANSFORM_TO_DRAGONHAWK,
		SPELL_SHAPE_OF_THE_DRAGONHAWK, SPELL_SHAPE_OF_THE_LYNX } };
...
public:
	...
	struct boss_zuljinAI: public ScriptedAI {
		...
		void SpawnAdds() {
			...
			for (uint8 i = 0; i < 4; ++i) {
				pCreature = me->SummonCreature(SpiritInfo[i].entry,
						SpiritInfo[i].x, SpiritInfo[i].y, SpiritInfo[i].z,
						SpiritInfo[i].orient, TEMPSUMMON_DEAD_DESPAWN, 0);
				if (pCreature) {
					pCreature->CastSpell(pCreature, SPELL_SPIRIT_AURA, true);
					pCreature->SetFlag(UNIT_FIELD_FLAGS,
		...
		void EnterPhase(uint32 NextPhase) {
			...
			case 4:
				...
				me->MonsterYell(Transform[Phase].text.c_str(), LANG_UNIVERSAL,
						NULL);
				...
				if (NextPhase == 2) {
					...
					for (uint8 i = 0; i < 4; ++i) {
						Creature* Vortex = DoSpawnCreature(
								CREATURE_FEATHER_VORTEX, 0, 0, 0, 0,
								TEMPSUMMON_CORPSE_DESPAWN, 0);
						if (Vortex) {
							Vortex->CastSpell(Vortex, SPELL_CYCLONE_PASSIVE,
									true);
							Vortex->CastSpell(Vortex, SPELL_CYCLONE_VISUAL,
									true);
							Vortex->SetFlag(UNIT_FIELD_FLAGS,
									UNIT_FLAG_NOT_SELECTABLE);
					...
					} else if (!Claw_Rage_Timer) // do not do this when Lynx_Rush
					{
						if (Claw_Loop_Timer <= diff) {
							...
							if (!pTarget || !pTarget->isTargetableForAttack())
								pTarget = Unit::GetUnit(*me, TankGUID);
							if (!pTarget || !pTarget->isTargetableForAttack())
								pTarget = SelectUnit(SELECT_TARGET_RANDOM, 0);
							if (pTarget) {
								AttackStart(pTarget);
								if (me->IsWithinMeleeRange(pTarget)) {
									DoCast(pTarget, SPELL_CLAW_RAGE_DAMAGE,
											true);
									++Claw_Counter;
									if (Claw_Counter == 12) {
...
public:
	...
	struct mob_zuljin_vortexAI: public ScriptedAI {
		mob_zuljin_vortexAI(Creature *c) :
				ScriptedAI(c) {
		}
