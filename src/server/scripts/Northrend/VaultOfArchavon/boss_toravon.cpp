...
// Events boss
enum Events {
	EVENT_FREEZING_GROUND = 1, EVENT_FROZEN_ORB = 2, EVENT_WHITEOUT = 3,
...
public:
	mob_frost_warder() :
			CreatureScript("mob_frost_warder") {
	}

	struct mob_frost_warderAI: public ScriptedAI {
		mob_frost_warderAI(Creature* c) :
				ScriptedAI(c) {
		}
...
public:
	mob_frozen_orb() :
			CreatureScript("mob_frozen_orb") {
	}

	struct mob_frozen_orbAI: public ScriptedAI {
		mob_frozen_orbAI(Creature* creature) :
				ScriptedAI(creature) {
		}
...
public:
	mob_frozen_orb_stalker() :
			CreatureScript("mob_frozen_orb_stalker") {
	}

	struct mob_frozen_orb_stalkerAI: public Scripted_NoMovementAI {
		mob_frozen_orb_stalkerAI(Creature* creature) :
		...
		void UpdateAI(const uint32 /*diff*/) {
			...
			spawned = true;
			Unit* toravon = me->GetCreature(*me,
					instance ? instance->GetData64(DATA_TORAVON) : 0);
			if (!toravon)
				return;
			...
			for (uint8 i = 0; i < num_orbs; ++i) {
				Position pos;
				me->GetNearPoint(toravon, pos.m_positionX, pos.m_positionY,
						pos.m_positionZ, 0.0f, 10.0f, 0.0f);
