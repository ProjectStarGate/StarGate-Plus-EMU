...
{
	NPC_APPLEBOUGH_A = 29547, NPC_SWEETBERRY_H = 29715,
};
...
public:
	...
	struct npc_mageguard_dalaranAI: public Scripted_NoMovementAI {
		npc_mageguard_dalaranAI(Creature* creature) :
			...
			creature->ApplySpellImmune(0, IMMUNITY_DAMAGE, SPELL_SCHOOL_NORMAL,
					true);
			creature->ApplySpellImmune(0, IMMUNITY_DAMAGE,
		...
		void MoveInLineOfSight(Unit* who) {
			...
			case 29254:
				if (player->GetTeam() == HORDE
						&& !(player->HasAura(SPELL_DISGUISE_H_M)
								|| player->HasAura(SPELL_DISGUISE_H_F))) // Horde unit found in Alliance area
						{
					if (GetClosestCreatureWithEntry(me, NPC_APPLEBOUGH_A,
							32.0f)) {
			...
			case 29255:
				if (player->GetTeam() == ALLIANCE
						&& !(player->HasAura(SPELL_DISGUISE_A_M)
								|| player->HasAura(SPELL_DISGUISE_A_F))) // Alliance unit found in Horde area
						{
					if (GetClosestCreatureWithEntry(me, NPC_SWEETBERRY_H,
							32.0f)) {
						if (me->isInBackInMap(who, 12.0f)) // In my line of sight, "outdoors", and behind me
...
public:
	npc_hira_snowdawn() :
			CreatureScript("npc_hira_snowdawn") {
	}
