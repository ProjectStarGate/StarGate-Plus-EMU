...
static Summons Group[] = { {
		...
		NPC_PHANTASMAL_FIRE } }, { { NPC_PHANTASMAL_OGRE, NPC_PHANTASMAL_OGRE,
		NPC_PHANTASMAL_NAGAL, NPC_PHANTASMAL_MURLOC } } };

static uint32 TeleportSpells[] = { SPELL_SUMMON_MENAGERIE,
		SPELL_SUMMON_MENAGERIE_2, SPELL_SUMMON_MENAGERIE_3 };

static int32 SayAggro[] = { SAY_AGGRO_1, SAY_AGGRO_2, SAY_AGGRO_3, SAY_AGGRO_4 };
...
public:
	...
	struct boss_uromAI: public BossAI {
		boss_uromAI(Creature* pCreature) :
				BossAI(pCreature, DATA_UROM_EVENT) {
		}
		...
		void EnterCombat(Unit* /*pWho*/) {
			...
			if (instance && instance->GetData(DATA_UROM_PLATAFORM) != 3)
				instance->SetData(DATA_UROM_PLATAFORM,
						instance->GetData(DATA_UROM_PLATAFORM) + 1);
		}
		...
		void SetGroups() {
			...
			while (uiGroup[0] == uiGroup[1] || uiGroup[0] == uiGroup[2]
					|| uiGroup[1] == uiGroup[2]) {
				uiGroup[0] = urand(0, 2);
				uiGroup[1] = urand(0, 2);
				uiGroup[2] = urand(0, 2);
		...
		void SummonGroups() {
			...
			for (uint8 uiI = 0; uiI < 4; uiI++) {
				SetPosition(uiI);
				me->SummonCreature(
						Group[uiGroup[instance->GetData(DATA_UROM_PLATAFORM)]].uiEntry[uiI],
						x, y, me->GetPositionZ(), me->GetOrientation());
			}
		}
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (uiTeleportTimer <= uiDiff) {
				...
				uiTeleportTimer -= uiDiff;

			if (bCanCast
					&& !me->FindCurrentSpellBySpellId(
				...
				if (uiCastArcaneExplosionTimer <= uiDiff) {
					...
					uiCastArcaneExplosionTimer -= uiDiff;
			}
			...
			if (bCanGoBack) {
				if (uiArcaneExplosionTimer <= uiDiff) {
					...
					me->NearTeleportTo(pPos.GetPositionX(), pPos.GetPositionY(),
							pPos.GetPositionZ(), pPos.GetOrientation());
