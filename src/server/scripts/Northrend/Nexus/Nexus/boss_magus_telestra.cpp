...
enum Creatures {
	MOB_FIRE_MAGUS = 26928, MOB_FROST_MAGUS = 26930, MOB_ARCANE_MAGUS = 26929
};
enum Yells {
	SAY_AGGRO = -1576000,
	SAY_KILL = -1576001,
...
enum Achievements {
	ACHIEV_SPLIT_PERSONALITY = 2150, ACHIEV_TIMER = 5 * IN_MILLISECONDS
};
...
public:
	boss_magus_telestra() :
			CreatureScript("boss_magus_telestra") {
	}
	...
	struct boss_magus_telestraAI: public BossAI {
		boss_magus_telestraAI(Creature* c) :
				BossAI(c, DATA_MAGUS_TELESTRA_EVENT) {
		...
		uint64 SplitPersonality(uint32 entry) {
			if (Creature* Summoned = me->SummonCreature(entry, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 1*IN_MILLISECONDS)) {
				switch (entry) {
				case MOB_FIRE_MAGUS: {
					Summoned->CastSpell(Summoned, SPELL_FIRE_MAGUS_VISUAL,
							false);
					break;
				}
				case MOB_FROST_MAGUS: {
					Summoned->CastSpell(Summoned, SPELL_FROST_MAGUS_VISUAL,
							false);
					break;
				}
				case MOB_ARCANE_MAGUS: {
					Summoned->CastSpell(Summoned, SPELL_ARCANE_MAGUS_VISUAL,
							false);
					break;
				}
				}
			...
			if ((Phase == 1) || (Phase == 3)) {
				...
				if (bFireMagusDead && bFrostMagusDead && bArcaneMagusDead) {
					...
					me->GetMap()->CreatureRelocation(me,
							CenterOfRoom.GetPositionX(),
							CenterOfRoom.GetPositionY(),
							CenterOfRoom.GetPositionZ(),
							CenterOfRoom.GetOrientation());
