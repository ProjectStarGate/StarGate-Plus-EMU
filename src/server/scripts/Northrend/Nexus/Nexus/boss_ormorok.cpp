...
public:
	...
	struct boss_ormorokAI: public BossAI {
		boss_ormorokAI(Creature *c) :
				BossAI(c, DATA_ORMOROK_EVENT) {
		...
		void UpdateAI(const uint32 diff) {
			...
			if (bCrystalSpikes) {
				if (uiCrystalSpikesTimer2 <= diff) {
					fSpikeXY[0][0] = fBaseX
							+ (SPIKE_DISTANCE * uiCrystalSpikesCount
									* cos(fBaseO));
					fSpikeXY[0][1] = fBaseY
							+ (SPIKE_DISTANCE * uiCrystalSpikesCount
									* sin(fBaseO));
					...
					fSpikeXY[3][1] = fBaseY
							- (SPIKE_DISTANCE * uiCrystalSpikesCount
									* sin(fBaseO - (M_PI / 2)));
					for (uint8 i = 0; i < 4; ++i)
						me->SummonCreature(MOB_CRYSTAL_SPIKE, fSpikeXY[i][0],
								fSpikeXY[i][1], fBaseZ, 0,
								TEMPSUMMON_TIMED_DESPAWN, 7 * IN_MILLISECONDS);
					if (++uiCrystalSpikesCount >= 13)
						bCrystalSpikes = false;
					uiCrystalSpikesTimer2 = 200;
				} else
					uiCrystalSpikesTimer2 -= diff;
			}
			...
			if (uiCrystalSpikesTimer <= diff) {
				...
				uiCrystalSpikesTimer -= diff;

			if (IsHeroic() && (uiSummonCrystallineTanglerTimer <= diff)) {
				Creature* Crystalline_Tangler = me->SummonCreature(
						MOB_CRYSTALLINE_TANGLER, me->GetPositionX(),
				...
				if (Crystalline_Tangler) {
					...
					for (uint8 j = 1; j <= 4; j++) {
						...
						for (;
								...
								++i) {
							Unit* pTemp = Unit::GetUnit((*me),
									(*i)->getUnitGuid());
							if (pTemp && pTemp->GetTypeId() == TYPEID_PLAYER
									&& pTemp->getClass() == Healer) {
								pTarget = pTemp;
								break;
							}
						}
					...
					if (pTarget) {
						...
						Crystalline_Tangler->getThreatManager().addThreat(
								pTarget, 1000000000.0f);
					}
				}
				uiSummonCrystallineTanglerTimer = 17 * IN_MILLISECONDS;
			} else
				uiSummonCrystallineTanglerTimer -= diff;
...
public:
	mob_crystal_spike() :
			CreatureScript("mob_crystal_spike") {
	}
	...
	struct mob_crystal_spikeAI: public Scripted_NoMovementAI {
		mob_crystal_spikeAI(Creature *c) :
...
public:
	mob_crystalline_tangler() :
			CreatureScript("mob_crystalline_tangler") {
	}
	...
	struct mob_crystalline_tanglerAI: public ScriptedAI {
		mob_crystalline_tanglerAI(Creature *c) :
				ScriptedAI(c) {
		}
