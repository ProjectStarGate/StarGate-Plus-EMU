...
public:
	boss_keristrasza() :
			CreatureScript("boss_keristrasza") {
	}
	...
	struct boss_keristraszaAI: public BossAI {
		boss_keristraszaAI(Creature *c) :
				BossAI(c, DATA_KERISTRASZA_EVENT) {
		...
		void Reset() {
			...
			uiCrystalChainsCrystalizeTimer = DUNGEON_MODE(30 * IN_MILLISECONDS,
					11 * IN_MILLISECONDS);
...
public:
	containment_sphere() :
			GameObjectScript("containment_sphere") {
	}

	bool OnGossipHello(Player * /*pPlayer*/, GameObject *pGO) {
		InstanceScript *pInstance = pGO->GetInstanceScript();

		Creature *pKeristrasza = Unit::GetCreature(*pGO,
				pInstance ? pInstance->GetData64(DATA_KERISTRASZA) : 0);
		if (pKeristrasza && pKeristrasza->isAlive()) {
			// maybe these are hacks :(
			pGO->SetFlag(GAMEOBJECT_FLAGS, GO_FLAG_UNK1);
			pGO->SetGoState(GO_STATE_ACTIVE);

			CAST_AI(boss_keristrasza::boss_keristraszaAI, pKeristrasza->AI())->CheckContainmentSpheres(
					true);
		}
		return true;
	}
