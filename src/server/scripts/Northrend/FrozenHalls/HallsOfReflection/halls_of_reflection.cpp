...
const Position HallsofReflectionLocs[] = { { 5283.234863f, 1990.946777f,
		707.695679f, 0.929097f }, // 2 Loralen Follows
...
const Position MoveThronePos = { 5306.952148f, 1998.499023f, 709.341431f,
		1.277278f }; // Jaina/Sylvanas walks to throne
const Position UtherSpawnPos = { 5308.310059f, 2003.857178f, 709.341431f,
		4.650315f };
const Position LichKingSpawnPos = { 5362.917480f, 2062.307129f, 707.695374f,
		3.945812f };
const Position LichKingMoveThronePos = { 5312.080566f, 2009.172119f,
		709.341431f, 3.973301f }; // Lich King walks to throne
const Position LichKingMoveAwayPos = { 5400.069824f, 2102.7131689f, 707.69525f,
		0.843803f }; // Lich King walks away
...
public:
	npc_jaina_or_sylvanas_hor(bool isSylvana, const char* name) :
			CreatureScript(name), m_isSylvana(isSylvana) {
	}
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature) {
		...
		QuestStatus status = pPlayer->GetQuestStatus(
				m_isSylvana ?
	...
	struct npc_jaina_or_sylvanas_horAI: public ScriptedAI {
		npc_jaina_or_sylvanas_horAI(Creature *pCreature) :
		...
		void UpdateAI(const uint32 diff) {
			...
			case EVENT_INTRO_A2_3:
				...
				pInstance->HandleGameObject(
						pInstance->GetData64(DATA_FROSTMOURNE), true);
			...
			case EVENT_INTRO_LK_1:
				...
				if (Creature* pLichKing = me->SummonCreature(NPC_LICH_KING_EVENT, LichKingSpawnPos, TEMPSUMMON_MANUAL_DESPAWN)) {
					pLichKing->GetMotionMaster()->MovePoint(0,
...
public:
	npc_ghostly_priest() :
			CreatureScript("npc_ghostly_priest") {
	}
	...
	struct npc_ghostly_priestAI: public ScriptedAI {
		npc_ghostly_priestAI(Creature *c) :
				ScriptedAI(c) {
		}
...
public:
	npc_phantom_mage() :
			CreatureScript("npc_phantom_mage") {
	}
...
public:
	npc_phantom_hallucination() :
			CreatureScript("npc_phantom_hallucination") {
	}
...
public:
	npc_shadowy_mercenary() :
			CreatureScript("npc_shadowy_mercenary") {
	}
...
public:
	npc_spectral_footman() :
			CreatureScript("npc_spectral_footman") {
	}
	...
	struct npc_spectral_footmanAI: public ScriptedAI {
		npc_spectral_footmanAI(Creature *c) :
				ScriptedAI(c) {
		}
...
public:
	npc_tortured_rifleman() :
			CreatureScript("npc_tortured_rifleman") {
	}
	...
	struct npc_tortured_riflemanAI: public ScriptedAI {
		npc_tortured_riflemanAI(Creature *c) :
				ScriptedAI(c) {
		}
