...
public:
	...
	struct boss_falricAI: public boss_horAI {
		boss_falricAI(Creature *pCreature) :
				boss_horAI(pCreature, DATA_FALRIC_EVENT) {
		...
		void UpdateAI(const uint32 diff) {
			...
			case EVENT_DEFILING_HORROR:
				...
				events.ScheduleEvent(EVENT_DEFILING_HORROR,
						urand(25000, 45000)); // TODO adjust timer.
				break;
			}
