...
enum Spells {
	...
	SPELL_PURPLE_BANISH_VISUAL = 68862
...
public:
	...
	struct boss_bronjahmAI: public BossAI {
		...
		void Reset() {
			...
			events.ScheduleEvent(EVENT_CORRUPT_SOUL, urand(25000, 35000), 0,
					PHASE_1);
		...
		void DamageTaken(Unit* /*attacker*/, uint32& damage) {
			if (events.GetPhaseMask() & (1 << PHASE_1) && !HealthAbovePct(30)) {
				...
				events.ScheduleEvent(EVENT_FEAR, urand(12000, 16000), 0,
						PHASE_2);
		...
		void JustSummoned(Creature* summon) {
			...
			summon->GetMotionMaster()->MoveFollow(me, me->GetObjectSize(),
					0.0f);
		...
		void UpdateAI(const uint32 diff) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_CORRUPT_SOUL:
					...
					events.ScheduleEvent(EVENT_CORRUPT_SOUL,
							urand(25000, 35000), 0, PHASE_1);
					break;
				...
				case EVENT_FEAR:
					if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 0.0f, true))
						me->CastCustomSpell(SPELL_FEAR, SPELLVALUE_MAX_TARGETS,
								1, target, false);
					events.ScheduleEvent(EVENT_FEAR, urand(8000, 12000), 0,
							PHASE_2);
					break;
				default:
					break;
...
public:
	spell_bronjahm_magic_bane() :
			SpellScriptLoader("spell_bronjahm_magic_bane") {
	}
	...
	class spell_bronjahm_magic_bane_SpellScript: public SpellScript {
		...
		void RecalculateDamage() {
			...
			const int32 maxDamage =
					GetCaster()->GetMap()->GetSpawnMode() == 1 ? 15000 : 10000;
		...
		void Register() {
			OnHit +=
					SpellHitFn(spell_bronjahm_magic_bane_SpellScript::RecalculateDamage);
		}
	};
...
public:
	...
	class spell_bronjahm_consume_soul_SpellScript: public SpellScript {
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_bronjahm_consume_soul_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
		}
	};
...
public:
	spell_bronjahm_soulstorm_channel() :
			SpellScriptLoader("spell_bronjahm_soulstorm_channel") {
	}
	...
	class spell_bronjahm_soulstorm_channel_AuraScript: public AuraScript {
		...
		void Register() {
			OnEffectPeriodic +=
					AuraEffectPeriodicFn(spell_bronjahm_soulstorm_channel_AuraScript::HandlePeriodicTick, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
		}
	};
...
public:
	...
	class spell_bronjahm_soulstorm_visual_AuraScript: public AuraScript {
		...
		void Register() {
			OnEffectPeriodic +=
					AuraEffectPeriodicFn(spell_bronjahm_soulstorm_visual_AuraScript::HandlePeriodicTick, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
		}
	};
...
class DistanceCheck {
public:
	explicit DistanceCheck(Unit* _caster) :
			caster(_caster) {
	}
...
public:
	...
	class spell_bronjahm_soulstorm_targeting_SpellScript: public SpellScript {
		...
		void Register() {
			OnUnitTargetSelect +=
					SpellUnitTargetFn(spell_bronjahm_soulstorm_targeting_SpellScript::FilterTargetsInitial, EFFECT_1, TARGET_UNIT_AREA_ENEMY_DST);
			OnUnitTargetSelect +=
					SpellUnitTargetFn(spell_bronjahm_soulstorm_targeting_SpellScript::FilterTargetsSubsequent, EFFECT_2, TARGET_UNIT_AREA_ENEMY_DST);
		}
