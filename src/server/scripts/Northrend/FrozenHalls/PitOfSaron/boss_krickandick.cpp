...
public:
	...
	struct boss_ickAI: public BossAI {
		boss_ickAI(Creature *c) :
				BossAI(c, DATA_KRICKANDICK_EVENT) {
		...
		Creature* GetKrick() {
			return me->GetCreature(*me,
					instance ? instance->GetData64(DATA_KRICK) : 0);
		}
		...
		void EnterCombat(Unit * /*who*/) {
			...
			if (!pKrick)
				pKrick = me->SummonCreature(CREATURE_KRICK, *me,
		...
		void UpdateAI(const uint32 diff) {
			...
			case EVENT_PURSUE:
				if (Creature* pKrick = GetKrick())
					DoScriptText(
							RAND(SAY_KRICK_CHASE_1, SAY_KRICK_CHASE_2,
									SAY_KRICK_CHASE_3), pKrick);
...
public:
	...
	struct boss_krickAI: public ScriptedAI {
		...
		Creature* GetIck() {
			return me->GetCreature(*me,
					pInstance ? pInstance->GetData64(DATA_ICK) : 0);
		}
		...
		void UpdateAI(const uint32 diff) {
			if (phase == PHASE_OUTRO) {
				...
				case EVENT_OUTRO_2: {
					Creature* pNpcDialog = me->GetCreature(*me,
							uiNpcOutroDialog);
					if (pNpcDialog) {
						if (pInstance->GetData(DATA_TEAM_IN_INSTANCE)
								== TEAM_ALLIANCE)
				...
				case EVENT_OUTRO_6:
					...
					uiTyrannus =
							(pInstance ? pInstance->GetData64(DATA_TYRANNUS) : 0);
