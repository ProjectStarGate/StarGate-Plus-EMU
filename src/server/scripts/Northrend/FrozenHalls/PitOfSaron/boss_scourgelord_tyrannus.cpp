...
enum Events {
	EVENT_NONE, EVENT_FORCEFUL_SMASH, EVENT_OVERLORDS_BRAND, EVENT_DARK_MIGHT,
...
public:
	...
	struct boss_tyrannusAI: public BossAI {
		boss_tyrannusAI(Creature *c) :
				BossAI(c, DATA_TYRANNUS_EVENT) {
...
public:
	player_overlord_brandAI(Player* pPlayer) :
			PlayerAI(pPlayer) {
		tyrannus = NULL;
	}
	...
	void DamageDealt(Unit* /*victim*/, uint32& damage,
		...
		me->CastCustomSpell(SPELL_OVERLORD_BRAND_DAMAGE, SPELLVALUE_BASE_POINT0,
				damage, tyrannus->getVictim(), true, NULL, NULL,
				tyrannus->GetGUID());
	}

	void HealDone(Unit* /*target*/, uint32& addHealth) {
		me->CastCustomSpell(SPELL_OVERLORD_BRAND_HEAL, SPELLVALUE_BASE_POINT0,
				int32(addHealth * 5.5f), tyrannus, true, NULL, NULL,
				tyrannus->GetGUID());
	}
...
public:
	spell_tyrannus_overlord_brand() :
			SpellScriptLoader("spell_tyrannus_overlord_brand") {
	}
	...
	class spell_tyrannus_overlord_brand_AuraScript: public AuraScript {
		...
		void OnApply(AuraEffect const* /*aurEff*/,
			...
			GetTarget()->SetAI(
					new player_overlord_brandAI(GetTarget()->ToPlayer()));
		...
		void Register() {
			OnEffectApply +=
					AuraEffectApplyFn(spell_tyrannus_overlord_brand_AuraScript::OnApply, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
			OnEffectRemove +=
					AuraEffectRemoveFn(spell_tyrannus_overlord_brand_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
		}
