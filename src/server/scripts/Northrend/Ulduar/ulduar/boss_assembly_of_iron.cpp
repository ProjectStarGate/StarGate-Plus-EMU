...
enum Events {
	EVENT_NONE, EVENT_UPDATEPHASE, EVENT_ENRAGE,
	// Steelbreaker
	EVENT_FUSION_PUNCH,
	...
	struct boss_steelbreakerAI: public BossAI {
		...
		void DoAction(uint32 action) {
			...
			case EVENT_UPDATEPHASE:
				...
				if (phase >= 3)
					events.RescheduleEvent(EVENT_OVERWHELMING_POWER,
							rand() % 5000);
				break;
			}
		}
		...
		void JustDied(Unit* /*Killer*/) {
			_JustDied();
			DoScriptText(
					RAND(SAY_STEELBREAKER_DEATH_1, SAY_STEELBREAKER_DEATH_2),
					me);
		...
		void UpdateAI(const uint32 diff) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_FUSION_PUNCH:
					DoCast(
							me->getVictim(),
							RAID_MODE(SPELL_FUSION_PUNCH,
									SPELL_FUSION_PUNCH_H));
					events.ScheduleEvent(EVENT_FUSION_PUNCH,
							urand(13000, 22000));
					break;
				case EVENT_STATIC_DISRUPTION:
					if (Unit *pTarget = SelectTarget(SELECT_TARGET_RANDOM))
						DoCast(
								pTarget,
								RAID_MODE(SPELL_STATIC_DISRUPTION,
										SPELL_STATIC_DISRUPTION_H));
				...
				case EVENT_OVERWHELMING_POWER:
					...
					DoCast(
							me->getVictim(),
							RAID_MODE(SPELL_OVERWHELMING_POWER,
									SPELL_OVERWHELMING_POWER_H));
					events.ScheduleEvent(EVENT_OVERWHELMING_POWER,
							RAID_MODE(60000, 35000));
					break;
				}
			}
	...
	struct boss_runemaster_molgeimAI: public BossAI {
		...
		void DoAction(uint32 action) {
			...
			case EVENT_UPDATEPHASE:
				...
				if (phase >= 3)
					events.RescheduleEvent(EVENT_RUNE_OF_SUMMONING,
							urand(20000, 30000));
				break;
			}
		}
		...
		void UpdateAI(const uint32 diff) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_SHIELD_OF_RUNES:
					DoCast(
							me,
							RAID_MODE(SPELL_SHIELD_OF_RUNES,
									SPELL_SHIELD_OF_RUNES_H));
					events.ScheduleEvent(EVENT_SHIELD_OF_RUNES,
							urand(27000, 34000));
					break;
				case EVENT_RUNE_OF_DEATH:
					...
					events.ScheduleEvent(EVENT_RUNE_OF_DEATH,
							urand(30000, 40000));
					break;
				case EVENT_RUNE_OF_SUMMONING:
					...
					events.ScheduleEvent(EVENT_RUNE_OF_SUMMONING,
							urand(20000, 30000));
					break;
				}
			}
...
public:
	mob_rune_of_power() :
			CreatureScript("mob_rune_of_power") {
	}
	...
	struct mob_rune_of_powerAI: public ScriptedAI {
		mob_rune_of_powerAI(Creature* c) :
				ScriptedAI(c) {
...
public:
	mob_lightning_elemental() :
			CreatureScript("mob_lightning_elemental") {
	}
	...
	struct mob_lightning_elementalAI: public ScriptedAI {
		mob_lightning_elementalAI(Creature *c) :
				ScriptedAI(c) {
			me->SetInCombatWithZone();
			me->AddAura(
					RAID_MODE(SPELL_LIGHTNING_ELEMENTAL_PASSIVE,
							SPELL_LIGHTNING_ELEMENTAL_PASSIVE_H), me);
		}
...
public:
	mob_rune_of_summoning() :
			CreatureScript("mob_rune_of_summoning") {
	}
	...
	struct mob_rune_of_summoningAI: public ScriptedAI {
		mob_rune_of_summoningAI(Creature *c) :
				ScriptedAI(c) {
...
public:
	...
	struct boss_stormcaller_brundirAI: public BossAI {
		...
		void DoAction(uint32 action) {
			...
			case EVENT_UPDATEPHASE:
				events.SetPhase(++phase);
				events.RescheduleEvent(EVENT_CHAIN_LIGHTNING,
						urand(9000, 17000));
				events.RescheduleEvent(EVENT_OVERLOAD, urand(60000, 125000));
				if (phase >= 2)
					events.RescheduleEvent(EVENT_LIGHTNING_WHIRL,
							urand(20000, 40000));
				if (phase >= 3) {
					DoCast(me, SPELL_STORMSHIELD);
					events.RescheduleEvent(EVENT_LIGHTNING_TENDRILS,
							urand(40000, 80000));
				}
				break;
			}
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_CHAIN_LIGHTNING:
					if (Unit* pTarget = SelectUnit(SELECT_TARGET_RANDOM, 0))
						DoCast(
								pTarget,
								RAID_MODE(SPELL_CHAIN_LIGHTNING_N,
										SPELL_CHAIN_LIGHTNING_H));
					events.ScheduleEvent(EVENT_CHAIN_LIGHTNING,
							urand(9000, 17000));
					break;
				case EVENT_OVERLOAD:
				...
				case EVENT_LIGHTNING_WHIRL:
					DoCast(
							RAID_MODE(SPELL_LIGHTNING_WHIRL,
									SPELL_LIGHTNING_WHIRL_H));
					events.ScheduleEvent(EVENT_LIGHTNING_WHIRL,
							urand(20000, 40000));
					break;
				case EVENT_LIGHTNING_TENDRILS:
					DoCast(
							RAID_MODE(SPELL_LIGHTNING_TENDRILS,
									SPELL_LIGHTNING_TENDRILS_H));
