...
enum eActions {
	ACTION_VAPORS_SPAWN, ACTION_VAPORS_DIE, ACTION_ANIMUS_DIE,
};
...
public:
	boss_general_vezax() :
			CreatureScript("boss_general_vezax") {
	}
	...
	struct boss_general_vezaxAI: public BossAI {
		boss_general_vezaxAI(Creature *pCreature) :
				BossAI(pCreature, TYPE_VEZAX) {
		...
		void EnterCombat(Unit * /*pWho*/) {
			...
			events.ScheduleEvent(EVENT_MARK_OF_THE_FACELESS,
					urand(35000, 40000));
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			while (uint32 uiEventId = events.ExecuteEvent()) {
				...
				case EVENT_SHADOW_CRASH:
					...
					events.ScheduleEvent(EVENT_SHADOW_CRASH,
							urand(8000, 12000));
					break;
				case EVENT_SEARING_FLAMES:
					DoCastAOE(SPELL_SEARING_FLAMES);
					events.ScheduleEvent(EVENT_SEARING_FLAMES,
							urand(14000, 17500));
					break;
				case EVENT_MARK_OF_THE_FACELESS:
					 ...
					 */
					if (!(pTarget = CheckPlayersInRange(RAID_MODE(4, 9), 15.0f,
							50.f)))
				...
				case EVENT_SURGE_OF_DARKNESS:
					...
					events.ScheduleEvent(EVENT_SURGE_OF_DARKNESS,
							urand(50000, 70000));
					break;
				case EVENT_SARONITE_VAPORS:
					DoCast(SPELL_SUMMON_SARONITE_VAPORS);
					events.ScheduleEvent(EVENT_SARONITE_VAPORS,
							urand(30000, 35000));
		...
		void SpellHitTarget(Unit * pWho, const SpellEntry * pSpell) {
			if (pWho && pWho->GetTypeId() == TYPEID_PLAYER
					&& pSpell->Id == SPELL_SHADOW_CRASH_HIT)
				bShadowDodger = false;
		}
		 ...
		 */
		Unit * CheckPlayersInRange(uint32 uiPlayersMin, float uiRangeMin,
				float uiRangeMax) {
			Map * pMap = me->GetMap();
			if (pMap && pMap->IsDungeon()) {
				...
				for (Map::PlayerList::const_iterator itr = Players.begin();
						itr != Players.end(); ++itr) {
					if (Player * pPlayer = itr->getSource()) {
						float uiDistance = pPlayer->GetDistance(
...
public:
	boss_saronite_animus() :
			CreatureScript("npc_saronite_animus") {
	}
	...
	struct boss_saronite_animusAI: public ScriptedAI {
		boss_saronite_animusAI(Creature *pCreature) :
...
public:
	npc_saronite_vapors() :
			CreatureScript("npc_saronite_vapors") {
	}
	...
	struct npc_saronite_vaporsAI: public ScriptedAI {
		npc_saronite_vaporsAI(Creature *pCreature) :
				ScriptedAI(pCreature) {
