	...
	struct boss_algalonAI: public ScriptedAI {
		...
		void DespawnCollapsingStar() {
			...
			for (std::list<uint64>::const_iterator itr =
					m_lCollapsingStarGUIDList.begin();
					itr != m_lCollapsingStarGUIDList.end(); ++itr) {
		...
		void SummonCollapsingStar(Unit* target) {
			...
			me->SummonCreature(CREATURE_COLLAPSING_STAR,
					target->GetPositionX() + 15.0f,
					target->GetPositionY() + 15.0f, target->GetPositionZ(), 0,
		...
		void UpdateAI(const uint32 diff) {
			...
			if (HealthBelowPct(2)) {
				me->SummonGameObject(GAMEOBJECT_GIVE_OF_THE_OBSERVER,
						1634.258667f, -295.101166f, 417.321381f, 0, 0, 0, 0, 0,
						0);
			...
			if (Phase == 1) {
				...
				if (QuantumStrike_Timer <= diff) {
					DoCast(
							me->getVictim(),
							RAID_MODE(SPELL_QUANTUM_STRIKE,
									H_SPELL_QUANTUM_STRIKE), true);

					QuantumStrike_Timer = urand(4000, 14000);
				} else
					QuantumStrike_Timer -= diff;

				if (BigBang_Timer <= diff) {
					DoScriptText(RAND(SAY_BIG_BANG_1, SAY_BIG_BANG_2), me);
					DoCast(me->getVictim(),
							RAID_MODE(SPELL_BIG_BANG, H_SPELL_BIG_BANG), true);
				...
				if (PhasePunch_Timer <= diff) {
					...
					PhasePunch_Timer -= diff;

				if (CosmicSmash_Timer <= diff) {
					DoCast(SelectUnit(SELECT_TARGET_RANDOM, 0),
							RAID_MODE(SPELL_COSMIC_SMASH, H_SPELL_COSMIC_SMASH),
							true);

					CosmicSmash_Timer = urand(30000, 60000);
				} else
					CosmicSmash_Timer -= diff;
...
public:
	mob_collapsing_star() :
			CreatureScript("mob_collapsing_star") {
	}
