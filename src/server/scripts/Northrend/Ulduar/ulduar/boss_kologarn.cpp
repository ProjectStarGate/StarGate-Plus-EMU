...
void EncounterInCombat(Creature* me, InstanceScript* pInstance) {
	Creature* c;
	c = Unit::GetCreature(*me,
			pInstance ? pInstance->GetData64(TYPE_KOLOGARN) : 0);
	if (c && c != me && c->isAlive())
		c->SetInCombatWithZone();

	c = Unit::GetCreature(*me,
			pInstance ? pInstance->GetData64(DATA_RIGHT_ARM) : 0);
	if (c && c != me && c->isAlive())
		c->SetInCombatWithZone();

	c = Unit::GetCreature(*me,
			pInstance ? pInstance->GetData64(DATA_LEFT_ARM) : 0);
	if (c && c != me && c->isAlive())
		c->SetInCombatWithZone();
}
...
public:
	...
	struct boss_kologarnAI: public BossAI {
		boss_kologarnAI(Creature *pCreature) :
				BossAI(pCreature, TYPE_KOLOGARN), vehicle(
						pCreature->GetVehicleKit()), uiArmCount(0) {
			ASSERT(vehicle);
		...
		void UpdateAI(const uint32 diff) {
			if (!me->getVictim()
					&& instance->GetBossState(TYPE_KOLOGARN) == NOT_STARTED)
				return;
			...
			case EVENT_RESPAWN_ARM: {
				Creature* curArm = Unit::GetCreature(*me,
						instance ? instance->GetData64(DATA_RIGHT_ARM) : 0);
				if (!(curArm && curArm->isAlive()))
					curArm = Unit::GetCreature(*me,
							instance ? instance->GetData64(DATA_LEFT_ARM) : 0);
				if (!(curArm && curArm->isAlive()))
					break;
...
public:
	...
	struct npc_right_armAI: public ScriptedAI {
		npc_right_armAI(Creature* pCreature) :
...
public:
	...
	struct npc_left_armAI: public ScriptedAI {
		npc_left_armAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
			pInstance = me->GetInstanceScript();
			SetCombatMovement(false);
			me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE);
