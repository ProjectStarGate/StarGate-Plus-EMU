...
public:
	...
	bool EffectDummyCreature(Unit* pCaster, uint32 uiSpellId, uint32 uiEffIndex,
			Creature* pCreatureTarget) {
		//always check spellid and effectindex
		if (uiSpellId == SPELL_TEMPER_DUMMY && uiEffIndex == 0) {
			if (pCaster->GetEntry() != NPC_VOLKHAN_ANVIL
					|| pCreatureTarget->GetEntry() != NPC_VOLKHAN)
				return true;

			for (uint8 i = 0; i < MAX_GOLEM; ++i) {
				pCreatureTarget->CastSpell(pCaster, SPELL_SUMMON_MOLTEN_GOLEM,
						true);
			}
	...
	struct boss_volkhanAI: public BossAI {
		boss_volkhanAI(Creature *pCreature) :
				BossAI(pCreature, DATA_VOLKHAN) {
		}
		...
		void JustDied(Unit* /*pKiller*/) {
			...
			if (IsHeroic() && GolemsShattered < 5) {
				AchievementEntry const *AchievShatterResistant =
		...
		void DespawnGolem() {
			...
			for (std::list<uint64>::const_iterator itr =
					m_lGolemGUIDList.begin(); itr != m_lGolemGUIDList.end();
					++itr) {
		...
		void ShatterGolem() {
			...
			for (std::list<uint64>::const_iterator itr =
				...
				if (Creature* pTemp = Unit::GetCreature(*me, *itr)) {
					...
					if (pTemp->isAlive()
							&& pTemp->GetEntry() == NPC_BRITTLE_GOLEM) {
						pTemp->CastSpell(pTemp,
								DUNGEON_MODE(SPELL_SHATTER_N, SPELL_SHATTER_H),
								false);
						GolemsShattered += 1;
					}
				}
		...
		void JustSummoned(Creature* pSummoned) {
			if (pSummoned->GetEntry() == NPC_MOLTEN_GOLEM) {
				...
				pSummoned->CastSpell(pSummoned,
						DUNGEON_MODE(SPELL_HEAT_N, SPELL_HEAT_H), false, NULL,
						NULL, me->GetGUID());
			}
		}
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			// Health check
			if (!m_bCanShatterGolem
					&& me->HealthBelowPct(100 - 20 * m_uiHealthAmountModifier)) {
...
public:
	npc_volkhan_anvil() :
			CreatureScript("npc_volkhan_anvil") {
	}
	...
	bool EffectDummyCreature(Unit* pCaster, uint32 uiSpellId, uint32 uiEffIndex,
		...
		if (uiSpellId == SPELL_TEMPER && uiEffIndex == 0) {
			if (pCaster->GetEntry() != NPC_VOLKHAN
					|| pCreatureTarget->GetEntry() != NPC_VOLKHAN_ANVIL)
				return true;
			...
			float fX, fY, fZ;
			pCreatureTarget->GetContactPoint(pCaster, fX, fY, fZ,
					INTERACTION_DISTANCE);

			pCaster->AttackStop();

			if (pCaster->GetMotionMaster()->GetCurrentMovementGeneratorType()
					== TARGETED_MOTION_TYPE)
				pCaster->GetMotionMaster()->MovementExpired();

			cre->GetMap()->CreatureRelocation(cre, fX, fY, fZ,
					pCreatureTarget->GetOrientation());
...
public:
	mob_molten_golem() :
			CreatureScript("mob_molten_golem") {
	}
	...
	struct mob_molten_golemAI: public ScriptedAI {
		mob_molten_golemAI(Creature *pCreature) :
		...
		void SpellHit(Unit* /*pCaster*/, const SpellEntry* pSpell) {
			...
			if (pSpell->Id == SPELL_SHATTER_N
					|| pSpell->Id == SPELL_SHATTER_H) {
