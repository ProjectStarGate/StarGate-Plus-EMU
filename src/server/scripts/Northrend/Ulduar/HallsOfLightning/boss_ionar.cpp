...
enum Misc {
	DATA_MAX_SPARKS = 5, DATA_MAX_SPARK_DISTANCE = 90, // Distance to boss - prevent runs through the whole instance
...
public:
	...
	bool EffectDummyCreature(Unit* /*pCaster*/, uint32 uiSpellId,
		...
		if (uiSpellId == SPELL_DISPERSE && uiEffIndex == 0) {
			...
			for (uint8 i = 0; i < DATA_MAX_SPARKS; ++i)
				pCreatureTarget->CastSpell(pCreatureTarget, SPELL_SUMMON_SPARK,
						true);

			pCreatureTarget->AttackStop();
			pCreatureTarget->SetVisible(false);
			pCreatureTarget->SetFlag(
					UNIT_FIELD_FLAGS,
	...
	struct boss_ionarAI: public BossAI {
		boss_ionarAI(Creature *pCreature) :
				BossAI(pCreature, DATA_IONAR) {
		}
		...
		void Reset() {
			...
			uiStaticOverloadTimer = urand(5 * IN_MILLISECONDS,
					6 * IN_MILLISECONDS);
			uiBallLightningTimer = urand(10 * IN_MILLISECONDS,
					11 * IN_MILLISECONDS);
		...
		void CallBackSparks() {
			...
			for (std::list<uint64>::const_iterator itr = summons.begin();
					itr != summons.end(); ++itr) {
				if (Creature* pSpark = Unit::GetCreature(*me, *itr)) {
					if (pSpark->isAlive()) {
						pSpark->SetSpeed(MOVE_RUN, 2.0f);
						pSpark->GetMotionMaster()->Clear();
						pSpark->GetMotionMaster()->MovePoint(
								DATA_POINT_CALLBACK, pos);
					} else
		...
		void JustSummoned(Creature* pSummoned) {
			if (pSummoned->GetEntry() == NPC_SPARK_OF_IONAR) {
				...
				pSummoned->CastSpell(
						pSummoned,
						DUNGEON_MODE(SPELL_SPARK_VISUAL_TRIGGER,
								H_SPELL_SPARK_VISUAL_TRIGGER), true);

				Unit* pTarget = SelectUnit(SELECT_TARGET_RANDOM, 0);
				if (pTarget) {
					pSummoned->SetInCombatWith(pTarget);
					pSummoned->GetMotionMaster()->Clear();
					pSummoned->GetMotionMaster()->MoveFollow(pTarget, 0.0f,
							0.0f);
				}
			}
		}
...
public:
	mob_spark_of_ionar() :
			CreatureScript("mob_spark_of_ionar") {
	}
	...
	struct mob_spark_of_ionarAI: public ScriptedAI {
		mob_spark_of_ionarAI(Creature *pCreature) :
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (uiCheckTimer <= uiDiff) {
				if (pInstance) {
					Creature* pIonar = pInstance->instance->GetCreature(
							pInstance->GetData64(DATA_IONAR));
