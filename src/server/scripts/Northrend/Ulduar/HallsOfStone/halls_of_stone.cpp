...
public:
	...
	struct mob_tribuna_controllerAI: public ScriptedAI {
		mob_tribuna_controllerAI(Creature *c) :
				ScriptedAI(c) {
		...
		void Reset() {
			...
			if (pInstance) {
				pInstance->HandleGameObject(
						pInstance->GetData64(DATA_GO_KADDRAK), false);
				pInstance->HandleGameObject(
						pInstance->GetData64(DATA_GO_MARNAK), false);
				pInstance->HandleGameObject(
						pInstance->GetData64(DATA_GO_ABEDNEUM), false);
				pInstance->HandleGameObject(
						pInstance->GetData64(DATA_GO_SKY_FLOOR), false);
			}
					...
					if (Unit* pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true))
						if (!KaddrakGUIDList.empty())
							for (std::list<uint64>::const_iterator itr =
									KaddrakGUIDList.begin();
									itr != KaddrakGUIDList.end(); ++itr) {
				...
				if (uiMarnakEncounterTimer <= diff) {
					if (Unit* pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true)) {
						if (Creature* pSummon = me->SummonCreature(CREATURE_DARK_MATTER_TARGET, pTarget->GetPositionX(), pTarget->GetPositionY(), pTarget->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_DESPAWN, 1000)) {
							...
							pSummon->CastSpell(
									pTarget,
									DUNGEON_MODE(SPELL_DARK_MATTER,
											H_SPELL_DARK_MATTER), true);
						}
					}
	...
	struct npc_brann_hosAI: public npc_escortAI {
		...
		void DespawnDwarf() {
			if (lDwarfGUIDList.empty())
				return;
			for (std::list<uint64>::const_iterator itr = lDwarfGUIDList.begin();
					itr != lDwarfGUIDList.end(); ++itr) {
		...
		void WaypointReached(uint32 uiPointId) {
			...
			case 17:
				...
				if (pInstance)
					pInstance->HandleGameObject(
							pInstance->GetData64(DATA_GO_TRIBUNAL_CONSOLE),
							true);
		...
		void SpawnDwarf(uint32 uiType) {
			...
			case 1: {
				...
				for (uint8 i = 0; i < uiSpawnNumber; ++i)
					me->SummonCreature(CREATURE_DARK_RUNE_PROTECTOR,
							SpawnLocations[0], TEMPSUMMON_CORPSE_TIMED_DESPAWN,
							30000);
				me->SummonCreature(CREATURE_DARK_RUNE_STORMCALLER,
						SpawnLocations[0], TEMPSUMMON_CORPSE_TIMED_DESPAWN,
						30000);
				break;
			}
			case 2:
				...
				break;
			case 3:
				me->SummonCreature(CREATURE_IRON_GOLEM_CUSTODIAN,
						SpawnLocations[0], TEMPSUMMON_CORPSE_TIMED_DESPAWN,
						30000);
				break;
			}
		...
		void UpdateEscortAI(const uint32 uiDiff) {
			if (uiPhaseTimer <= uiDiff) {
				...
				case 8:
					...
					if (pInstance)
						pInstance->HandleGameObject(
								pInstance->GetData64(DATA_GO_KADDRAK), true);
					if (Creature* pTemp = Unit::GetCreature(*me, uiControllerGUID))
						CAST_AI(mob_tribuna_controller::mob_tribuna_controllerAI, pTemp->AI())->bKaddrakActivated =
								true;
					JumpToNextStep(5000);
					break;
				case 9:
				...
				case 12:
					...
					if (pInstance)
						pInstance->HandleGameObject(
								pInstance->GetData64(DATA_GO_MARNAK), true);
					if (Creature* pTemp = Unit::GetCreature(*me, uiControllerGUID))
						CAST_AI(mob_tribuna_controller::mob_tribuna_controllerAI, pTemp->AI())->bMarnakActivated =
								true;
				...
				case 18:
					...
					if (pInstance)
						pInstance->HandleGameObject(
								pInstance->GetData64(DATA_GO_ABEDNEUM), true);
					if (Creature* pTemp = Unit::GetCreature(*me, uiControllerGUID))
						CAST_AI(mob_tribuna_controller::mob_tribuna_controllerAI, pTemp->AI())->bAbedneumActivated =
								true;
					JumpToNextStep(5000);
					break;
				case 19:
				...
				case 28:
					...
					if (pInstance)
						pInstance->HandleGameObject(
								pInstance->GetData64(DATA_GO_SKY_FLOOR), true);
					if (Creature* pTemp = Unit::GetCreature(*me, uiControllerGUID))
						pTemp->DealDamage(pTemp, pTemp->GetHealth(), NULL,
				...
				case 29:
					...
					if (pInstance) {
						...
						if (!bHasBeenDamaged)
							pInstance->DoCompleteAchievement(
									ACHIEV_BRANN_SPANKIN_NEW);
