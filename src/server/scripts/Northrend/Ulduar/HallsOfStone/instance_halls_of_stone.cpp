...
public:
	...
	struct instance_halls_of_stone_InstanceMapScript: public InstanceScript {
		instance_halls_of_stone_InstanceMapScript(Map* pMap) :
				InstanceScript(pMap) {
			Initialize();
		}
		;
		...
		void OnCreatureCreate(Creature* pCreature, bool /*add*/) {
			...
			case CREATURE_MAIDEN:
				uiMaidenOfGrief = pCreature->GetGUID();
				break;
			case CREATURE_KRYSTALLUS:
				uiKrystallus = pCreature->GetGUID();
				break;
			case CREATURE_SJONNIR:
				uiSjonnir = pCreature->GetGUID();
				break;
			case CREATURE_MARNAK:
				uiMarnak = pCreature->GetGUID();
				break;
			case CREATURE_KADDRAK:
				uiKaddrak = pCreature->GetGUID();
				break;
			case CREATURE_ABEDNEUM:
				uiAbedneum = pCreature->GetGUID();
				break;
			case CREATURE_BRANN:
				uiBrann = pCreature->GetGUID();
				break;
			}
		}
		...
		uint64 GetData64(uint32 identifier) {
			...
			case DATA_GO_SKY_FLOOR:
				return uiTribunalSkyFloor;
			case DATA_SJONNIR_DOOR:
				return uiSjonnirDoor;
			case DATA_MAIDEN_DOOR:
				return uiMaidenOfGriefDoor;
			}
		...
		std::string GetSaveData() {
			...
			saveStream << "H S " << m_auiEncounter[0] << " "
