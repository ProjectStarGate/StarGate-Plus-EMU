...
public:
	...
	struct instance_gundrak_InstanceMapScript: public InstanceScript {
		instance_gundrak_InstanceMapScript(Map* pMap) :
				InstanceScript(pMap) {
		...
		void OnCreatureCreate(Creature* creature) {
			...
			case CREATURE_SLAD_RAN:
				uiSladRan = creature->GetGUID();
				break;
			case CREATURE_MOORABI:
				uiMoorabi = creature->GetGUID();
				break;
			case CREATURE_GALDARAH:
				uiGalDarah = creature->GetGUID();
				break;
			case CREATURE_DRAKKARICOLOSSUS:
				uiDrakkariColossus = creature->GetGUID();
				break;
			case CREATURE_ECK:
				uiEckTheFerocious = creature->GetGUID();
				break;
			...
			case DATA_DRAKKARI_COLOSSUS_EVENT:
				...
				if (data == DONE) {
					GameObject* go = instance->GetGameObject(
							uiDrakkariColossusAltar);
					if (go)
						go->RemoveFlag(GAMEOBJECT_FLAGS, GO_FLAG_UNK1);
				} else if (data == NOT_STARTED) {
					for (std::vector<uint64>::const_iterator itr =
							LivingMojoGUIDs.begin();
							itr != LivingMojoGUIDs.end(); ++itr)
		...
		std::string GetSaveData() {
			...
			saveStream << "G D " << m_auiEncounter[0] << " "
					<< m_auiEncounter[1] << " " << m_auiEncounter[2] << " "
					<< m_auiEncounter[3] << " " << m_auiEncounter[4] << " "
					<< (uiSladRanStatue ?
							GetObjState(uiSladRanStatue) : GO_STATE_ACTIVE)
					<< " "
					<< (uiMoorabiStatue ?
							GetObjState(uiMoorabiStatue) : GO_STATE_ACTIVE)
					<< " "
					<< (uiDrakkariColossusStatue ?
							GetObjState(uiDrakkariColossusStatue) :
							GO_STATE_ACTIVE) << " "
					<< (uiGalDarahStatue ?
							GetObjState(uiGalDarahStatue) : GO_STATE_READY)
					<< " "
					<< (uiBridge ? GetObjState(uiBridge) : GO_STATE_ACTIVE)
					<< " "
					<< (uiCollision ? GetObjState(uiCollision) : GO_STATE_READY);
		...
		void Update(uint32 diff) {
			...
			if (spawnSupport) {
				if (GameObject* pCollision = instance->GetGameObject(uiCollision))
					pCollision->SummonGameObject(192743,
							pCollision->GetPositionX(),
							pCollision->GetPositionY(),
							pCollision->GetPositionZ(),
							pCollision->GetOrientation(), 0, 0, 0, 0, 0);
				spawnSupport = false;
			}
			...
			if (timer < diff) {
				...
				if (toActivate == uiBridge) {
					...
					GameObject* pCollision = instance->GetGameObject(
							uiCollision);
					GameObject* pSladRanStatue = instance->GetGameObject(
							uiSladRanStatue);
					GameObject* pMoorabiStatue = instance->GetGameObject(
							uiMoorabiStatue);
					GameObject* pDrakkariColossusStatue =
							instance->GetGameObject(uiDrakkariColossusStatue);
					GameObject* pGalDarahStatue = instance->GetGameObject(
							uiGalDarahStatue);

					if (pBridge && pCollision && pSladRanStatue
							&& pMoorabiStatue && pDrakkariColossusStatue
							&& pGalDarahStatue) {
						...
						pDrakkariColossusStatue->SetGoState(
								GO_STATE_ACTIVE_ALTERNATIVE);
						pGalDarahStatue->SetGoState(
								GO_STATE_ACTIVE_ALTERNATIVE);
...
public:
	go_gundrak_altar() :
			GameObjectScript("go_gundrak_altar") {
	}
	...
	bool OnGossipHello(Player * /*pPlayer*/, GameObject* pGO) {
		...
		if (pInstance) {
			...
			if (CAST_INST(instance_gundrak::instance_gundrak_InstanceMapScript, pInstance)->QueueActivation(
					uiStatue, 3500)) {
				pGO->SetFlag(GAMEOBJECT_FLAGS, GO_FLAG_UNK1);
				pGO->SetGoState(GO_STATE_ACTIVE);
			}
