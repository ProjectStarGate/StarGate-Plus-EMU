...
static Position EckSpawnPoint = { 1643.877930f, 936.278015f, 107.204948f,
		0.668432f };
...
public:
	...
	struct boss_eckAI: public BossAI {
		boss_eckAI(Creature *c) :
				BossAI(c, DATA_ECK_THE_FEROCIOUS_EVENT) {
		...
		void UpdateAI(const uint32 diff) {
			...
			if (uiSpringTimer <= diff) {
				...
				if (pTarget && pTarget->GetTypeId() == TYPEID_PLAYER) {
					DoCast(pTarget,
							RAND(SPELL_ECK_SPRING_1, SPELL_ECK_SPRING_2));
					uiSpringTimer = urand(5 * IN_MILLISECONDS,
							10 * IN_MILLISECONDS);
				}
			} else
				uiSpringTimer -= diff;
...
public:
	npc_ruins_dweller() :
			CreatureScript("npc_ruins_dweller") {
	}
	...
	struct npc_ruins_dwellerAI: public ScriptedAI {
		npc_ruins_dwellerAI(Creature *c) :
				ScriptedAI(c) {
		...
		void JustDied(Unit * /*who*/) {
			if (pInstance) {
				...
				if (pInstance->GetData(DATA_ALIVE_RUIN_DWELLERS) == 0)
					me->SummonCreature(CREATURE_ECK, EckSpawnPoint,
