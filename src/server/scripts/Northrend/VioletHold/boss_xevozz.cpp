...
public:
	...
	struct boss_xevozzAI: public BossAI {
		boss_xevozzAI(Creature* pCreature) :
				BossAI(pCreature, DATA_XEVOZZ) {
		...
		void Reset() {
			...
			uiArcaneBuffet_Timer = uiSummonEtherealSphere_Timer
					+ urand(5000, 6000);
			DespawnSphere();
		}
		...
		void DespawnSphere() {
			...
			GetCreatureListWithEntryInGrid(assistList, me, NPC_ETHEREAL_SPHERE,
					150.0f);
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (uiArcaneBarrageVolley_Timer < uiDiff) {
				...
				uiArcaneBarrageVolley_Timer -= uiDiff;
			...
			if (uiSummonEtherealSphere_Timer < uiDiff) {
				...
				if (IsHeroic()) // extra one for heroic
					me->SummonCreature(NPC_ETHEREAL_SPHERE,
							me->GetPositionX() - 5 + rand() % 10,
							me->GetPositionY() - 5 + rand() % 10,
							me->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN,
							40000);

				uiSummonEtherealSphere_Timer = urand(45000, 47000);
				uiArcaneBuffet_Timer = urand(5000, 6000);
			} else
				uiSummonEtherealSphere_Timer -= uiDiff;
...
public:
	mob_ethereal_sphere() :
			CreatureScript("mob_ethereal_sphere") {
	}
	...
	struct mob_ethereal_sphereAI: public ScriptedAI {
		mob_ethereal_sphereAI(Creature* pCreature) :
