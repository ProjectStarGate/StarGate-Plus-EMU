...
public:
	...
	struct boss_erekemAI: public BossAI {
		boss_erekemAI(Creature *c) :
				BossAI(c, BOSS_EREKEM) {
		...
		void UpdateAI(const uint32 diff) {
			...
			if (uiChainHealTimer <= diff) {
				if (uint64 TargetGUID = GetChainHealTargetGUID()) {
					...
					Creature *pGuard1 = Unit::GetCreature(
							*me,
							instance ?
									instance->GetData64(DATA_EREKEM_GUARD_1) :
									0);
					Creature *pGuard2 = Unit::GetCreature(
							*me,
							instance ?
									instance->GetData64(DATA_EREKEM_GUARD_2) :
									0);
					uiChainHealTimer = (
							(pGuard1 && !pGuard1->isAlive())
		...
		uint64 GetChainHealTargetGUID() {
			...
			Creature *pGuard1 = Unit::GetCreature(*me,
					instance ? instance->GetData64(DATA_EREKEM_GUARD_1) : 0);
			if (pGuard1 && pGuard1->isAlive() && !pGuard1->HealthAbovePct(75))
				return pGuard1->GetGUID();

			Creature *pGuard2 = Unit::GetCreature(*me,
					instance ? instance->GetData64(DATA_EREKEM_GUARD_2) : 0);
...
public:
	mob_erekem_guard() :
			CreatureScript("mob_erekem_guard") {
	}
