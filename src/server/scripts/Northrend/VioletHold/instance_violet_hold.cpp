...
enum AzureSaboteurSpells {
	SABOTEUR_SHIELD_DISRUPTION = 58291, SABOTEUR_SHIELD_EFFECT = 45775
};
...
const Position PortalLocation[] = { { 1877.51f, 850.104f, 44.6599f, 4.7822f }, // WP 1
...
const Position CyanigosasSpawnLocation = { 1930.281250f, 804.407715f,
		52.410946f, 3.139621f };
const Position MiddleRoomLocation = { 1892.291260f, 805.696838f, 38.438862f,
		3.139621f };
const Position MiddleRoomPortalSaboLocation = { 1896.622925f, 804.854126f,
		38.504772f, 3.139621f };
...
enum Spells {
	CYANIGOSA_SPELL_TRANSFORM = 58668, CYANIGOSA_BLUE_AURA = 47759,
};
enum Achievements {
...
public:
	instance_violet_hold() :
			InstanceMapScript("instance_violet_hold", 608) {
	}
	...
	struct instance_violet_hold_InstanceMapScript: public InstanceScript {
		...
		void OnCreatureCreate(Creature* pCreature, bool add) {
			...
			if (add
					&& (pCreature->GetGUID() == uiFirstBoss
							|| pCreature->GetGUID() == uiSecondBoss)) {
		...
		void OnGameObjectCreate(GameObject* pGo, bool /*add*/) {
			...
			case GO_ACTIVATION_CRYSTAL:
				if (uiCountActivationCrystals < 3)
					uiActivationCrystal[uiCountActivationCrystals++] =
							pGo->GetGUID();
				break;
			}
		}
		...
		uint64 GetData64(uint32 identifier) {
			...
			case DATA_EREKEM_GUARD_1:
				return uiErekemGuard[0];
			case DATA_EREKEM_GUARD_2:
				return uiErekemGuard[1];
			case DATA_ICHORON:
				return uiIchoron;
			case DATA_LAVANTHOR:
				return uiLavanthor;
		...
		void AddWave() {
			...
			case 6:
				...
				if (Creature *pSinclari = instance->GetCreature(uiSinclari)) {
					...
					if (Creature *pAzureSaboteur = pSinclari->SummonCreature(CREATURE_SABOTEOUR, MiddleRoomLocation, TEMPSUMMON_DEAD_DESPAWN))
						pAzureSaboteur->CastSpell(pAzureSaboteur,
								SABOTEUR_SHIELD_EFFECT, false);
				}
				break;
			case 12:
				if (uiSecondBoss == 0)
			...
			case 18: {
				...
				if (pSinclari)
					pSinclari->SummonCreature(CREATURE_CYANIGOSA,
							CyanigosasSpawnLocation, TEMPSUMMON_DEAD_DESPAWN);
				break;
			}
			case 1: {
				if (GameObject* pMainDoor = instance->GetGameObject(uiMainDoor))
		...
		std::string GetSaveData() {
			...
			saveStream << "V H " << (uint16) m_auiEncounter[0] << " "
					...
					<< (uint16) m_auiEncounter[2] << " " << (uint16) uiFirstBoss
					<< " " << (uint16) uiSecondBoss;
		...
		void Update(uint32 diff) {
			...
			if (uiMainEventPhase == IN_PROGRESS && CheckWipe()) {
				...
				if (Creature* pSinclari = instance->GetCreature(uiSinclari)) {
					...
					pSinclari->GetCreatureListWithEntryInGrid(GuardList,
							NPC_VIOLET_HOLD_GUARD, 40.0f);
					if (!GuardList.empty()) {
						for (std::list<Creature*>::const_iterator itr =
								GuardList.begin(); itr != GuardList.end();
								++itr) {
							if (Creature* pGuard = *itr) {
								pGuard->SetVisible(true);
								pGuard->SetReactState(REACT_AGGRESSIVE);
								pGuard->GetMotionMaster()->MovePoint(1,
										pGuard->GetHomePosition());
							}
						}
					}
					pSinclari->GetMotionMaster()->MovePoint(1,
							pSinclari->GetHomePosition());
					pSinclari->RemoveFlag(UNIT_FIELD_FLAGS,
			...
			if (pCyanigosa && !pCyanigosa->HasAura(CYANIGOSA_SPELL_TRANSFORM)) {
				if (uiCyanigosaEventTimer <= diff) {
					...
					case 1:
						pCyanigosa->CastSpell(pCyanigosa, CYANIGOSA_BLUE_AURA,
								false);
						...
						break;
					case 2:
						pCyanigosa->GetMotionMaster()->MoveJump(
								MiddleRoomLocation.GetPositionX(),
								MiddleRoomLocation.GetPositionY(),
								MiddleRoomLocation.GetPositionZ(), 10.0f,
								20.0f);
						pCyanigosa->CastSpell(pCyanigosa, CYANIGOSA_BLUE_AURA,
								false);
						uiCyanigosaEventTimer = 7 * IN_MILLISECONDS;
					...
					case 3:
						...
						pCyanigosa->CastSpell(pCyanigosa,
								CYANIGOSA_SPELL_TRANSFORM, 0);
						pCyanigosa->RemoveFlag(
								UNIT_FIELD_FLAGS,
								UNIT_FLAG_OOC_NOT_ATTACKABLE
										| UNIT_FLAG_NON_ATTACKABLE);
			...
			if (GetData(DATA_NPC_PRESENCE_AT_DOOR)
					&& uiMainEventPhase == IN_PROGRESS) {
				// if door integrity is > 0 then decrase it's integrity state
				if (GetData(DATA_DOOR_INTEGRITY)) {
					if (uiDoorSpellTimer < diff) {
						SetData(DATA_DOOR_INTEGRITY,
								GetData(DATA_DOOR_INTEGRITY) - 1);
						uiDoorSpellTimer = 2000;
					} else
		...
		void ActivateCrystal() {
			...
			for (std::set<uint64>::const_iterator itr = trashMobs.begin();
					itr != trashMobs.end(); ++itr) {
				Creature* pCreature = instance->GetCreature(*itr);
				if (pCreature && pCreature->isAlive())
					pCreature->CastSpell(pCreature, SPELL_ARCANE_LIGHTNING,
							true); // Who should cast the spell?
			}
		}
