...
enum Events {
	EVENT_NONE, EVENT_SHIFT, EVENT_CHAIN, EVENT_BERSERK,
};
	...
	struct boss_thaddiusAI: public BossAI {
		boss_thaddiusAI(Creature *c) :
			...
			if (!checkFeugenAlive && !checkStalaggAlive) {
				me->RemoveFlag(
						UNIT_FIELD_FLAGS,
						UNIT_FLAG_OOC_NOT_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE
								| UNIT_FLAG_STUNNED);
		...
		void UpdateAI(const uint32 diff) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_CHAIN:
					DoCast(
							me->getVictim(),
							RAID_MODE(SPELL_CHAIN_LIGHTNING,
			...
			if (magneticPullTimer <= uiDiff) {
				if (Creature *pFeugen = me->GetCreature(*me, pInstance->GetData64(DATA_FEUGEN))) {
					...
					if (pFeugenVictim && pStalaggVictim) {
						...
						pFeugen->getThreatManager().modifyThreatPercent(
								pFeugenVictim, -100);
