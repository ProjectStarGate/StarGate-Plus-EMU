...
enum Events {
	EVENT_NONE, EVENT_DISRUPT, EVENT_FEVER, EVENT_ERUPT, EVENT_PHASE,
};

enum Phases {
	PHASE_FIGHT = 1, PHASE_DANCE,
};
		...
		void UpdateAI(const uint32 diff) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_PHASE:
					...
					EnterPhase(
							phase == PHASE_FIGHT ? PHASE_DANCE : PHASE_FIGHT);
					break;
				case EVENT_ERUPT:
					...
					events.ScheduleEvent(EVENT_ERUPT,
							phase == PHASE_FIGHT ? 10000 : 3000);
					break;
				}
			}
