...
enum Creatures {
	MOB_WEB_WRAP = 16486, MOB_SPIDERLING = 17055,
};

#define MAX_POS_WRAP            3
const Position PosWrap[MAX_POS_WRAP] = {
		{ 3546.796f, -3869.082f, 296.450f, 0.0f }, { 3531.271f, -3847.424f,
				299.450f, 0.0f }, { 3497.067f, -3843.384f, 302.384f, 0.0f }, };
...
public:
	...
	struct mob_webwrapAI: public NullCreatureAI {
		mob_webwrapAI(Creature *c) :
				NullCreatureAI(c), victimGUID(0) {
		}
		...
		void SetGUID(const uint64 &guid, int32 /*param*/) {
			...
			if (me->m_spells[0] && victimGUID)
				if (Unit *victim = Unit::GetUnit(*me, victimGUID))
					victim->CastSpell(victim, me->m_spells[0], true, NULL, NULL,
							me->GetGUID());
		}
