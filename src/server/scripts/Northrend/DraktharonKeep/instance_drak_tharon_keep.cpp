...
public:
	instance_drak_tharon() :
			InstanceMapScript("instance_drak_tharon", 600) {
	}

	struct instance_drak_tharon_InstanceScript: public InstanceScript {
		instance_drak_tharon_InstanceScript(Map* pMap) :
				InstanceScript(pMap) {
			Initialize();
		}
		;
		...
		uint64 GetData64(uint32 identifier) {
			...
			case DATA_NOVOS_CRYSTAL_1:
				return uiNovosCrystal1;
			case DATA_NOVOS_CRYSTAL_2:
				return uiNovosCrystal2;
			case DATA_NOVOS_CRYSTAL_3:
				return uiNovosCrystal3;
			case DATA_NOVOS_CRYSTAL_4:
				return uiNovosCrystal4;
			}
		...
		std::string GetSaveData() {
			...
			saveStream << "D K " << m_auiEncounter[0] << " "
