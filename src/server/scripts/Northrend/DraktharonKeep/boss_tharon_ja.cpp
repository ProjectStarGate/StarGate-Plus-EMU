...
enum Models {
	MODEL_FLESH = 27073, MODEL_SKELETON = 27511
};
enum CombatPhase {
	SKELETAL, GOING_FLESH, FLESH, GOING_SKELETAL
};
...
public:
	...
	struct boss_tharon_jaAI: public BossAI {
		boss_tharon_jaAI(Creature *c) :
				BossAI(c, DATA_THARON_JA_EVENT) {
		...
		void Reset() {
			...
			uiRainOfFireTimer = urand(14 * IN_MILLISECONDS,
					18 * IN_MILLISECONDS);
			uiShadowVolleyTimer = urand(8 * IN_MILLISECONDS,
					10 * IN_MILLISECONDS);
			Phase = SKELETAL;
		...
		void UpdateAI(const uint32 diff) {
			...
			case SKELETAL:
				if (uiCurseOfLifeTimer < diff) {
					...
					uiCurseOfLifeTimer = urand(10 * IN_MILLISECONDS,
							15 * IN_MILLISECONDS);
				} else
					uiCurseOfLifeTimer -= diff;
			...
			case GOING_FLESH:
				if (uiPhaseTimer < diff) {
					...
					SelectTargetList(playerList, 5, SELECT_TARGET_TOPAGGRO, 0,
							true);
					for (std::list<Unit*>::const_iterator itr =
							playerList.begin(); itr != playerList.end();
							++itr) {
						Unit *pTemp = (*itr);
					...
					uiPhaseTimer = 20 * IN_MILLISECONDS;
					uiLightningBreathTimer = urand(3 * IN_MILLISECONDS,
							4 * IN_MILLISECONDS);
					uiEyeBeamTimer = urand(4 * IN_MILLISECONDS,
							8 * IN_MILLISECONDS);
					uiPoisonCloudTimer = urand(6 * IN_MILLISECONDS,
							7 * IN_MILLISECONDS);
					Phase = FLESH;
				} else
					uiPhaseTimer -= diff;
			...
			case FLESH:
				if (uiLightningBreathTimer < diff) {
					...
					uiLightningBreathTimer = urand(6 * IN_MILLISECONDS,
							7 * IN_MILLISECONDS);
				} else
					uiLightningBreathTimer -= diff;
				...
				if (uiPoisonCloudTimer < diff) {
					...
					uiPoisonCloudTimer = urand(10 * IN_MILLISECONDS,
							12 * IN_MILLISECONDS);
				} else
