...
enum Creatures {
	NPC_RAPTOR_1 = 26641, NPC_RAPTOR_2 = 26628
};
...
public:
	...
	struct boss_dredAI: public BossAI {
		boss_dredAI(Creature *c) :
				BossAI(c, DATA_DRED_EVENT) {
		...
		void Reset() {
			...
			uiFearsomeRoarTimer = urand(10 * IN_MILLISECONDS,
					20 * IN_MILLISECONDS);
			uiPiercingSlashTimer = 17 * IN_MILLISECONDS;
			uiRaptorCallTimer = urand(20 * IN_MILLISECONDS,
					25 * IN_MILLISECONDS);
		}
		...
		void UpdateAI(const uint32 diff) {
			...
			if (uiFearsomeRoarTimer < diff) {
				...
				uiFearsomeRoarTimer = urand(16 * IN_MILLISECONDS,
						18 * IN_MILLISECONDS);
			} else
			...
			if (uiRaptorCallTimer < diff) {
				...
				me->SummonCreature(RAND(NPC_RAPTOR_1, NPC_RAPTOR_2), x, y, z, 0,
						TEMPSUMMON_DEAD_DESPAWN, 1 * IN_MILLISECONDS);
...
public:
	npc_drakkari_gutripper() :
			CreatureScript("npc_drakkari_gutripper") {
	}

	struct npc_drakkari_gutripperAI: public ScriptedAI {
		npc_drakkari_gutripperAI(Creature *c) :
				ScriptedAI(c) {
		...
		void JustDied(Unit* /*killer*/) {
			if (pInstance) {
				if (IsHeroic()
						&& pInstance->GetData(DATA_DRED_EVENT) == IN_PROGRESS
						&& pInstance->GetData(DATA_KING_DRED_ACHIEV) < 6) {
					pInstance->SetData(DATA_KING_DRED_ACHIEV,
							pInstance->GetData(DATA_KING_DRED_ACHIEV) + 1);
				}
			}
		}
	};
...
public:
	npc_drakkari_scytheclaw() :
			CreatureScript("npc_drakkari_scytheclaw") {
	}

	struct npc_drakkari_scytheclawAI: public ScriptedAI {
		npc_drakkari_scytheclawAI(Creature *c) :
				ScriptedAI(c) {
			pInstance = c->GetInstanceScript();
		}
