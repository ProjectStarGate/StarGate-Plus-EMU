...
static Position CrystalHandlerSpawnPoint = { -326.626343f, -709.956604f,
		27.813314f, 0.0f };
static Position AddDestinyPoint =
		{ -379.314545f, -772.577637f, 28.58837f, 0.0f };
...
public:
	...
	struct boss_novosAI: public BossAI {
		boss_novosAI(Creature *c) :
				BossAI(c, DATA_NOVOS_EVENT) {
		...
		void Reset() {
			...
			if (instance) {
				...
				luiCrystals.push_back(
						instance->GetData64(DATA_NOVOS_CRYSTAL_1));
				luiCrystals.push_back(
						instance->GetData64(DATA_NOVOS_CRYSTAL_2));
				luiCrystals.push_back(
						instance->GetData64(DATA_NOVOS_CRYSTAL_3));
				luiCrystals.push_back(
						instance->GetData64(DATA_NOVOS_CRYSTAL_4));
				for (std::list<uint64>::const_iterator itr =
						luiCrystals.begin(); itr != luiCrystals.end(); ++itr) {
		...
		void UpdateAI(const uint32 diff) {
			...
			case PHASE_1:
				if (uiTimer <= diff) {
					Creature *pSummon = me->SummonCreature(
							RAND(CREATURE_FETID_TROLL_CORPSE,
									CREATURE_HULKING_CORPSE,
									CREATURE_RISEN_SHADOWCASTER), AddSpawnPoint,
				...
				if (crystalHandlerAmount < 4) {
					if (uiCrystalHandlerTimer <= diff) {
						...
						Creature *pCrystalHandler = me->SummonCreature(
								CREATURE_CRYSTAL_HANDLER,
								CrystalHandlerSpawnPoint,
								TEMPSUMMON_CORPSE_TIMED_DESPAWN,
								20 * IN_MILLISECONDS);
						pCrystalHandler->GetMotionMaster()->MovePoint(0,
								AddDestinyPoint);
						uiCrystalHandlerTimer = urand(20 * IN_MILLISECONDS,
								30 * IN_MILLISECONDS);
					} else
				...
				if (uiTimer <= diff) {
					if (Unit *pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true))
						DoCast(
								pTarget,
								DUNGEON_MODE(
										RAND(SPELL_ARCANE_BLAST, SPELL_BLIZZARD,
												SPELL_FROSTBOLT,
												SPELL_WRATH_OF_MISERY),
										RAND(H_SPELL_ARCANE_BLAST,
												H_SPELL_BLIZZARD,
												H_SPELL_FROSTBOLT,
												H_SPELL_WRATH_OF_MISERY)));
...
enum CrystalHandlerSpells {
	SPELL_FLASH_OF_DARKNESS = 49668, H_SPELL_FLASH_OF_DARKNESS = 59004
};

class mob_crystal_handler: public CreatureScript {
public:
	mob_crystal_handler() :
			CreatureScript("mob_crystal_handler") {
	}

	struct mob_crystal_handlerAI: public ScriptedAI {
		mob_crystal_handlerAI(Creature *c) :
				ScriptedAI(c) {
		...
		void UpdateAI(const uint32 diff) {
			...
			if (uiFlashOfDarknessTimer <= diff) {
				DoCast(
						me->getVictim(),
						DUNGEON_MODE(SPELL_FLASH_OF_DARKNESS,
								H_SPELL_FLASH_OF_DARKNESS));
				uiFlashOfDarknessTimer = 5 * IN_MILLISECONDS;
			} else
				uiFlashOfDarknessTimer -= diff;
...
public:
	mob_novos_minion() :
			CreatureScript("mob_novos_minion") {
	}

	struct mob_novos_minionAI: public ScriptedAI {
		mob_novos_minionAI(Creature *c) :
				ScriptedAI(c) {
			pInstance = c->GetInstanceScript();
		}
		...
		void MovementInform(uint32 type, uint32 id) {
			if (type != POINT_MOTION_TYPE || id != 0)
				return;
			if (Creature* pNovos = Unit::GetCreature(*me, pInstance ? pInstance->GetData64(DATA_NOVOS) : 0)) {
				CAST_AI(boss_novos::boss_novosAI, pNovos->AI())->bAchiev =
						false;
				if (Unit *pTarget = CAST_AI(boss_novos::boss_novosAI, pNovos->AI())->GetRandomTarget())
					AttackStart(pTarget);
			}
