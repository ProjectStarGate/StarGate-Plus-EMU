...
enum Shadowmourne {
	QUEST_BLOOD_INFUSION = 24756, ITEM_SHADOW_S_EDGE = 49888,

	SPELL_GUSHING_WOUND = 72132, SPELL_THIRST_QUENCHED = 72154,
};

static const uint32 vampireAuras[3][MAX_DIFFICULTY] =
		{ { 70867, 71473, 71532, 71533 }, { 70879, 71525, 71530, 71531 }, {
...
enum ePoints {
	POINT_CENTER = 1, POINT_AIR = 2, POINT_GROUND = 3,
};
...
public:
	...
	struct boss_blood_queen_lana_thelAI: public BossAI {
		...
		void Reset() {
			...
			events.ScheduleEvent(EVENT_DELIRIOUS_SLASH, urand(20000, 24000),
					EVENT_GROUP_NORMAL);
			events.ScheduleEvent(EVENT_PACT_OF_THE_DARKFALLEN, 15000,
					EVENT_GROUP_NORMAL);
			events.ScheduleEvent(EVENT_SWARMING_SHADOWS, 30500,
					EVENT_GROUP_NORMAL);
			events.ScheduleEvent(EVENT_TWILIGHT_BLOODBOLT, urand(20000, 25000),
					EVENT_GROUP_NORMAL);
			events.ScheduleEvent(EVENT_AIR_PHASE,
					124000 + uint32(Is25ManRaid() ? 3000 : 0));
		...
		void MovementInform(uint32 type, uint32 id) {
			...
			case POINT_CENTER:
				...
				events.ScheduleEvent(EVENT_AIR_PHASE,
						100000 + uint32(Is25ManRaid() ? 0 : 20000));
				events.RescheduleEvent(EVENT_SWARMING_SHADOWS, 30500,
						EVENT_GROUP_NORMAL);
				events.RescheduleEvent(EVENT_PACT_OF_THE_DARKFALLEN, 25500,
						EVENT_GROUP_NORMAL);
		...
		void UpdateAI(const uint32 diff) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_BLOOD_MIRROR: {
					...
					if (me->getVictim()) {
						...
						if (offtank != newOfftank) {
							offtank = newOfftank;
							if (offtank) {
								offtank->CastSpell(me->getVictim(),
										SPELL_BLOOD_MIRROR_DAMAGE, true);
								me->getVictim()->CastSpell(offtank,
										SPELL_BLOOD_MIRROR_DUMMY, true);
								DoCastVictim(SPELL_BLOOD_MIRROR_VISUAL);
								if (Item* shadowsEdge = offtank->GetWeaponForAttack(BASE_ATTACK, true))
									if (!offtank->HasAura(SPELL_THIRST_QUENCHED)
											&& shadowsEdge->GetEntry()
													== ITEM_SHADOW_S_EDGE
											&& !offtank->HasAura(
													SPELL_GUSHING_WOUND))
										offtank->CastSpell(offtank,
												SPELL_GUSHING_WOUND, true);
							}
						}
					}
					events.ScheduleEvent(EVENT_BLOOD_MIRROR, 2500,
				...
				case EVENT_DELIRIOUS_SLASH:
					if (offtank
							&& !me->HasByteFlag(UNIT_FIELD_BYTES_1, 3, 0x03))
						DoCast(offtank, SPELL_DELIRIOUS_SLASH);
					events.ScheduleEvent(EVENT_DELIRIOUS_SLASH,
							urand(20000, 24000), EVENT_GROUP_NORMAL);
					break;
				case EVENT_PACT_OF_THE_DARKFALLEN: {
					...
					if (targets.size() > 1) {
						...
						for (std::list<Player*>::iterator itr = targets.begin();
								itr != targets.end(); ++itr)
							DoCast(*itr, SPELL_PACT_OF_THE_DARKFALLEN);
					}
					events.ScheduleEvent(EVENT_PACT_OF_THE_DARKFALLEN, 30500,
							EVENT_GROUP_NORMAL);
					break;
				}
				...
				case EVENT_TWILIGHT_BLOODBOLT: {
					std::list<Player*> targets;
					SelectRandomTarget(false, &targets);
					Trinity::RandomResizeList<Player*>(targets,
							uint32(Is25ManRaid() ? 4 : 2));
					for (std::list<Player*>::iterator itr = targets.begin();
							itr != targets.end(); ++itr)
						DoCast(*itr, SPELL_TWILIGHT_BLOODBOLT);
					DoCast(me, SPELL_TWILIGHT_BLOODBOLT_TARGET);
					events.ScheduleEvent(EVENT_TWILIGHT_BLOODBOLT,
							urand(10000, 15000), EVENT_GROUP_NORMAL);
					break;
				}
	...
	private:
		...
		Player* SelectRandomTarget(bool includeOfftank,
			...
			for (std::list<HostileReference*>::const_iterator itr =
				...
				if (Unit* refTarget = (*itr)->getTarget())
					if (refTarget != me->getVictim()
							&& refTarget->GetTypeId() == TYPEID_PLAYER
							&& (includeOfftank ? true : (refTarget != offtank)))
			...
			if (includeOfftank) {
				tempTargets.sort(
						Trinity::ObjectDistanceOrderPred(me->getVictim()));
	...
	class spell_blood_queen_vampiric_bite_SpellScript: public SpellScript {
		...
		void OnCast() {
			...
			if (IsVampire(GetHitUnit())) {
				...
				Spell::SendCastResult(GetCaster()->ToPlayer(), GetSpellInfo(),
						0, SPELL_FAILED_BAD_TARGETS);
				return;
			}
			...
			spell = sSpellMgr->GetSpellForDifficultyFromSpell(spell,
					GetCaster());
			GetCaster()->RemoveAura(spell->Id, 0, 0,
					AURA_REMOVE_BY_ENEMY_SPELL);
			GetCaster()->CastSpell(GetCaster(),
					SPELL_ESSENCE_OF_THE_BLOOD_QUEEN_PLR, true);
			// Presence of the Darkfallen buff on Blood-Queen
			if (GetCaster()->GetMap()->IsHeroic())
				GetCaster()->CastSpell(GetCaster(),
						SPELL_PRESENCE_OF_THE_DARKFALLEN, true);
			// Shadowmourne questline
			if (GetCaster()->ToPlayer()->GetQuestStatus(QUEST_BLOOD_INFUSION)
					== QUEST_STATUS_INCOMPLETE) {
				if (Aura* aura = GetCaster()->GetAura(SPELL_GUSHING_WOUND)) {
					if (aura->GetStackAmount() == 3) {
						GetCaster()->CastSpell(GetCaster(),
								SPELL_THIRST_QUENCHED, true);
						GetCaster()->RemoveAura(aura);
					} else
						GetCaster()->CastSpell(GetCaster(), SPELL_GUSHING_WOUND,
								true);
				}
			}
		...
		void Register() {
			BeforeHit +=
					SpellHitFn(spell_blood_queen_vampiric_bite_SpellScript::OnCast);
		}
	};
			...
			if (GetTargetApplication()->GetRemoveMode()
				...
				if (InstanceScript* instance = target->GetInstanceScript())
					if (Creature* bloodQueen = ObjectAccessor::GetCreature(*target, instance->GetData64(DATA_BLOOD_QUEEN_LANA_THEL))) {
						...
						bloodQueen->CastSpell(target,
								SPELL_UNCONTROLLABLE_FRENZY, true);
					}
		}

		void Register() {
			OnEffectRemove +=
					AuraEffectRemoveFn(spell_blood_queen_frenzied_bloodthirst_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_OVERRIDE_SPELLS, AURA_EFFECT_HANDLE_REAL);
		}
	};
	...
	class spell_blood_queen_bloodbolt_SpellScript: public SpellScript {
		...
		void HandleDummy() {
			if (GetHitUnit()->GetTypeId() == TYPEID_PLAYER)
				GetCaster()->CastSpell(GetHitUnit(), SPELL_TWILIGHT_BLOODBOLT,
						true);
		}

		void Register() {
			AfterHit +=
					SpellHitFn(spell_blood_queen_bloodbolt_SpellScript::HandleDummy);
		}
	};
...
public:
	PactOfTheDarkfallenCheck(bool hasPact) :
			_hasPact(hasPact) {
	}
...
public:
	...
	class spell_blood_queen_pact_of_the_darkfallen_SpellScript: public SpellScript {
		...
		void FilterTargets(std::list<Unit*>& unitList) {
			...
			for (itr = unitList.begin(); itr != itrEnd && remove; ++itr) {
				...
				for (itr2 = unitList.begin(); itr2 != itrEnd && remove; ++itr2)
					if (itr != itr2
							&& !(*itr2)->IsWithinDist(*itr, 5.0f, false))
						remove = false;
			}
		...
		void Register() {
			OnUnitTargetSelect +=
					SpellUnitTargetFn(spell_blood_queen_pact_of_the_darkfallen_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_AREA_ALLY_SRC);
		}
	};
...
public:
	...
	class spell_blood_queen_pact_of_the_darkfallen_dmg_AuraScript: public AuraScript {
		...
		void PeriodicTick(AuraEffect const* aurEff) {
			SpellEntry const* damageSpell = sSpellStore.LookupEntry(
					SPELL_PACT_OF_THE_DARKFALLEN_DAMAGE);
			int32 damage = SpellMgr::CalculateSpellEffectAmount(damageSpell,
					EFFECT_0);
			float multiplier = 0.3375f
					+ 0.1f * uint32(aurEff->GetTickNumber() / 10); // do not convert to 0.01f - we need tick number/10 as INT (damage increases every 10 ticks)
			damage = uint32(damage * multiplier);
			GetTarget()->CastCustomSpell(SPELL_PACT_OF_THE_DARKFALLEN_DAMAGE,
					SPELLVALUE_BASE_POINT0, damage, GetTarget(), true);
		}

		void Register() {
			OnEffectPeriodic +=
					AuraEffectPeriodicFn(spell_blood_queen_pact_of_the_darkfallen_dmg_AuraScript::PeriodicTick, EFFECT_1, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
		}
	};
...
public:
	...
	class spell_blood_queen_pact_of_the_darkfallen_dmg_SpellScript: public SpellScript {
		...
		void Register() {
			OnUnitTargetSelect +=
					SpellUnitTargetFn(spell_blood_queen_pact_of_the_darkfallen_dmg_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_AREA_ALLY_SRC);
		}
	};
...
public:
	achievement_once_bitten_twice_shy_n() :
			AchievementCriteriaScript("achievement_once_bitten_twice_shy_n") {
	}
...
public:
	achievement_once_bitten_twice_shy_v() :
			AchievementCriteriaScript("achievement_once_bitten_twice_shy_v") {
	}
