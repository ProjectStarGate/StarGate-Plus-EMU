...
enum MovementPoints {
	POINT_TARGET_BONESTORM_PLAYER = 36612631, POINT_TARGET_COLDFLAME = 36672631,
};
...
public:
	...
	struct boss_lord_marrowgarAI: public BossAI {
		...
		void Reset() {
			...
			events.ScheduleEvent(EVENT_BONE_SPIKE_GRAVEYARD,
					urand(10000, 15000), EVENT_GROUP_SPECIAL);
		...
		void UpdateAI(const uint32 diff) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_BONE_SPIKE_GRAVEYARD:
					...
					events.ScheduleEvent(EVENT_BONE_SPIKE_GRAVEYARD,
							urand(15000, 20000), EVENT_GROUP_SPECIAL);
					break;
				case EVENT_COLDFLAME:
					coldflameLastPos.Relocate(me);
					if (!me->HasAura(SPELL_BONE_STORM))
						me->CastCustomSpell(SPELL_COLDFLAME_NORMAL,
								SPELLVALUE_MAX_TARGETS, 1, me);
					else
						DoCast(me, SPELL_COLDFLAME_BONE_STORM);
					events.ScheduleEvent(EVENT_COLDFLAME, 5000,
							EVENT_GROUP_SPECIAL);
					break;
				case EVENT_WARN_BONE_STORM:
					...
					events.ScheduleEvent(EVENT_WARN_BONE_STORM,
							urand(90000, 95000));
					break;
				case EVENT_BONE_STORM_BEGIN:
					...
					Talk(SAY_BONE_STORM);
					events.ScheduleEvent(EVENT_BONE_STORM_END,
							boneStormDuration + 1);
					// no break here
				case EVENT_BONE_STORM_MOVE: {
					events.ScheduleEvent(EVENT_BONE_STORM_MOVE,
							boneStormDuration / 3);
				...
				case EVENT_BONE_STORM_END:
					...
					events.ScheduleEvent(EVENT_ENABLE_BONE_SLICE, 10000);
					if (!IsHeroic())
						events.RescheduleEvent(EVENT_BONE_SPIKE_GRAVEYARD,
								urand(15000, 20000), EVENT_GROUP_SPECIAL);
					break;
		...
		void MovementInform(uint32 type, uint32 id) {
			if (type != POINT_MOTION_TYPE
					|| id != POINT_TARGET_BONESTORM_PLAYER)
				return;
	...
	struct npc_coldflameAI: public ScriptedAI {
		...
		void IsSummonedBy(Unit* owner) {
			...
			if (!owner->HasAura(SPELL_BONE_STORM)) {
				...
				Unit* target = creOwner->AI()->SelectTarget(
						SELECT_TARGET_RANDOM, 1, -owner->GetObjectSize(), true,
						-SPELL_IMPALED);
				if (!target)
					target = creOwner->AI()->SelectTarget(SELECT_TARGET_RANDOM,
							0, 0.0f, true); // or the tank if its solo
				if (!target) {
				...
				owner->GetNearPosition(pos, owner->GetObjectSize() / 2.0f,
						0.0f);
			} else {
				MarrowgarAI* marrowgarAI = CAST_AI(MarrowgarAI, creOwner->AI());
				Position const* ownerPos =
						marrowgarAI->GetLastColdflamePosition();
		...
		void UpdateAI(const uint32 diff) {
			...
			if (events.ExecuteEvent() == EVENT_COLDFLAME_TRIGGER) {
				...
				me->NearTeleportTo(newPos.GetPositionX(), newPos.GetPositionY(),
						me->GetPositionZ(), me->GetOrientation());
...
public:
	...
	struct npc_bone_spikeAI: public Scripted_NoMovementAI {
		npc_bone_spikeAI(Creature* creature) :
				Scripted_NoMovementAI(creature), hasTrappedUnit(false) {
...
public:
	...
	class spell_marrowgar_coldflame_SpellScript: public SpellScript {
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_marrowgar_coldflame_SpellScript::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
		}
	};
...
public:
	spell_marrowgar_coldflame_damage() :
			SpellScriptLoader("spell_marrowgar_coldflame_damage") {
	}
	...
	class spell_marrowgar_coldflame_damage_AuraScript: public AuraScript {
		...
		void OnPeriodic(AuraEffect const* /*aurEff*/) {
			if (DynamicObject* owner = GetDynobjOwner())
				if (GetTarget()->GetExactDist2d(owner) > owner->GetRadius()
						|| GetTarget()->HasAura(SPELL_IMPALED))
					PreventDefaultAction();
		}

		void Register() {
			OnEffectPeriodic +=
					AuraEffectPeriodicFn(spell_marrowgar_coldflame_damage_AuraScript::OnPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);
		}
	};
	...
	class spell_marrowgar_bone_spike_graveyard_SpellScript: public SpellScript {
		...
		void HandleSpikes(SpellEffIndex effIndex) {
			...
			if (Creature* marrowgar = GetCaster()->ToCreature()) {
				...
				uint8 boneSpikeCount = uint8(
						GetCaster()->GetMap()->GetSpawnMode() & 1 ? 3 : 1);
				for (uint8 i = 0; i < boneSpikeCount; ++i) {
					// select any unit but not the tank
					Unit* target = marrowgarAI->SelectTarget(
							SELECT_TARGET_RANDOM, 1, 100.0f, true,
							-SPELL_IMPALED);
					// try the tank only in first iteration
					if (!target && !i)
						target = marrowgarAI->SelectTarget(SELECT_TARGET_RANDOM,
								0, 100.0f, true, -SPELL_IMPALED);
					if (!target)
						return;
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_marrowgar_bone_spike_graveyard_SpellScript::HandleSpikes, EFFECT_1, SPELL_EFFECT_APPLY_AURA);
		}
	};
...
public:
	...
	class spell_marrowgar_bone_storm_SpellScript: public SpellScript {
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_marrowgar_bone_storm_SpellScript::RecalculateDamage, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
		}
	};
