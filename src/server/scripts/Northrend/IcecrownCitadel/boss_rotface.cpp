	...
	struct boss_rotfaceAI: public BossAI {
		...
		Unit* GetAuraEffectTriggerTarget(uint32 spellId, uint8 /*effIndex*/) {
			if (spellId == SPELL_SLIME_SPRAY) {
				for (std::list<uint64>::iterator itr = summons.begin();
						itr != summons.end();) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_SLIME_SPRAY:
					if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 0.0f, true)) {
						...
						DoSummon(NPC_OOZE_SPRAY_STALKER, pos, 8000,
				...
				case EVENT_MUTATED_INFECTION: {
					Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 0.0f,
							true, -MUTATED_INFECTION);
					if (!target)
...
public:
	npc_little_ooze() :
			CreatureScript("npc_little_ooze") {
	}
...
public:
	npc_precious_icc() :
			CreatureScript("npc_precious_icc") {
	}
	...
	struct npc_precious_iccAI: public ScriptedAI {
		...
		void UpdateAI(const uint32 diff) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_MORTAL_WOUND:
					...
					events.ScheduleEvent(EVENT_MORTAL_WOUND,
							urand(10000, 12500));
					break;
				default:
					break;
		...
		void JustDied(Unit* /*who*/) {
			uint64 rotfaceGUID =
					instance ? instance->GetData64(DATA_ROTFACE) : 0;
...
public:
	spell_rotface_ooze_flood() :
			SpellScriptLoader("spell_rotface_ooze_flood") {
	}
	...
	class spell_rotface_ooze_flood_SpellScript: public SpellScript {
		...
		void HandleScript(SpellEffIndex /*effIndex*/) {
			...
			GetHitUnit()->GetCreatureListWithEntryInGrid(list,
					GetHitUnit()->GetEntry(), 12.5f);
			list.sort(Trinity::ObjectDistanceOrderPred(GetHitUnit()));
			GetHitUnit()->CastSpell(list.back(), uint32(GetEffectValue()),
					false, NULL, NULL,
					GetOriginalCaster() ? GetOriginalCaster()->GetGUID() : 0);
		}
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_rotface_ooze_flood_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
			OnUnitTargetSelect +=
					SpellUnitTargetFn(spell_rotface_ooze_flood_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_AREA_ENTRY_SRC);
		}
	};
...
public:
	...
	class spell_rotface_little_ooze_combine_SpellScript: public SpellScript {
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_rotface_little_ooze_combine_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
		}
	};
...
public:
	...
	class spell_rotface_large_ooze_combine_SpellScript: public SpellScript {
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_rotface_large_ooze_combine_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
		}
	};
...
public:
	...
	class spell_rotface_large_ooze_buff_combine_SpellScript: public SpellScript {
			...
			if (Aura* unstable = GetCaster()->GetAura(SPELL_UNSTABLE_OOZE)) {
				...
				if (newStack >= 5) {
					...
					GetCaster()->CastSpell(GetCaster(),
							SPELL_UNSTABLE_OOZE_EXPLOSION, false, NULL, NULL,
							GetCaster()->GetGUID());
					if (InstanceScript* instance = GetCaster()->GetInstanceScript())
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_rotface_large_ooze_buff_combine_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
		}
	};
...
public:
	...
	class spell_rotface_unstable_ooze_explosion_init_SpellScript: public SpellScript {
		...
		void HandleCast(SpellEffIndex effIndex) {
			...
			Creature* dummy = GetCaster()->SummonCreature(
					NPC_UNSTABLE_EXPLOSION_STALKER, x, y, z, 0.0f,
					TEMPSUMMON_TIMED_DESPAWN, 60000);
			GetCaster()->CastSpell(dummy, SPELL_UNSTABLE_OOZE_EXPLOSION_TRIGGER,
					true);
		}

		void Register() {
			OnEffect +=
					SpellEffectFn(spell_rotface_unstable_ooze_explosion_init_SpellScript::HandleCast, EFFECT_0, SPELL_EFFECT_FORCE_CAST);
		}
	};
	...
	class spell_rotface_unstable_ooze_explosion_SpellScript: public SpellScript {
		...
		void CheckTarget(SpellEffIndex effIndex) {
			...
			uint32 triggered_spell_id =
					GetSpellInfo()->EffectTriggerSpell[effIndex];
			...
			if (InstanceScript* script = GetTargetUnit()->GetInstanceScript())
				if (Creature* rotface = script->instance->GetCreature(script->GetData64(DATA_ROTFACE)))
					rotface->CastSpell(x, y, z, triggered_spell_id, true, NULL,
							NULL, GetCaster()->GetGUID(), GetTargetUnit());
		}

		void Register() {
			OnEffect +=
					SpellEffectFn(spell_rotface_unstable_ooze_explosion_SpellScript::CheckTarget, EFFECT_0, SPELL_EFFECT_TRIGGER_MISSILE);
		}
	};
...
public:
	...
	class spell_rotface_unstable_ooze_explosion_suicide_AuraScript: public AuraScript {
		...
		void Register() {
			OnEffectPeriodic +=
					AuraEffectPeriodicFn(spell_rotface_unstable_ooze_explosion_suicide_AuraScript::DespawnSelf, EFFECT_2, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
		}
	};
...
public:
	spell_rotface_slime_spray() :
			SpellScriptLoader("spell_rotface_slime_spray") {
	}
	...
	class spell_rotface_slime_spray_SpellScript: public SpellScript {
		...
		void ExtraEffect() {
			...
			GetHitUnit()->CastSpell(GetHitUnit(), SPELL_GREEN_BLIGHT_RESIDUE,
					true);
		}

		void Register() {
			AfterHit +=
					SpellHitFn(spell_rotface_slime_spray_SpellScript::ExtraEffect);
		}
	};
