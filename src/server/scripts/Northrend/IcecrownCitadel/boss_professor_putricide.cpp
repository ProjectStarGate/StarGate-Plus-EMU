...
enum Phases {
	...
	PHASE_MASK_COMBAT = (1 << PHASE_COMBAT_1) | (1 << PHASE_COMBAT_2)
			| (1 << PHASE_COMBAT_3),
	PHASE_MASK_NOT_SELF = (1 << PHASE_FESTERGUT) | (1 << PHASE_ROTFACE),
};

enum Points {
	POINT_FESTERGUT = 366260, POINT_ROTFACE = 366270, POINT_TABLE = 366780,
};

static const Position festergutWatchPos = { 4324.820f, 3166.03f, 389.3831f,
		3.316126f }; //emote 432 (release gas)
static const Position rotfaceWatchPos = { 4390.371f, 3164.50f, 389.3890f,
		5.497787f }; //emote 432 (release ooze)
...
public:
	...
	struct boss_professor_putricideAI: public BossAI {
		boss_professor_putricideAI(Creature* creature) :
				BossAI(creature, DATA_PROFESSOR_PUTRICIDE), baseSpeed(
						creature->GetSpeedRate(MOVE_RUN)), experimentState(
		...
		void InitializeAI() {
			if (!instance
					|| static_cast<InstanceMap*>(me->GetMap())->GetScriptId()
							!= GetScriptId(ICCScriptName))
		...
		void EnterCombat(Unit* who) {
			...
			events.ScheduleEvent(EVENT_UNSTABLE_EXPERIMENT,
					urand(25000, 30000));
			if (IsHeroic())
		...
		void JustSummoned(Creature* summon) {
			...
			case NPC_GAS_CLOUD:
				...
				summon->SetFlag(UNIT_FIELD_AURASTATE,
						1 << (AURA_STATE_UNKNOWN22 - 1));
				summon->CastSpell(summon, SPELL_GASEOUS_BLOAT_PROC, true);
				if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 0.0f, true, -SPELL_GASEOUS_BLOAT_PROTECTION)) {
					summon->AI()->AttackStart(target);
					summon->CastCustomSpell(SPELL_GASEOUS_BLOAT,
							SPELLVALUE_MAX_TARGETS, 1, target, false);
					summon->ClearUnitState(UNIT_STAT_CASTING);
					summon->GetMotionMaster()->MoveIdle();
					summon->m_Events.AddEvent(new StartMovementEvent(*summon),
							summon->m_Events.CalculateTime(3500));
			...
			case NPC_VOLATILE_OOZE:
				// no possible aura seen in sniff adding the aurastate
				summon->SetFlag(UNIT_FIELD_AURASTATE,
						1 << (AURA_STATE_UNKNOWN19 - 1));
				summon->CastSpell(summon, SPELL_OOZE_ERUPTION_SEARCH_PERIODIC,
						true);
				if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 0.0f, true, -SPELL_VOLATILE_OOZE_PROTECTION)) {
					summon->AI()->AttackStart(target);
					summon->CastCustomSpell(SPELL_VOLATILE_OOZE_ADHESIVE,
							SPELLVALUE_MAX_TARGETS, 1, target, false);
					summon->ClearUnitState(UNIT_STAT_CASTING);
					summon->GetMotionMaster()->MoveIdle();
					summon->m_Events.AddEvent(new StartMovementEvent(*summon),
			...
			case NPC_CHOKING_GAS_BOMB:
				summon->CastSpell(summon, SPELL_CHOKING_GAS_BOMB_PERIODIC,
						true);
				summon->CastSpell(summon, SPELL_CHOKING_GAS_EXPLOSION_TRIGGER,
						true);
				return;
		...
		void MovementInform(uint32 type, uint32 id) {
			...
			case POINT_FESTERGUT:
				...
				if (Creature* festergut = Unit::GetCreature(*me, instance->GetData64(DATA_FESTERGUT)))
					festergut->CastSpell(festergut, SPELL_GASEOUS_BLIGHT_LARGE,
							false, NULL, NULL, festergut->GetGUID());
				break;
			case POINT_ROTFACE:
				instance->SetBossState(DATA_ROTFACE, IN_PROGRESS); // needed here for delayed gate close
				me->SetSpeed(MOVE_RUN, baseSpeed, true);
				DoAction(ACTION_ROTFACE_OOZE);
				events.ScheduleEvent(EVENT_ROTFACE_OOZE_FLOOD, 25000, 0,
						PHASE_ROTFACE);
				break;
			case POINT_TABLE:
				// stop attack
				...
				case PHASE_COMBAT_2: {
					...
					spell = sSpellMgr->GetSpellForDifficultyFromSpell(spell,
							me);
					DoCast(me, SPELL_CREATE_CONCOCTION);
					events.ScheduleEvent(EVENT_PHASE_TRANSITION,
							GetSpellCastTime(spell) + 100);
					break;
				}
				case PHASE_COMBAT_3: {
					SpellEntry const* spell = sSpellStore.LookupEntry(
		...
		void DoAction(const int32 action) {
			...
			case ACTION_FESTERGUT_COMBAT:
				...
				me->GetMotionMaster()->MovePoint(POINT_FESTERGUT,
						festergutWatchPos);
				me->SetReactState(REACT_PASSIVE);
				DoZoneInCombat(me);
				if (IsHeroic())
					events.ScheduleEvent(EVENT_FESTERGUT_GOO,
							urand(15000, 20000), 0, PHASE_FESTERGUT);
				break;
			...
			case ACTION_FESTERGUT_DEATH:
				events.ScheduleEvent(EVENT_FESTERGUT_DIES, 4000, 0,
						PHASE_FESTERGUT);
				break;
			case ACTION_ROTFACE_COMBAT: {
				_SetPhase(PHASE_ROTFACE);
				me->SetSpeed(MOVE_RUN, baseSpeed * 2.0f, true);
				me->GetMotionMaster()->MovePoint(POINT_ROTFACE,
						rotfaceWatchPos);
				me->SetReactState(REACT_PASSIVE);
				oozeFloodStage = 0;
				DoZoneInCombat(me);
				if (IsHeroic())
					events.ScheduleEvent(EVENT_ROTFACE_VILE_GAS,
							urand(15000, 20000), 0, PHASE_ROTFACE);
				// init random sequence of floods
				if (Creature* rotface = Unit::GetCreature(*me, instance->GetData64(DATA_ROTFACE))) {
					std::list<Creature*> list;
					GetCreatureListWithEntryInGrid(list, rotface,
							NPC_PUDDLE_STALKER, 36.0f);
					if (list.size() > 4) {
			...
			case ACTION_ROTFACE_OOZE:
				...
				if (Creature* dummy = Unit::GetCreature(*me, oozeFloodDummy[oozeFloodStage]))
					dummy->CastSpell(dummy, oozeFloodSpells[oozeFloodStage],
							true, NULL, NULL, me->GetGUID()); // cast from self for LoS (with prof's GUID for logs)
				if (++oozeFloodStage == 4)
					oozeFloodStage = 0;
				break;
			case ACTION_ROTFACE_DEATH:
				events.ScheduleEvent(EVENT_ROTFACE_DIES, 4500, 0,
						PHASE_ROTFACE);
				break;
			case ACTION_CHANGE_PHASE:
				...
				if (!IsHeroic()) {
					...
					if (Is25ManRaid()) {
						...
						{
							const std::list<HostileReference*>& threatlist =
									me->getThreatManager().getThreatList();
							for (std::list<HostileReference*>::const_iterator itr =
									threatlist.begin(); itr != threatlist.end();
									++itr)
								if ((*itr)->getTarget()->GetTypeId()
										== TYPEID_PLAYER)
						...
						for (std::list<Unit*>::iterator itr =
								targetList.begin(); itr != targetList.end();
								++itr)
		...
		void UpdateAI(const uint32 diff) {
			if ((!UpdateVictim()
					&& !(events.GetPhaseMask() & PHASE_MASK_NOT_SELF))
					|| !CheckInRoom())
				return;
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_SLIME_PUDDLE: {
					...
					SelectTargetList(targets, 2, SELECT_TARGET_RANDOM, 0.0f,
							true);
					if (!targets.empty())
				...
				case EVENT_MALLEABLE_GOO:
					if (Is25ManRaid()) {
						std::list<Unit*> targets;
						SelectTargetList(targets, 2, SELECT_TARGET_RANDOM,
								-7.0f, true);
						if (!targets.empty()) {
							Talk(EMOTE_MALLEABLE_GOO);
							for (std::list<Unit*>::iterator itr =
									targets.begin(); itr != targets.end();
									++itr)
				...
				case EVENT_UNBOUND_PLAGUE:
					if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 0.0f, true)) {
						me->CastCustomSpell(SPELL_UNBOUND_PLAGUE,
								SPELLVALUE_BASE_POINT0, 775, target);
				...
				case EVENT_PHASE_TRANSITION: {
					...
					case PHASE_COMBAT_2:
						...
						events.ScheduleEvent(EVENT_RESUME_ATTACK, 5500, 0,
								PHASE_COMBAT_2);
						break;
					case PHASE_COMBAT_3:
						if (Creature* face = me->FindNearestCreature(NPC_TEAR_GAS_TARGET_STALKER, 50.0f))
						...
						events.ScheduleEvent(EVENT_RESUME_ATTACK, 8500, 0,
								PHASE_COMBAT_3);
						break;
					default:
						break;
...
public:
	...
	struct npc_volatile_oozeAI: public ScriptedAI {
		...
		void SpellHitTarget(Unit* /*target*/, SpellEntry const* spell) {
			SpellEntry const* explode = sSpellStore.LookupEntry(
					SPELL_OOZE_ERUPTION);
		...
		void UpdateAI(const uint32 diff) {
			...
			if (newTargetSelectTimer <= diff) {
				...
				if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 0.0f, true, -SPELL_VOLATILE_OOZE_PROTECTION)) {
					AttackStart(target);
					me->CastCustomSpell(SPELL_VOLATILE_OOZE_ADHESIVE,
							SPELLVALUE_MAX_TARGETS, 1, target, false);
				} else
...
public:
	...
	class spell_putricide_gaseous_bloat_SpellScript: public SpellScript {
		...
		void Register() {
			AfterHit +=
					SpellHitFn(spell_putricide_gaseous_bloat_SpellScript::ModAuraStack);
		}
	};
	...
	class spell_putricide_gaseous_bloat_AuraScript: public AuraScript {
		...
		void HandleExtraEffect(AuraEffect const* /*aurEff*/) {
			GetTarget()->RemoveAuraFromStack(GetSpellProto()->Id,
					GetCasterGUID());
		}

		void Register() {
			OnEffectPeriodic +=
					AuraEffectPeriodicFn(spell_putricide_gaseous_bloat_AuraScript::HandleExtraEffect, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);
		}
	};
...
public:
	spell_putricide_expunged_gas() :
			SpellScriptLoader("spell_putricide_expunged_gas") {
	}
	...
	class spell_putricide_expunged_gas_SpellScript: public SpellScript {
		...
		void CalcDamage(SpellEffIndex /*effIndex*/) {
			...
			Creature* professor = Unit::GetCreature(*GetCaster(),
					instance->GetData64(DATA_PROFESSOR_PUTRICIDE));
			if (!professor)
				return;
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_putricide_expunged_gas_SpellScript::CalcDamage, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
		}
	};
		...
		void HandleTriggerSpell(AuraEffect const* aurEff) {
			...
			if (Unit* caster = GetCaster()) {
				...
				uint32 triggerSpellId =
						GetSpellProto()->EffectTriggerSpell[aurEff->GetEffIndex()];
				caster->CastCustomSpell(triggerSpellId, SPELLVALUE_RADIUS_MOD,
						radiusMod * 100, caster, true);
			}
		}

		void Register() {
			OnEffectPeriodic +=
					AuraEffectPeriodicFn(spell_putricide_slime_puddle_AuraScript::HandleTriggerSpell, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
		}
	};
...
public:
	...
	class spell_putricide_unstable_experiment_SpellScript: public SpellScript {
		...
		void HandleScript(SpellEffIndex effIndex) {
			...
			uint32 stage = GetCaster()->ToCreature()->AI()->GetData(
					DATA_EXPERIMENT_STAGE);
			Creature* target = NULL;
			std::list<Creature*> creList;
			GetCreatureListWithEntryInGrid(creList, GetCaster(),
					NPC_ABOMINATION_WING_MAD_SCIENTIST_STALKER, 100.0f);
			...
			for (std::list<Creature*>::iterator itr = creList.begin();
				...
				GetCreatureListWithEntryInGrid(tmp, target,
						NPC_ABOMINATION_WING_MAD_SCIENTIST_STALKER, 1.0f);
				if ((!stage && tmp.size() > 1) || (stage && tmp.size() == 1))
					break;
			}

			GetCaster()->CastSpell(target,
					uint32(GetSpellInfo()->EffectBasePoints[stage] + 1), true,
					NULL, NULL, GetCaster()->GetGUID());
		}

		void Register() {
			OnEffect +=
					SpellEffectFn(spell_putricide_unstable_experiment_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
		}
	};
...
public:
	spell_putricide_ooze_summon() :
			SpellScriptLoader("spell_putricide_ooze_summon") {
	}
	...
	class spell_putricide_ooze_summon_AuraScript: public AuraScript {
		...
		void HandleTriggerSpell(AuraEffect const* aurEff) {
			...
			if (Unit* caster = GetCaster()) {
				...
				caster->CastSpell(x, y, z, triggerSpellId, true, NULL, NULL,
		...
		void Register() {
			OnEffectPeriodic +=
					AuraEffectPeriodicFn(spell_putricide_ooze_summon_AuraScript::HandleTriggerSpell, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
		}
	};
...
public:
	...
	class spell_putricide_ooze_eruption_searcher_SpellScript: public SpellScript {
		...
		void HandleDummy(SpellEffIndex /*effIndex*/) {
			SpellEntry const* adhesive = sSpellStore.LookupEntry(
					SPELL_VOLATILE_OOZE_ADHESIVE);
			adhesive = sSpellMgr->GetSpellForDifficultyFromSpell(adhesive,
					GetCaster());
			if (GetHitUnit()->HasAura(adhesive->Id)) {
				GetCaster()->CastSpell(GetHitUnit(), SPELL_OOZE_ERUPTION, true);
				GetHitUnit()->RemoveAurasDueToSpell(adhesive->Id,
						GetCaster()->GetGUID());
			}
		}
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_putricide_ooze_eruption_searcher_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
		}
	};
	...
	class spell_putricide_choking_gas_bomb_SpellScript: public SpellScript {
		...
		void HandleScript(SpellEffIndex /*effIndex*/) {
			...
			for (uint32 i = 0; i < 3; ++i) {
				...
				uint32 spellId = uint32(
						SpellMgr::CalculateSpellEffectAmount(GetSpellInfo(),
								uint8(i)));
				GetCaster()->CastSpell(GetCaster(), spellId, true, NULL, NULL,
						GetCaster()->GetGUID());
			}
		}
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_putricide_choking_gas_bomb_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
		}
	};
...
public:
	...
	class spell_putricide_unbound_plague_SpellScript: public SpellScript {
		...
		void HandleScript(SpellEffIndex /*effIndex*/) {
			...
			SpellEntry const* plague = sSpellStore.LookupEntry(
					SPELL_UNBOUND_PLAGUE);
			SpellEntry const* searcher = sSpellStore.LookupEntry(
					SPELL_UNBOUND_PLAGUE_SEARCHER);
			Creature* professor = NULL;
			if (InstanceScript* instance = GetCaster()->GetInstanceScript()) {
				professor = Unit::GetCreature(*GetCaster(),
						instance->GetData64(DATA_PROFESSOR_PUTRICIDE));
				if (professor) {
					plague = sSpellMgr->GetSpellForDifficultyFromSpell(plague,
							professor);
					searcher = sSpellMgr->GetSpellForDifficultyFromSpell(
							searcher, professor);
				}
			}
					...
					if (Aura* oldPlague = GetCaster()->GetAura(plague->Id, professor->GetGUID()))
						if (Aura* newPlague = professor->AddAura(plague->Id, GetHitUnit())) {
							...
							GetCaster()->CastSpell(GetCaster(),
									SPELL_PLAGUE_SICKNESS, true);
							GetCaster()->CastSpell(GetCaster(),
									SPELL_UNBOUND_PLAGUE_PROTECTION, true);
							professor->CastSpell(GetHitUnit(),
									SPELL_UNBOUND_PLAGUE_SEARCHER, true);
						}
			}
		}
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_putricide_unbound_plague_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
		}
	};
...
public:
	...
	class spell_putricide_eat_ooze_SpellScript: public SpellScript {
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_putricide_eat_ooze_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
		}
	};
...
public:
	...
	class spell_putricide_mutated_plague_AuraScript: public AuraScript {
		...
		void HandleTriggerSpell(AuraEffect const* aurEff) {
			...
			int32 damage = SpellMgr::CalculateSpellEffectAmount(spell, 0,
					caster);
			...
			GetTarget()->CastCustomSpell(triggerSpell, SPELLVALUE_BASE_POINT0,
					damage, GetTarget(), true, NULL, aurEff, GetCasterGUID());
		}
		...
		void OnRemove(AuraEffect const* /*aurEff*/,
			...
			if (GetTargetApplication()->GetRemoveMode() == AURA_REMOVE_BY_STACK)
				return;
			uint32 healSpell = uint32(
					SpellMgr::CalculateSpellEffectAmount(GetSpellProto(), 0));
			GetTarget()->CastSpell(GetTarget(), healSpell, true, NULL, NULL,
					GetCasterGUID());
		}

		void Register() {
			OnEffectPeriodic +=
					AuraEffectPeriodicFn(spell_putricide_mutated_plague_AuraScript::HandleTriggerSpell, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
			OnEffectRemove +=
					AuraEffectRemoveFn(spell_putricide_mutated_plague_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL, AURA_EFFECT_HANDLE_REAL);
		}
	};
...
public:
	...
	class spell_putricide_mutation_init_AuraScript: public AuraScript {
		...
		void Register() {
			OnEffectRemove +=
					AuraEffectRemoveFn(spell_putricide_mutation_init_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
		}
	};
...
public:
	...
	class spell_putricide_mutated_transformation_dismiss_AuraScript: public AuraScript {
		...
		void Register() {
			OnEffectRemove +=
					AuraEffectRemoveFn(spell_putricide_mutated_transformation_dismiss_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL, AURA_EFFECT_HANDLE_REAL);
		}
	};
...
public:
	...
	class spell_putricide_mutated_transformation_SpellScript: public SpellScript {
		...
		void HandleSummon(SpellEffIndex effIndex) {
			...
			TempSummon* summon = caster->GetMap()->SummonCreature(entry, pos,
					properties, duration, caster);
			if (!summon || !summon->IsVehicle())
				return;

			caster->CastSpell(summon, SPELL_MUTATED_TRANSFORMATION_NAME, true);
			summon->CastSpell(summon, SPELL_ABOMINATION_VEHICLE_POWER_DRAIN,
					true);
			summon->CastSpell(summon, SPELL_MUTATED_TRANSFORMATION_DAMAGE,
					true);
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_putricide_mutated_transformation_SpellScript::HandleSummon, EFFECT_0, SPELL_EFFECT_SUMMON);
		}
	};
...
public:
	...
	class spell_putricide_mutated_transformation_dmg_SpellScript: public SpellScript {
		...
		void Register() {
			OnUnitTargetSelect +=
					SpellUnitTargetFn(spell_putricide_mutated_transformation_dmg_SpellScript::FilterTargetsInitial, EFFECT_0, TARGET_UNIT_AREA_ALLY_SRC);
		}
	};
...
public:
	...
	class spell_putricide_regurgitated_ooze_SpellScript: public SpellScript {
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_putricide_regurgitated_ooze_SpellScript::ExtraEffect, EFFECT_0, SPELL_EFFECT_APPLY_AURA);
		}
	};
...
public:
	...
	class spell_stinky_precious_decimate_SpellScript: public SpellScript {
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_stinky_precious_decimate_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
		}
	};
