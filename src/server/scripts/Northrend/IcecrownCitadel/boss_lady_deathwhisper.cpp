...
enum Phases {
	...
	PHASE_INTRO_MASK = 1 << PHASE_INTRO, PHASE_ONE_MASK = 1 << PHASE_ONE,
};
...
static const Position addSpawnPos[7] = { { -578.7066f, 2154.167f, 51.01529f,
...
public:
	DaranavanMoveEvent(Creature& _darnavan) :
			darnavan(_darnavan) {
	}
...
public:
	...
	struct boss_lady_deathwhisperAI: public BossAI {
		...
		void Reset() {
			...
			me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_TAUNT,
					false);
		...
		void EnterCombat(Unit* who) {
			...
			events.ScheduleEvent(EVENT_P1_SHADOW_BOLT, urand(5500, 6000), 0,
					PHASE_ONE);
			events.ScheduleEvent(EVENT_P1_EMPOWER_CULTIST, urand(20000, 30000),
					0, PHASE_ONE);
		...
		void DamageTaken(Unit* /*damageDealer*/, uint32& uiDamage) {
			...
			if (events.GetPhaseMask() & PHASE_ONE_MASK
				...
				events.ScheduleEvent(EVENT_P2_FROSTBOLT, urand(10000, 12000), 0,
						PHASE_TWO);
				events.ScheduleEvent(EVENT_P2_FROSTBOLT_VOLLEY,
						urand(19000, 21000), 0, PHASE_TWO);
				events.ScheduleEvent(EVENT_P2_TOUCH_OF_INSIGNIFICANCE,
						urand(6000, 9000), 0, PHASE_TWO);
				events.ScheduleEvent(EVENT_P2_SUMMON_SHADE, urand(12000, 15000),
						0, PHASE_TWO);
				...
				if (IsHeroic()) {
					...
					me->ApplySpellImmune(0, IMMUNITY_EFFECT,
							SPELL_EFFECT_ATTACK_ME, true);
					events.ScheduleEvent(EVENT_P2_SUMMON_WAVE,
							urand(30000, 60000), 0, PHASE_TWO);
				}
			}
		}
		...
		void UpdateAI(const uint32 diff) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_DEATH_AND_DECAY:
					...
					events.ScheduleEvent(EVENT_DEATH_AND_DECAY,
							urand(10000, 12000));
					break;
				case EVENT_DOMINATE_MIND_H:
					...
					events.ScheduleEvent(EVENT_DOMINATE_MIND_H,
							urand(40000, 45000));
					break;
				case EVENT_P1_SUMMON_WAVE:
					SummonWaveP1();
					events.ScheduleEvent(EVENT_P1_SUMMON_WAVE, 60000, 0,
							PHASE_ONE);
					break;
				case EVENT_P1_SHADOW_BOLT:
					if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM))
						DoCast(target, SPELL_SHADOW_BOLT);
					events.ScheduleEvent(EVENT_P1_SHADOW_BOLT,
							urand(5000, 8000), 0, PHASE_ONE);
					break;
				...
				case EVENT_P1_EMPOWER_CULTIST:
					EmpowerCultist();
					events.ScheduleEvent(EVENT_P1_EMPOWER_CULTIST,
							urand(18000, 25000));
					break;
				case EVENT_P2_FROSTBOLT:
					DoCastVictim(SPELL_FROSTBOLT);
					events.ScheduleEvent(EVENT_P2_FROSTBOLT,
							urand(10000, 11000), 0, PHASE_TWO);
					break;
				case EVENT_P2_FROSTBOLT_VOLLEY:
					DoCastAOE(SPELL_FROSTBOLT_VOLLEY);
					events.ScheduleEvent(EVENT_P2_FROSTBOLT_VOLLEY,
							urand(13000, 15000), 0, PHASE_TWO);
					break;
				case EVENT_P2_TOUCH_OF_INSIGNIFICANCE:
					DoCastVictim(SPELL_TOUCH_OF_INSIGNIFICANCE);
					events.ScheduleEvent(EVENT_P2_TOUCH_OF_INSIGNIFICANCE,
							urand(9000, 13000), 0, PHASE_TWO);
					break;
				case EVENT_P2_SUMMON_SHADE:
					...
					events.ScheduleEvent(EVENT_P2_SUMMON_SHADE,
							urand(18000, 23000), 0, PHASE_TWO);
					break;
				case EVENT_P2_SUMMON_WAVE:
					SummonWaveP2();
					events.ScheduleEvent(EVENT_P2_SUMMON_WAVE, 60000, 0,
							PHASE_TWO);
					break;
				case EVENT_BERSERK:
		...
		void SummonWaveP1() {
			...
			if (addWaveCounter
					|| !sPoolMgr->IsSpawnedObject<Quest>(QUEST_DEPROGRAMMING))
				_SummonAdd(addEntries[addIndex], addSpawnPos[addIndex * 3]);
			else
				_SummonAdd(NPC_DARNAVAN, addSpawnPos[addIndex * 3]);
			_SummonAdd(addEntries[addIndexOther],
					addSpawnPos[addIndex * 3 + 1]);
			_SummonAdd(addEntries[addIndex], addSpawnPos[addIndex * 3 + 2]);
			if (Is25ManRaid()) {
				_SummonAdd(addEntries[addIndexOther],
						addSpawnPos[addIndexOther * 3]);
				_SummonAdd(addEntries[addIndex],
						addSpawnPos[addIndexOther * 3 + 1]);
				_SummonAdd(addEntries[addIndexOther],
						addSpawnPos[addIndexOther * 3 + 2]);
		...
		void SummonWaveP2() {
			if (Is25ManRaid()) {
				uint8 addIndex = addWaveCounter & 1;
				_SummonAdd(addEntries[addIndex], addSpawnPos[addIndex * 3]);
				_SummonAdd(addEntries[addIndex ^ 1],
						addSpawnPos[addIndex * 3 + 1]);
				_SummonAdd(addEntries[addIndex], addSpawnPos[addIndex * 3 + 2]);
			} else
				_SummonAdd(addEntries[urand(0, 1)], addSpawnPos[6]);
		...
		void AddToReanimationQueue(Unit* summon) {
			...
			events.ScheduleEvent(EVENT_P1_REANIMATE_CULTIST, 3000, 0,
					PHASE_ONE);
		}
		...
		void SpellHitTarget(Unit* target, SpellEntry const* spell) {
			if (spell->Id == SPELL_DARK_MARTYRDOM_T) {
				...
				if (target->GetEntry() == NPC_CULT_FANATIC)
					me->SummonCreature(NPC_REANIMATED_FANATIC, pos,
							TEMPSUMMON_CORPSE_TIMED_DESPAWN, 10000);
				else
					me->SummonCreature(NPC_REANIMATED_ADHERENT, pos,
							TEMPSUMMON_CORPSE_TIMED_DESPAWN, 10000);
...
public:
	...
	struct npc_cult_fanaticAI: public ScriptedAI {
		...
		void Reset() {
			events.Reset();
			events.ScheduleEvent(EVENT_FANATIC_NECROTIC_STRIKE,
					urand(10000, 12000));
			events.ScheduleEvent(EVENT_FANATIC_SHADOW_CLEAVE,
					urand(14000, 16000));
			events.ScheduleEvent(EVENT_FANATIC_VAMPIRIC_MIGHT,
					urand(20000, 27000));
			if (me->GetEntry() == NPC_CULT_FANATIC)
				events.ScheduleEvent(EVENT_CULTIST_DARK_MARTYRDOM,
						urand(18000, 32000));
		}
		...
		void UpdateAI(const uint32 diff) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_FANATIC_NECROTIC_STRIKE:
					...
					events.ScheduleEvent(SPELL_NECROTIC_STRIKE,
							urand(11000, 13000));
					break;
...
public:
	...
	struct npc_cult_adherentAI: public ScriptedAI {
		...
		void Reset() {
			events.Reset();
			events.ScheduleEvent(EVENT_ADHERENT_FROST_FEVER,
					urand(10000, 12000));
			events.ScheduleEvent(EVENT_ADHERENT_DEATHCHILL,
					urand(14000, 16000));
			events.ScheduleEvent(EVENT_ADHERENT_CURSE_OF_TORPOR,
					urand(14000, 16000));
			events.ScheduleEvent(EVENT_ADHERENT_SHORUD_OF_THE_OCCULT,
					urand(32000, 39000));
...
public:
	...
	struct npc_darnavanAI: public ScriptedAI {
		...
		void Reset() {
			...
			events.ScheduleEvent(EVENT_DARNAVAN_INTIMIDATING_SHOUT,
					urand(20000, 25000));
			events.ScheduleEvent(EVENT_DARNAVAN_MORTAL_STRIKE,
					urand(25000, 30000));
			events.ScheduleEvent(EVENT_DARNAVAN_SUNDER_ARMOR,
					urand(5000, 8000));
			canCharge = true;
			canShatter = true;
		}
		...
		void UpdateAI(const uint32 diff) {
			...
			if (canShatter
					&& me->getVictim()->IsImmunedToDamage(
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_DARNAVAN_BLADESTORM:
					...
					events.ScheduleEvent(EVENT_DARNAVAN_BLADESTORM,
							urand(90000, 100000));
					break;
...
public:
	...
	class spell_deathwhisper_mana_barrier_AuraScript: public AuraScript {
		...
		void Register() {
			OnEffectPeriodic +=
					AuraEffectPeriodicFn(spell_deathwhisper_mana_barrier_AuraScript::HandlePeriodicTick, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
		}
	};
			...
			if (GetCaster()->isSummon())
				if (Unit* owner = GetCaster()->ToTempSummon()->GetSummoner())
					if (owner->GetEntry() == NPC_LADY_DEATHWHISPER)
						CAST_AI(DeathwisperAI, owner->ToCreature()->AI())->AddToReanimationQueue(
								GetCaster());

			GetCaster()->Kill(GetCaster());
			GetCaster()->SetDisplayId(
					uint32(
							GetCaster()->GetEntry() == NPC_CULT_FANATIC ?
									38009 : 38010));
		}

		void Register() {
			OnEffect +=
					SpellEffectFn(spell_cultist_dark_martyrdom_SpellScript::HandleEffect, EFFECT_2, SPELL_EFFECT_FORCE_DESELECT);
		}
	};
