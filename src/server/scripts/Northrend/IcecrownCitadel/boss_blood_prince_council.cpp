...
enum Events {
	EVENT_INTRO_1 = 1, EVENT_INTRO_2 = 2,

	EVENT_INVOCATION_OF_BLOOD = 3, EVENT_BERSERK = 4,
...
enum Points {
	POINT_INTRO_DESPAWN = 380040, POINT_KINETIC_BOMB_IMPACT = 384540,
};
...
static const Position introFinalPos = { 4660.490f, 2769.200f, 430.0000f,
		0.000000f };
static const Position triggerPos =
		{ 4680.231f, 2769.134f, 379.9256f, 3.121708f };
static const Position triggerEndPos = { 4680.180f, 2769.150f, 365.5000f,
		3.121708f };

class boss_blood_council_controller: public CreatureScript {
public:
	boss_blood_council_controller() :
			CreatureScript("boss_blood_council_controller") {
	}
	...
	struct boss_blood_council_controllerAI: public BossAI {
		...
		void EnterCombat(Unit* who) {
			if (instance->GetBossState(DATA_BLOOD_PRINCE_COUNCIL)
					== IN_PROGRESS)
				return;
			...
			invocationOrder[0] = InvocationData(
					instance->GetData64(DATA_PRINCE_VALANAR_GUID),
					SPELL_INVOCATION_OF_BLOOD_VALANAR, EMOTE_VALANAR_INVOCATION,
					71070);
			if (urand(0, 1)) {
				invocationOrder[1] = InvocationData(
						instance->GetData64(DATA_PRINCE_TALDARAM_GUID),
						SPELL_INVOCATION_OF_BLOOD_TALDARAM,
						EMOTE_TALDARAM_INVOCATION, 71081);
				invocationOrder[2] = InvocationData(
						instance->GetData64(DATA_PRINCE_KELESETH_GUID),
						SPELL_INVOCATION_OF_BLOOD_KELESETH,
						EMOTE_KELESETH_INVOCATION, 71080);
			} else {
				invocationOrder[1] = InvocationData(
						instance->GetData64(DATA_PRINCE_KELESETH_GUID),
						SPELL_INVOCATION_OF_BLOOD_KELESETH,
		...
		void JustReachedHome() {
			resetCounter = 0;
			if (Creature* keleseth = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_PRINCE_KELESETH_GUID)))
				keleseth->RemoveFlag(UNIT_FIELD_FLAGS,
						UNIT_FLAG_NON_ATTACKABLE);

			if (Creature* taldaram = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_PRINCE_TALDARAM_GUID)))
				taldaram->RemoveFlag(UNIT_FIELD_FLAGS,
						UNIT_FLAG_NON_ATTACKABLE);
		...
		void UpdateAI(const uint32 diff) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_INVOCATION_OF_BLOOD: {
					Creature* oldPrince = ObjectAccessor::GetCreature(*me,
							invocationOrder[invocationStage].guid);
					if (++invocationStage == 3)
						invocationStage = 0;

					Creature* newPrince = ObjectAccessor::GetCreature(*me,
							invocationOrder[invocationStage].guid);
					if (oldPrince) {
						...
						if (newPrince)
							oldPrince->CastSpell(
									newPrince,
									invocationOrder[invocationStage].visualSpell,
									true);
					}
					...
					if (newPrince) {
						...
						newPrince->AI()->Talk(
								invocationOrder[invocationStage].textId);
					}
	...
	private:
		struct InvocationData {
			...
			InvocationData(uint64 _guid, uint32 _spellId, int32 _textId,
					uint32 _visualSpell) {
				guid = _guid;
				spellId = _spellId;
				textId = _textId;
			...
			InvocationData() :
					guid(0), spellId(0), textId(0), visualSpell(0) {
			}
		} invocationOrder[3];
	...
	struct boss_prince_kelesethAI: public BossAI {
		...
		void JustSummoned(Creature* summon) {
			...
			if (summon->GetExactDist2d(&roomCenter) > 50.0f) {
				...
				summon->MovePosition(pos, float(rand_norm()) * 50.0f,
						summon->GetAngle(&roomCenter));
			}
		}
		...
		void DamageDealt(Unit* /*target*/, uint32& damage,
			...
			if (damage > RAID_MODE<uint32>(23000, 25000, 23000, 25000))
				instance->SetData(DATA_ORB_WHISPERER_ACHIEVEMENT,
						uint32(false));
		}
		...
		void UpdateAI(const uint32 diff) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_SHADOW_RESONANCE:
					...
					events.ScheduleEvent(EVENT_SHADOW_RESONANCE,
							urand(10000, 15000));
					break;
...
const Position boss_prince_keleseth_icc::boss_prince_kelesethAI::roomCenter = {
		4658.0f, 2769.26f, 361.2f, 0.0f };
		...
		void UpdateAI(const uint32 diff) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_GLITTERING_SPARKS:
					...
					events.ScheduleEvent(EVENT_GLITTERING_SPARKS,
							urand(15000, 50000));
					break;
				case EVENT_CONJURE_FLAME:
					if (isEmpowered) {
						DoCast(me, SPELL_CONJURE_EMPOWERED_FLAME);
						events.ScheduleEvent(EVENT_CONJURE_FLAME,
								urand(15000, 25000));
					} else {
	...
	struct boss_prince_valanarAI: public BossAI {
		...
		void JustSummoned(Creature* summon) {
			...
			case NPC_KINETIC_BOMB_TARGET:
				...
				summon->CastSpell(summon, SPELL_KINETIC_BOMB, true, NULL, NULL,
						me->GetGUID());
				break;
			case NPC_KINETIC_BOMB: {
				float x, y, z;
				summon->GetPosition(x, y, z);
				float ground_Z = summon->GetMap()->GetHeight(x, y, z, true,
						500.0f);
				summon->GetMotionMaster()->MovePoint(POINT_KINETIC_BOMB_IMPACT,
						x, y, ground_Z);
				break;
			}
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_SHOCK_VORTEX:
					if (isEmpowered) {
						...
						events.ScheduleEvent(EVENT_SHOCK_VORTEX,
								urand(18000, 23000));
					}
					break;
				default:
...
public:
	...
	struct npc_ball_of_flameAI: public ScriptedAI {
		...
		void MovementInform(uint32 type, uint32 id) {
			if (type == TARGETED_MOTION_TYPE && id == GUID_LOPART(chaseGUID)
					&& chaseGUID) {
...
public:
	...
	struct npc_kinetic_bombAI: public ScriptedAI {
		...
		void Reset() {
			...
			me->CastCustomSpell(SPELL_KINETIC_BOMB_VISUAL,
					SPELLVALUE_BASE_POINT0, 0x7FFFFFFF, me, true);
			me->SetReactState(REACT_PASSIVE);
		...
		void DoAction(const int32 action) {
			if (action == SPELL_KINETIC_BOMB_EXPLOSION)
				...
				me->GetMotionMaster()->MoveJump(x, y, me->GetPositionZ() + 7.0f,
						1.0f, 7.0f);
		...
		void UpdateAI(const uint32 diff) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_CONTINUE_FALLING:
					me->GetMotionMaster()->Clear();
					me->GetMotionMaster()->MovePoint(POINT_KINETIC_BOMB_IMPACT,
							x, y, groundZ);
					break;
				default:
					break;
			...
			if (!lockedTarget) {
				if (Unit* victim = me->SelectVictim()) {
					if (me->getVictim() && me->getVictim() != victim) {
						me->getVictim()->RemoveAurasDueToSpell(
								SPELL_SHADOW_RESONANCE_RESIST, me->GetGUID());
						lockedTarget = true;
					}
...
public:
	...
	class spell_taldaram_glittering_sparks_SpellScript: public SpellScript {
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_taldaram_glittering_sparks_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
		}
	};
...
public:
	...
	class spell_taldaram_summon_flame_ball_SpellScript: public SpellScript {
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_taldaram_summon_flame_ball_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_DUMMY);
		}
	};
...
public:
	spell_taldaram_flame_ball_visual() :
			SpellScriptLoader("spell_taldaram_flame_ball_visual") {
	}
	...
	class spell_flame_ball_visual_AuraScript: public AuraScript {
		...
		bool Load() {
			if (GetCaster()->GetEntry() == NPC_BALL_OF_FLAME
					|| GetCaster()->GetEntry() == NPC_BALL_OF_INFERNO_FLAME)
				return true;
			return false;
		}
		...
		void Register() {
			OnEffectRemove +=
					AuraEffectRemoveFn(spell_flame_ball_visual_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_MOD_STUN, AURA_EFFECT_HANDLE_REAL);
		}
	};
...
public:
	...
	class spell_taldaram_ball_of_inferno_flame_SpellScript: public SpellScript {
		...
		void Register() {
			AfterHit +=
					SpellHitFn(spell_taldaram_ball_of_inferno_flame_SpellScript::ModAuraStack);
		}
	};
...
public:
	...
	class spell_valanar_kinetic_bomb_SpellScript: public SpellScript {
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_valanar_kinetic_bomb_SpellScript::ChangeSummonPos, EFFECT_0, SPELL_EFFECT_SUMMON);
		}
	};
	...
	class spell_valanar_kinetic_bomb_AuraScript: public AuraScript {
		...
		void Register() {
			OnEffectPeriodic +=
					AuraEffectPeriodicFn(spell_valanar_kinetic_bomb_AuraScript::HandleDummyTick, EFFECT_1, SPELL_AURA_PERIODIC_DUMMY);
		}
	};
...
public:
	...
	class spell_valanar_kinetic_bomb_knockback_SpellScript: public SpellScript {
		...
		void Register() {
			AfterHit +=
					SpellHitFn(spell_valanar_kinetic_bomb_knockback_SpellScript::KnockIntoAir);
		}
	};
	...
	class spell_blood_council_shadow_prison_AuraScript: public AuraScript {
		...
		void HandleDummyTick(AuraEffect const* aurEff) {
			if (GetTarget()->isMoving())
				GetTarget()->CastSpell(GetTarget(), SPELL_SHADOW_PRISON_DAMAGE,
						true, NULL, aurEff);
		}

		void Register() {
			OnEffectPeriodic +=
					AuraEffectPeriodicFn(spell_blood_council_shadow_prison_AuraScript::HandleDummyTick, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
		}
	};
...
public:
	...
	class spell_blood_council_shadow_prison_SpellScript: public SpellScript {
		...
		void Register() {
			OnHit +=
					SpellHitFn(spell_blood_council_shadow_prison_SpellScript::AddExtraDamage);
		}
	};
