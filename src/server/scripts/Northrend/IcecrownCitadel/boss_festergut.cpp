			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_INHALE_BLIGHT: {
					...
					if (inhaleCounter == 3) {
						...
						events.RescheduleEvent(EVENT_GAS_SPORE,
								urand(20000, 25000));
					} else {
						...
						++inhaleCounter;
						if (inhaleCounter < 3)
							me->CastSpell(me, gaseousBlight[inhaleCounter],
									true, NULL, NULL, me->GetGUID());
					}
					events.ScheduleEvent(EVENT_INHALE_BLIGHT,
							urand(33500, 35000));
					break;
				}
				case EVENT_VILE_GAS: {
					...
					SelectTargetList(targets, minTargets, SELECT_TARGET_RANDOM,
							-5.0f, true);
					float minDist = 0.0f;
				...
				case EVENT_GAS_SPORE:
					...
					me->CastCustomSpell(SPELL_GAS_SPORE, SPELLVALUE_MAX_TARGETS,
							RAID_MODE<int32>(2, 3, 2, 3), me);
				...
				case EVENT_GASTRIC_BLOAT:
					...
					events.ScheduleEvent(EVENT_GASTRIC_BLOAT,
							urand(15000, 17500));
					break;
				case EVENT_BERSERK:
	...
	struct npc_stinky_iccAI: public ScriptedAI {
		...
		void JustDied(Unit* /*who*/) {
			uint64 festergutGUID =
					instance ? instance->GetData64(DATA_FESTERGUT) : 0;
...
public:
	...
	class spell_festergut_pungent_blight_SpellScript: public SpellScript {
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_festergut_pungent_blight_SpellScript::HandleScript, EFFECT_1, SPELL_EFFECT_SCRIPT_EFFECT);
		}
	};
...
public:
	...
	class spell_festergut_gastric_bloat_SpellScript: public SpellScript {
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_festergut_gastric_bloat_SpellScript::HandleScript, EFFECT_2, SPELL_EFFECT_SCRIPT_EFFECT);
		}
	};
...
public:
	...
	class spell_festergut_blighted_spores_AuraScript: public AuraScript {
		...
		void ExtraEffect(AuraEffect const* /*aurEff*/,
			...
			uint32 inoculateId =
					CAST_AI(ScriptedAI, GetCaster()->ToCreature()->AI())->INOCULATED_HELPER;
			uint32 currStack = 0;
			...
			++currStack;
			GetCaster()->ToCreature()->AI()->SetData(DATA_INOCULATED_STACK,
					currStack);
		}

		void Register() {
			OnEffectApply +=
					AuraEffectApplyFn(spell_festergut_blighted_spores_AuraScript::ExtraEffect, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE, AURA_EFFECT_HANDLE_REAL);
		}
	};
...
public:
	spell_festergut_gaseous_blight() :
			SpellScriptLoader("spell_festergut_gaseous_blight") {
	}
	...
	class spell_festergut_gaseous_blight_SpellScript: public SpellScript {
		...
		void ExtraEffect() {
			...
			GetHitUnit()->CastSpell(GetHitUnit(), SPELL_ORANGE_BLIGHT_RESIDUE,
					true);
		}

		void Register() {
			AfterHit +=
					SpellHitFn(spell_festergut_gaseous_blight_SpellScript::ExtraEffect);
		}
	};
