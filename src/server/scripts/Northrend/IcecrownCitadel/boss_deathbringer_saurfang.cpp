...
enum Phases {
	PHASE_INTRO_A = 1, PHASE_INTRO_H = 2, PHASE_COMBAT = 3,
...
static const Position deathbringerPos =
		{ -496.3542f, 2211.33f, 541.1138f, 0.0f };
...
public:
	boss_deathbringer_saurfang() :
			CreatureScript("boss_deathbringer_saurfang") {
	}
	...
	struct boss_deathbringer_saurfangAI: public BossAI {
		...
		void EnterCombat(Unit* who) {
			...
			Talk(SAY_AGGRO);
			events.ScheduleEvent(EVENT_SUMMON_BLOOD_BEAST, 30000, 0,
					PHASE_COMBAT);
		...
		void MovementInform(uint32 type, uint32 id) {
			...
			instance->HandleGameObject(instance->GetData64(GO_SAURFANG_S_DOOR),
					false);
		}
		...
		void SpellHitTarget(Unit* target, SpellEntry const* spell) {
			...
			case 72446:
				if (me->GetPower(POWER_ENERGY) != me->GetMaxPower(POWER_ENERGY))
					target->CastCustomSpell(SPELL_BLOOD_LINK_DUMMY,
							SPELLVALUE_BASE_POINT0, 1, me, true);
				break;
			default:
				break;
			}
		...
		void UpdateAI(const uint32 diff) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_SUMMON_BLOOD_BEAST:
					...
					events.ScheduleEvent(EVENT_SUMMON_BLOOD_BEAST, 40000, 0,
							PHASE_COMBAT);
					break;
				case EVENT_BLOOD_NOVA: {
					// select at range only
					Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, -10.0f,
							true);
					if (!target)
						target = SelectTarget(SELECT_TARGET_RANDOM, 1, 10.0f,
								true); // noone? select melee
					if (target)
				...
				case EVENT_RUNE_OF_BLOOD:
					...
					events.ScheduleEvent(EVENT_RUNE_OF_BLOOD,
				...
				case EVENT_BOILING_BLOOD:
					...
					events.ScheduleEvent(EVENT_BOILING_BLOOD,
							urand(15000, 20000), 0, PHASE_COMBAT);
					break;
				case EVENT_BERSERK:
		...
		void DoAction(const int32 action) {
			...
			case PHASE_INTRO_H:
				...
				me->SetHomePosition(deathbringerPos.GetPositionX(),
						deathbringerPos.GetPositionY(),
						deathbringerPos.GetPositionZ(), me->GetOrientation());
				me->GetMotionMaster()->MovePoint(POINT_SAURFANG,
						deathbringerPos.GetPositionX(),
						deathbringerPos.GetPositionY(),
						deathbringerPos.GetPositionZ());

				events.ScheduleEvent(EVENT_INTRO_ALLIANCE_2, 2500, 0,
						PHASE_INTRO_A);
				events.ScheduleEvent(EVENT_INTRO_ALLIANCE_3, 20000, 0,
						PHASE_INTRO_A);

				events.ScheduleEvent(EVENT_INTRO_HORDE_2, 5000, 0,
						PHASE_INTRO_H);
				break;
			case ACTION_CONTINUE_INTRO:
				if (introDone)
					return;
				events.ScheduleEvent(EVENT_INTRO_ALLIANCE_6, 6500 + 500, 0,
						PHASE_INTRO_A);
				events.ScheduleEvent(EVENT_INTRO_FINISH, 8000, 0,
						PHASE_INTRO_A);

				events.ScheduleEvent(EVENT_INTRO_HORDE_4, 6500, 0,
						PHASE_INTRO_H);
				events.ScheduleEvent(EVENT_INTRO_HORDE_9, 46700 + 1000 + 500, 0,
						PHASE_INTRO_H);
				events.ScheduleEvent(EVENT_INTRO_FINISH, 46700 + 1000 + 8000, 0,
						PHASE_INTRO_H);
				break;
...
public:
	...
	struct npc_high_overlord_saurfangAI: public ScriptedAI {
		npc_high_overlord_saurfangAI(Creature* creature) :
				ScriptedAI(creature), vehicle(creature->GetVehicleKit()) {
			ASSERT(vehicle);
		...
		void DoAction(const int32 action) {
			...
			case ACTION_START_EVENT: {
				...
				GetCreatureListWithEntryInGrid(guardList, me,
						NPC_SE_KOR_KRON_REAVER, 20.0f);
				guardList.sort(Trinity::ObjectDistanceOrderPred(me));
				uint32 x = 1;
				for (std::list<Creature*>::iterator itr = guardList.begin();
						itr != guardList.end(); ++x, ++itr)
					(*itr)->AI()->SetData(0, x);

				me->RemoveFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP);
				Talk(SAY_INTRO_HORDE_1);
				events.SetPhase(PHASE_INTRO_H);
				events.ScheduleEvent(EVENT_INTRO_HORDE_3, 18500, 0,
						PHASE_INTRO_H);
				if (instance) {
					deathbringerSaurfangGUID = instance->GetData64(
							DATA_DEATHBRINGER_SAURFANG);
					instance->HandleGameObject(
							instance->GetData64(GO_SAURFANG_S_DOOR), true);
				}
			...
			case ACTION_START_OUTRO: {
				...
				me->SendMonsterMove(me->GetPositionX(), me->GetPositionY(),
						539.2917f, SPLINEFLAG_FALLING, 0, 0.0f);
				for (std::list<Creature*>::iterator itr = guardList.begin();
						itr != guardList.end(); ++itr)
		...
		void MovementInform(uint32 type, uint32 id) {
			...
			case POINT_FIRST_STEP:
				...
				events.ScheduleEvent(EVENT_INTRO_HORDE_5, 15500, 0,
						PHASE_INTRO_H);
				events.ScheduleEvent(EVENT_INTRO_HORDE_6, 29500, 0,
						PHASE_INTRO_H);
				events.ScheduleEvent(EVENT_INTRO_HORDE_7, 43800, 0,
						PHASE_INTRO_H);
				events.ScheduleEvent(EVENT_INTRO_HORDE_8, 47000, 0,
						PHASE_INTRO_H);
				if (Creature* deathbringer = ObjectAccessor::GetCreature(*me, deathbringerSaurfangGUID))
			...
			case POINT_CORPSE:
				if (Creature* deathbringer = ObjectAccessor::GetCreature(*me, deathbringerSaurfangGUID)) {
					...
					deathbringer->SetFlag(UNIT_FIELD_FLAGS,
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_INTRO_HORDE_8:
					...
					me->GetMotionMaster()->MoveCharge(
							chargePos[0].GetPositionX(),
							chargePos[0].GetPositionY(),
							chargePos[0].GetPositionZ(), 8.5f, POINT_CHARGE);
					break;
	...
	struct npc_muradin_bronzebeard_iccAI: public ScriptedAI {
		...
		void DoAction(const int32 action) {
			...
			case ACTION_START_EVENT: {
				...
				GetCreatureListWithEntryInGrid(guardList, me,
						NPC_SE_SKYBREAKER_MARINE, 20.0f);
				guardList.sort(Trinity::ObjectDistanceOrderPred(me));
				uint32 x = 1;
				for (std::list<Creature*>::iterator itr = guardList.begin();
						itr != guardList.end(); ++x, ++itr)
				...
				events.ScheduleEvent(EVENT_INTRO_ALLIANCE_4,
						2500 + 17500 + 9500, 0, PHASE_INTRO_A);
				if (instance) {
					deathbringerSaurfangGUID = instance->GetData64(
							DATA_DEATHBRINGER_SAURFANG);
					instance->HandleGameObject(
		...
		void MovementInform(uint32 type, uint32 id) {
			...
			events.ScheduleEvent(EVENT_INTRO_ALLIANCE_5, 5000, 0,
					PHASE_INTRO_A);
			if (Creature* deathbringer = ObjectAccessor::GetCreature(*me, deathbringerSaurfangGUID))
				deathbringer->AI()->DoAction(ACTION_CONTINUE_INTRO);
		}
...
public:
	npc_saurfang_event() :
			CreatureScript("npc_saurfang_event") {
	}
	...
	struct npc_saurfang_eventAI: public ScriptedAI {
		...
		void SpellHit(Unit* /*caster*/, SpellEntry const* spell) {
			if (spell->Id == SPELL_GRIP_OF_AGONY) {
				me->AddUnitMovementFlag(MOVEMENTFLAG_LEVITATING);
				me->GetMotionMaster()->MovePoint(POINT_CHOKE,
						chokePos[npcIndex]);
			}
		}
		...
		void DoAction(const int32 action) {
			if (action == ACTION_CHARGE && npcIndex)
				me->GetMotionMaster()->MoveCharge(
						chargePos[npcIndex].GetPositionX(),
						chargePos[npcIndex].GetPositionY(),
						chargePos[npcIndex].GetPositionZ(), 13.0f,
						POINT_CHARGE);
	...
	class spell_deathbringer_blood_link_SpellScript: public SpellScript {
		...
		void HandleDummy(SpellEffIndex /*effIndex*/) {
			GetHitUnit()->CastCustomSpell(SPELL_BLOOD_LINK_POWER,
					SPELLVALUE_BASE_POINT0, GetEffectValue(), GetHitUnit(),
					true);
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_deathbringer_blood_link_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
		}
	};
...
public:
	...
	class spell_deathbringer_blood_link_AuraScript: public AuraScript {
		...
		void Register() {
			OnEffectPeriodic +=
					AuraEffectPeriodicFn(spell_deathbringer_blood_link_AuraScript::HandlePeriodicTick, EFFECT_1, SPELL_AURA_PERIODIC_DUMMY);
		}
	};
...
public:
	...
	class spell_deathbringer_blood_power_SpellScript: public SpellScript {
		...
		void Register() {
			AfterHit +=
					SpellHitFn(spell_deathbringer_blood_power_SpellScript::ModAuraValue);
		}
	};
	...
	class spell_deathbringer_blood_power_AuraScript: public AuraScript {
		...
		void RecalculateHook(AuraEffect const* /*aurEffect*/, int32& amount,
		...
		void Register() {
			DoEffectCalcAmount +=
					AuraEffectCalcAmountFn(spell_deathbringer_blood_power_AuraScript::RecalculateHook, EFFECT_0, SPELL_AURA_MOD_SCALE);
			DoEffectCalcAmount +=
					AuraEffectCalcAmountFn(spell_deathbringer_blood_power_AuraScript::RecalculateHook, EFFECT_1, SPELL_AURA_MOD_DAMAGE_PERCENT_DONE);
		}
...
public:
	spell_deathbringer_rune_of_blood() :
			SpellScriptLoader("spell_deathbringer_rune_of_blood") {
	}
	...
	class spell_deathbringer_rune_of_blood_SpellScript: public SpellScript {
		...
		void HandleScript(SpellEffIndex effIndex) {
			...
			if (GetCaster()->GetPower(POWER_ENERGY)
					!= GetCaster()->GetMaxPower(POWER_ENERGY))
				GetHitUnit()->CastCustomSpell(SPELL_BLOOD_LINK_DUMMY,
						SPELLVALUE_BASE_POINT0, 1, GetCaster(), true);
		}

		void Register() {
			OnEffect +=
					SpellEffectFn(spell_deathbringer_rune_of_blood_SpellScript::HandleScript, EFFECT_1, SPELL_EFFECT_SCRIPT_EFFECT);
		}
	};
...
public:
	spell_deathbringer_blood_nova() :
			SpellScriptLoader("spell_deathbringer_blood_nova") {
	}
	...
	class spell_deathbringer_blood_nova_SpellScript: public SpellScript {
		...
		void HandleScript(SpellEffIndex /*effIndex*/) {
			...
			if (GetCaster()->GetPower(POWER_ENERGY)
					!= GetCaster()->GetMaxPower(POWER_ENERGY))
				GetHitUnit()->CastCustomSpell(SPELL_BLOOD_LINK_DUMMY,
						SPELLVALUE_BASE_POINT0, 2, GetCaster(), true);
		}

		void Register() {
			OnEffect +=
					SpellEffectFn(spell_deathbringer_blood_nova_SpellScript::HandleScript, EFFECT_1, SPELL_EFFECT_SCRIPT_EFFECT);
		}
	};
	...
	class spell_deathbringer_blood_nova_targeting_SpellScript: public SpellScript {
		...
		void FilterTargetsInitial(std::list<Unit*>& unitList) {
			...
			uint32 const minTargets =
					GetCaster()->GetMap()->GetSpawnMode() & 1 ? 10 : 4;
			...
			for (std::list<Unit*>::iterator itr = unitList.begin();
					itr != unitList.end(); ++itr, ++targetsAtRange)
				if ((*itr)->GetDistance(GetCaster()) < 12.0f)
					break;

			// set the upper cap
			if (targetsAtRange < minTargets)
				targetsAtRange = std::min < uint32
						> (unitList.size() - 1, minTargets);
		...
		void Register() {
			OnUnitTargetSelect +=
					SpellUnitTargetFn(spell_deathbringer_blood_nova_targeting_SpellScript::FilterTargetsInitial, EFFECT_0, TARGET_UNIT_AREA_ENEMY_SRC);
			OnUnitTargetSelect +=
					SpellUnitTargetFn(spell_deathbringer_blood_nova_targeting_SpellScript::FilterTargetsSubsequent, EFFECT_1, TARGET_UNIT_AREA_ENEMY_SRC);
		}
...
public:
	spell_deathbringer_mark_of_the_fallen_champion() :
			SpellScriptLoader("spell_deathbringer_mark_of_the_fallen_champion") {
	}
	...
	class spell_deathbringer_mark_of_the_fallen_champion_SpellScript: public SpellScript {
		...
		void Register() {
			OnUnitTargetSelect +=
					SpellUnitTargetFn(spell_deathbringer_mark_of_the_fallen_champion_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_AREA_ENEMY_SRC);
		}
	};
