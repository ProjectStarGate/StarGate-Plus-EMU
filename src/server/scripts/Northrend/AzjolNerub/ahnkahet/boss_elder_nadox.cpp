...
//1x 30176  -- every 25secs
};

enum Creatures {
	MOB_AHNKAHAR_SWARMER = 30178, MOB_AHNKAHAR_GUARDIAN_ENTRY = 30176
};
...
public:
	boss_elder_nadox() :
			CreatureScript("boss_elder_nadox") {
	}

	struct boss_elder_nadoxAI: public BossAI {
		boss_elder_nadoxAI(Creature *c) :
				BossAI(c, DATA_ELDER_NADOX_EVENT) {
...
public:
	mob_nadox_eggs() :
			CreatureScript("mob_nadox_eggs") {
	}

	struct mob_nadox_eggsAI: public Scripted_NoMovementAI {
		mob_nadox_eggsAI(Creature* c) :
