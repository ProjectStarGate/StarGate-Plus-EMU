...
enum Creatures {
	NPC_HEALTHY_MUSHROOM = 30391, NPC_POISONOUS_MUSHROOM = 30435
};

class boss_amanitar: public CreatureScript {
public:
	boss_amanitar() :
			CreatureScript("boss_amanitar") {
	}

	struct boss_amanitarAI: public BossAI {
		boss_amanitarAI(Creature *c) :
				BossAI(c, DATA_AMANITAR_EVENT) {
			bFirstTime = true;
		...
		void Reset() {
			...
			me->ApplySpellImmune(0, IMMUNITY_SCHOOL, SPELL_SCHOOL_MASK_NATURE,
					true);
		...
		void SpawnAdds() {
			for (uint8 i = 0; i < 30; ++i) {
				...
				if (victim) {
					...
					me->SummonCreature(NPC_POISONOUS_MUSHROOM, pos,
							TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN,
							30 * IN_MILLISECONDS);
					me->GetRandomNearPosition(pos, float(urand(5, 80)));
					me->SummonCreature(NPC_HEALTHY_MUSHROOM, pos,
							TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN,
							30 * IN_MILLISECONDS);
				}
			}
		...
		void UpdateAI(const uint32 diff) {
			...
			if (uiSpawnTimer <= diff) {
				SpawnAdds();
				uiSpawnTimer = urand(35 * IN_MILLISECONDS,
						40 * IN_MILLISECONDS);
			} else
				uiSpawnTimer -= diff;
...
public:
	mob_amanitar_mushrooms() :
			CreatureScript("mob_amanitar_mushrooms") {
	}

	struct mob_amanitar_mushroomsAI: public Scripted_NoMovementAI {
		mob_amanitar_mushroomsAI(Creature* c) :
