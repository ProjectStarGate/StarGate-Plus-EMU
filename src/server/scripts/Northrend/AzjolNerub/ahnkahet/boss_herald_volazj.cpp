...
public:
	boss_volazj() :
			CreatureScript("boss_volazj") {
	}

	struct boss_volazjAI: public BossAI {
		boss_volazjAI(Creature* pCreature) :
				BossAI(pCreature, DATA_HERALD_VOLAZJ_EVENT) {
		...
		void SpellHitTarget(Unit *pTarget, const SpellEntry *spell) {
			if (spell->Id == SPELL_INSANITY) {
				...
				pTarget->CastSpell(pTarget,
						SPELL_INSANITY_TARGET + insanityHandled, true);
				...
				for (Map::PlayerList::const_iterator i = players.begin();
					...
					if (Unit *summon = me->SummonCreature(MOB_TWISTED_VISAGE, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation(), TEMPSUMMON_CORPSE_DESPAWN, 0)) {
						...
						// set phase
						summon->SetPhaseMask((1 << (4 + insanityHandled)),
								true);
					}
				}
				++insanityHandled;
		...
		void ResetPlayersPhaseMask() {
			...
			for (Map::PlayerList::const_iterator i = players.begin();
				...
				pPlayer->RemoveAurasDueToSpell(
						GetSpellForPhaseMask(pPlayer->GetPhaseMask()));
			}
		}
		...
		void SummonedCreatureDespawn(Creature *summon) {
			...
			for (SummonList::const_iterator iter = summons.begin();
					iter != summons.end(); ++iter) {
