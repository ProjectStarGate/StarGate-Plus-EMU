...
public:
	boss_hadronox() :
			CreatureScript("boss_hadronox") {
	}

	struct boss_hadronoxAI: public BossAI {
		boss_hadronoxAI(Creature* c) :
				BossAI(c, DATA_HADRONOX_EVENT) {
			fMaxDistance = 50.0f;
			bFirstTime = true;
		...
		void Reset() {
			...
			if (instance
					&& (instance->GetData(DATA_HADRONOX_EVENT) != DONE
							&& !bFirstTime))
		...
		void KilledUnit(Unit* Victim) {
			...
			if (!Victim
					|| !Victim->HasAura(
							DUNGEON_MODE(SPELL_LEECH_POISON,
									H_SPELL_LEECH_POISON)) || !me->isAlive())
				return;
		...
		void UpdateAI(const uint32 diff) {
			...
			if (me->HasAura(SPELL_WEB_FRONT_DOORS)
					|| me->HasAura(SPELL_WEB_SIDE_DOORS)) {
				if (IsCombatMovement())
			...
			if (uiLeechTimer <= diff) {
				...
				uiLeechTimer = urand(11 * IN_MILLISECONDS,
						14 * IN_MILLISECONDS);
			} else
				uiLeechTimer -= diff;
			...
			if (uiDoorsTimer <= diff) {
				uiDoorsTimer = urand(30 * IN_MILLISECONDS,
						60 * IN_MILLISECONDS);
			} else
				uiDoorsTimer -= diff;
