...
public:
	...
	struct instance_pinnacle: public InstanceScript {
		...
		void OnCreatureCreate(Creature* pCreature, bool /*add*/) {
			...
			case BOSS_SVALA_SORROWGRAVE:
				uiSvalaSorrowgrave = pCreature->GetGUID();
				break;
			case BOSS_GORTOK_PALEHOOF:
				uiGortokPalehoof = pCreature->GetGUID();
				break;
			case BOSS_SKADI_RUTHLESS:
				uiSkadiTheRuthless = pCreature->GetGUID();
				break;
			case BOSS_KING_YMIRON:
				uiKingYmiron = pCreature->GetGUID();
				break;
			case MOB_FRENZIED_WORGEN:
				uiFrenziedWorgen = pCreature->GetGUID();
				break;
			case MOB_RAVENOUS_FURBOLG:
				uiRavenousFurbolg = pCreature->GetGUID();
				break;
			case MOB_MASSIVE_JORMUNGAR:
				uiMassiveJormungar = pCreature->GetGUID();
				break;
			case MOB_FEROCIOUS_RHINO:
				uiFerociousRhino = pCreature->GetGUID();
				break;
			case MOB_SVALA:
				uiSvala = pCreature->GetGUID();
				break;
			case MOB_PALEHOOF_ORB:
				uiPalehoofOrb = pCreature->GetGUID();
				break;
			}
		}
		...
		uint64 GetData64(uint32 identifier) {
			...
			case DATA_SVALA_SORROWGRAVE:
				return uiSvalaSorrowgrave;
			case DATA_GORTOK_PALEHOOF:
				return uiGortokPalehoof;
			case DATA_SKADI_THE_RUTHLESS:
				return uiSkadiTheRuthless;
			case DATA_KING_YMIRON:
				return uiKingYmiron;
			case DATA_MOB_FRENZIED_WORGEN:
				return uiFrenziedWorgen;
			case DATA_MOB_RAVENOUS_FURBOLG:
				return uiRavenousFurbolg;
			case DATA_MOB_MASSIVE_JORMUNGAR:
				return uiMassiveJormungar;
			case DATA_MOB_FEROCIOUS_RHINO:
				return uiFerociousRhino;
			case DATA_MOB_ORB:
				return uiPalehoofOrb;
			case DATA_SVALA:
				return uiSvala;
			case DATA_GORTOK_PALEHOOF_SPHERE:
				return uiGortokPalehoofSphere;
			}
		...
		std::string GetSaveData() {
			...
			saveStream << "U P " << m_auiEncounter[0] << " "
