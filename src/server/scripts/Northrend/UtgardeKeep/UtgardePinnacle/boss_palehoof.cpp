...
//Orb spells
enum OrbSpells {
	SPELL_ORB_VISUAL = 48044, SPELL_ORB_CHANNEL = 48048
};
...
struct Locations moveLocs[] = { { 261.6f, -449.3f, 109.5f }, { 263.3f, -454.0f,
		109.5f }, { 291.5f, -450.4f, 109.5f }, { 291.5f, -454.0f, 109.5f }, {
		310.0f, -453.4f, 109.5f }, { 238.6f, -460.7f, 109.5f } };
...
public:
	...
	struct boss_palehoofAI: public BossAI {
		boss_palehoofAI(Creature *c) :
				BossAI(c, DATA_GORTOK_PALEHOOF_EVENT) {
		...
		void Reset() {
			...
			if (instance) {
				...
				Creature* pTemp = Unit::GetCreature((*me),
						instance->GetData64(DATA_MOB_FRENZIED_WORGEN));
				if (pTemp && !pTemp->isAlive())
					pTemp->Respawn();

				pTemp = Unit::GetCreature((*me),
						instance->GetData64(DATA_MOB_FEROCIOUS_RHINO));
				if (pTemp && !pTemp->isAlive())
					pTemp->Respawn();

				pTemp = Unit::GetCreature((*me),
						instance->GetData64(DATA_MOB_MASSIVE_JORMUNGAR));
				if (pTemp && !pTemp->isAlive())
					pTemp->Respawn();

				pTemp = Unit::GetCreature((*me),
						instance->GetData64(DATA_MOB_RAVENOUS_FURBOLG));
				if (pTemp && !pTemp->isAlive())
					pTemp->Respawn();

				GameObject* pGo = instance->instance->GetGameObject(
						instance->GetData64(DATA_GORTOK_PALEHOOF_SPHERE));
				if (pGo) {
		...
		void NextPhase() {
			if (currentPhase == PHASE_NONE) {
				...
				me->SummonCreature(MOB_STASIS_CONTROLLER, moveLocs[5].x,
						moveLocs[5].y, moveLocs[5].z, 0,
			...
			Creature *pOrb = Unit::GetCreature((*me),
					instance ? instance->GetData64(DATA_MOB_ORB) : 0);
			if (pOrb && pOrb->isAlive()) {
				if (currentPhase == PHASE_NONE)
					pOrb->CastSpell(me, SPELL_ORB_VISUAL, true);
				pOrb->GetMotionMaster()->MovePoint(move, moveLocs[move].x,
						moveLocs[move].y, moveLocs[move].z);
			}
			currentPhase = move;
		}
...
public:
	mob_ravenous_furbolg() :
			CreatureScript("mob_ravenous_furbolg") {
	}
	...
	struct mob_ravenous_furbolgAI: public ScriptedAI {
		mob_ravenous_furbolgAI(Creature *c) :
				ScriptedAI(c) {
		...
		void Reset() {
			...
			if (pInstance)
				if (pInstance->GetData(DATA_GORTOK_PALEHOOF_EVENT)
						== IN_PROGRESS) {
					Creature *pPalehoof = Unit::GetCreature(
							(*me),
							pInstance ?
									pInstance->GetData64(DATA_GORTOK_PALEHOOF) :
									0);
		...
		void JustDied(Unit* /*killer*/) {
			if (pInstance) {
				Creature *pPalehoof = Unit::GetCreature(
						(*me),
						pInstance ?
								pInstance->GetData64(DATA_GORTOK_PALEHOOF) : 0);
				if (pPalehoof)
...
public:
	mob_frenzied_worgen() :
			CreatureScript("mob_frenzied_worgen") {
	}
	...
	struct mob_frenzied_worgenAI: public ScriptedAI {
		mob_frenzied_worgenAI(Creature *c) :
				ScriptedAI(c) {
			pInstance = c->GetInstanceScript();
		}
		...
		void JustDied(Unit* /*killer*/) {
			if (pInstance) {
				Creature *pPalehoof = Unit::GetCreature((*me),
						pInstance->GetData64(DATA_GORTOK_PALEHOOF));
				if (pPalehoof)
					CAST_AI(boss_palehoof::boss_palehoofAI, pPalehoof->AI())->NextPhase();
			}
...
public:
	mob_ferocious_rhino() :
			CreatureScript("mob_ferocious_rhino") {
	}
	...
	struct mob_ferocious_rhinoAI: public ScriptedAI {
		mob_ferocious_rhinoAI(Creature *c) :
				ScriptedAI(c) {
			pInstance = c->GetInstanceScript();
		}
...
public:
	mob_massive_jormungar() :
			CreatureScript("mob_massive_jormungar") {
	}
	...
	struct mob_massive_jormungarAI: public ScriptedAI {
		mob_massive_jormungarAI(Creature *c) :
				ScriptedAI(c) {
			pInstance = c->GetInstanceScript();
		}
...
public:
	mob_palehoof_orb() :
			CreatureScript("mob_palehoof_orb") {
	}
		...
		void MovementInform(uint32 type, uint32 id) {
			...
			case PHASE_FRENZIED_WORGEN:
				pNext = Unit::GetCreature(
						(*me),
						pInstance ?
								pInstance->GetData64(DATA_MOB_FRENZIED_WORGEN) :
								0);
				break;
			case PHASE_RAVENOUS_FURLBORG:
				pNext = Unit::GetCreature(
			...
			case PHASE_FEROCIOUS_RHINO:
				pNext = Unit::GetCreature(
						(*me),
						pInstance ?
								pInstance->GetData64(DATA_MOB_FEROCIOUS_RHINO) :
								0);
				break;
			case PHASE_GORTOK_PALEHOOF:
				pNext = Unit::GetCreature(
...
public:
	...
	bool OnGossipHello(Player * /*pPlayer*/, GameObject *pGO) {
		...
		Creature *pPalehoof = Unit::GetCreature(*pGO,
				pInstance ? pInstance->GetData64(DATA_GORTOK_PALEHOOF) : 0);
		if (pPalehoof && pPalehoof->isAlive()) {
