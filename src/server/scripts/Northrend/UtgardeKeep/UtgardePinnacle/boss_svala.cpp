...
enum Spells {
	SPELL_CALL_FLAMES = 48258, SPELL_RITUAL_OF_THE_SWORD = 48276, //Effect #1 Teleport,  Effect #2 Dummy
...
static Position RitualChannelerPos[] = { { 296.42f, -355.01f, 90.94f, 0.0f }, {
		302.36f, -352.01f, 90.54f, 0.0f }, { 291.39f, -350.89f, 90.54f, 0.0f } };
...
public:
	...
	struct boss_svalaAI: public BossAI {
		boss_svalaAI(Creature *c) :
				BossAI(c, DATA_SVALA_SORROWGRAVE_EVENT) {
		...
		void MoveInLineOfSight(Unit* pWho) {
			...
			if (Phase == IDLE && pWho->isTargetableForAttack()
					&& me->IsHostileTo(pWho)
					&& me->IsWithinDistInMap(pWho, 40)) {
				Phase = INTRO;
		...
		void UpdateAI(const uint32 diff) {
			...
			if (uiIntroTimer <= diff) {
				...
				case 5:
					...
					break;
				case 6:
					if (me->SummonCreature(CREATURE_SVALA_SORROWGRAVE, SvalaPos,
...
public:
	mob_ritual_channeler() :
			CreatureScript("mob_ritual_channeler") {
	}
	...
	struct mob_ritual_channelerAI: public Scripted_NoMovementAI {
		mob_ritual_channelerAI(Creature *c) :
...
public:
	boss_svala_sorrowgrave() :
			CreatureScript("boss_svala_sorrowgrave") {
	}
	...
	struct boss_svala_sorrowgraveAI: public ScriptedAI {
		boss_svala_sorrowgraveAI(Creature *c) :
				ScriptedAI(c), summons(c) {
			pInstance = c->GetInstanceScript();
		}
		...
		void JustDied(Unit* pKiller) {
			if (pInstance) {
				Creature* pSvala = Unit::GetCreature((*me),
						pInstance->GetData64(DATA_SVALA));
