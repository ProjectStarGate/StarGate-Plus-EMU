			...
			} else // In Undead form used to summon weapon
			{
				if (uiEnrageTimer <= diff) {
					if (!me->HasUnitState(UNIT_STAT_CASTING)) {
						...
						if (pTarget) {
							me->SummonCreature(ENTRY_THROW_TARGET,
...
public:
	mob_annhylde_the_caller() :
			CreatureScript("mob_annhylde_the_caller") {
	}
	...
	struct mob_annhylde_the_callerAI: public ScriptedAI {
		mob_annhylde_the_callerAI(Creature *c) :
				ScriptedAI(c) {
		...
		void Reset() {
			...
			Unit* ingvar = Unit::GetUnit(*me,
					pInstance ? pInstance->GetData64(DATA_INGVAR) : 0);
			if (ingvar) {
		...
		void MovementInform(uint32 type, uint32 id) {
			...
			if (ingvar) {
				...
				case 1:
					...
					ingvar->CastSpell(ingvar, SPELL_SCOURG_RESURRECTION_DUMMY,
							true);
			...
			if (uiResurectTimer) {
				if (uiResurectTimer <= diff) {
					if (uiResurectPhase == 1) {
						Unit* ingvar = Unit::GetUnit(
								*me,
								pInstance ?
										pInstance->GetData64(DATA_INGVAR) : 0);
						if (ingvar) {
							ingvar->SetStandState(UNIT_STAND_STATE_STAND);
							ingvar->CastSpell(ingvar,
									SPELL_SCOURG_RESURRECTION_HEAL, false);
						}
						...
						if (Creature* ingvar = Unit::GetCreature(*me, pInstance ? pInstance->GetData64(DATA_INGVAR) : 0)) {
							...
							me->GetMotionMaster()->MovePoint(2, x + 1, y,
									z + 30);
							++uiResurectPhase;
							uiResurectTimer = 0;
						}
...
enum eShadowAxe {
	SPELL_SHADOW_AXE_DAMAGE = 42750, H_SPELL_SHADOW_AXE_DAMAGE = 59719
};

class mob_ingvar_throw_dummy: public CreatureScript {
public:
	mob_ingvar_throw_dummy() :
			CreatureScript("mob_ingvar_throw_dummy") {
	}
	...
	struct mob_ingvar_throw_dummyAI: public ScriptedAI {
		mob_ingvar_throw_dummyAI(Creature *c) :
				ScriptedAI(c) {
		}
