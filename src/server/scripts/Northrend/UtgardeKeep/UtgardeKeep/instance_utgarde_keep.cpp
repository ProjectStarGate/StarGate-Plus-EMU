...
public:
	...
	struct instance_utgarde_keep_InstanceMapScript: public InstanceScript {
		...
		Player* GetPlayerInMap() {
			...
			sLog->outDebug(
					LOG_FILTER_TSCR,
					"TSCR: Instance Utgarde Keep: GetPlayerInMap, but PlayerList is empty!");
			return NULL;
		}
		...
		void OnCreatureCreate(Creature* pCreature, bool /*add*/) {
			...
			case 23953:
				Keleseth = pCreature->GetGUID();
				break;
			case 24201:
				Dalronn = pCreature->GetGUID();
				break;
			case 24200:
				Skarvald = pCreature->GetGUID();
				break;
			case 23954:
				Ingvar = pCreature->GetGUID();
				break;
			}
		}
		...
		std::string GetSaveData() {
			...
			saveStream << "U K " << m_auiEncounter[0] << " "
					<< m_auiEncounter[1] << " " << m_auiEncounter[2] << " "
					<< forge_event[0] << " " << forge_event[1] << " "
					<< forge_event[2];
