...
float SkeletonSpawnPoint[5][5] = { { 156.2559f, 259.2093f }, { 156.2559f,
		259.2093f }, { 156.2559f, 259.2093f }, { 156.2559f, 259.2093f }, {
		156.2559f, 259.2093f }, };
...
public:
	...
	struct mob_frost_tombAI: public ScriptedAI {
		...
		void UpdateAI(const uint32 /*diff*/) {
			...
			if ((temp && temp->isAlive() && !temp->HasAura(SPELL_FROST_TOMB))
					|| !temp)
	...
	struct boss_kelesethAI: public BossAI {
		...
		void JustDied(Unit* /*killer*/) {
			...
			if (IsHeroic() && !ShatterFrostTomb) {
				AchievementEntry const *AchievOnTheRocks =
...
public:
	mob_vrykul_skeleton() :
			CreatureScript("mob_vrykul_skeleton") {
	}
	...
	struct mob_vrykul_skeletonAI: public ScriptedAI {
		mob_vrykul_skeletonAI(Creature *c) :
				ScriptedAI(c) {
		...
		void UpdateAI(const uint32 diff) {
			if (pInstance
					&& pInstance->GetData(DATA_PRINCEKELESETH_EVENT)
							== IN_PROGRESS) {
				if (isDead) {
