...
public:
	...
	struct boss_skarvald_the_constructorAI: public BossAI {
			...
			if (!ghost && instance) {
				...
				if (dalronn) {
					if (dalronn->isDead()) {
						...
						me->RemoveFlag(UNIT_DYNAMIC_FLAGS,
								UNIT_DYNFLAG_LOOTABLE);
						//DoCast(me, SPELL_SUMMON_SKARVALD_GHOST, true);
						Creature* temp = me->SummonCreature(MOB_SKARVALD_GHOST,
								me->GetPositionX(), me->GetPositionY(),
								me->GetPositionZ(), 0,
								TEMPSUMMON_CORPSE_DESPAWN, 5000);
						if (temp) {
							temp->SetFlag(UNIT_FIELD_FLAGS,
		...
		void UpdateAI(const uint32 diff) {
			if (ghost) {
				if (instance
						&& instance->GetData(DATA_SKARVALD_DALRONN_EVENT)
								!= IN_PROGRESS)
			...
			if (!ghost) {
				if (Check_Timer) {
					if (Check_Timer <= diff) {
						Check_Timer = 5000;
						Unit* dalronn = Unit::GetUnit(
								*me,
								instance ?
										instance->GetData64(DATA_DALRONN) : 0);
						if (dalronn && dalronn->isDead()) {
							Dalronn_isDead = true;
							Response_Timer = 2000;
							Check_Timer = 0;
	...
	struct boss_dalronn_the_controllerAI: public ScriptedAI {
		...
		void EnterCombat(Unit * who) {
			if (!ghost && pInstance) {
				...
				if (pInstance)
					pInstance->SetData(DATA_SKARVALD_DALRONN_EVENT,
							IN_PROGRESS);
			}
		}
			...
			if (!ghost && pInstance) {
				...
				if (skarvald) {
					if (skarvald->isDead()) {
						...
						Creature* temp = me->SummonCreature(MOB_DALRONN_GHOST,
								me->GetPositionX(), me->GetPositionY(),
								me->GetPositionZ(), 0,
								TEMPSUMMON_CORPSE_DESPAWN, 5000);
						if (temp) {
		...
		void UpdateAI(const uint32 diff) {
			if (ghost) {
				if (pInstance
						&& pInstance->GetData(DATA_SKARVALD_DALRONN_EVENT)
								!= IN_PROGRESS)
					me->DealDamage(me, me->GetHealth(), NULL, DIRECT_DAMAGE,
							SPELL_SCHOOL_MASK_NORMAL, NULL, false);
			}
