...
Vector3 VMapManager2::convertPositionToInternalRep(float x, float y,
		float z) const {
	Vector3 pos;
...
Vector3 VMapManager2::convertPositionToMangosRep(float x, float y,
		float z) const {
	Vector3 pos;
	const float mid = 0.5f * 64.0f * 533.33333333f;
	pos.x = mid - x;
...
void VMapManager2::preventMapsFromBeingUsed(const char* pMapIdString) {
	...
	if (pMapIdString != NULL) {
		...
		while (std::getline(map_ss, map_str, ',')) {
			...
			ss2 >> map_num;
			if (map_num >= 0) {
				sLog->outDebug(LOG_FILTER_MAPS, "Ignoring Map %i for VMaps",
						map_num);
...
int VMapManager2::loadMap(const char* pBasePath, unsigned int pMapId, int x,
		int y) {
...
bool VMapManager2::_loadMap(unsigned int pMapId, const std::string &basePath,
	...
	if (instanceTree == iInstanceMapTrees.end()) {
		...
		if (!newTree->InitMap(mapFileName, this))
			return false;
		instanceTree = iInstanceMapTrees.insert(
				InstanceTreeMap::value_type(pMapId, newTree)).first;
	}
...
bool VMapManager2::isInLineOfSight(unsigned int pMapId, float x1, float y1,
 ...
 */
bool VMapManager2::getObjectHitPos(unsigned int pMapId, float x1, float y1,
		float z1, float x2, float y2, float z2, float& rx, float &ry, float& rz,
		float pModifyDist) {
	bool result = false;
	rx = x2;
	ry = y2;
	...
	if (isLineOfSightCalcEnabled()) {
		...
		if (instanceTree != iInstanceMapTrees.end()) {
			...
			Vector3 pos2 = convertPositionToInternalRep(x2, y2, z2);
			Vector3 resultPos;
			result = instanceTree->second->getObjectHitPos(pos1, pos2,
					resultPos, pModifyDist);
			resultPos = convertPositionToMangosRep(resultPos.x, resultPos.y,
					resultPos.z);
			rx = resultPos.x;
			ry = resultPos.y;
			rz = resultPos.z;
...
bool VMapManager2::getAreaInfo(unsigned int pMapId, float x, float y, float &z,
	...
	bool result = false;
	InstanceTreeMap::const_iterator instanceTree = iInstanceMapTrees.find(
			pMapId);
	if (instanceTree != iInstanceMapTrees.end()) {
		Vector3 pos = convertPositionToInternalRep(x, y, z);
		result = instanceTree->second->getAreaInfo(pos, flags, adtId, rootId,
				groupId);
...
WorldModel* VMapManager2::acquireModelInstance(const std::string &basepath,
	...
	if (model == iLoadedModelFiles.end()) {
		...
		sLog->outDebug(LOG_FILTER_MAPS, "VMapManager2: loading file '%s%s'",
				basepath.c_str(), filename.c_str());
		model =
				iLoadedModelFiles.insert(
						std::pair<std::string, ManagedModel>(filename,
								ManagedModel())).first;
...
void VMapManager2::releaseModelInstance(const std::string &filename) {
	...
	if (model->second.decRefCount() == 0) {
		sLog->outDebug(LOG_FILTER_MAPS, "VMapManager2: unloading file '%s'",
				filename.c_str());
...
bool VMapManager2::existsMap(const char* pBasePath, unsigned int pMapId, int x,
		int y) {
