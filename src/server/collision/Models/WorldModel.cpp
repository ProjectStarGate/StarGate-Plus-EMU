...
bool IntersectTriangle(const MeshTriangle &tri,
		std::vector<Vector3>::const_iterator points, const G3D::Ray &ray,
		float &distance) {
...
bool WmoLiquid::GetLiquidHeight(const Vector3 &pos, float &liqHeight) const {
	...
	if (dx > dy) // case (a)
			{
		float sx = iHeight[tx + 1 + ty * rowOffset]
	...
	} else // case (b)
	{
		...
		float sy = iHeight[tx + (ty + 1) * rowOffset]
				- iHeight[tx + ty * rowOffset];
		liqHeight = iHeight[tx + ty * rowOffset] + dx * sx + dy * sy;
	}
	return true;
...
uint32 WmoLiquid::GetFileSize() {
	return 2 * sizeof(uint32) + sizeof(Vector3)
...
GroupModel::GroupModel(const GroupModel &other) :
		iBound(other.iBound), iMogpFlags(other.iMogpFlags), iGroupWMOID(
				other.iGroupWMOID), vertices(other.vertices), triangles(
				other.triangles), meshTree(other.meshTree), iLiquid(0) {
	if (other.iLiquid)
...
bool GroupModel::IntersectRay(const G3D::Ray &ray, float &distance,
...
bool GroupModel::IsInsideObject(const Vector3 &pos, const Vector3 &down,
		float &z_dist) const {
...
struct WModelRayCallBack {
	...
	bool operator()(const G3D::Ray& ray, uint32 entry, float& distance,
			bool pStopAtFirstHit) {
		bool result = models[entry].IntersectRay(ray, distance,
				pStopAtFirstHit);
		if (result)
			hit = true;
		return hit;
...
bool WorldModel::IntersectRay(const G3D::Ray &ray, float &distance,
		bool stopAtFirstHit) const {
...
public:
	WModelAreaCallback(const std::vector<GroupModel> &vals, const Vector3 &down) :
			prims(vals.begin()), hit(vals.end()), minVol(G3D::inf()), zDist(
					G3D::inf()), zVec(down) {
	}
...
bool WorldModel::IntersectPoint(const G3D::Vector3 &p, const G3D::Vector3 &down,
...
bool WorldModel::GetLocationInfo(const G3D::Vector3 &p,
		const G3D::Vector3 &down, float &dist, LocationInfo &info) const {
	if (!groupModels.size())
		return false;
	WModelAreaCallback callback(groupModels, down);
	groupTree.intersectPoint(p, callback);
