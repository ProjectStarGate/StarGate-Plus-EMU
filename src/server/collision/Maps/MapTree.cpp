...
std::string StaticMapTree::getTileFileName(uint32 mapID, uint32 tileX,
		uint32 tileY) {
...
bool StaticMapTree::getIntersectionTime(const G3D::Ray& pRay, float &pMaxDist,
...
bool StaticMapTree::isInLineOfSight(const Vector3& pos1,
	...
	G3D::Ray ray = G3D::Ray::fromOriginAndDirection(pos1,
			(pos2 - pos1) / maxDist);
...
bool StaticMapTree::getObjectHitPos(const Vector3& pPos1, const Vector3& pPos2,
		Vector3& pResultHitPos, float pModifyDist) const {
	bool result = false;
...
bool StaticMapTree::CanLoadMap(const std::string &vmapPath, uint32 mapID,
...
bool StaticMapTree::LoadMapTile(uint32 tileX, uint32 tileY, VMapManager2 *vm) {
	...
	if (!iTreeValues) {
		sLog->outError(
				"StaticMapTree::LoadMapTile() : tree has not been initialized [%u,%u]",
				tileX, tileY);
		return false;
	}
	bool result = true;
	...
	if (tf) {
		...
		for (uint32 i = 0; i < numSpawns && result; ++i) {
			...
			if (result) {
				...
				WorldModel *model = vm->acquireModelInstance(iBasePath,
						spawn.name);
				if (!model)
					sLog->outError(
							"StaticMapTree::LoadMapTile() : could not acquire WorldModel pointer [%u,%u]",
							tileX, tileY);
...
#endif
						iTreeValues[referencedVal] = ModelInstance(spawn,
								model);
...
void StaticMapTree::UnloadMapTile(uint32 tileX, uint32 tileY,
	...
	{
		std::string tilefile = iBasePath
				+ getTileFileName(iMapID, tileX, tileY);
		FILE* tf = fopen(tilefile.c_str(), "rb");
		if (tf) {
			bool result = true;
			char chunk[8];
				...
				if (result) {
					...
					if (fread(&referencedNode, sizeof(uint32), 1, tf) != 1)
						...
						if (!iLoadedSpawns.count(referencedNode))
							sLog->outError(
									"StaticMapTree::UnloadMapTile() : trying to unload non-referenced model '%s' (ID:%u)",
