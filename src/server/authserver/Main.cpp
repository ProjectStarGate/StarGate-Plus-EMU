...
#ifndef _STARGATE_REALM_CONFIG
# define _STARGATE_REALM_CONFIG  "authserver.conf"
#endif //_STARGATE_REALM_CONFIG
#ifdef _WIN32
#include "ServiceWin32.h"
char serviceName[] = "authserver";
char serviceLongName[] = "StarGate Auth service";
char serviceDescription[] = "StarGate World of Warcraft emulator world auth service";
/*
...
extern int main(int argc, char **argv) {
	...
	///- Command line parsing to get the configuration file name
			char const* cfg_file = _STARGATE_REALM_CONFIG;
			int c = 1;
			while(c < argc)
			{
    ...
    sLog->outDetail("%s (Library: %s)", OPENSSL_VERSION_TEXT, SSLeay_version(SSLEAY_VERSION));
			...
			{
				sLog->outError("StarGate Auth can not bind to %s:%d", bind_ip.c_str(), rmport);
				return 1;
			}
...
#ifdef _WIN32
	{
		...
		if (Prio)
		{
			...
			sLog->outString("StarGate Auth process priority class set to HIGH");
			else
