...
int RASocket::open(void *) {
	...
	if (peer().get_remote_addr(remote_addr) == -1) {
		sLog->outError("RASocket::open: peer().get_remote_addr error is %s",
...
int RASocket::send(const std::string& line) {
	return size_t(peer().send(line.c_str(), line.length())) == line.length() ?
			0 : -1;
}
...
int RASocket::check_access_level(const std::string& user) {
	...
	QueryResult result =
			LoginDatabase.PQuery(
					"SELECT a.id, aa.gmlevel, aa.RealmID FROM account a LEFT JOIN account_access aa ON (a.id = aa.id) WHERE a.username = '%s'",
					safe_user.c_str());
...
int RASocket::svc(void) {
	...
	for (;;) {
		...
		if (size_t(peer().send(tc_prompt, strlen(tc_prompt)))
				!= strlen(tc_prompt))
			return -1;
...
void RASocket::zprint(void* callbackArg, const char * szText) {
	...
	if (socket->putq(mb, const_cast<ACE_Time_Value*>(&ACE_Time_Value::zero))
			== -1) {
		sLog->outRemote(
				"Failed to enqueue message, queue is full or closed. Error is %s",
				ACE_OS::strerror(errno));
		mb->release();
	}
}
