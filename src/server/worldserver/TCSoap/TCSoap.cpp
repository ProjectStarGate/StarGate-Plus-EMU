...
void TCSoapRunnable::run() {
	...
	if (soap_bind(&soap, m_host.c_str(), m_port, 100) < 0) {
		sLog->outError("TCSoap: couldn't bind to %s:%d", m_host.c_str(),
				m_port);
		exit(-1);
	}
	...
	while (!World::IsStopped()) {
		...
		sLog->outDebug(LOG_FILTER_NETWORKIO,
				"TCSoap: accepted connection from IP=%d.%d.%d.%d",
...
int ns1__executeCommand(soap* soap, char* command, char** result) {
	...
	if (!soap->userid || !soap->passwd) {
		sLog->outDebug(LOG_FILTER_NETWORKIO,
				"TCSoap: Client didn't provide login information");
		return 401;
	}
	...
	if (!accountId) {
		sLog->outDebug(LOG_FILTER_NETWORKIO,
				"TCSoap: Client used invalid username '%s'", soap->userid);
		return 401;
	}

	if (!sAccountMgr->CheckPassword(accountId, soap->passwd)) {
		sLog->outDebug(LOG_FILTER_NETWORKIO,
				"TCSoap: invalid password for account '%s'", soap->userid);
		return 401;
	}

	if (sAccountMgr->GetSecurity(accountId) < SEC_ADMINISTRATOR) {
		sLog->outDebug(LOG_FILTER_NETWORKIO, "TCSoap: %s's gmlevel is too low",
				soap->userid);
		return 403;
	}

	if (!command || !*command)
		return soap_sender_fault(soap, "Command mustn't be empty",
	...
	// commands are executed in the world thread. We have to wait for them to be completed
	{
		...
		CliCommandHolder* cmd = new CliCommandHolder(&connection, command,
				&SOAPCommand::print, &SOAPCommand::commandFinished);
	...
	if (acc) {
		sLog->outError(
				"TCSoap: Error while acquiring lock, acc = %i, errno = %u", acc,
				errno);
	}
...
struct Namespace namespaces[] = { { "SOAP-ENV",
		"http://schemas.xmlsoap.org/soap/envelope/", NULL, NULL }, // must be first
		...
		{ "xsi", "http://www.w3.org/1999/XMLSchema-instance",
				...
				"http://www.w3.org/*/XMLSchema", NULL }, { "ns1", "urn:TC",
				NULL, NULL }, // "ns1" namespace prefix
		{ NULL, NULL, NULL, NULL } };
