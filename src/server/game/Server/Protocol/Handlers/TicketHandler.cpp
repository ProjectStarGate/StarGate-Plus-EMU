...
void WorldSession::HandleGMTicketCreateOpcode(WorldPacket & recv_data) {
	...
	if (GetPlayer()->getLevel()
		...
		SendNotification(GetStarGateString(LANG_TICKET_REQ),
				sWorld->getIntConfig(CONFIG_TICKET_LEVEL_REQ));
		return;
	}
	...
	sWorld->SendGMText(LANG_COMMAND_TICKETNEW, GetPlayer()->GetName(),
			ticket->guid);
}
...
void WorldSession::HandleGMTicketUpdateOpcode(WorldPacket & recv_data) {
	...
	sWorld->SendGMText(LANG_COMMAND_TICKETUPDATED, GetPlayer()->GetName(),
			ticket->guid);
}
...
void WorldSession::HandleGMTicketGetTicketOpcode(WorldPacket & /*recv_data*/) {
	...
	if (GM_Ticket * ticket = sTicketMgr->GetGMTicketByPlayer(
			GetPlayer()->GetGUID())) {
		if (ticket->completed)
			SendGMTicketResponse(ticket);
		else
			SendGMTicketGetTicket(GMTICKET_STATUS_HASTEXT,
					ticket->message.c_str(), ticket);
	} else
		SendGMTicketGetTicket(GMTICKET_STATUS_DEFAULT, NULL);
}

void WorldSession::HandleGMTicketSystemStatusOpcode(
...
void WorldSession::SendGMTicketGetTicket(uint32 status, char const* text,
	...
	if (status == GMTICKET_STATUS_HASTEXT) {
		...
		if (ticket) {
			...
			if (GM_Ticket * oldestTicket =
					sTicketMgr->GetOldestOpenGMTicket()) {
				// get ticketage, but it's stored in seconds so we have to do it in days
				float oldestTicketAge = (float) time(NULL)
						- (float) oldestTicket->timestamp;
				oldestTicketAge /= DAY;
			...
			data
					<< uint8(
							std::min(ticket->escalated,
									uint8(TICKET_IN_ESCALATION_QUEUE))); // escalated data
			data
					<< uint8(
							ticket->viewed ?
									GMTICKET_OPENEDBYGM_STATUS_OPENED :
									GMTICKET_OPENEDBYGM_STATUS_NOT_OPENED); // whether or not it has been viewed
		} else {
...
void WorldSession::HandleGMSurveySubmit(WorldPacket& recv_data) {
	...
	ss
			<< "INSERT INTO gm_surveys (player, surveyid, mainSurvey, overall_comment, timestamp) VALUES (";
	...
	for (uint8 i = 0; i < 10; i++) {
		...
		os
				<< "INSERT INTO gm_subsurveys (surveyid, subsurveyid, rank, comment) VALUES (";
...
void WorldSession::HandleReportLag(WorldPacket& recv_data) {
	...
	std::ostringstream os;
	os
			<< "INSERT INTO lag_reports (player, lag_type, map, posX, posY, posZ) VALUES (";
