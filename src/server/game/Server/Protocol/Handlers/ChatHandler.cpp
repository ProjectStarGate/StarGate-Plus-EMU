...
void WorldSession::HandleMessagechatOpcode(WorldPacket & recv_data) {
	...
	// LANG_ADDON should not be changed nor be affected by flood control
		...
		if (!_player->CanSpeak()) {
			...
			SendNotification(GetStarGateString(LANG_WAIT_BEFORE_SPEAKING),
					timeStr.c_str());
			return;
		}
	...
	if (GetPlayer()->HasAura(1852) && type != CHAT_MSG_WHISPER) {
		...
		SendNotification(GetStarGateString(LANG_GM_SILENCE),
				GetPlayer()->GetName());
		return;
	}
	...
	case CHAT_MSG_YELL: {
		...
		if (GetSecurity() == SEC_PLAYER)
        {
			...
			SendNotification(GetStarGateString(LANG_SAY_REQ),
					sWorld->getIntConfig(CONFIG_CHAT_SAY_LEVEL_REQ));
			return;
			}
		}
	...
	case CHAT_MSG_PARTY_LEADER: {
		...
		if ((type == CHAT_MSG_PARTY_LEADER)
				&& !group->IsLeader(_player->GetGUID()))
			return;
	...
	case CHAT_MSG_RAID: {
		...
		WorldPacket data;
		ChatHandler::FillMessageData(&data, this, CHAT_MSG_RAID, lang, "", 0,
				msg.c_str(), NULL);
		group->BroadcastPacket(&data, false);
	}
		break;
	case CHAT_MSG_RAID_LEADER: {
		...
		WorldPacket data;
		ChatHandler::FillMessageData(&data, this, CHAT_MSG_RAID_LEADER, lang,
				"", 0, msg.c_str(), NULL);
		group->BroadcastPacket(&data, false);
	}
		break;
	case CHAT_MSG_RAID_WARNING: {
		...
		ChatHandler::FillMessageData(&data, this, CHAT_MSG_RAID_WARNING, lang,
				"", 0, msg.c_str(), NULL);
		group->BroadcastPacket(&data, false);
	}
		break;
	case CHAT_MSG_BATTLEGROUND: {
		...
		WorldPacket data;
		ChatHandler::FillMessageData(&data, this, CHAT_MSG_BATTLEGROUND, lang,
				"", 0, msg.c_str(), NULL);
		group->BroadcastPacket(&data, false);
	}
		break;
	...
	case CHAT_MSG_BATTLEGROUND_LEADER: {
		...
		WorldPacket data;
		ChatHandler::FillMessageData(&data, this, CHAT_MSG_BATTLEGROUND_LEADER,
...
public:
	EmoteChatBuilder(Player const& pl, uint32 text_emote, uint32 emote_num,
			Unit const* target) :
			i_player(pl), i_text_emote(text_emote), i_emote_num(emote_num), i_target(
					target) {
	}

	void operator()(WorldPacket& data, LocaleConstant loc_idx) {
		char const* nam =
				i_target ? i_target->GetNameForLocaleIdx(loc_idx) : NULL;
