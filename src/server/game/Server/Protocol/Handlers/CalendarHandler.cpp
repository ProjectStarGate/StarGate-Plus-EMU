...
void WorldSession::HandleCalendarGetCalendar(WorldPacket & /*recv_data*/) {
	...
	for (int i = 0; i < MAX_DIFFICULTY; ++i) {
		for (Player::BoundInstancesMap::const_iterator itr =
				_player->m_boundInstances[i].begin();
				itr != _player->m_boundInstances[i].end(); ++itr) {
			if (itr->second.perm) {
...
void WorldSession::HandleCalendarEventRemoveInvite(WorldPacket &recv_data) {
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: CMSG_CALENDAR_EVENT_REMOVE_INVITE");
	recv_data.hexlike();
	recv_data.rpos(recv_data.wpos()); // set to end to avoid warnings spam
...
void WorldSession::HandleCalendarEventModeratorStatus(WorldPacket &recv_data) {
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: CMSG_CALENDAR_EVENT_MODERATOR_STATUS");
	recv_data.hexlike();
	recv_data.rpos(recv_data.wpos()); // set to end to avoid warnings spam
...
void WorldSession::HandleCalendarGetNumPending(WorldPacket & /*recv_data*/) {
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: CMSG_CALENDAR_GET_NUM_PENDING"); // empty
