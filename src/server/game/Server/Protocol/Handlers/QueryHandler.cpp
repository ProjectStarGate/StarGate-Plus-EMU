...
void WorldSession::HandleCreatureQueryOpcode(WorldPacket & recv_data) {
	...
	if (ci) {
		...
		sLog->outDetail("WORLD: CMSG_CREATURE_QUERY '%s' - Entry: %u.",
				ci->Name, entry);
		// guess size
		...
		sLog->outDebug(
				...
				"WORLD: CMSG_CREATURE_QUERY - NO CREATURE INFO! (GUID: %u, ENTRY: %u)",
	...
	if (info) {
		...
		if (loc_idx >= 0) {
			if (GameObjectLocale const *gl = sObjectMgr->GetGameObjectLocale(entryID)) {
				...
				sObjectMgr->GetLocaleString(gl->CastBarCaption, loc_idx,
						CastBarCaption);
			}
		}
		sLog->outDetail("WORLD: CMSG_GAMEOBJECT_QUERY '%s' - Entry: %u. ",
				info->name, entryID);
			...
			if (POI) {
				...
				for (QuestPOIVector::const_iterator itr = POI->begin();
					...
					for (std::vector<QuestPOIPoint>::const_iterator itr2 =
							itr->points.begin(); itr2 != itr->points.end();
							++itr2) {
