...
template<>
void ActivePoolData::ActivateObject<GameObject>(uint32 db_guid,
		uint32 pool_id) {
...
PoolObject* PoolGroup<T>::RollOne(ActivePoolData& spawns, uint32 triggerFrom) {
		...
		for (uint32 i = 0; i < ExplicitlyChanced.size(); ++i) {
			...
			if (roll < 0
					&& (ExplicitlyChanced[i].guid == triggerFrom
							|| !spawns.IsActiveObject<T>(
									ExplicitlyChanced[i].guid)))
	...
	if (!EqualChanced.empty()) {
		...
		// Triggering object is marked as spawned at this time and can be also rolled (respawn case)
		// so this need explicit check for this case
		if (EqualChanced[index].guid == triggerFrom
				|| !spawns.IsActiveObject<T>(EqualChanced[index].guid))
...
void PoolGroup<Creature>::Despawn1Object(uint32 guid) {
	if (CreatureData const* data = sObjectMgr->GetCreatureData(guid)) {
		...
		if (Creature * pCreature = sObjectAccessor->GetObjectInWorld(
				MAKE_NEW_GUID(guid, data->id, HIGHGUID_UNIT), (Creature*) NULL))
...
void PoolGroup<GameObject>::Despawn1Object(uint32 guid) {
	if (GameObjectData const* data = sObjectMgr->GetGOData(guid)) {
		...
		if (GameObject * pGameobject = sObjectAccessor->GetObjectInWorld(
				MAKE_NEW_GUID(guid, data->id, HIGHGUID_GAMEOBJECT),
				(GameObject*) NULL))
...
void PoolGroup<Quest>::Despawn1Object(uint32 quest_id) {
	...
	for (PooledQuestRelation::iterator itr = qr.first; itr != qr.second;
		...
		if (qitr == questMap->end())
			continue;
		QuestRelations::iterator lastElement = questMap->upper_bound(
				itr->second);
...
void PoolGroup<Quest>::Spawn1Object(PoolObject* obj) {
	...
	for (PooledQuestRelation::iterator itr = qr.first; itr != qr.second;
			++itr) {
		sLog->outDebug(LOG_FILTER_POOLSYS,
				"PoolGroup<Quest>: Adding quest %u to creature %u", itr->first,
				itr->second);
	...
	for (PooledQuestRelation::iterator itr = qr.first; itr != qr.second;
			++itr) {
		sLog->outDebug(LOG_FILTER_POOLSYS,
				"PoolGroup<Quest>: Adding quest %u to GO %u", itr->first,
				itr->second);
		questMap->insert(QuestRelations::value_type(itr->second, itr->first));
	}
}
...
template<>
void PoolGroup<Quest>::SpawnObject(ActivePoolData& spawns, uint32 limit,
		uint32 triggerFrom) {
	sLog->outDebug(LOG_FILTER_POOLSYS, "PoolGroup<Quest>: Spawning pool %u",
			poolId);
	// load state from db
	if (!triggerFrom) {
		QueryResult result = CharacterDatabase.PQuery(
				"SELECT quest_id FROM pool_quest_save WHERE pool_id = %u",
				poolId);
		if (result) {
			do {
...
void PoolGroup<Creature>::ReSpawn1Object(PoolObject* obj) {
	if (CreatureData const* data = sObjectMgr->GetCreatureData(obj->guid))
		if (Creature * pCreature = sObjectAccessor->GetObjectInWorld(
				MAKE_NEW_GUID(obj->guid, data->id, HIGHGUID_UNIT),
				(Creature*) NULL))
...
void PoolGroup<GameObject>::ReSpawn1Object(PoolObject* obj) {
	if (GameObjectData const* data = sObjectMgr->GetGOData(obj->guid))
		if (GameObject * pGameobject = sObjectAccessor->GetObjectInWorld(
				MAKE_NEW_GUID(obj->guid, data->id, HIGHGUID_GAMEOBJECT),
				(GameObject*) NULL))
...
void PoolMgr::LoadFromDB() {
	...
	sLog->outString(">> Loaded %u objects pools in %u ms", count,
	...
	if (!result) {
		...
		do {
			...
			if (!data) {
				sLog->outErrorDb(
						"`pool_creature` has a non existing creature spawn (GUID: %u) defined for pool id (%u), skipped.",
						guid, pool_id);
				continue;
			}
			if (pool_id > max_pool_id) {
				sLog->outErrorDb(
						"`pool_creature` pool id (%u) is out of range compared to max pool id in `pool_template`, skipped.",
						pool_id);
				continue;
			}
			if (chance < 0 || chance > 100) {
				sLog->outErrorDb(
						"`pool_creature` has an invalid chance (%f) for creature guid (%u) in pool id (%u), skipped.",
						chance, guid, pool_id);
				continue;
			}
			PoolTemplateData *pPoolTemplate = &mPoolTemplate[pool_id];
		...
		sLog->outString(">> Loaded %u creatures in pools in %u ms", count,
				GetMSTimeDiffToNow(oldMSTime));
		sLog->outString();
	}
	...
	if (!result) {
		...
		do {
			...
			if (!data) {
				sLog->outErrorDb(
						"`pool_gameobject` has a non existing gameobject spawn (GUID: %u) defined for pool id (%u), skipped.",
						guid, pool_id);
				continue;
			}
			GameObjectInfo const* goinfo = ObjectMgr::GetGameObjectInfo(
					data->id);
			if (goinfo->type != GAMEOBJECT_TYPE_CHEST
					&& goinfo->type != GAMEOBJECT_TYPE_GOOBER
					&& goinfo->type != GAMEOBJECT_TYPE_FISHINGHOLE) {
				sLog->outErrorDb(
						"`pool_gameobject` has a not lootable gameobject spawn (GUID: %u, type: %u) defined for pool id (%u), skipped.",
						guid, goinfo->type, pool_id);
				continue;
			}
			if (pool_id > max_pool_id) {
				sLog->outErrorDb(
						"`pool_gameobject` pool id (%u) is out of range compared to max pool id in `pool_template`, skipped.",
						pool_id);
				continue;
			}
			if (chance < 0 || chance > 100) {
				sLog->outErrorDb(
						"`pool_gameobject` has an invalid chance (%f) for gameobject guid (%u) in pool id (%u), skipped.",
						chance, guid, pool_id);
				continue;
			}
			PoolTemplateData *pPoolTemplate = &mPoolTemplate[pool_id];
		...
		sLog->outString(">> Loaded %u gameobject in pools in %u ms", count,
				GetMSTimeDiffToNow(oldMSTime));
		sLog->outString();
	}
	...
	if (!result) {
		...
		do {
			...
			if (mother_pool_id > max_pool_id) {
				sLog->outErrorDb(
						"`pool_pool` mother_pool id (%u) is out of range compared to max pool id in `pool_template`, skipped.",
						mother_pool_id);
				continue;
			}
			if (child_pool_id > max_pool_id) {
				sLog->outErrorDb(
						"`pool_pool` included pool_id (%u) is out of range compared to max pool id in `pool_template`, skipped.",
						child_pool_id);
				continue;
			}
			if (mother_pool_id == child_pool_id) {
				sLog->outErrorDb(
						"`pool_pool` pool_id (%u) includes itself, dead-lock detected, skipped.",
						child_pool_id);
				continue;
			}
			if (chance < 0 || chance > 100) {
				sLog->outErrorDb(
						"`pool_pool` has an invalid chance (%f) for pool id (%u) in mother pool id (%u), skipped.",
			...
			for (SearchMap::iterator poolItr = mPoolSearchMap.find(i);
				...
				if (checkedPools.find(poolItr->second) != checkedPools.end()) {
					...
					for (std::set<uint32>::const_iterator itr =
							checkedPools.begin(); itr != checkedPools.end();
							++itr)
						ss << *itr << " ";
					ss
		...
		sLog->outString(">> Loaded %u pools in mother pools in %u ms", count,
				GetMSTimeDiffToNow(oldMSTime));
		sLog->outString();
	}
}
...
void PoolMgr::LoadQuestPools() {
	...
	do {
		...
		if (!pQuest) {
			sLog->outErrorDb(
					"`pool_quest` has a non existing quest template (Entry: %u) defined for pool id (%u), skipped.",
					entry, pool_id);
			continue;
		}
		...
		if (pool_id > max_pool_id) {
			sLog->outErrorDb(
					"`pool_quest` pool id (%u) is out of range compared to max pool id in `pool_template`, skipped.",
					pool_id);
			continue;
		}
		...
		if (!pQuest->IsDailyOrWeekly()) {
			sLog->outErrorDb(
					"`pool_quest` has an quest (%u) which is not daily or weekly in pool id (%u), use ExclusiveGroup instead, skipped.",
					entry, pool_id);
			continue;
		}
		...
		if (poolTypeMap[pool_id] == QUEST_NONE)
			poolTypeMap[pool_id] =
					pQuest->IsDaily() ? QUEST_DAILY : QUEST_WEEKLY;
		...
		if (poolTypeMap[pool_id] != currType) {
			sLog->outErrorDb(
					...
					entry,
					currType == QUEST_DAILY ? "QUEST_DAILY" : "QUEST_WEEKLY",
					pool_id,
		...
		if (creBounds.first == creBounds.second
			...
			sLog->outErrorDb(
					"`pool_quest` lists entry (%u) as member of pool (%u) but is not started anywhere, skipped.",
					entry, pool_id);
			continue;
		}
	...
	sLog->outString(">> Loaded %u quests in pools in %u ms", count,
			GetMSTimeDiffToNow(oldMSTime));
	sLog->outString();
}
...
void PoolMgr::Initialize() {
		...
		do {
			...
			if (!CheckPool(pool_entry)) {
				if (pool_pool_id)
					...
					sLog->outErrorDb(
							"Pool Id %u has no equal chance pooled entites defined and explicit chance sum is not 100. This broken pool is a child pool of Id %u and cannot be safely removed.",
							pool_entry, fields[2].GetUInt32());
				else
					sLog->outErrorDb(
							"Pool Id %u has no equal chance pooled entites defined and explicit chance sum is not 100. The pool will not be spawned.",
							pool_entry);
				continue;
			}
	...
	sLog->outBasic("Pool handling system initialized, %u pools spawned.",
			count);
}
...
void PoolMgr::SpawnPool<Creature>(uint32 pool_id, uint32 db_guid) {
	if (!mPoolCreatureGroups[pool_id].isEmpty())
		mPoolCreatureGroups[pool_id].SpawnObject(mSpawnedData,
...
void PoolMgr::SpawnPool<GameObject>(uint32 pool_id, uint32 db_guid) {
	if (!mPoolGameobjectGroups[pool_id].isEmpty())
		mPoolGameobjectGroups[pool_id].SpawnObject(mSpawnedData,
				mPoolTemplate[pool_id].MaxLimit, db_guid);
}
...
void PoolMgr::SpawnPool<Pool>(uint32 pool_id, uint32 sub_pool_id) {
	if (!mPoolPoolGroups[pool_id].isEmpty())
		mPoolPoolGroups[pool_id].SpawnObject(mSpawnedData,
...
void PoolMgr::SpawnPool<Quest>(uint32 pool_id, uint32 quest_id) {
	if (!mPoolQuestGroups[pool_id].isEmpty())
		mPoolQuestGroups[pool_id].SpawnObject(mSpawnedData,
				mPoolTemplate[pool_id].MaxLimit, quest_id);
}
