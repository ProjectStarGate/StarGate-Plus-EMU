...
enum GeneralScriptTexts {
	...
	EMOTE_GENERIC_BERSERK_RAID = -1000005,
...
struct DefaultTargetSelector: public std::unary_function<Unit *, bool> {
	...
	DefaultTargetSelector(const Unit *pUnit, float dist, bool playerOnly,
			int32 aura) :
...
public:
	...
	Unit* SelectTarget(SelectAggroTarget targetType, uint32 position = 0,
			float dist = 0.0f, bool playerOnly = false, int32 aura = 0);
	void SelectTargetList(std::list<Unit*> &targetList, uint32 num,
			SelectAggroTarget targetType, float dist = 0.0f, bool playerOnly =
					false, int32 aura = 0);
	...
	template<class PREDICATE> Unit* SelectTarget(SelectAggroTarget targetType,
			uint32 position, PREDICATE predicate) {
	...
	virtual void sGossipSelect(Player* /*player*/, uint32 /*sender*/,
			uint32 /*action*/) {
	}
	virtual void sGossipSelectCode(Player* /*player*/, uint32 /*sender*/,
	...
	virtual void sQuestReward(Player* /*player*/, Quest const* /*quest*/,
			uint32 /*opt*/) {
	}
	virtual bool sOnDummyEffect(Unit* /*caster*/, uint32 /*spellId*/,
