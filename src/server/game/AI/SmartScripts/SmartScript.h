...
public:
	...
	void GetScript();
	void FillScript(SmartAIEventList e, WorldObject* obj,
			AreaTriggerEntry const* at);

	void ProcessEventsFor(SMART_EVENT e, Unit* unit = NULL, uint32 var0 = 0,
			uint32 var1 = 0, bool bvar = false, const SpellEntry* spell = NULL,
			GameObject* gob = NULL);
	void ProcessEvent(SmartScriptHolder& e, Unit* unit = NULL, uint32 var0 = 0,
			uint32 var1 = 0, bool bvar = false, const SpellEntry* spell = NULL,
			GameObject* gob = NULL);
	...
	void ProcessAction(SmartScriptHolder& e, Unit* unit = NULL, uint32 var0 = 0,
			uint32 var1 = 0, bool bvar = false, const SpellEntry* spell = NULL,
			GameObject* gob = NULL);
	...
	bool IsSmart(Creature* c = NULL) {
		...
		if (!smart)
			sLog->outErrorDb(
					"SmartScript: Action target Creature(entry: %u) is not using SmartAI, action skipped to prevent crash.",
	...
	bool IsSmartGO(GameObject* g = NULL) {
		...
		if (!smart)
			...
			sLog->outErrorDb(
					"SmartScript: Action target GameObject(entry: %u) is not using SmartGameObjectAI, action skipped to prevent crash.",
	...
	GameObject* FindGameObjectNear(WorldObject* pSearchObject,
			uint32 guid) const {
	...
	Creature* FindCreatureNear(WorldObject* pSearchObject, uint32 guid) const {
		...
		Trinity::CreatureSearcher<Trinity::CreatureWithDbGUIDCheck> checker(
				pSearchObject, crea, target_check);
