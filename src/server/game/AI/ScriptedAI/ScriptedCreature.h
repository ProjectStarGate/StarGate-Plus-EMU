...
struct ScriptedAI: public CreatureAI {
	...
	void DoCastSpell(Unit* pTarget, SpellEntry const* pSpellInfo,
	...
	std::list<Creature*> DoFindFriendlyMissingBuff(float fRange,
			uint32 uiSpellId);
	...
	SpellEntry const* SelectSpell(Unit* Target, uint32 School, uint32 Mechanic,
			SelectTargetType Targets, uint32 PowerCostMin, uint32 PowerCostMax,
			float RangeMin, float RangeMax, SelectEffect Effect);

	//Checks if you can cast the specified spell
	bool CanCast(Unit* pTarget, SpellEntry const* pSpell, bool bTriggered =
			false);

	void SetEquipmentSlots(bool bLoadDefault,
			int32 uiMainHand = EQUIP_NO_CHANGE, int32 uiOffHand =
					EQUIP_NO_CHANGE, int32 uiRanged = EQUIP_NO_CHANGE);
	...
	template<class T> inline const T& DUNGEON_MODE(const T& normal5,
			const T& heroic10) {
		switch (m_difficulty) {
	...
	template<class T> inline const T& RAID_MODE(const T& normal10,
			const T& normal25) {
		switch (m_difficulty) {
	...
	template<class T> inline const T& RAID_MODE(const T& normal10,
			const T& normal25, const T& heroic10, const T& heroic25) {
		switch (m_difficulty) {
		case RAID_DIFFICULTY_10MAN_NORMAL:
			return normal10;
		case RAID_DIFFICULTY_25MAN_NORMAL:
...
Creature *GetClosestCreatureWithEntry(WorldObject *pSource, uint32 uiEntry,
		float fMaxSearchRange, bool bAlive = true);
