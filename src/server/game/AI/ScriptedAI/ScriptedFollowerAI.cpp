...
FollowerAI::FollowerAI(Creature* pCreature) :
		ScriptedAI(pCreature), m_uiLeaderGUID(0), m_uiUpdateFollowTimer(2500), m_uiFollowState(
				STATE_FOLLOW_NONE), m_pQuestForFollow(NULL) {
}
...
bool FollowerAI::AssistPlayerInCombat(Unit* pWho) {
	...
	if (me->IsWithinDistInMap(pWho, MAX_PLAYER_DISTANCE)
...
void FollowerAI::EnterEvadeMode() {
	...
	if (HasFollowState(STATE_FOLLOW_INPROGRESS)) {
		sLog->outDebug(
				LOG_FILTER_TSCR,
				"TSCR: FollowerAI left combat, returning to CombatStartPosition.");

		if (me->GetMotionMaster()->GetCurrentMovementGeneratorType()
				== TARGETED_MOTION_TYPE) {
			float fPosX, fPosY, fPosZ;
			me->GetPosition(fPosX, fPosY, fPosZ);
			me->GetMotionMaster()->MovePoint(POINT_COMBAT_START, fPosX, fPosY,
					fPosZ);
		}
	} else {
		if (me->GetMotionMaster()->GetCurrentMovementGeneratorType()
		...
		if (m_uiUpdateFollowTimer <= uiDiff) {
			if (HasFollowState(STATE_FOLLOW_COMPLETE)
				...
				sLog->outDebug(LOG_FILTER_TSCR,
						"TSCR: FollowerAI is set completed, despawns.");
				me->ForcedDespawn();
				return;
			}
			...
			if (Player* pPlayer = GetLeaderForFollower()) {
				if (HasFollowState(STATE_FOLLOW_RETURNING)) {
					sLog->outDebug(LOG_FILTER_TSCR,
							"TSCR: FollowerAI is returning to leader.");

					RemoveFollowState(STATE_FOLLOW_RETURNING);
					me->GetMotionMaster()->MoveFollow(pPlayer, PET_FOLLOW_DIST,
							PET_FOLLOW_ANGLE);
					return;
				}

				if (Group* pGroup = pPlayer->GetGroup()) {
					for (GroupReference* pRef = pGroup->GetFirstMember();
			...
			if (bIsMaxRangeExceeded) {
				sLog->outDebug(
						LOG_FILTER_TSCR,
						"TSCR: FollowerAI failed because player/group was to far away or not found");
				me->ForcedDespawn();
				return;
			}
...
void FollowerAI::StartFollow(Player* pLeader, uint32 uiFactionForFollower,
		const Quest* pQuest) {
	if (me->getVictim()) {
		sLog->outDebug(LOG_FILTER_TSCR,
				"TSCR: FollowerAI attempt to StartFollow while in combat.");
		return;
	}

	if (HasFollowState(STATE_FOLLOW_INPROGRESS)) {
		sLog->outError(
				"TSCR: FollowerAI attempt to StartFollow while already following.");
		return;
	}
	...
	if (me->GetMotionMaster()->GetCurrentMovementGeneratorType()
		...
		sLog->outDebug(
				LOG_FILTER_TSCR,
				"TSCR: FollowerAI start with WAYPOINT_MOTION_TYPE, set to MoveIdle.");
	}
	...
	me->GetMotionMaster()->MoveFollow(pLeader, PET_FOLLOW_DIST,
			PET_FOLLOW_ANGLE);

	sLog->outDebug(LOG_FILTER_TSCR,
			"TSCR: FollowerAI start follow %s (GUID " UI64FMTD ")",
			pLeader->GetName(), m_uiLeaderGUID);
}
...
Player* FollowerAI::GetLeaderForFollower() {
			...
			if (Group* pGroup = pLeader->GetGroup()) {
				for (GroupReference* pRef = pGroup->GetFirstMember();
					...
					if (pMember && pMember->isAlive()
						...
						sLog->outDebug(
								LOG_FILTER_TSCR,
								"TSCR: FollowerAI GetLeader changed and returned new leader.");
	...
	sLog->outDebug(LOG_FILTER_TSCR,
			"TSCR: FollowerAI GetLeader can not find suitable leader.");
	return NULL;
}
