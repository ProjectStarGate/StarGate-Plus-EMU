...
struct LootStoreItem {
	...
	LootStoreItem(uint32 _itemid, float _chanceOrQuestChance, uint16 _lootmode,
			uint8 _group, int32 _mincountOrRef, uint8 _maxcount) :
			itemid(_itemid), chance(fabs(_chanceOrQuestChance)), mincountOrRef(
					_mincountOrRef), lootmode(_lootmode), group(_group), needs_quest(
					_chanceOrQuestChance < 0), maxcount(_maxcount) {
	}
...
struct LootItem {
	...
	AllowedLooterSet* GetAllowedLooters() {
		return &allowedGUIDs;
	}
};
...
public:
	...
	void Process(Loot& loot, bool rate, uint16 lootMode,
...
struct Loot {
	...
	Loot(uint32 _gold = 0) :
			gold(_gold), unlootedCount(0), loot_type(LOOT_CORPSE) {
	}
	~Loot() {
		clear();
	}
	...
	bool FillLoot(uint32 lootId, LootStore const& store, Player* lootOwner,
			bool personal, bool noEmptyError = false, uint16 lootMode =
					LOOT_MODE_DEFAULT);
	...
	LootItem* LootItemInSlot(uint32 lootslot, Player* player,
			QuestItem** qitem = NULL, QuestItem** ffaitem = NULL,
...
struct LootView {
	...
	LootView(Loot &_loot, Player *_viewer, PermissionTypes _permission =
			ALL_PERMISSION) :
