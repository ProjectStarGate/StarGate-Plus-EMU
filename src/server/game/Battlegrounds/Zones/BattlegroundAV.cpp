...
void BattlegroundAV::HandleKillUnit(Creature *unit, Player *killer) {
	sLog->outDebug(LOG_FILTER_BATTLEGROUND, "bg_av HandleKillUnit %i",
			unit->GetEntry());
	if (GetStatus() != STATUS_IN_PROGRESS)
		return;
	...
	if (entry == BG_AV_CreatureInfo[AV_NPC_A_BOSS][0]) {
		...
		if (creature)
			YellToAll(creature, GetStarGateString(LANG_BG_AV_A_CAPTAIN_DEAD),
					LANG_UNIVERSAL);
		...
		Creature* creature = GetBGCreature(AV_CPLACE_HERALD);
		if (creature)
			YellToAll(creature, GetStarGateString(LANG_BG_AV_H_CAPTAIN_DEAD),
					LANG_UNIVERSAL);
		DelCreature(AV_CPLACE_TRIGGER18);
	} else if (entry == BG_AV_CreatureInfo[AV_NPC_N_MINE_N_4][0]
			|| entry == BG_AV_CreatureInfo[AV_NPC_N_MINE_A_4][0]
			|| entry == BG_AV_CreatureInfo[AV_NPC_N_MINE_H_4][0])
		ChangeMineOwner(AV_NORTH_MINE, killer->GetTeam());
	else if (entry == BG_AV_CreatureInfo[AV_NPC_S_MINE_N_4][0]
			|| entry == BG_AV_CreatureInfo[AV_NPC_S_MINE_A_4][0]
			|| entry == BG_AV_CreatureInfo[AV_NPC_S_MINE_H_4][0])
...
void BattlegroundAV::HandleQuestComplete(uint32 questid, Player *player) {
	...
	sLog->outDebug(LOG_FILTER_BATTLEGROUND, "BG_AV Quest %i completed",
			questid);
	switch (questid) {
	...
	case AV_QUEST_H_SCRAPS2:
		...
		if (m_Team_QuestStatus[team][0] == 500
				|| m_Team_QuestStatus[team][0] == 1000
				|| m_Team_QuestStatus[team][0] == 1500) //25, 50, 75 turn ins
				{
			sLog->outDebug(LOG_FILTER_BATTLEGROUND,
					"BG_AV Quest %i completed starting with unit upgrading..",
					questid);
			for (BG_AV_Nodes i = BG_AV_NODES_FIRSTAID_STATION;
					i <= BG_AV_NODES_FROSTWOLF_HUT; ++i)
				if (m_Nodes[i].Owner == player->GetTeam()
	...
	case AV_QUEST_H_COMMANDER1:
		...
		if (m_Team_QuestStatus[team][1] == 30)
			sLog->outDebug(
					...
					"BG_AV Quest %i completed (need to implement some events here",
					questid);
		break;
	...
	case AV_QUEST_H_NEAR_MINE:
		...
		if (m_Team_QuestStatus[team][5] == 28) {
			...
			if (m_Team_QuestStatus[team][6] == 7)
				sLog->outDebug(
						LOG_FILTER_BATTLEGROUND,
						"BG_AV Quest %i completed (need to implement some events here - ground assault ready",
						questid);
		}
		break;
	...
	case AV_QUEST_H_RIDER_HIDE:
		...
		if (m_Team_QuestStatus[team][7] == 25) {
			...
			if (m_Team_QuestStatus[team][8] == 25)
				sLog->outDebug(
						LOG_FILTER_BATTLEGROUND,
						"BG_AV Quest %i completed (need to implement some events here - rider assault ready",
						questid);
		}
		break;
	...
	default:
		sLog->outDebug(LOG_FILTER_BATTLEGROUND,
				"BG_AV Quest %i completed but is not interesting at all",
				questid);
...
void BattlegroundAV::UpdateScore(uint16 team, int16 points) { //note: to remove reinforcementpoints points must be negative, for adding reinforcements points must be positive
	ASSERT(team == ALLIANCE || team == HORDE);
	uint8 teamindex = GetTeamIndexByTeamId(team); //0=ally 1=horde
	m_Team_Scores[teamindex] += points;

	UpdateWorldState(
			((teamindex == BG_TEAM_HORDE) ? AV_Horde_Score : AV_Alliance_Score),
			m_Team_Scores[teamindex]);
	if (points < 0) {
		if (m_Team_Scores[teamindex] < 1) {
			m_Team_Scores[teamindex] = 0;
			EndBattleground(((teamindex == BG_TEAM_HORDE) ? ALLIANCE : HORDE));
		} else if (!m_IsInformedNearVictory[teamindex]
				&& m_Team_Scores[teamindex] < SEND_MSG_NEAR_LOSE) {
			SendMessageToAll(
					teamindex == BG_TEAM_HORDE ?
							LANG_BG_AV_H_NEAR_LOSE : LANG_BG_AV_A_NEAR_LOSE,
					teamindex == BG_TEAM_HORDE ?
...
Creature* BattlegroundAV::AddAVCreature(uint16 cinfoid, uint16 type) {
	...
	if (type >= AV_CPLACE_MAX) //static
		...
		creature = AddCreature(BG_AV_StaticCreatureInfo[cinfoid][0],
				(type + AV_CPLACE_MAX), BG_AV_StaticCreatureInfo[cinfoid][1],
				BG_AV_StaticCreaturePos[type][0],
				BG_AV_StaticCreaturePos[type][1],
				BG_AV_StaticCreaturePos[type][2],
				BG_AV_StaticCreaturePos[type][3]);
		level = (BG_AV_StaticCreatureInfo[cinfoid][2]
				== BG_AV_StaticCreatureInfo[cinfoid][3]) ?
				BG_AV_StaticCreatureInfo[cinfoid][2] :
				urand(BG_AV_StaticCreatureInfo[cinfoid][2],
						BG_AV_StaticCreatureInfo[cinfoid][3]);
		isStatic = true;
	} else {
		creature = AddCreature(BG_AV_CreatureInfo[cinfoid][0], type,
				BG_AV_CreatureInfo[cinfoid][1], BG_AV_CreaturePos[type][0],
				BG_AV_CreaturePos[type][1], BG_AV_CreaturePos[type][2],
				BG_AV_CreaturePos[type][3]);
		level = (BG_AV_CreatureInfo[cinfoid][2]
				== BG_AV_CreatureInfo[cinfoid][3]) ?
				BG_AV_CreatureInfo[cinfoid][2] :
				urand(BG_AV_CreatureInfo[cinfoid][2],
						BG_AV_CreatureInfo[cinfoid][3]);
	}
	if (!creature)
		return NULL;
	if (creature->GetEntry() == BG_AV_CreatureInfo[AV_NPC_A_CAPTAIN][0]
			|| creature->GetEntry() == BG_AV_CreatureInfo[AV_NPC_H_CAPTAIN][0])
		creature->SetRespawnDelay(RESPAWN_ONE_DAY); // TODO: look if this can be done by database + also add this for the wingcommanders

	if ((isStatic && cinfoid >= 10 && cinfoid <= 14)
			|| (!isStatic
					&& ((cinfoid >= AV_NPC_A_GRAVEDEFENSE0
		...
		if (!isStatic
				&& ((cinfoid >= AV_NPC_A_GRAVEDEFENSE0
						...
						|| (cinfoid >= AV_NPC_H_GRAVEDEFENSE0
								&& cinfoid <= AV_NPC_H_GRAVEDEFENSE3))) {
			CreatureData &data = sObjectMgr->NewOrExistCreatureData(
	...
	if (creature->GetEntry() == BG_AV_CreatureInfo[AV_NPC_A_CAPTAIN][0]) {
		...
		triggerSpawnID = AV_CPLACE_TRIGGER17;
		newFaction = 84;
	} else if (creature->GetEntry()
			== BG_AV_CreatureInfo[AV_NPC_H_CAPTAIN][0]) {
		triggerSpawnID = AV_CPLACE_TRIGGER18;
		newFaction = 83;
	} else if (creature->GetEntry() == BG_AV_CreatureInfo[AV_NPC_H_BOSS][0]) {
		triggerSpawnID = AV_CPLACE_TRIGGER19;
		...
		for (uint8 i = 0; i <= 1; i++) //0=alliance, 1=horde
			...
			if (m_CaptainBuffTimer[i] > diff)
				...
				if (i == 0) {
					...
					if (creature)
						YellToAll(creature, LANG_BG_AV_A_CAPTAIN_BUFF,
								LANG_COMMON);
				} else {
					...
					if (creature)
						YellToAll(creature, LANG_BG_AV_H_CAPTAIN_BUFF,
								LANG_ORCISH);
				}
...
void BattlegroundAV::StartingEventOpenDoors() {
	...
	for (uint16 i = BG_AV_OBJECT_MINE_SUPPLY_N_MIN;
			i <= BG_AV_OBJECT_MINE_SUPPLY_N_MAX; i++)
		SpawnBGObject(i, RESPAWN_IMMEDIATELY);
	for (uint16 i = BG_AV_OBJECT_MINE_SUPPLY_S_MIN;
			i <= BG_AV_OBJECT_MINE_SUPPLY_S_MAX; i++)
		SpawnBGObject(i, RESPAWN_IMMEDIATELY);
...
void BattlegroundAV::AddPlayer(Player *plr) {
	...
	if (m_MaxLevel == 0)
		m_MaxLevel =
				(plr->getLevel() % 10 == 0) ?
						plr->getLevel() :
						(plr->getLevel() - (plr->getLevel() % 10)) + 10; //TODO: just look at the code \^_^/ --but queue-info should provide this information..
}
...
void BattlegroundAV::EndBattleground(uint32 winner) {
	...
	for (int i = 0; i <= 1; i++) //0=ally 1=horde
		...
		if (rep[i] != 0)
			RewardReputationToTeam((i == 0) ? 730 : 729, rep[i],
					(i == 0) ? ALLIANCE : HORDE);
		if (kills[i] != 0)
			RewardHonorToTeam(GetBonusHonor(kills[i]),
					(i == 0) ? ALLIANCE : HORDE);
	}
...
void BattlegroundAV::UpdatePlayerScore(Player* Source, uint32 type,
		uint32 value, bool doAddHonor) {
	...
	case SCORE_GRAVEYARDS_ASSAULTED:
		...
		Source->GetAchievementMgr().UpdateAchievementCriteria(
...
void BattlegroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node) {
	...
	sLog->outDebug(LOG_FILTER_BATTLEGROUND,
			"bg_av: player destroyed point node %i object %i", node, object);
	...
	if (IsTower(node)) {
		...
		if (m_BgCreatures[AV_CPLACE_A_MARSHAL_SOUTH + tmp])
			...
			sLog->outError(
					"BG_AV: playerdestroyedpoint: marshal %i doesn't exist",
		...
		UpdateScore((owner == ALLIANCE) ? HORDE : ALLIANCE,
				(-1) * BG_AV_RES_TOWER);
		RewardReputationToTeam((owner == ALLIANCE) ? 730 : 729, BG_AV_REP_TOWER,
				owner);
		RewardHonorToTeam(GetBonusHonor(BG_AV_KILL_TOWER), owner);

		SpawnBGObject(
				BG_AV_OBJECT_TAURA_A_DUNBALDAR_SOUTH
						+ GetTeamIndexByTeamId(owner) + (2 * tmp),
				RESPAWN_ONE_DAY);
		SpawnBGObject(
		...
		SpawnBGObject(
				BG_AV_OBJECT_AURA_A_FIRSTAID_STATION
						+ GetTeamIndexByTeamId(owner) + 3 * node,
				RESPAWN_IMMEDIATELY);
		PopulateNode(node);
			...
			for (uint8 i = 0; i < 4; i++) {
				SpawnBGObject(
						((owner == ALLIANCE) ?
								...
								BG_AV_OBJECT_SNOW_EYECANDY_PH) + i,
						RESPAWN_ONE_DAY);
				SpawnBGObject(
						((owner == ALLIANCE) ?
								BG_AV_OBJECT_SNOW_EYECANDY_A :
								BG_AV_OBJECT_SNOW_EYECANDY_H) + i,
						RESPAWN_IMMEDIATELY);
			}
		}
	}
	...
	if (IsTower(node))
		sprintf(
				buf,
				GetStarGateString(LANG_BG_AV_TOWER_TAKEN),
				GetNodeName(node),
				(owner == ALLIANCE) ?
						GetStarGateString(LANG_BG_AV_ALLY) :
						GetStarGateString(LANG_BG_AV_HORDE));
	else
		sprintf(
				buf,
				GetStarGateString(LANG_BG_AV_GRAVE_TAKEN),
				GetNodeName(node),
				(owner == ALLIANCE) ?
						GetStarGateString(LANG_BG_AV_ALLY) :
						GetStarGateString(LANG_BG_AV_HORDE));
...
void BattlegroundAV::ChangeMineOwner(uint8 mine, uint32 team, bool initial) { //mine=0 northmine mine=1 southmin
//changing the owner results in setting respawntim to infinite for current creatures, spawning new mine owners creatures and changing the chest-objects so that the current owning team can use them
	...
	if (team != ALLIANCE && team != HORDE)
		...
		PlaySoundToAll(
				(team == ALLIANCE) ?
						AV_SOUND_ALLIANCE_GOOD : AV_SOUND_HORDE_GOOD);
	...
	if (!initial) {
		sLog->outDebug(LOG_FILTER_BATTLEGROUND,
				"bg_av depopulating mine %i (0=north, 1=south)", mine);
		...
		for (uint16 i = (
				(mine == AV_NORTH_MINE) ?
						AV_CPLACE_MINE_N_1_MIN : AV_CPLACE_MINE_S_1_MIN);
				i
						<= ((mine == AV_NORTH_MINE) ?
								AV_CPLACE_MINE_N_3 : AV_CPLACE_MINE_S_3); i++)
			if (m_BgCreatures[i])
	...
	sLog->outDebug(LOG_FILTER_BATTLEGROUND,
			"bg_av populating mine %i (0=north, 1=south)", mine);
	uint16 miner;
	...
	for (uint16 i = (
			...
			i
					<= ((mine == AV_NORTH_MINE) ?
							AV_CPLACE_MINE_N_1_MAX : AV_CPLACE_MINE_S_1_MAX);
			i++)
		AddAVCreature(miner, i);
	//the next chooses randomly between 2 cretures
	for (uint16 i = (
			(mine == AV_NORTH_MINE) ?
					AV_CPLACE_MINE_N_2_MIN : AV_CPLACE_MINE_S_2_MIN);
			i
					<= ((mine == AV_NORTH_MINE) ?
							AV_CPLACE_MINE_N_2_MAX : AV_CPLACE_MINE_S_2_MAX);
			i++)
		AddAVCreature(miner + (urand(1, 2)), i);
	AddAVCreature(miner + 3,
			(mine == AV_NORTH_MINE) ? AV_CPLACE_MINE_N_3 : AV_CPLACE_MINE_S_3);
...
//    }
	if (team == ALLIANCE || team == HORDE) {
		...
		sprintf(
				buf,
				GetStarGateString(LANG_BG_AV_MINE_TAKEN),
				GetStarGateString(
						(mine == AV_NORTH_MINE) ?
		...
		if (mine == AV_SOUTH_MINE) //i think this gets called all the time
			...
			if (Creature* creature = GetBGCreature(AV_CPLACE_MINE_S_3))
				YellToAll(creature, LANG_BG_AV_S_MINE_BOSS_CLAIMS,
						LANG_UNIVERSAL);
		}
	}
	return;
...
void BattlegroundAV::PopulateNode(BG_AV_Nodes node) {
	...
	if (IsTower(node))
		creatureid =
				(owner == ALLIANCE) ?
						AV_NPC_A_TOWERDEFENSE : AV_NPC_H_TOWERDEFENSE;
	else {
		uint8 team2 = GetTeamIndexByTeamId(owner);
		if (m_Team_QuestStatus[team2][0] < 500)
			creatureid =
					(owner == ALLIANCE) ?
							AV_NPC_A_GRAVEDEFENSE0 : AV_NPC_H_GRAVEDEFENSE0;
		else if (m_Team_QuestStatus[team2][0] < 1000)
			creatureid =
					(owner == ALLIANCE) ?
							AV_NPC_A_GRAVEDEFENSE1 : AV_NPC_H_GRAVEDEFENSE1;
		else if (m_Team_QuestStatus[team2][0] < 1500)
			creatureid =
					(owner == ALLIANCE) ?
							AV_NPC_A_GRAVEDEFENSE2 : AV_NPC_H_GRAVEDEFENSE2;
		else
			creatureid =
					(owner == ALLIANCE) ?
							AV_NPC_A_GRAVEDEFENSE3 : AV_NPC_H_GRAVEDEFENSE3;
		//spiritguide
		if (m_BgCreatures[node])
			DelCreature(node);
		if (!AddSpiritGuide(node, BG_AV_CreaturePos[node][0],
				BG_AV_CreaturePos[node][1], BG_AV_CreaturePos[node][2],
				BG_AV_CreaturePos[node][3], owner))
	...
	if (!trigger)
		trigger = AddCreature(WORLD_TRIGGER, node + 302, owner,
...
BG_AV_Nodes BattlegroundAV::GetNodeThroughObject(uint32 object) {
	sLog->outDebug(LOG_FILTER_BATTLEGROUND, "bg_AV getnodethroughobject %i",
			object);
...
uint32 BattlegroundAV::GetObjectThroughNode(BG_AV_Nodes node) { //this function is the counterpart to GetNodeThroughObject()
	sLog->outDebug(LOG_FILTER_BATTLEGROUND, "bg_AV GetObjectThroughNode %i",
			node);
	...
	sLog->outError(
			"BattlegroundAV: Error! GetPlaceNode couldn't resolve node %i",
			node);
	ASSERT(false);
	return 0;
}
...
void BattlegroundAV::EventPlayerClickedOnFlag(Player *source,
	...
	sLog->outDebug(LOG_FILTER_BATTLEGROUND,
			"BG_AV using gameobject %i with type %i", target_obj->GetEntry(),
			object);
	if (object < 0)
		return;
...
void BattlegroundAV::EventPlayerDefendsPoint(Player* player, uint32 object) {
	...
	if (owner == player->GetTeam() || m_Nodes[node].State != POINT_ASSAULTED)
		return;
	if (m_Nodes[node].TotalOwner == AV_NEUTRAL_TEAM) { //until snowfall doesn't belong to anyone it is better handled in assault-code
		ASSERT(node == BG_AV_NODES_SNOWFALL_GRAVE); //currently the only neutral grave
		EventPlayerAssaultsPoint(player, object);
		return;
	}
	sLog->outDebug(LOG_FILTER_BATTLEGROUND,
			"player defends point object: %i node: %i", object, node);
	if (m_Nodes[node].PrevOwner != team) {
		sLog->outError(
				"BG_AV: player defends point which doesn't belong to his team %i",
				node);
		return;
	}
	...
	if (!IsTower(node)) {
		...
		SpawnBGObject(
				BG_AV_OBJECT_AURA_A_FIRSTAID_STATION
						+ GetTeamIndexByTeamId(team) + 3 * node,
				RESPAWN_IMMEDIATELY);
	}
	// despawn old go
	SpawnBGObject(object, RESPAWN_ONE_DAY);
	...
	if (IsTower(node)) {
		...
		SpawnBGObject(
				...
				(team == ALLIANCE) ? RESPAWN_IMMEDIATELY : RESPAWN_ONE_DAY);
		SpawnBGObject(
				BG_AV_OBJECT_TAURA_H_DUNBALDAR_SOUTH
						+ (2 * (node - BG_AV_NODES_DUNBALDAR_SOUTH)),
				(team == HORDE) ? RESPAWN_IMMEDIATELY : RESPAWN_ONE_DAY);
		SpawnBGObject(
				BG_AV_OBJECT_TFLAG_A_DUNBALDAR_SOUTH
						+ (2 * (node - BG_AV_NODES_DUNBALDAR_SOUTH)),
				(team == ALLIANCE) ? RESPAWN_IMMEDIATELY : RESPAWN_ONE_DAY);
	...
	UpdatePlayerScore(player,
			(IsTower(node)) ? SCORE_TOWERS_DEFENDED : SCORE_GRAVEYARDS_DEFENDED,
			1);
	if (IsTower(node))
...
void BattlegroundAV::EventPlayerAssaultsPoint(Player* player, uint32 object) {
	...
	uint32 team = player->GetTeam();
	sLog->outDebug(LOG_FILTER_BATTLEGROUND,
			"bg_av: player assaults point object %i node %i", object, node);
	...
	if (m_Nodes[node].TotalOwner != AV_NEUTRAL_TEAM) {
		...
		if (team == ALLIANCE)
			...
			SpawnBGObject(object + 22, RESPAWN_IMMEDIATELY);
		if (IsTower(node)) { //spawning/despawning of bigflag+aura
			SpawnBGObject(
					BG_AV_OBJECT_TAURA_A_DUNBALDAR_SOUTH
							+ (2 * (node - BG_AV_NODES_DUNBALDAR_SOUTH)),
					(team == ALLIANCE) ? RESPAWN_IMMEDIATELY : RESPAWN_ONE_DAY);
			...
			if (!ghost_list.empty()) {
				...
				for (std::vector<uint64>::iterator itr = ghost_list.begin();
						itr != ghost_list.end(); ++itr) {
					...
					if (!ClosestGrave)
						...
						plr->TeleportTo(GetMapId(), ClosestGrave->x,
								ClosestGrave->y, ClosestGrave->z,
								plr->GetOrientation());
				}
	...
	sprintf(
			buf,
			(IsTower(node)) ?
					GetStarGateString(LANG_BG_AV_TOWER_ASSAULTED) :
					GetStarGateString(LANG_BG_AV_GRAVE_ASSAULTED),
			GetNodeName(node),
			(team == ALLIANCE) ?
					GetStarGateString(LANG_BG_AV_ALLY) :
					GetStarGateString(LANG_BG_AV_HORDE));
	...
	UpdatePlayerScore(
			player,
			(IsTower(node)) ?
					SCORE_TOWERS_ASSAULTED : SCORE_GRAVEYARDS_ASSAULTED, 1);
	PlaySoundToAll(
			(team == ALLIANCE) ?
					AV_SOUND_ALLIANCE_ASSAULTS : AV_SOUND_HORDE_ASSAULTS);
}
...
void BattlegroundAV::FillInitialWorldStates(WorldPacket& data) {
	...
	for (uint8 i = BG_AV_NODES_FIRSTAID_STATION; i <= BG_AV_NODES_FROSTWOLF_HUT;
		...
		for (uint8 j = 1; j <= 3; j += 2) { //j=1=assaulted j=3=controled
			...
			data
					<< uint32(
							BG_AV_NodeWorldStates[i][GetWorldStateType(j,
									ALLIANCE)])
					<< uint32((m_Nodes[i].Owner == ALLIANCE && stateok) ? 1 : 0);
			data
					<< uint32(
							BG_AV_NodeWorldStates[i][GetWorldStateType(j, HORDE)])
					<< uint32((m_Nodes[i].Owner == HORDE && stateok) ? 1 : 0);
		}
	}
...
uint8 BattlegroundAV::GetWorldStateType(uint8 state, uint16 team) //this is used for node worldstates and returns values which fit good into the worldstatesarray
	...
	sLog->outError("BG_AV: should update a strange worldstate state:%i team:%i",
			state, team);
...
void BattlegroundAV::UpdateNodeWorldState(BG_AV_Nodes node) {
	UpdateWorldState(
			BG_AV_NodeWorldStates[node][GetWorldStateType(m_Nodes[node].State,
					m_Nodes[node].Owner)], 1);
	if (m_Nodes[node].PrevOwner == AV_NEUTRAL_TEAM) //currently only snowfall is supported as neutral node (i don't want to make an extra row (neutral states) in worldstatesarray just for one node
		...
		UpdateWorldState(
				BG_AV_NodeWorldStates[node][GetWorldStateType(
						m_Nodes[node].PrevState, m_Nodes[node].PrevOwner)], 0);
}
...
WorldSafeLocsEntry const* BattlegroundAV::GetClosestGraveYard(Player* player) {
	...
	pGraveyard = sWorldSafeLocsStore.LookupEntry(
			BG_AV_GraveyardIds[GetTeamIndexByTeamId(player->GetTeam()) + 7]);
...
bool BattlegroundAV::SetupBattleground() {
	...
	!AddObject(BG_AV_OBJECT_DOOR_A, BG_AV_OBJECTID_GATE_A,
			BG_AV_DoorPositons[0][0], BG_AV_DoorPositons[0][1],
			BG_AV_DoorPositons[0][2], BG_AV_DoorPositons[0][3], 0, 0,
			sin(BG_AV_DoorPositons[0][3] / 2),
			cos(BG_AV_DoorPositons[0][3] / 2), RESPAWN_IMMEDIATELY)
	// horde gates
			|| !AddObject(BG_AV_OBJECT_DOOR_H, BG_AV_OBJECTID_GATE_H,
					BG_AV_DoorPositons[1][0], BG_AV_DoorPositons[1][1],
					BG_AV_DoorPositons[1][2], BG_AV_DoorPositons[1][3], 0, 0,
					sin(BG_AV_DoorPositons[1][3] / 2),
					cos(BG_AV_DoorPositons[1][3] / 2), RESPAWN_IMMEDIATELY)) {
		sLog->outErrorDb(
	...
	for (uint8 i = BG_AV_NODES_FIRSTAID_STATION; i < BG_AV_NODES_MAX; ++i) {
		if (i <= BG_AV_NODES_FROSTWOLF_HUT) {
			if (!AddObject(i, BG_AV_OBJECTID_BANNER_A_B, BG_AV_ObjectPos[i][0],
					BG_AV_ObjectPos[i][1], BG_AV_ObjectPos[i][2],
					BG_AV_ObjectPos[i][3], 0, 0, sin(BG_AV_ObjectPos[i][3] / 2),
					cos(BG_AV_ObjectPos[i][3] / 2), RESPAWN_ONE_DAY)
					|| !AddObject(i + 11, BG_AV_OBJECTID_BANNER_CONT_A_B,
							BG_AV_ObjectPos[i][0], BG_AV_ObjectPos[i][1],
							BG_AV_ObjectPos[i][2], BG_AV_ObjectPos[i][3], 0, 0,
							sin(BG_AV_ObjectPos[i][3] / 2),
							cos(BG_AV_ObjectPos[i][3] / 2), RESPAWN_ONE_DAY)
			...
			//aura
					|| !AddObject(BG_AV_OBJECT_AURA_N_FIRSTAID_STATION + i * 3,
							BG_AV_OBJECTID_AURA_N, BG_AV_ObjectPos[i][0],
							BG_AV_ObjectPos[i][1], BG_AV_ObjectPos[i][2],
							BG_AV_ObjectPos[i][3], 0, 0,
							sin(BG_AV_ObjectPos[i][3] / 2),
							cos(BG_AV_ObjectPos[i][3] / 2), RESPAWN_ONE_DAY)
					|| !AddObject(BG_AV_OBJECT_AURA_A_FIRSTAID_STATION + i * 3,
							BG_AV_OBJECTID_AURA_A, BG_AV_ObjectPos[i][0],
							BG_AV_ObjectPos[i][1], BG_AV_ObjectPos[i][2],
							BG_AV_ObjectPos[i][3], 0, 0,
							sin(BG_AV_ObjectPos[i][3] / 2),
							cos(BG_AV_ObjectPos[i][3] / 2), RESPAWN_ONE_DAY)
					|| !AddObject(BG_AV_OBJECT_AURA_H_FIRSTAID_STATION + i * 3,
							BG_AV_OBJECTID_AURA_H, BG_AV_ObjectPos[i][0],
							BG_AV_ObjectPos[i][1], BG_AV_ObjectPos[i][2],
							BG_AV_ObjectPos[i][3], 0, 0,
							sin(BG_AV_ObjectPos[i][3] / 2),
							cos(BG_AV_ObjectPos[i][3] / 2), RESPAWN_ONE_DAY)) {
			...
			if (i <= BG_AV_NODES_STONEHEART_BUNKER) //alliance towers
				...
				if (!AddObject(i, BG_AV_OBJECTID_BANNER_A,
						...
						|| !AddObject(i + 22, BG_AV_OBJECTID_BANNER_CONT_H,
								BG_AV_ObjectPos[i][0], BG_AV_ObjectPos[i][1],
								BG_AV_ObjectPos[i][2], BG_AV_ObjectPos[i][3], 0,
								0, sin(BG_AV_ObjectPos[i][3] / 2),
								cos(BG_AV_ObjectPos[i][3] / 2), RESPAWN_ONE_DAY)
						|| !AddObject(
								BG_AV_OBJECT_TAURA_A_DUNBALDAR_SOUTH
								...
								BG_AV_ObjectPos[i + 8][3], 0, 0,
								sin(BG_AV_ObjectPos[i + 8][3] / 2),
		...
		for (uint8 j = 0; j <= 9; j++) {
			if (j < 5) {
				if (!AddObject(
						...
						0,
						sin(
								BG_AV_ObjectPos[AV_OPLACE_BURN_BUILDING_A
										+ (i * 10) + j][3] / 2),
						cos(
								BG_AV_ObjectPos[AV_OPLACE_BURN_BUILDING_A
										+ (i * 10) + j][3] / 2),
	...
	for (uint16 i = 0;
		...
		if (!AddObject(BG_AV_OBJECT_MINE_SUPPLY_N_MIN + i,
				...
				BG_AV_ObjectPos[AV_OPLACE_MINE_SUPPLY_N_MIN + i][3], 0, 0,
				sin(BG_AV_ObjectPos[AV_OPLACE_MINE_SUPPLY_N_MIN + i][3] / 2),
	...
	for (uint16 i = 0;
		...
		if (!AddObject(BG_AV_OBJECT_MINE_SUPPLY_S_MIN + i,
				...
				BG_AV_ObjectPos[AV_OPLACE_MINE_SUPPLY_S_MIN + i][3], 0, 0,
				sin(BG_AV_ObjectPos[AV_OPLACE_MINE_SUPPLY_S_MIN + i][3] / 2),
	...
	if (!AddObject(BG_AV_OBJECT_FLAG_N_SNOWFALL_GRAVE,
			...
			BG_AV_ObjectPos[BG_AV_NODES_SNOWFALL_GRAVE][3], 0, 0,
			sin(BG_AV_ObjectPos[BG_AV_NODES_SNOWFALL_GRAVE][3] / 2),
	...
	for (uint8 i = 0; i < 4; i++) {
		if (!AddObject(BG_AV_OBJECT_SNOW_EYECANDY_A + i,
				...
				BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i][3], 0, 0,
				sin(BG_AV_ObjectPos[AV_OPLACE_SNOW_1 + i][3] / 2),
	...
	for (i = BG_AV_OBJECT_TFLAG_H_ICEBLOOD_TOWER;
			i <= BG_AV_OBJECT_TFLAG_H_FROSTWOLF_WTOWER; i += 2) {
		SpawnBGObject(i, RESPAWN_IMMEDIATELY); //flag
		SpawnBGObject(i + 16, RESPAWN_IMMEDIATELY); //aura
	}
	...
	AddSpiritGuide(7, BG_AV_CreaturePos[7][0], BG_AV_CreaturePos[7][1],
			BG_AV_CreaturePos[7][2], BG_AV_CreaturePos[7][3], ALLIANCE);
	AddSpiritGuide(8, BG_AV_CreaturePos[8][0], BG_AV_CreaturePos[8][1],
			BG_AV_CreaturePos[8][2], BG_AV_CreaturePos[8][3], HORDE);
...
const char* BattlegroundAV::GetNodeName(BG_AV_Nodes node) {
	...
	case BG_AV_NODES_FIRSTAID_STATION:
		return GetStarGateString(LANG_BG_AV_NODE_GRAVE_STORM_AID);
	case BG_AV_NODES_DUNBALDAR_SOUTH:
		return GetStarGateString(LANG_BG_AV_NODE_TOWER_DUN_S);
	case BG_AV_NODES_DUNBALDAR_NORTH:
		return GetStarGateString(LANG_BG_AV_NODE_TOWER_DUN_N);
	case BG_AV_NODES_STORMPIKE_GRAVE:
		return GetStarGateString(LANG_BG_AV_NODE_GRAVE_STORMPIKE);
	case BG_AV_NODES_ICEWING_BUNKER:
		return GetStarGateString(LANG_BG_AV_NODE_TOWER_ICEWING);
	case BG_AV_NODES_STONEHEART_GRAVE:
		return GetStarGateString(LANG_BG_AV_NODE_GRAVE_STONE);
	case BG_AV_NODES_STONEHEART_BUNKER:
		return GetStarGateString(LANG_BG_AV_NODE_TOWER_STONE);
	case BG_AV_NODES_SNOWFALL_GRAVE:
		return GetStarGateString(LANG_BG_AV_NODE_GRAVE_SNOW);
	case BG_AV_NODES_ICEBLOOD_TOWER:
		return GetStarGateString(LANG_BG_AV_NODE_TOWER_ICE);
	case BG_AV_NODES_ICEBLOOD_GRAVE:
		return GetStarGateString(LANG_BG_AV_NODE_GRAVE_ICE);
	case BG_AV_NODES_TOWER_POINT:
		return GetStarGateString(LANG_BG_AV_NODE_TOWER_POINT);
	case BG_AV_NODES_FROSTWOLF_GRAVE:
		return GetStarGateString(LANG_BG_AV_NODE_GRAVE_FROST);
	case BG_AV_NODES_FROSTWOLF_ETOWER:
		return GetStarGateString(LANG_BG_AV_NODE_TOWER_FROST_E);
	case BG_AV_NODES_FROSTWOLF_WTOWER:
		return GetStarGateString(LANG_BG_AV_NODE_TOWER_FROST_W);
	case BG_AV_NODES_FROSTWOLF_HUT:
		return GetStarGateString(LANG_BG_AV_NODE_GRAVE_FROST_HUT);
	default:
...
void BattlegroundAV::AssaultNode(BG_AV_Nodes node, uint16 team) {
	...
	m_Nodes[node].Timer =
			(m_Nodes[node].PrevOwner) ? BG_AV_CAPTIME : BG_AV_SNOWFALL_FIRSTCAP;
