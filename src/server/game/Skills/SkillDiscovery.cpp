...
void LoadSkillDiscoveryTable() {
	...
	do {
		...
		if (chance <= 0) // chance
			...
			ssNonDiscoverableEntries << "spellId = " << spellId
					...
					<< " reqSkillValue = " << reqSkillValue << " chance = "
		...
		if (reqSkillOrSpell > 0) // spell case
				{
			SpellEntry const* reqSpellEntry = sSpellStore.LookupEntry(
					reqSkillOrSpell);
			if (!reqSpellEntry) {
				if (reportedReqSpells.find(reqSkillOrSpell)
						== reportedReqSpells.end()) {
					sLog->outErrorDb(
							"Spell (ID: %u) have not existed spell (ID: %i) in `reqSpell` field in `skill_discovery_template` table",
			...
			if (bounds.first == bounds.second) {
				sLog->outErrorDb(
						"Spell (ID: %u) not listed in `SkillLineAbility.dbc` but listed with `reqSpell`=0 in `skill_discovery_template` table",
						spellId);
				continue;
			}
			...
			sLog->outErrorDb(
					"Spell (ID: %u) have negative value in `reqSpell` field in `skill_discovery_template` table",
					spellId);
			continue;
		}
	...
	for (uint32 spell_id = 1; spell_id < sSpellStore.GetNumRows(); ++spell_id) {
		...
		if (SkillDiscoveryStore.find(spell_id) == SkillDiscoveryStore.end())
			sLog->outErrorDb(
					"Spell (ID: %u) is 100%% chance random discovery ability but not have data in `skill_discovery_template` table",
					spell_id);
	}

	sLog->outString(">> Loaded %u skill discovery definitions in %u ms", count,
...
uint32 GetExplicitDiscoverySpell(uint32 spellId, Player* player) {
	...
	uint32 skillvalue =
			bounds.first != bounds.second ?
					player->GetSkillValue(bounds.first->second->skillId) : 0;

	float full_chance = 0;
	for (SkillDiscoveryList::const_iterator item_iter = tab->second.begin();
			item_iter != tab->second.end(); ++item_iter)
...
uint32 GetSkillDiscoverySpell(uint32 skillId, uint32 spellId, Player* player) {
	...
	if (tab != SkillDiscoveryStore.end()) {
		for (SkillDiscoveryList::const_iterator item_iter = tab->second.begin();
				item_iter != tab->second.end(); ++item_iter) {
			if (roll_chance_f(
					item_iter->chance * sWorld->getRate(RATE_SKILL_DISCOVERY))
