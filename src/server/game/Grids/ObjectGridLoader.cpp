...
void ObjectGridRespawnMover::Move(GridType &grid) {
	TypeContainerVisitor<ObjectGridRespawnMover, GridTypeMapContainer> mover(
			*this);
	grid.Visit(mover);
}
...
public:
	explicit ObjectWorldLoader(ObjectGridLoader& gloader) :
			i_cell(gloader.i_cell), i_grid(gloader.i_grid), i_map(
					gloader.i_map), i_corpses(0) {
	}
...
template<class T>
void AddObjectHelper(CellPair &cell, GridRefManager<T> &m, uint32 &count,
		Map* map, T *obj) {
...
void LoadHelper(CellCorpseSet const& cell_corpses, CellPair &cell,
		CorpseMapType &m, uint32 &count, Map* map) {
	if (cell_corpses.empty())
		return;

	for (CellCorpseSet::const_iterator itr = cell_corpses.begin();
			itr != cell_corpses.end(); ++itr) {
...
void ObjectGridLoader::Visit(GameObjectMapType &m) {
	...
	CellObjectGuids const& cell_guids = sObjectMgr->GetCellObjectGuids(
			i_map->GetId(), i_map->GetSpawnMode(), cell_id);
...
void ObjectGridLoader::Load(GridType &grid) {
...

	{
		...
		TypeContainerVisitor<ObjectWorldLoader, WorldTypeMapContainer> loader(
				wloader);
		grid.Visit(loader);
...
void ObjectGridLoader::LoadN(void) {
	...
	sLog->outDebug(
			...
			i_gameObjects, i_creatures, i_corpses, i_grid.GetGridId(),
			i_map->GetId());
}
...
void ObjectGridUnloader::Unload(GridType &grid) {
	TypeContainerVisitor<ObjectGridUnloader, GridTypeMapContainer> unloader(
			*this);
	grid.Visit(unloader);
}
