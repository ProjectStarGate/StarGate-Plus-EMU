...
void VisibleNotifier::SendToSelf() {
	...
	for (Player::ClientGUIDs::const_iterator it = vis_guids.begin();
			it != vis_guids.end(); ++it) {
		...
		if (IS_PLAYER_GUID(*it)) {
			...
			if (plr && plr->IsInWorld()
					&& !plr->isNeedNotify(NOTIFY_VISIBILITY_CHANGED))
	...
	for (std::set<Unit*>::const_iterator it = i_visibleNow.begin();
			it != i_visibleNow.end(); ++it)
...
inline void CreatureUnitRelocationWorker(Creature* c, Unit* u) {
	...
	if (c->HasReactState(REACT_AGGRESSIVE)
			&& !c->HasUnitState(UNIT_STAT_SIGHTLESS))
...
void DelayedUnitRelocation::Visit(CreatureMapType &m) {
	for (CreatureMapType::iterator iter = m.begin(); iter != m.end(); ++iter) {
		...
		TypeContainerVisitor<CreatureRelocationNotifier, WorldTypeMapContainer> c2world_relocation(
				relocate);
		TypeContainerVisitor<CreatureRelocationNotifier, GridTypeMapContainer> c2grid_relocation(
				relocate);
...
void DelayedUnitRelocation::Visit(PlayerMapType &m) {
	for (PlayerMapType::iterator iter = m.begin(); iter != m.end(); ++iter) {
		...
		CellPair pair2(
				Trinity::ComputeCellPair(viewPoint->GetPositionX(),
						viewPoint->GetPositionY()));
		Cell cell2(pair2);
		...
		TypeContainerVisitor<PlayerRelocationNotifier, WorldTypeMapContainer> c2world_relocation(
				relocate);
		TypeContainerVisitor<PlayerRelocationNotifier, GridTypeMapContainer> c2grid_relocation(
				relocate);
