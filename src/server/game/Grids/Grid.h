...
public:
	 ...
	 */
	template<class SPECIFIC_OBJECT> void RemoveWorldObject(
			SPECIFIC_OBJECT *obj) {
	...
	template<class T> void Visit(
			TypeContainerVisitor<T, TypeMapContainer<GRID_OBJECT_TYPES> > &visitor) {
	...
	unsigned int ActiveObjectsInGrid(void) const {
		return /*m_activeGridObjects.size()+*/i_objects.template Count<
				ACTIVE_OBJECT>();
	}
	 ...
	 */
	template<class SPECIFIC_OBJECT> void RemoveGridObject(
			SPECIFIC_OBJECT *obj) {
