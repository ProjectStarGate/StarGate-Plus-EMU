...
bool GameEventMgr::CheckOneGameEvent(uint16 entry) const {
	...
	case GAMEEVENT_WORLD_INACTIVE: {
		...
		for (std::set<uint16>::const_iterator itr =
				...
				itr != mGameEvent[entry].prerequisite_events.end(); ++itr) {
			if ((mGameEvent[*itr].state != GAMEEVENT_WORLD_NEXTPHASE
					&& mGameEvent[*itr].state != GAMEEVENT_WORLD_FINISHED) || // if prereq not in nextphase or finished state, then can't start this one
	...
	if (mGameEvent[event_id].state == GAMEEVENT_NORMAL
		...
		if (overwrite) {
			...
			if (mGameEvent[event_id].end <= mGameEvent[event_id].start)
				mGameEvent[event_id].end = mGameEvent[event_id].start
						+ mGameEvent[event_id].length;
		}
		return false;
	} else {
...
void GameEventMgr::StopEvent(uint16 event_id, bool overwrite) {
	...
	if (overwrite && !serverwide_evt) {
		mGameEvent[event_id].start = time(NULL)
				- mGameEvent[event_id].length * MINUTE;
		if (mGameEvent[event_id].end <= mGameEvent[event_id].start)
			mGameEvent[event_id].end = mGameEvent[event_id].start
					+ mGameEvent[event_id].length;
		...
		if (overwrite
				|| mGameEvent[event_id].state != GAMEEVENT_WORLD_FINISHED) {
			// reset conditions
			...
			for (itr = mGameEvent[event_id].conditions.begin();
					itr != mGameEvent[event_id].conditions.end(); ++itr)
				itr->second.done = 0;
			SQLTransaction trans = CharacterDatabase.BeginTransaction();
			trans->PAppend("DELETE FROM game_event_save WHERE event_id = '%u'",
					event_id);
			trans->PAppend(
					"DELETE FROM game_event_condition_save WHERE event_id = '%u'",
					event_id);
...
void GameEventMgr::LoadFromDB() {
	...
	if (!result) {
		sLog->outString(
				">> Loaded 0 weather definitions. DB table `game_event` is empty.");
		sLog->outString();
		return;
	}
	...
	result =
			WorldDatabase.Query(
					"SELECT entry, UNIX_TIMESTAMP(start_time), UNIX_TIMESTAMP(end_time), occurence, length, holiday, description, world_event FROM game_event");
	if (!result) {
		mGameEvent.clear();
		sLog->outErrorDb(
	...
	do {
		...
		if (event_id == 0) {
			sLog->outErrorDb(
					"`game_event` game event id (%i) is reserved and can't be used.",
					event_id);
			continue;
		}
		...
		if (pGameEvent.length == 0 && pGameEvent.state == GAMEEVENT_NORMAL) // length>0 is validity check
				{
			sLog->outErrorDb(
					"`game_event` game event id (%i) isn't a world event and has length = 0, thus it can't be used.",
					event_id);
			continue;
		}
		...
		if (pGameEvent.holiday_id != HOLIDAY_NONE) {
			if (!sHolidaysStore.LookupEntry(pGameEvent.holiday_id)) {
				sLog->outErrorDb(
						"`game_event` game event id (%i) have not existed holiday id %u.",
						event_id, pGameEvent.holiday_id);
	...
	sLog->outString(">> Loaded %u game events in %u ms", count,
	...
	result = CharacterDatabase.Query(
			"SELECT event_id, state, next_start FROM game_event_save");
	...
	if (!result) {
		...
		do {
			...
			if (event_id >= mGameEvent.size()) {
				sLog->outErrorDb(
						"`game_event_save` game event id (%i) is out of range compared to max event id in `game_event`",
						event_id);
				continue;
			}
			...
			if (mGameEvent[event_id].state != GAMEEVENT_NORMAL
				...
				mGameEvent[event_id].state =
						(GameEventState) (fields[1].GetUInt8());
				mGameEvent[event_id].nextstart = time_t(fields[2].GetUInt32());
			} else {
				sLog->outErrorDb(
						"game_event_save includes event save for non-worldevent id %u",
						event_id);
				continue;
			}
		...
		sLog->outString(">> Loaded %u game event saves in game events in %u ms",
	...
	result = WorldDatabase.Query(
			"SELECT event_id, prerequisite_event FROM game_event_prerequisite");
	if (!result) {
		sLog->outString(
		...
		do {
			...
			if (event_id >= mGameEvent.size()) {
				sLog->outErrorDb(
						"`game_event_prerequisite` game event id (%i) is out of range compared to max event id in `game_event`",
						event_id);
				continue;
			}
			...
			if (mGameEvent[event_id].state != GAMEEVENT_NORMAL
				...
				if (prerequisite_event >= mGameEvent.size()) {
					sLog->outErrorDb(
							"`game_event_prerequisite` game event prerequisite id (%i) is out of range compared to max event id in `game_event`",
							prerequisite_event);
					continue;
				}
				...
				sLog->outErrorDb(
						"game_event_prerequisiste includes event entry for non-worldevent id %u",
						event_id);
				continue;
			}
		...
		sLog->outString(
				">> Loaded %u game event prerequisites in game events in %u ms",
				count, GetMSTimeDiffToNow(oldMSTime));
		sLog->outString();
	}
	...
	if (!result) {
		...
		do {
			...
			if (internal_event_id < 0
				...
				sLog->outErrorDb(
						"`game_event_creature` game event id (%i) is out of range compared to max event id in `game_event`",
						event_id);
				continue;
			}
		...
		sLog->outString(">> Loaded %u creatures in game events in %u ms", count,
				GetMSTimeDiffToNow(oldMSTime));
		sLog->outString();
	}
	...
	if (!result) {
		...
		do {
			...
			if (internal_event_id < 0
				...
				sLog->outErrorDb(
						"`game_event_gameobject` game event id (%i) is out of range compared to max event id in `game_event`",
						event_id);
				continue;
			}
		...
		sLog->outString(">> Loaded %u gameobjects in game events in %u ms",
				count, GetMSTimeDiffToNow(oldMSTime));
		sLog->outString();
	}
	...
	result =
			WorldDatabase.Query(
					"SELECT creature.guid, game_event_model_equip.event, game_event_model_equip.modelid, "
					//   3
	...
	if (!result) {
		...
		do {
			...
			if (event_id >= mGameEventModelEquip.size()) {
				sLog->outErrorDb(
						"`game_event_model_equip` game event id (%u) is out of range compared to max event id in `game_event`",
						event_id);
				continue;
			}
			...
			if (newModelEquipSet.equipment_id > 0) {
				if (!sObjectMgr->GetEquipmentInfo(
						newModelEquipSet.equipment_id)) {
					sLog->outErrorDb(
							"Table `game_event_model_equip` have creature (Guid: %u) with equipment_id %u not found in table `creature_equip_template`, set to no equipment.",
			...
			equiplist.push_back(
					std::pair<uint32, ModelEquip>(guid, newModelEquipSet));
		} while (result->NextRow());

		sLog->outString(
				">> Loaded %u model/equipment changes in game events in %u ms",
				count, GetMSTimeDiffToNow(oldMSTime));
		sLog->outString();
	}
	...
	//                                   0   1      2
	result = WorldDatabase.Query(
			"SELECT id, quest, event FROM game_event_creature_quest");
	...
	if (!result) {
		...
		do {
			...
			if (event_id >= mGameEventCreatureQuests.size()) {
				sLog->outErrorDb(
						"`game_event_creature_quest` game event id (%u) is out of range compared to max event id in `game_event`",
						event_id);
				continue;
			}
		...
		sLog->outString(">> Loaded %u quests additions in game events in %u ms",
				count, GetMSTimeDiffToNow(oldMSTime));
		sLog->outString();
	}
	...
	//                                   0   1      2
	result = WorldDatabase.Query(
			"SELECT id, quest, event FROM game_event_gameobject_quest");
	...
	if (!result) {
		...
		do {
			...
			if (event_id >= mGameEventGameObjectQuests.size()) {
				sLog->outErrorDb(
						"`game_event_gameobject_quest` game event id (%u) is out of range compared to max event id in `game_event`",
						event_id);
				continue;
			}
	...
	result =
			WorldDatabase.Query(
					"SELECT quest, event_id, condition_id, num FROM game_event_quest_condition");
	...
	if (!result) {
		...
		do {
			...
			if (event_id >= mGameEvent.size()) {
				sLog->outErrorDb(
						"`game_event_quest_condition` game event id (%u) is out of range compared to max event id in `game_event`",
						event_id);
				continue;
			}
		...
		sLog->outString(
				">> Loaded %u quest event conditions in game events in %u ms",
				count, GetMSTimeDiffToNow(oldMSTime));
		sLog->outString();
	}
	...
	result =
			WorldDatabase.Query(
					"SELECT event_id, condition_id, req_num, max_world_state_field, done_world_state_field FROM game_event_condition");
	...
	if (!result) {
		...
		do {
			...
			if (event_id >= mGameEvent.size()) {
				sLog->outErrorDb(
						"`game_event_condition` game event id (%u) is out of range compared to max event id in `game_event`",
						event_id);
				continue;
			}
		...
		sLog->outString(">> Loaded %u conditions in game events in %u ms",
				count, GetMSTimeDiffToNow(oldMSTime));
		sLog->outString();
	}
	...
	result =
			CharacterDatabase.Query(
					"SELECT event_id, condition_id, done FROM game_event_condition_save");
	...
	if (!result) {
		...
		do {
			...
			if (event_id >= mGameEvent.size()) {
				sLog->outErrorDb(
						"`game_event_condition_save` game event id (%u) is out of range compared to max event id in `game_event`",
						event_id);
				continue;
			}
			...
			if (itr != mGameEvent[event_id].conditions.end()) {
				...
				sLog->outErrorDb(
						"game_event_condition_save contains not present condition evt id %u cond id %u",
						event_id, condition);
				continue;
			}
		...
		sLog->outString(">> Loaded %u condition saves in game events in %u ms",
				count, GetMSTimeDiffToNow(oldMSTime));
		sLog->outString();
	}
	...
	//                                   0         1        2
	result = WorldDatabase.Query(
			"SELECT guid, event_id, npcflag FROM game_event_npcflag");
	...
	if (!result) {
		...
		do {
			...
			if (event_id >= mGameEvent.size()) {
				sLog->outErrorDb(
						"`game_event_npcflag` game event id (%u) is out of range compared to max event id in `game_event`",
						event_id);
				continue;
			}
		...
		sLog->outString(">> Loaded %u npcflags in game events in %u ms", count,
				GetMSTimeDiffToNow(oldMSTime));
		sLog->outString();
	}
	...
	result =
			WorldDatabase.Query(
					"SELECT event, guid, item, maxcount, incrtime, ExtendedCost FROM game_event_npc_vendor ORDER BY guid, slot ASC");
	...
	if (!result) {
		...
		do {
			...
			if (event_id >= mGameEventVendors.size()) {
				sLog->outErrorDb(
						"`game_event_npc_vendor` game event id (%u) is out of range compared to max event id in `game_event`",
						event_id);
				continue;
			}
			...
			if (!sObjectMgr->IsVendorItemValid(newEntry.entry, newEntry.item,
					newEntry.maxcount, newEntry.incrtime, newEntry.ExtendedCost,
					NULL, NULL, event_npc_flag))
				continue;
			++count;
			vendors.push_back(newEntry);
		} while (result->NextRow());

		sLog->outString(">> Loaded %u vendor additions in game events in %u ms",
				count, GetMSTimeDiffToNow(oldMSTime));
		sLog->outString();
	}
	...
	//                                   0     1
	result = WorldDatabase.Query(
			"SELECT event, bgflag FROM game_event_battleground_holiday");

	if (!result) {
		sLog->outString(
				">> Loaded 0 battleground holidays in game events. DB table `game_event_condition` is empty.");
		sLog->outString();
	} else {
		count = 0;
		do {
			...
			if (event_id >= mGameEvent.size()) {
				sLog->outErrorDb(
						"`game_event_battleground_holiday` game event id (%u) is out of range compared to max event id in `game_event`",
						event_id);
				continue;
			}
		...
		sLog->outString(
				">> Loaded %u battleground holidays in game events in %u ms",
				count, GetMSTimeDiffToNow(oldMSTime));
		sLog->outString();
	}
	...
	result =
			WorldDatabase.Query(
					"SELECT pool_template.entry, game_event_pool.event "
	...
	if (!result) {
		...
		do {
			...
			if (internal_event_id < 0
				...
				sLog->outErrorDb(
						"`game_event_pool` game event id (%i) is out of range compared to max event id in `game_event`",
						event_id);
				continue;
			}
			...
			if (!sPoolMgr->CheckPool(entry)) {
				sLog->outErrorDb(
						"Pool Id (%u) has all creatures or gameobjects with explicit chance sum <>100 and no equal chance defined. The pool system cannot pick one to spawn.",
						entry);
				continue;
			}
		...
		sLog->outString(">> Loaded %u pools for game events in %u ms", count,
				GetMSTimeDiffToNow(oldMSTime));
		sLog->outString();
	}
}
...
uint32 GameEventMgr::GetNPCFlag(Creature * cr) {
	...
	for (ActiveEvents::iterator e_itr = m_ActiveEvents.begin();
			e_itr != m_ActiveEvents.end(); ++e_itr) {
		for (NPCFlagList::iterator itr = mGameEventNPCFlags[*e_itr].begin();
				itr != mGameEventNPCFlags[*e_itr].end(); ++itr)
			if (itr->first == guid)
				mask |= itr->second;
	}
...
void GameEventMgr::StartArenaSeason() {
	QueryResult result = WorldDatabase.PQuery(
			"SELECT event FROM season_linked_event WHERE season = '%i'",
			sWorld->getIntConfig(CONFIG_ARENA_SEASON_ID));

	if (!result) {
		sLog->outError("ArenaSeason (%i) must be an existant Arena Season",
				sWorld->getIntConfig(CONFIG_ARENA_SEASON_ID));
		return;
	}
	...
	sLog->outString("Arena Season %i started...",
			sWorld->getIntConfig(CONFIG_ARENA_SEASON_ID));
	sLog->outString();
}
...
void GameEventMgr::UnApplyEvent(uint16 event_id) {
	sLog->outDetail("GameEvent %u \"%s\" removed.", event_id,
...
void GameEventMgr::ApplyNewEvent(uint16 event_id) {
	...
	sLog->outDetail("GameEvent %u \"%s\" started.", event_id,
			mGameEvent[event_id].description.c_str());
...
void GameEventMgr::UpdateEventNPCFlags(uint16 event_id) {
	...
	for (NPCFlagList::iterator itr = mGameEventNPCFlags[event_id].begin();
			itr != mGameEventNPCFlags[event_id].end(); ++itr) {
		// get the creature data from the low guid to get the entry, to be able to find out the whole guid
		if (CreatureData const* data = sObjectMgr->GetCreatureData(itr->first)) {
			Creature * cr = HashMapHolder<Creature>::Find(
					MAKE_NEW_GUID(itr->first, data->id, HIGHGUID_UNIT));
...
void GameEventMgr::UpdateEventNPCVendor(uint16 event_id, bool activate) {
	for (NPCVendorList::iterator itr = mGameEventVendors[event_id].begin();
			itr != mGameEventVendors[event_id].end(); ++itr) {
		if (activate)
			sObjectMgr->AddVendorItem(itr->entry, itr->item, itr->maxcount,
					itr->incrtime, itr->ExtendedCost, false);
		else
...
void GameEventMgr::GameEventSpawn(int16 event_id) {
	...
	for (GuidList::iterator itr =
			mGameEventCreatureGuids[internal_event_id].begin();
			itr != mGameEventCreatureGuids[internal_event_id].end(); ++itr) {
		// Add to correct cell
		if (CreatureData const* data = sObjectMgr->GetCreatureData(*itr)) {
			...
			if (!map->Instanceable() && map->IsLoaded(data->posX, data->posY)) {
				...
				if (ci->ScriptID)
					pCreature = sScriptMgr->GetCreatureScriptedClass(
							ci->ScriptID);
				if (pCreature == NULL)
	...
	if (internal_event_id < 0
			|| internal_event_id >= int32(mGameEventGameobjectGuids.size())) {
		sLog->outError(
	...
	for (GuidList::iterator itr =
			...
			itr != mGameEventGameobjectGuids[internal_event_id].end(); ++itr) {
		// Add to correct cell
	...
	for (IdList::iterator itr = mGameEventPoolIds[internal_event_id].begin();
			itr != mGameEventPoolIds[internal_event_id].end(); ++itr)
...
void GameEventMgr::GameEventUnspawn(int16 event_id) {
	...
	for (GuidList::iterator itr =
		...
		if (CreatureData const* data = sObjectMgr->GetCreatureData(*itr)) {
			...
			if (Creature * pCreature = sObjectAccessor->GetObjectInWorld(
					MAKE_NEW_GUID(*itr, data->id, HIGHGUID_UNIT),
					(Creature*) NULL))
	...
	for (GuidList::iterator itr =
		...
		if (GameObjectData const* data = sObjectMgr->GetGOData(*itr)) {
			...
			if (GameObject * pGameobject = sObjectAccessor->GetObjectInWorld(
					MAKE_NEW_GUID(*itr, data->id, HIGHGUID_GAMEOBJECT),
					(GameObject*) NULL))
	...
	if (internal_event_id < 0
			|| internal_event_id >= int32(mGameEventPoolIds.size())) {
		sLog->outError(
				"GameEventMgr::GameEventUnspawn attempt access to out of range mGameEventPoolIds element %u (size: " SIZEFMTD ")",
				internal_event_id, mGameEventPoolIds.size());
		return;
	}
...
void GameEventMgr::ChangeEquipOrModel(int16 event_id, bool activate) {
	for (ModelEquipList::iterator itr = mGameEventModelEquip[event_id].begin();
			itr != mGameEventModelEquip[event_id].end(); ++itr) {
		// Remove the creature from grid
		...
		// Update if spawned
		Creature* pCreature = sObjectAccessor->GetObjectInWorld(
				MAKE_NEW_GUID(itr->first, data->id, HIGHGUID_UNIT),
				(Creature*) NULL);
		if (pCreature) {
			if (activate) {
				itr->second.equipement_id_prev =
						pCreature->GetCurrentEquipmentId();
				...
				if (itr->second.modelid > 0
					...
					if (minfo) {
						...
						pCreature->SetFloatValue(UNIT_FIELD_BOUNDINGRADIUS,
								minfo->bounding_radius);
						pCreature->SetFloatValue(UNIT_FIELD_COMBATREACH,
								minfo->combat_reach);
					}
				}
			} else {
	...
	for (ActiveEvents::iterator e_itr = m_ActiveEvents.begin();
		...
		if ((*e_itr) != event_id)
			for (QuestRelList::iterator itr =
					...
					itr != mGameEventGameObjectQuests[*e_itr].end(); ++itr)
				if (itr->second == quest_id)
					return true;
	}
	return false;
}
bool GameEventMgr::hasCreatureActiveEventExcept(uint32 creature_id,
		uint16 event_id) {
	for (ActiveEvents::iterator e_itr = m_ActiveEvents.begin();
			e_itr != m_ActiveEvents.end(); ++e_itr) {
		if ((*e_itr) != event_id) {
...
bool GameEventMgr::hasGameObjectActiveEventExcept(uint32 go_id,
		uint16 event_id) {
	for (ActiveEvents::iterator e_itr = m_ActiveEvents.begin();
			e_itr != m_ActiveEvents.end(); ++e_itr) {
		if ((*e_itr) != event_id) {
...
void GameEventMgr::UpdateEventQuests(uint16 event_id, bool activate) {
	...
	for (itr = mGameEventCreatureQuests[event_id].begin();
			itr != mGameEventCreatureQuests[event_id].end(); ++itr) {
		...
		if (activate) // Add the pair(id, quest) to the multimap
			...
			if (!hasCreatureQuestActiveEventExcept(itr->second, event_id)) {
				...
				QuestRelations::iterator qitr = CreatureQuestMap->find(
						itr->first);
	...
	for (itr = mGameEventGameObjectQuests[event_id].begin();
			itr != mGameEventGameObjectQuests[event_id].end(); ++itr) {
		...
		if (activate) // Add the pair(id, quest) to the multimap
			...
			if (!hasGameObjectQuestActiveEventExcept(itr->second, event_id)) {
				// Remove the pair(id, quest) from the multimap
				QuestRelations::iterator qitr = GameObjectQuestMap->find(
						itr->first);
...
void GameEventMgr::UpdateWorldStates(uint16 event_id, bool Activate) {
	...
	if (event.holiday_id != HOLIDAY_NONE) {
		BattlegroundTypeId bgTypeId = BattlegroundMgr::WeekendHolidayIdToBGType(
				event.holiday_id);
		if (bgTypeId != BATTLEGROUND_TYPE_NONE) {
			...
			if (bl && bl->HolidayWorldStateId) {
				WorldPacket data;
				sBattlegroundMgr->BuildUpdateWorldStatePacket(&data,
						bl->HolidayWorldStateId, Activate ? 1 : 0);
...
void GameEventMgr::HandleQuestComplete(uint32 quest_id) {
	...
	QuestIdToEventConditionMap::iterator itr = mQuestToEventConditions.find(
			quest_id);
	// quest is registered
	if (itr != mQuestToEventConditions.end()) {
		...
		if (mGameEvent[event_id].state != GAMEEVENT_WORLD_CONDITIONS)
			return;
		std::map<uint32, GameEventFinishCondition>::iterator citr =
				mGameEvent[event_id].conditions.find(condition);
		...
		if (citr != mGameEvent[event_id].conditions.end()) {
			...
			if (citr->second.done < citr->second.reqNum) {
				...
				SQLTransaction trans = CharacterDatabase.BeginTransaction();
				trans->PAppend(
						"DELETE FROM game_event_condition_save WHERE event_id = '%u' AND condition_id = '%u'",
						event_id, condition);
				trans->PAppend(
						"INSERT INTO game_event_condition_save (event_id, condition_id, done) VALUES (%u, %u, %f)",
						event_id, condition, citr->second.done);
				CharacterDatabase.CommitTransaction(trans);
...
bool GameEventMgr::CheckOneGameEventConditions(uint16 event_id) {
	for (std::map<uint32, GameEventFinishCondition>::iterator itr =
	...
	if (!mGameEvent[event_id].nextstart) {
		time_t currenttime = time(NULL);
		mGameEvent[event_id].nextstart = currenttime
				+ mGameEvent[event_id].length * 60;
	}
	return true;
}
...
void GameEventMgr::SaveWorldEventStateToDB(uint16 event_id) {
	...
	if (mGameEvent[event_id].nextstart)
		trans->PAppend(
				"INSERT INTO game_event_save (event_id, state, next_start) VALUES ('%u', '%u', "UI64FMTD")",
				event_id, mGameEvent[event_id].state,
				(uint64) (mGameEvent[event_id].nextstart));
	else
		trans->PAppend(
				"INSERT INTO game_event_save (event_id, state, next_start) VALUES ('%u', '%u', '0')",
				event_id, mGameEvent[event_id].state);
	CharacterDatabase.CommitTransaction(trans);
}
...
void GameEventMgr::SendWorldStateUpdate(Player * plr, uint16 event_id) {
	...
	for (itr = mGameEvent[event_id].conditions.begin();
		...
		if (itr->second.max_world_state)
			plr->SendUpdateWorldState(itr->second.max_world_state,
