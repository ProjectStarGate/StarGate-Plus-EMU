...
enum GMTicketSystemStatus {
	GMTICKET_QUEUE_STATUS_DISABLED = 0, GMTICKET_QUEUE_STATUS_ENABLED = 1,
};

enum GMTicketStatus {
	GMTICKET_STATUS_HASTEXT = 0x06, GMTICKET_STATUS_DEFAULT = 0x0A,
};
...
// ticket has been opened by a gm
};
...
public:
	...
	GM_Ticket *GetGMTicket(uint64 ticketGuid) {
		for (GmTicketList::const_iterator i = m_GMTicketList.begin();
	...
	void RemoveGMTicket(uint64 ticketGuid, int64 source = -1, bool permanently =
			false);
	void RemoveGMTicket(GM_Ticket *ticket, int64 source = -1, bool permanently =
			false);
	...
	uint64 GetOpenTicketCount() const {
		return m_openTickets;
	}
