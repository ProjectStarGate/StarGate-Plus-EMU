...
void ObjectPosSelector::AddUsedPos(float size, float angle, float dist) {
	if (angle >= 0)
		m_UsedPosLists[USED_POS_PLUS].insert(
				UsedPosList::value_type(angle, UsedPos(1.0, size, dist)));
	else
...
bool ObjectPosSelector::FirstAngle(float& angle) {
	if (m_UsedPosLists[USED_POS_PLUS].empty()
		...
		return NextAngleFor(*m_UsedPosLists[USED_POS_MINUS].begin(), 1.0,
				USED_POS_PLUS, angle);
	else if (m_UsedPosLists[USED_POS_MINUS].empty()
			&& !m_UsedPosLists[USED_POS_PLUS].empty())
		return NextAngleFor(*m_UsedPosLists[USED_POS_PLUS].begin(), -1.0,
				USED_POS_MINUS, angle);
...
bool ObjectPosSelector::NextPosibleAngle(float& angle) {
	...
	if (m_nextUsedPos[USED_POS_PLUS] != m_UsedPosLists[USED_POS_PLUS].end()
		...
		if (m_smallStepOk[USED_POS_PLUS])
			...
			ok = NextAngleFor(*m_nextUsedPos[USED_POS_PLUS], 1.0, USED_POS_PLUS,
					angle);
	...
	// -- direction less updated
		...
		if (m_smallStepOk[USED_POS_MINUS])
			...
			ok = NextAngleFor(*m_nextUsedPos[USED_POS_MINUS], -1.0,
					USED_POS_MINUS, angle);
