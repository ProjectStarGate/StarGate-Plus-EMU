...
TempSummon::TempSummon(SummonPropertiesEntry const *properties, Unit *owner) :
		Creature(), m_Properties(properties), m_type(TEMPSUMMON_MANUAL_DESPAWN), m_timer(
				0), m_lifetime(0) {
...
Unit* TempSummon::GetSummoner() const {
	return m_summonerGUID ?
			ObjectAccessor::GetUnit(*this, m_summonerGUID) : NULL;
}
	...
	if (owner) {
		if (uint32 slot = m_Properties->Slot) {
			if (owner->m_SummonSlot[slot]
				...
				Creature *oldSummon = GetMap()->GetCreature(
						owner->m_SummonSlot[slot]);
