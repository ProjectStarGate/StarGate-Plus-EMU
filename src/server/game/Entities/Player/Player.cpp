	...
	if (pProto) {
		...
		if (pProto->RequiredSkill != 0) {
			if (GetSkillValue(pProto->RequiredSkill) == 0)
				return EQUIP_ERR_NO_REQUIRED_PROFICIENCY;
			else if (GetSkillValue(pProto->RequiredSkill)
...
void Player::_LoadInventory(PreparedQueryResult result, uint32 timediff) {
	...
	if (result) {
		...
		while (!problematicItems.empty()) {
			std::string subject = GetSession()->GetStarGateString(
	...
	if (!isGameMaster() && ar) {
		...
		if (sDisableMgr->IsDisabledFor(DISABLE_TYPE_MAP, target_map, this)) {
			GetSession()->SendAreaTriggerMessage("%s",
					GetSession()->GetStarGateString(LANG_INSTANCE_CLOSED));
			return false;
		}
