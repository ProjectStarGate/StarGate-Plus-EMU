...
Roll::Roll(uint64 _guid, LootItem const& li) :
		itemGUID(_guid), itemid(li.itemid), itemRandomPropId(
				li.randomPropertyId), itemRandomSuffix(li.randomSuffix), itemCount(
				li.count), totalPlayersRolling(0), totalNeed(0), totalGreed(0), totalPass(
				0), itemSlot(0), rollVoteMask(ROLL_ALL_TYPE_NO_DISENCHANT) {
}
...
Group::Group() :
		m_leaderGuid(0), m_groupType(GROUPTYPE_NORMAL), m_bgGroup(NULL), m_lootMethod(
				FREE_FOR_ALL), m_looterGuid(0), m_lootThreshold(
				ITEM_QUALITY_UNCOMMON), m_subGroupsCounts(NULL), m_guid(0), m_counter(
				0), m_maxEnchantingLevel(0) {
...
Group::~Group() {
	...
	for (uint8 i = 0; i < MAX_DIFFICULTY; ++i)
		for (BoundInstancesMap::iterator itr2 = m_boundInstances[i].begin();
				itr2 != m_boundInstances[i].end(); ++itr2)
...
bool Group::Create(const uint64 &guid, const char * name) {
	...
	if (!isBGGroup()) {
		...
		trans->PAppend(
				"DELETE FROM group_member WHERE guid = %u OR memberGuid = %u",
				lowguid, GUID_LOPART(guid));
		trans->PAppend(
				"INSERT INTO group_member (guid, memberGuid, subgroup) VALUES (%u, %u, 0)",
				lowguid, GUID_LOPART(guid));
		trans->PAppend(
				...
				GUID_LOPART(m_looterGuid), uint32(m_lootThreshold),
				m_targetIcons[0], m_targetIcons[1], m_targetIcons[2],
				m_targetIcons[3], m_targetIcons[4], m_targetIcons[5],
				m_targetIcons[6], m_targetIcons[7], uint8(m_groupType),
				uint32(m_dungeonDifficulty), m_raidDifficulty);
...
bool Group::LoadGroupFromDB(const uint32 &groupGuid, QueryResult result,
		bool loadMembers) {
	if (isBGGroup())
		return false;
	...
	if (loadMembers) {
		...
		result =
				CharacterDatabase.PQuery(
						"SELECT memberGuid, memberFlags, subgroup, roles FROM group_member WHERE guid=%u",
						groupGuid);
		if (!result)
			return false;
		...
		do {
			fields = result->Fetch();
			LoadMemberFromDB(fields[0].GetUInt32(), fields[1].GetUInt8(),
					fields[2].GetUInt8(), fields[3].GetUInt8());
...
bool Group::LoadMemberFromDB(uint32 guidLow, uint8 memberFlags, uint8 subgroup,
	...
	if (!sObjectMgr->GetPlayerNameByGUID(member.guid, member.name)) {
		CharacterDatabase.PQuery("DELETE FROM group_member WHERE memberGuid=%u",
				guidLow);
		return false;
	}
...
void Group::ConvertToLFG() {
	...
	if (!isBGGroup())
		CharacterDatabase.PExecute(
				...
				uint8(m_groupType), GUID_LOPART(m_guid));
	SendUpdate();
}
	...
	if (GetMembersCount() > (isBGGroup() ? 1u : 2u)) {
		...
		if (Player * player = sObjectMgr->GetPlayer(guid)) {
			...
			if (Group* group = player->GetGroup())
				...
				data << uint64(m_guid) << uint32(m_counter) << uint32(0)
						<< uint64(0);
				player->GetSession()->SendPacket(&data);
			}
		...
		if (leaderChanged) {
			WorldPacket data(SMSG_GROUP_SET_LEADER,
					(m_memberSlots.front().name.size() + 1));
...
void Group::SendLootRoll(const uint64& SourceGuid, const uint64& TargetGuid,
		uint8 RollNumber, uint8 RollType, const Roll &r) {
...
void Group::SendLootRollWon(const uint64& SourceGuid, const uint64& TargetGuid,
		uint8 RollNumber, uint8 RollType, const Roll &r) {
...
void Group::GroupLoot(Loot *loot, WorldObject* pLootedObject) {
	...
	for (i = loot->items.begin(); i != loot->items.end(); ++i, ++itemSlot) {
		...
		if (item->Quality >= uint32(m_lootThreshold)) {
			uint64 newitemGUID =
					MAKE_NEW_GUID(sObjectMgr->GenerateLowGuid(HIGHGUID_ITEM), 0, HIGHGUID_ITEM);
			Roll* r = new Roll(newitemGUID, *i);

			//a vector is filled with only near party members
			for (GroupReference *itr = GetFirstMember(); itr != NULL;
					itr = itr->next()) {
				Player *member = itr->getSource();
				if (!member || !member->GetSession())
					continue;
				if (i->AllowedForPlayer(member)) {
					if (member->IsWithinDistInMap(pLootedObject,
							sWorld->getFloatConfig(CONFIG_GROUP_XP_DISTANCE), false))
							{
			...
			if (r->totalPlayersRolling > 0) {
				...
				if (r->totalPass) {
					for (Roll::PlayerVote::const_iterator itr =
							r->playerVote.begin(); itr != r->playerVote.end();
							++itr) {
						Player *p = sObjectMgr->GetPlayer(itr->first);
						if (!p || !p->GetSession())
							continue;
						...
						if (itr->second == PASS)
							SendLootRoll(newitemGUID, p->GetGUID(), 128,
									ROLL_PASS, *r);
					}
				}
	...
	for (std::vector<LootItem>::iterator i = loot->items.begin();
		...
		if (item->Quality >= uint32(m_lootThreshold)) {
			...
			for (GroupReference *itr = GetFirstMember(); itr != NULL;
				...
				if (playerToRoll->CanUseItem(item) == EQUIP_ERR_OK
						&& allowedForPlayer) {
					if (playerToRoll->IsWithinDistInMap(pLootedObject,
							sWorld->getFloatConfig(CONFIG_GROUP_XP_DISTANCE), false))
							{
								r->totalPlayersRolling++;
...
void Group::MasterLoot(Loot* /*loot*/, WorldObject* pLootedObject) {
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"Group::MasterLoot (SMSG_LOOT_MASTER_LIST, 330)");
	...
	for (GroupReference *itr = GetFirstMember(); itr != NULL;
		...
		if (looter->IsWithinDistInMap(pLootedObject,
				sWorld->getFloatConfig(CONFIG_GROUP_XP_DISTANCE), false))
				{
...
void Group::CountRollVote(const uint64& playerGUID, const uint64& Guid,
		uint32 NumberOfPlayers, uint8 Choice) {
...
void Group::CountTheRoll(Rolls::iterator rollI, uint32 NumberOfPlayers) {
	...
	if (roll->totalNeed > 0) {
		if (!roll->playerVote.empty()) {
			...
			for (Roll::PlayerVote::const_iterator itr =
					roll->playerVote.begin(); itr != roll->playerVote.end();
					++itr) {
				if (itr->second != NEED)
					continue;
			...
			if (player && player->GetSession()) {
				player->GetAchievementMgr().UpdateAchievementCriteria(
						...
						roll->itemid, maxresul);

				ItemPosCountVec dest;
				LootItem *item = &(roll->getLoot()->items[roll->itemSlot]);
				uint8 msg = player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest,
						roll->itemid, item->count);
				if (msg == EQUIP_ERR_OK) {
					item->is_looted = true;
					roll->getLoot()->NotifyItemRemoved(roll->itemSlot);
					roll->getLoot()->unlootedCount--;
					AllowedLooterSet* looters = item->GetAllowedLooters();
					player->StoreNewItem(dest, roll->itemid, true,
							item->randomPropertyId,
							(looters->size() > 1) ? looters : NULL);
				} else {
		...
		if (!roll->playerVote.empty()) {
			...
			if (player && player->GetSession()) {
				...
				if (rollvote == GREED) {
					ItemPosCountVec dest;
					uint8 msg = player->CanStoreNewItem(NULL_BAG, NULL_SLOT,
							dest, roll->itemid, item->count);
					if (msg == EQUIP_ERR_OK) {
						item->is_looted = true;
						roll->getLoot()->NotifyItemRemoved(roll->itemSlot);
						roll->getLoot()->unlootedCount--;
...
void Group::GetDataForXPAtKill(Unit const* victim, uint32& count,
	...
	for (GroupReference *itr = GetFirstMember(); itr != NULL;
		...
		if (!member_with_max_level
				|| member_with_max_level->getLevel() < member->getLevel())
...
void Group::SendUpdate() {
	...
	for (member_citerator citr = m_memberSlots.begin();
		...
		if (isLFGGroup()) {
			data
					<< uint8(
							sLFGMgr->GetState(m_guid)
									== LFG_STATE_FINISHED_DUNGEON ? 2 : 0); // FIXME - Dungeon save status? 2 = done
		...
		for (member_citerator citr2 = m_memberSlots.begin();
			...
			uint8 onlineState =
					(member) ? MEMBER_STATUS_ONLINE : MEMBER_STATUS_OFFLINE;
...
bool Group::_addMember(const uint64 &guid, const char* name, uint8 group) {
	...
	if (!isBGGroup())
		CharacterDatabase.PExecute(
				"INSERT INTO group_member (guid, memberGuid, memberFlags, subgroup, roles) VALUES(%u, %u, %u, %u, %u)",
				GUID_LOPART(m_guid), GUID_LOPART(member.guid), member.flags,
				member.group, member.roles);
...
void Group::_setLeader(const uint64 &guid) {
	...
	if (!isBGGroup()) {
		...
		if (player) {
			for (uint8 i = 0; i < MAX_DIFFICULTY; ++i) {
				for (BoundInstancesMap::iterator itr =
						m_boundInstances[i].begin();
						itr != m_boundInstances[i].end();) {
					if (itr->second.perm) {
		...
		trans->PAppend("UPDATE groups SET leaderGuid='%u' WHERE guid='%u'",
				GUID_LOPART(slot->guid), GUID_LOPART(m_guid));
		CharacterDatabase.CommitTransaction(trans);
	}
...
void Group::_removeRolls(const uint64 &guid) {
	for (Rolls::iterator it = RollId.begin(); it != RollId.end(); ++it) {
		...
		CountRollVote(guid, roll->itemGUID, GetMembersCount() - 1,
				MAX_ROLL_TYPE);
	}
}
...
bool Group::_setAssistantFlag(const uint64 &guid, const bool &apply) {
	...
	if (!isBGGroup())
		CharacterDatabase.PExecute(
				"UPDATE group_member SET memberFlags='%u' WHERE memberGuid='%u'",
				slot->flags, GUID_LOPART(guid));
...
void Group::UpdateLooterGuid(WorldObject* pLootedObject, bool ifneed) {
	...
	if (guid_itr != m_memberSlots.end()) {
		if (ifneed) {
			...
			if (looter
					&& looter->IsWithinDistInMap(pLootedObject,
							sWorld->getFloatConfig(CONFIG_GROUP_XP_DISTANCE), false))
							return;
						}
						++guid_itr;
	...
	for (member_citerator itr = guid_itr; itr != m_memberSlots.end(); ++itr) {
		if (Player* pl = ObjectAccessor::FindPlayer(itr->guid))
			if (pl->IsWithinDistInMap(pLootedObject,
					sWorld->getFloatConfig(CONFIG_GROUP_XP_DISTANCE), false))
					{
						pNewLooter = pl;
						break;
...
GroupJoinBattlegroundResult Group::CanJoinBattlegroundQueue(
		Battleground const* bgOrTemplate, BattlegroundQueueTypeId bgQueueTypeId,
		uint32 MinPlayerCount, uint32 /*MaxPlayerCount*/, bool isRated,
		uint32 arenaSlot) {
	...
	PvPDifficultyEntry const* bracketEntry = GetBattlegroundBracketByLevel(
			bgOrTemplate->GetMapId(), reference->getLevel());
	if (!bracketEntry)
		return ERR_BATTLEGROUND_JOIN_FAILED;
	...
	for (GroupReference *itr = GetFirstMember(); itr != NULL;
		...
		if (bgOrTemplate->GetTypeID() == BATTLEGROUND_RB
...
void Group::SetDungeonDifficulty(Difficulty difficulty) {
	...
	if (!isBGGroup())
		CharacterDatabase.PExecute(
				"UPDATE groups SET difficulty = %u WHERE guid ='%u'",
				m_dungeonDifficulty, GUID_LOPART(m_guid));
...
void Group::SetRaidDifficulty(Difficulty difficulty) {
	...
	if (!isBGGroup())
		CharacterDatabase.PExecute(
				"UPDATE groups SET raiddifficulty = %u WHERE guid ='%u'",
				m_raidDifficulty, GUID_LOPART(m_guid));
...
bool Group::InCombatToInstance(uint32 instanceId) {
	for (GroupReference *itr = GetFirstMember(); itr != NULL;
		...
		if (pPlayer && pPlayer->getAttackers().size()
				&& pPlayer->GetInstanceId() == instanceId
				&& (pPlayer->GetMap()->IsRaidOrHeroicDungeon()))
			for (std::set<Unit*>::const_iterator i =
					pPlayer->getAttackers().begin();
					i != pPlayer->getAttackers().end(); ++i)
				if ((*i) && (*i)->GetTypeId() == TYPEID_UNIT
						&& (*i)->ToCreature()->GetCreatureInfo()->flags_extra
...
void Group::ResetInstances(uint8 method, bool isRaid, Player* SendMsgTo) {
	...
	for (BoundInstancesMap::iterator itr = m_boundInstances[diff].begin();
		...
		if (!entry || entry->IsRaid() != isRaid
				|| (!p->CanReset() && method != INSTANCE_RESET_GROUP_DISBAND)) {
			++itr;
			continue;
		}
		...
		if (map && map->IsDungeon()
				&& !(method == INSTANCE_RESET_GROUP_DISBAND && !p->CanReset())) {
			if (p->CanReset())
...
InstanceGroupBind* Group::GetBoundInstance(Map* aMap) {
	...
	BoundInstancesMap::iterator itr = m_boundInstances[difficulty].find(
			aMap->GetId());
...
InstanceGroupBind* Group::BindToInstance(InstanceSave *save, bool permanent,
		bool load) {
	if (!save || isBGGroup())
		return NULL;
	...
	if (!load && (!bind.save || permanent != bind.perm || save != bind.save))
		CharacterDatabase.PExecute(
				"REPLACE INTO group_instance (guid, instance, permanent) VALUES (%u, %u, %u)",
				GUID_LOPART(GetGUID()), save->GetInstanceId(), permanent);
	...
	if (!load)
		sLog->outDebug(
				LOG_FILTER_MAPS,
				"Group::BindToInstance: %d is now bound to map %d, instance %d, difficulty %d",
				GUID_LOPART(GetGUID()), save->GetMapId(), save->GetInstanceId(),
				save->GetDifficulty());
	return &bind;
}
	...
	if (itr != m_boundInstances[difficulty].end()) {
		if (!unload)
			CharacterDatabase.PExecute(
					...
					GUID_LOPART(GetGUID()), itr->second.save->GetInstanceId());
...
void Group::BroadcastGroupUpdate(void) {
	...
	for (member_citerator citr = m_memberSlots.begin();
		...
		if (pp && pp->IsInWorld()) {
			...
			sLog->outStaticDebug("-- Forced group value update for '%s'",
					pp->GetName());
		}
	}
}
	...
	for (member_citerator citr = m_memberSlots.begin();
		...
		if (pMember
				&& m_maxEnchantingLevel
						< pMember->GetSkillValue(SKILL_ENCHANTING))
...
bool Group::IsFull() const {
	return isRaidGroup() ?
			(m_memberSlots.size() >= MAXRAIDSIZE) :
...
void Group::ToggleGroupMemberFlag(member_witerator slot, uint8 flag,
		bool apply) {
	if (apply)
		slot->flags |= flag;
	else
