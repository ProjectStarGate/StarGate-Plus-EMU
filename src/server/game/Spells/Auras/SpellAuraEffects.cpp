		...
		if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE) {
			...
			// Serpent String
                if (GetSpellProto()->Id == 1978)
					...
					damage = int32(GetBase()->GetCaster()->GetTotalAttackPowerValue(RANGED_ATTACK) * 0.8f); //Anti-Fix
...
void AuraEffect::HandleAuraDummy(AuraApplication const *aurApp, uint8 mode,
	...
	if (mode & AURA_EFFECT_HANDLE_REAL) {
			...
			case 52916: // Honor Among Thieves
				if (target == caster)
					if (Player* pTarget = caster->ToPlayer()) {
						Unit* spellTarget = ObjectAccessor::GetUnit(*pTarget,
								pTarget->GetComboTarget());
						if (!spellTarget)
		...
		// AT REMOVE
			...
			case SPELLFAMILY_PRIEST:
				// Vampiric Touch
				if (m_spellProto->SpellFamilyFlags[1] & 0x0400
						&& aurApp->GetRemoveMode() == AURA_REMOVE_BY_ENEMY_SPELL
						&& GetEffIndex() == 0) {
					if (AuraEffect const * aurEff = GetBase()->GetEffect(1)) {
						int32 damage = aurEff->GetAmount() * 8;
						// backfire damage
						target->CastCustomSpell(target, 64085, &damage, NULL,
								NULL, true, NULL, NULL, GetCasterGUID());
					}
				}
				break;
			case SPELLFAMILY_HUNTER:
