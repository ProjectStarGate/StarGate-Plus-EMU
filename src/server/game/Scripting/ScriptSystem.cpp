...
void SystemMgr::LoadVersion() {
	...
	QueryResult Result = WorldDatabase.Query(
			"SELECT script_version FROM version LIMIT 1");

	if (Result) {
		Field* pFields = Result->Fetch();

		sLog->outString("TSCR: Database version is: %s",
				pFields[0].GetCString());
		sLog->outString();
	} else {
...
void SystemMgr::LoadScriptTexts() {
	...
	LoadStarGateStrings("script_texts", TEXT_SOURCE_RANGE,
...
void SystemMgr::LoadScriptTextsCustom() {
	...
	LoadStarGateStrings("custom_texts", TEXT_SOURCE_RANGE * 2,
			1 + (TEXT_SOURCE_RANGE * 3));
	...
	do {
		...
		if (iId >= 0) {
			sLog->outErrorDb(
					"TSCR: Entry %i in table `custom_texts` is not a negative value.",
					iId);
			continue;
		}
		...
		if (iId > TEXT_SOURCE_RANGE * 2 || iId <= TEXT_SOURCE_RANGE * 3) {
			sLog->outErrorDb(
					"TSCR: Entry %i in table `custom_texts` is out of accepted entry range for table.",
					iId);
			continue;
		}
		...
		if (pTemp.uiSoundId) {
			if (!GetSoundEntriesStore()->LookupEntry(pTemp.uiSoundId))
				sLog->outErrorDb(
						"TSCR: Entry %i in table `custom_texts` has soundId %u but sound does not exist.",
						iId, pTemp.uiSoundId);
		}

		if (!GetLanguageDescByID(pTemp.uiLanguage))
			sLog->outErrorDb(
					"TSCR: Entry %i in table `custom_texts` using Language %u but Language does not exist.",
					iId, pTemp.uiLanguage);

		if (pTemp.uiType > CHAT_TYPE_ZONE_YELL)
			sLog->outErrorDb(
					"TSCR: Entry %i in table `custom_texts` has Type %u but this Chat Type does not exist.",
					iId, pTemp.uiType);
...
void SystemMgr::LoadScriptWaypoints() {
	...
	sLog->outString(
			"TSCR: Loading Script Waypoints for " UI64FMTD " creature(s)...",
			uiCreatureCount);
	...
	do {
		...
		CreatureInfo const* pCInfo = GetCreatureTemplateStore(
				pTemp.uiCreatureEntry);

		if (!pCInfo) {
			sLog->outErrorDb(
					"TSCR: DB table script_waypoint has waypoint for non-existant creature entry %u",
					pTemp.uiCreatureEntry);
			continue;
		}
		...
		if (!pCInfo->ScriptID)
			sLog->outErrorDb(
					"TSCR: DB table script_waypoint has waypoint for creature entry %u, but creature does not have ScriptName defined and then useless.",
					pTemp.uiCreatureEntry);
	...
	sLog->outString(">> Loaded %u Script Waypoint nodes in %u ms", count,
