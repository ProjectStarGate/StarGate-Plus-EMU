...
template<>
bool WaypointMovementGenerator<Creature>::GetDestination(float &x, float &y,
		float &z) const {
...
template<>
bool WaypointMovementGenerator<Player>::GetDestination(float & /*x*/,
...
void WaypointMovementGenerator<Creature>::Initialize(Creature &u) {
	...
	if (waypoints && waypoints->size()) {
		...
		i_destinationHolder.SetDestination(traveller, node->x, node->y,
				node->z);
...
template<class T>
bool WaypointMovementGenerator<T>::Update(T & /*unit*/,
		const uint32 & /*diff*/) {
	return false;
}

template<>
bool WaypointMovementGenerator<Creature>::Update(Creature &unit,
		const uint32 &diff) {
	if (!&unit)
		return true;
		...
		if (unit.IsStopped()) {
			...
			if (i_currentNode == waypoints->size() - 1) // If that's our last waypoint
				...
				if (repeating) // If the movement is repeating
					...
					unit.SetHomePosition(node->x, node->y, node->z,
							unit.GetOrientation());
			...
			if (unit.GetFormation()
					&& unit.GetFormation()->getLeader() == &unit)
			...
			if (node->event_id && urand(0, 99) < node->event_chance)
				unit.GetMap()->ScriptsStart(sWaypointScripts, node->event_id,
						&unit, NULL/*, false*/);
...
template bool WaypointMovementGenerator<Player>::Update(Player &,
		const uint32 &);
		...
		if (i_destinationHolder.UpdateTraveller(traveller, diff)) {
			...
			if (i_destinationHolder.HasArrived()) {
				...
				if (MovementInProgress()) {
					...
					sLog->outStaticDebug("loading node %u for player %s",
							i_currentNode, player.GetName());
					if ((*i_path)[i_currentNode].mapid == curMap) {
						// do not send movement, it was sent already
						i_destinationHolder.SetDestination(traveller,
								(*i_path)[i_currentNode].x,
								(*i_path)[i_currentNode].y,
								(*i_path)[i_currentNode].z, false);
					}
...
void FlightPathMovementGenerator::PreloadEndGrid() {
	...
	// Load the grid
	if (endMap) {
		sLog->outDetail("Preloading flightmaster at grid (%f, %f) for map %u",
				m_endGridX, m_endGridY, m_endMapId);
...
void FlightPathMovementGenerator::DoEventIfAny(Player& player,
	...
	if (uint32 eventid = departure ? node.departureEventID : node.arrivalEventID) {
		sLog->outDebug(LOG_FILTER_MAPSCRIPTS,
				...
				departure ? "departure" : "arrival", eventid, node.index,
				node.path, player.GetName());
