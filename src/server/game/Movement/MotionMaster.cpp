...
void MotionMaster::InitDefault() {
	if (i_owner->GetTypeId() == TYPEID_UNIT) {
		MovementGenerator* movement = FactorySelector::selectMovementGenerator(
				i_owner->ToCreature());
		Mutate(movement == NULL ? &si_idleMovement : movement,
				MOTION_SLOT_IDLE);
	} else {
...
void MotionMaster::MoveRandom(float spawndist) {
	if (i_owner->GetTypeId() == TYPEID_UNIT) {
		sLog->outStaticDebug("Creature (GUID: %u) start moving random",
				i_owner->GetGUIDLow());
...
void MotionMaster::MoveTargetedHome() {
	...
	if (i_owner->GetTypeId() == TYPEID_UNIT) {
		sLog->outStaticDebug("Creature (Entry: %u GUID: %u) targeted home",
				i_owner->GetEntry(), i_owner->GetGUIDLow());
		Mutate(new HomeMovementGenerator<Creature>(), MOTION_SLOT_ACTIVE);
	} else {
		sLog->outError("Player (GUID: %u) attempt targeted home",
				i_owner->GetGUIDLow());
	}
}
...
void MotionMaster::MoveConfused() {
	if (i_owner->GetTypeId() == TYPEID_PLAYER) {
		sLog->outStaticDebug("Player (GUID: %u) move confused",
				i_owner->GetGUIDLow());
...
void MotionMaster::MoveChase(Unit* target, float dist, float angle) {
	...
	if (!target || target == i_owner
			|| i_owner->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE))
		return;
	...
	if (i_owner->GetTypeId() == TYPEID_PLAYER) {
		sLog->outStaticDebug(
				...
				target->GetTypeId() == TYPEID_PLAYER ?
						target->GetGUIDLow() :
						target->ToCreature()->GetDBTableGUIDLow());
...
void MotionMaster::MoveKnockbackFrom(float srcX, float srcY, float speedXY,
		float speedZ) {
	...
	i_owner->GetNearPoint(i_owner, x, y, z, i_owner->GetObjectSize(), dist,
			i_owner->GetAngle(srcX, srcY) + M_PI);
...
void MotionMaster::MoveJump(float x, float y, float z, float speedXY,
		float speedZ) {
	...
	if (i_owner->GetTypeId() == TYPEID_PLAYER) {
		sLog->outStaticDebug(
				"Player (GUID: %u) jump to point (X: %f Y: %f Z: %f)",
				i_owner->GetGUIDLow(), x, y, z);
		Mutate(new PointMovementGenerator<Player>(0, x, y, z),
				MOTION_SLOT_CONTROLLED);
	} else {
		sLog->outStaticDebug(
				"Creature (Entry: %u GUID: %u) jump to point (X: %f Y: %f Z: %f)",
...
void MotionMaster::MoveFall(float z, uint32 id) {
	...
	MoveCharge(i_owner->GetPositionX(), i_owner->GetPositionY(), z,
			SPEED_CHARGE, id);
}

void MotionMaster::MoveSeekAssistance(float x, float y, float z) {
	if (i_owner->GetTypeId() == TYPEID_PLAYER) {
		sLog->outError("Player (GUID: %u) attempt to seek assistance",
				i_owner->GetGUIDLow());
	} else {
		sLog->outStaticDebug(
...
void MotionMaster::MoveFleeing(Unit* enemy, uint32 time) {
	...
	if (i_owner->GetTypeId() == TYPEID_PLAYER) {
		sLog->outStaticDebug(
				...
				enemy->GetTypeId() == TYPEID_PLAYER ?
						enemy->GetGUIDLow() :
						enemy->ToCreature()->GetDBTableGUIDLow());
...
void MotionMaster::MoveDistract(uint32 timer) {
	...
	if (i_owner->GetTypeId() == TYPEID_PLAYER) {
		sLog->outStaticDebug("Player (GUID: %u) distracted (timer: %u)",
