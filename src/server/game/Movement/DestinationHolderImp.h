...
void DestinationHolder<TRAVELLER>::_findOffSetPoint(float x1, float y1,
		float x2, float y2, float offset, float &x, float &y) {
	...
	if (offset == 0) {
		...
		if (distance_d == 0) {
			...
			double distance_ratio = (double) (distance_d - offset)
					/ (double) distance_d;
...
template<typename TRAVELLER>
uint32 DestinationHolder<TRAVELLER>::SetDestination(TRAVELLER &traveller,
		float dest_x, float dest_y, float dest_z, bool sendMove) {
	i_destSet = true;
	i_destX = dest_x;
	i_destY = dest_y;
	i_destZ = dest_z;
...
template<typename TRAVELLER>
uint32 DestinationHolder<TRAVELLER>::StartTravel(TRAVELLER &traveller,
		bool sendMove) {
	if (!i_destSet)
		return 0;
	...
	i_totalTravelTime = traveller.GetTotalTrevelTimeTo(i_destX, i_destY,
			i_destZ);
	i_timeElapsed = 0;
	if (sendMove)
...
template<typename TRAVELLER>
bool DestinationHolder<TRAVELLER>::UpdateTraveller(TRAVELLER &traveller,
		uint32 diff, bool micro_movement) {
	i_timeElapsed += diff;
	...
	if (!micro_movement)
		...
		if (!traveller.GetTraveller().HasUnitState(
				UNIT_STAT_MOVING | UNIT_STAT_IN_FLIGHT))
			return true;

		if (traveller.GetTraveller().HasUnitState(UNIT_STAT_IN_FLIGHT))
			GetLocationNow(traveller.GetTraveller().GetBaseMap(), x, y, z,
	...
	if (traveller.GetTraveller().GetPositionX() != x
			|| traveller.GetTraveller().GetPositionY() != y
			|| traveller.GetTraveller().GetPositionZ() != z) {
...
template<typename TRAVELLER>
void DestinationHolder<TRAVELLER>::GetLocationNow(const Map * map, float &x,
		float &y, float &z, bool is3D) const {
	if (HasArrived()) {
		x = i_destX;
		y = i_destY;
		z = i_destZ;
	} else if (HasDestination()) {
		double percent_passed = (double) i_timeElapsed
...
template<typename TRAVELLER>
float DestinationHolder<TRAVELLER>::GetDistance3dFromDestSq(
...
template<typename TRAVELLER>
float DestinationHolder<TRAVELLER>::GetDestinationDiff(float x, float y,
		float z) const {
	return sqrt(
			((x - i_destX) * (x - i_destX)) + ((y - i_destY) * (y - i_destY))
					+ ((z - i_destZ) * (z - i_destZ)));
}

template<typename TRAVELLER>
void DestinationHolder<TRAVELLER>::GetLocationNowNoMicroMovement(float &x,
