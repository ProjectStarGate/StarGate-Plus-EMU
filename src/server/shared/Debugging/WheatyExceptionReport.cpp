...
BOOL WheatyExceptionReport::_GetProcessorName(TCHAR* sProcessorName,
	...
	lRet = ::RegOpenKeyEx(HKEY_LOCAL_MACHINE,
			_T("HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0"), 0,
	...
	*szVersion = _T('\0');
	...
	case VER_PLATFORM_WIN32_NT:
		...
		if (osvi.dwMajorVersion == 6)
			_tcsncat(szVersion, _T("Windows Vista or Windows Server 2008 "),
					cntMax);
		...
		if (osvi.dwMajorVersion == 4
			...
			} else // Windows NT 4.0 prior to SP6a
			{
				_stprintf(wszTmp, _T("%s (Version %d.%d, Build %d)"),
						osvi.szCSDVersion, osvi.dwMajorVersion,
		...
		break;
	default:
		_stprintf(wszTmp, _T("%s (Version %d.%d, Build %d)"), osvi.szCSDVersion,
				osvi.dwMajorVersion, osvi.dwMinorVersion,
				osvi.dwBuildNumber & 0xFFFF);
		_tcsncat(szVersion, wszTmp, cntMax);
		break;
	...
	do {
		if (te32.th32OwnerProcessID == dwOwnerPID) {
			...
			HANDLE threadHandle = OpenThread(
					THREAD_GET_CONTEXT | THREAD_QUERY_INFORMATION, false,
					te32.th32ThreadID);
...
//    #ifdef _M_IX86                                          // X86 Only!
	...
	SymEnumSymbols(GetCurrentProcess(),
			(DWORD64) GetModuleHandle(szFaultingModule), 0,
			EnumerateSymbolsCallback, 0);
	//  #endif                                                  // X86 Only!
...
#define EXCEPTION(x) case EXCEPTION_##x: return _T(#x);
	...
	FormatMessage(FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_FROM_HMODULE,
			GetModuleHandle(_T("NTDLL.DLL")), dwCode, 0, szBuffer,
			sizeof(szBuffer), 0);
...
//=============================================================================
BOOL WheatyExceptionReport::GetLogicalAddress(PVOID addr, PTSTR szModule,
	...
	PIMAGE_NT_HEADERS pNtHdr = (PIMAGE_NT_HEADERS)(
			hMod + DWORD_PTR(pDosHdr->e_lfanew));
	...
	for (unsigned i = 0; i < pNtHdr->FileHeader.NumberOfSections;
		...
		DWORD_PTR sectionEnd = sectionStart
				+ DWORD_PTR(
						std::max(pSection->SizeOfRawData,
...
void WheatyExceptionReport::WriteStackDetails(PCONTEXT pContext,
	...
	while (1) {
		...
		if (!StackWalk64(dwMachineType, m_hProcess,
				pThreadHandle != NULL ? pThreadHandle : GetCurrentThread(), &sf,
				pContext, 0, SymFunctionTableAccess64, SymGetModuleBase64, 0))
			break;
		...
		} else // No symbol found.  Print out the logical address instead.
		{
			...
			GetLogicalAddress((PVOID) sf.AddrPC.Offset, szModule,
		...
		if (SymGetLineFromAddr64(m_hProcess, sf.AddrPC.Offset,
			...
			_tprintf(_T("  %s line %u"), lineInfo.FileName,
					lineInfo.LineNumber);
		}
...
BOOL CALLBACK
WheatyExceptionReport::EnumerateSymbolsCallback(PSYMBOL_INFO pSymInfo,
...
char * WheatyExceptionReport::DumpTypeIndex(char * pszCurrBuffer,
		DWORD64 modBase, DWORD dwTypeIndex, unsigned nestingLevel,
		DWORD_PTR offset, bool & bHandled, char* Name) {
	bHandled = false;
	...
	for (unsigned i = 0; i < dwChildrenCount; i++) {
		...
		pszCurrBuffer = DumpTypeIndex(pszCurrBuffer, modBase,
				children.ChildId[i], nestingLevel + 1, offset, bHandled2,
				""/*Name */);
		...
		if (!bHandled2) {
			...
			pszCurrBuffer = FormatOutputValue(pszCurrBuffer, basicType, length,
					(PVOID) dwFinalOffset);
...
BasicType WheatyExceptionReport::GetBasicType(DWORD typeIndex,
		DWORD64 modBase) {
	BasicType basicType;
	if (SymGetTypeInfo(m_hProcess, modBase, typeIndex, TI_GET_BASETYPE,
			&basicType)) {
		return basicType;
	}
	...
	// SymGetTypeInfo(TI_GET_TYPEID) call below.
	DWORD typeId;
	if (SymGetTypeInfo(m_hProcess, modBase, typeIndex, TI_GET_TYPEID,
			&typeId)) {
