...
int DelayExecutor::activate(int num_threads, ACE_Method_Request* pre_svc_hook,
...
int DelayExecutor::execute(ACE_Method_Request* new_req) {
	...
	if (queue_.enqueue(new_req, (ACE_Time_Value*) &ACE_Time_Value::zero)
			== -1) {
		delete new_req;
		ACE_ERROR_RETURN(
				(LM_ERROR, ACE_TEXT("(%t) %p\n"), ACE_TEXT("DelayExecutor::execute enqueue")),
				-1);
	}
