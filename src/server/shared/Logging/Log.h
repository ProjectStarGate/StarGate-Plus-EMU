...
enum DebugLogFilters {
	LOG_FILTER_NONE = 0x00000000, LOG_FILTER_UNITS = 0x00000001, // Anything related to units that doesn't fit in other categories. ie. creature formations
...
enum LogLevel {
	LOGL_NORMAL = 0, LOGL_BASIC, LOGL_DETAIL, LOGL_DEBUG
};
...
public:
	...
	void outCharDump(const char * str, uint32 account_id, uint32 guid,
			const char * name);
	...
	void SetSQLDriverQueryLogging(bool newStatus) {
		m_sqlDriverQueryLogging = newStatus;
	}
	...
	bool GetSQLDriverQueryLogging() const {
		return m_sqlDriverQueryLogging;
	}
private:
	FILE* openLogFile(char const* configFileName,
