...
Log::Log() :
		raLogfile(NULL), logfile(NULL), gmLogfile(NULL), charLogfile(NULL), dberLogfile(
				NULL), chatLogfile(NULL), arenaLogFile(NULL), sqlLogFile(NULL), sqlDevLogFile(
				NULL), m_gmlog_per_account(false), m_enableLogDBLater(false), m_enableLogDB(
				false), m_colored(false) {
	Initialize();
}
...
void Log::Initialize() {
	...
	if (!m_gmlog_per_account)
		...
		if (!m_gmlog_filename_format.empty()) {
			bool m_gmlog_timestamp = sConfig->GetBoolDefault("GmLogTimestamp",
					false);
	...
	m_sqlDriverQueryLogging = sConfig->GetBoolDefault("SQLDriverQueryLogging",
			false);

	m_DebugLogMask = DebugLogFilters(
			sConfig->GetIntDefault("DebugLogMask", LOG_FILTER_NONE));

	// Char log settings
	m_charLog_Dump = sConfig->GetBoolDefault("CharLogDump", false);
	m_charLog_Dump_Separate = sConfig->GetBoolDefault("CharLogDump.Separate",
			false);
	if (m_charLog_Dump_Separate) {
		m_dumpsDir = sConfig->GetStringDefault("CharLogDump.SeparateDir", "");
		if (!m_dumpsDir.empty())
			if ((m_dumpsDir.at(m_dumpsDir.length() - 1) != '/')
					&& (m_dumpsDir.at(m_dumpsDir.length() - 1) != '\\'))
				m_dumpsDir.append("/");
	}
}

FILE* Log::openLogFile(char const* configFileName,
		char const* configTimeStampFlag, char const* mode) {
	std::string logfn = sConfig->GetStringDefault(configFileName, "");
	if (logfn.empty())
		return NULL;

	if (configTimeStampFlag
			&& sConfig->GetBoolDefault(configTimeStampFlag, false)) {
...
FILE* Log::openGmlogPerAccount(uint32 account) {
	...
	snprintf(namebuf, ARKCORE_PATH_MAX, m_gmlog_filename_format.c_str(),
			account);
...
void Log::outTimestamp(FILE* file) {
	...
	fprintf(file, "%-4d-%02d-%02d %02d:%02d:%02d ", aTm->tm_year + 1900,
...
void Log::SetColor(bool stdout_stream, ColorTypes color) {
	...
	static uint8 UnixColorFG[Colors] = { FG_BLACK, // BLACK
			FG_RED, // RED
			FG_GREEN, // GREEN
			FG_BROWN, // BROWN
			FG_BLUE, // BLUE
...
void Log::outDB(LogTypes type, const char * str) {
	...
	LoginDatabase.PExecute("INSERT INTO logs (time, realm, type, string) "
			"VALUES (" UI64FMTD ", %u, %u, '%s');", uint64(time(0)), realm,
			type, new_str.c_str());
}
