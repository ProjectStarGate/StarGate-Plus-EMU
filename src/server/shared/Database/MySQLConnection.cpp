...
bool MySQLConnection::ExecuteTransaction(SQLTransaction& transaction) {
	...
	for (itr = queries.begin(); itr != queries.end(); ++itr) {
		...
		case SQL_ELEMENT_PREPARED: {
			...
			if (!Execute(stmt)) {
				sLog->outSQLDriver(
						"[Warning] Transaction aborted. %u queries not executed.",
						(uint32) queries.size());
				RollbackTransaction();
				return false;
			}
...
MySQLPreparedStatement* MySQLConnection::GetPreparedStatement(uint32 index) {
	...
	if (!ret)
		sLog->outSQLDriver(
				"ERROR: Could not fetch prepared statement %u on database `%s`, connection type: %s.",
				index,
				m_connectionInfo.database.c_str(),
...
void MySQLConnection::PrepareStatement(uint32 index, const char* sql,
	...
	if (!stmt) {
		sLog->outSQLDriver("[ERROR]: In mysql_stmt_init() id: %u, sql: \"%s\"",
				index, sql);
		...
		if (mysql_stmt_prepare(stmt, sql,
			...
			sLog->outSQLDriver(
					"[ERROR]: In mysql_stmt_prepare() id: %u, sql: \"%s\"",
					index, sql);
...
bool MySQLConnection::_HandleMySQLErrno(uint32 errNo) {
	...
	default:
		sLog->outSQLDriver(
				"Unhandled MySQL errno %u. Unexpected behaviour possible.",
				errNo);
		return false;
	}
}
