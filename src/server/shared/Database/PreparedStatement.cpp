...
void PreparedStatement::BindParameters() {
	...
	sLog->outSQLDriver("Warnung: BindParameters() for statement %u did not bind all allocated parameters", m_index);
#endif
}
...
bool MySQLPreparedStatement::CheckValidIndex(uint8 index) {
	...
	if (m_paramsSet[index])
		sLog->outSQLDriver(
				"Warnung: Prepared Statement (id: %u) trying to bind value on already bound index (%u).",
				m_stmt->m_index, index);
	return true;
}
...
void MySQLPreparedStatement::setValue(MYSQL_BIND* param, enum_field_types type,
...
PreparedStatementTask::PreparedStatementTask(PreparedStatement* stmt) :
		m_stmt(stmt), m_has_result(false) {
}

PreparedStatementTask::PreparedStatementTask(PreparedStatement* stmt,
		PreparedQueryResultFuture result) :
		m_stmt(stmt), m_has_result(true), m_result(result) {
}
