...
template<class T>
void SQLStorageLoaderBase<T>::convert_str_to_str(uint32 /*field_pos*/,
		char* src, char*& dst) {
	if (!src) {
		dst = new char[1];
		*dst = 0;
...
template<class T>
template<class S>
void SQLStorageLoaderBase<T>::convert_to_str(uint32 /*field_pos*/, S /*src*/,
		char*& dst) {
	dst = new char[1];
	*dst = 0;
}
...
template<class T>
template<class D>
void SQLStorageLoaderBase<T>::convert_from_str(uint32 /*field_pos*/,
		char* /*src*/, D& dst) {
	dst = 0;
}
...
template<class T>
template<class V>
void SQLStorageLoaderBase<T>::storeValue(V value, SQLStorage& store, char* p,
		int x, uint32 &offset) {
...
template<class T>
void SQLStorageLoaderBase<T>::storeValue(char* value, SQLStorage& store,
		char* p, int x, uint32& offset) {
	T* subclass = (static_cast<T*>(this));
	switch (store.dst_format[x]) {
	case FT_LOGIC:
...
void SQLStorageLoaderBase<T>::Load(SQLStorage& store) {
	...
	if (store.iNumFields != result->GetFieldCount()) {
		store.RecordCount = 0;
		sLog->outError(
				"Error loading table %s, probably table format was updated (there should be %u fields in sql).\n",
	...
	for (uint32 x = 0; x < store.iNumFields; x++)
		if (store.dst_format[x] == FT_STRING)
			...
			++bb;
	recordsize = (store.iNumFields - sc - bo - bb) * 4 + sc * sizeof(char*)
			+ bo * sizeof(bool) + bb * sizeof(char);
	...
	do {
		...
		for (uint32 x = 0; x < store.iNumFields; x++)
			...
			case FT_LOGIC:
				storeValue((bool) (fields[x].GetUInt32() > 0), store, p, x,
						offset);
				break;
			case FT_BYTE:
